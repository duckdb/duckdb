...
TEST_CASE("Test SQLColAttribute TIMESTAMP", "[odbc][timestamp]") {
	...
	EXECUTE_AND_CHECK("SQLExecDirect", SQLExecDirect, hstmt, ConvertToSQLCHAR("SELECT TIMESTAMP '1992-09-20 12:34:56.123456' as ts"), SQL_NTS);
	...
	expected_int[SQL_DESC_CONCISE_TYPE] = ExpectedResult(SQL_TYPE_TIMESTAMP);	// For the datetime and interval data types, this field returns
