...
TEST_CASE("Test SQLColAttribute TIME", "[odbc][time]") {
	SQLHANDLE env;
	SQLHANDLE dbc;
	...
	EXECUTE_AND_CHECK("SQLExecDirect", SQLExecDirect, hstmt, ConvertToSQLCHAR("SELECT TIME '12:34:56.123456' as t"), SQL_NTS);
	...
	expected_int[SQL_DESC_UNNAMED] = ExpectedResult(SQL_NAMED);				// If the SQL_DESC_NAME field of the IRD contains
	...
	expected_int[SQL_DESC_DISPLAY_SIZE] = ExpectedResult(9);

	// ODBC 2 - see ODBC3 sectiion below for details
	expected_int[SQL_COLUMN_LENGTH] = ExpectedResult(8);
	expected_int[SQL_COLUMN_PRECISION] = ExpectedResult(8);
	//expected_int[SQL_COLUMN_SCALE] = ExpectedResult(0);

	// ODBC 3
	expected_int[SQL_DESC_LENGTH] = ExpectedResult(8);						// A numeric value that is either the maximum 
																			//   or actual character length of a character
																			//   string or binary data type.
	expected_int[SQL_DESC_PRECISION] = ExpectedResult(8);					// A numeric value that for a numeric data type
	...
	//expected_int[SQL_DESC_SCALE] = ExpectedResult(11);					// A numeric value that is the applicable scale for a
	...
	expected_int[SQL_DESC_TYPE] = ExpectedResult(SQL_DATETIME);				// For the datetime and interval data types, this field returns
	...
	expected_int[SQL_DESC_CONCISE_TYPE] =
	    ExpectedResult(SQL_TYPE_TIME); // For the datetime and interval data types, this field returns
