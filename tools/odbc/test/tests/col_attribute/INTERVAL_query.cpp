...
TEST_CASE("Test SQLColAttribute INTERVAL 1 YEAR", "[odbc][interval]") {
	...
	expected_int[SQL_DESC_TYPE] = ExpectedResult(SQL_INTERVAL);				// For the datetime and interval data types, this field returns
...
TEST_CASE("Test SQLColAttribute INTERVAL 1 MINUTE", "[odbc][interval]") {
	...
	// run a simple query  with ints to get a result set
	EXECUTE_AND_CHECK("SQLExecDirect", SQLExecDirect, hstmt, ConvertToSQLCHAR("SELECT INTERVAL 1 MINUTE as m"), SQL_NTS);
	EXECUTE_AND_CHECK("SQLFetch ()", SQLFetch, hstmt);
...
TEST_CASE("Test SQLColAttribute INTERVAL 1 SECOND", "[odbc][interval]") {
	...
	// run a simple query  with ints to get a result set
	EXECUTE_AND_CHECK("SQLExecDirect", SQLExecDirect, hstmt, ConvertToSQLCHAR("SELECT INTERVAL 1 SECOND as s"), SQL_NTS);
	EXECUTE_AND_CHECK("SQLFetch ()", SQLFetch, hstmt);
...
TEST_CASE("Test SQLColAttribute add 1 year to a specific date", "[odbc][interval]") {
	...
	// run a simple query  with ints to get a result set
	EXECUTE_AND_CHECK("SQLExecDirect", SQLExecDirect, hstmt, ConvertToSQLCHAR("SELECT DATE '2000-01-01' + INTERVAL 1 YEAR"),
	                  SQL_NTS);
	EXECUTE_AND_CHECK("SQLFetch ()", SQLFetch, hstmt);
	...
	expected_int[SQL_DESC_CONCISE_TYPE] = ExpectedResult(SQL_TYPE_DATE);// For the datetime and interval data types, this field returns
																		//   the concise data type; for example, SQL_TYPE_TIME or
																		//   SQL_INTERVAL_YEAR.
...
TEST_CASE("Test SQLColAttribute subtract 1 year from a specific date", "[odbc][interval]") {
	...
	// run a simple query  with ints to get a result set
	EXECUTE_AND_CHECK("SQLExecDirect", SQLExecDirect, hstmt,ConvertToSQLCHAR("SELECT DATE '2000-01-01' - INTERVAL 1 YEAR"), SQL_NTS);
	EXECUTE_AND_CHECK("SQLFetch ()", SQLFetch, hstmt);
...
TEST_CASE("Test SQLColAttribute construct an interval from a column, instead of a constant", "[odbc][interval]") {
	...
	// run a simple query  with ints to get a result set
	EXECUTE_AND_CHECK("SQLExecDirect", SQLExecDirect, hstmt, ConvertToSQLCHAR("SELECT INTERVAL (i) YEAR FROM range(1, 5) t(i)"), SQL_NTS);
	EXECUTE_AND_CHECK("SQLFetch ()", SQLFetch, hstmt);
	DATA_CHECK(hstmt, 1, "1 year");
...
TEST_CASE("Test SQLColAttribute construct an interval with mixed units", "[odbc][interval]") {
	...
	// run a simple query  with ints to get a result set
	EXECUTE_AND_CHECK("SQLExecDirect", SQLExecDirect, hstmt, ConvertToSQLCHAR("SELECT INTERVAL '1 month 1 day'"), SQL_NTS);
	EXECUTE_AND_CHECK("SQLFetch ()", SQLFetch, hstmt);
