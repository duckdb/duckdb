...
static void TestMicrosoftExample(HSTMT &hstmt) {
	...
	ExecuteCmdAndCheckODBC("SQLSetStmtAttr", SQLSetStmtAttr, hstmt, hstmt, SQL_ATTR_ROW_ARRAY_SIZE,
...
TEST_CASE("row_wise_fetching", "[odbc") {
	...
	ExecuteCmdAndCheckODBC("SQLAllocHandle (HSTMT)", SQLAllocHandle, hstmt, SQL_HANDLE_STMT, dbc, &hstmt);

	// Free the statement handle
	ExecuteCmdAndCheckODBC("SQLFreeHandle (HSTMT)", SQLFreeHandle, hstmt, SQL_HANDLE_STMT, hstmt);

	// Disconnect from the database
	ExecuteCmdAndCheckODBC("SQLDisconnect (HDBC)", SQLDisconnect, dbc, dbc);
}
