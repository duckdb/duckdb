...
static void DataAtExecution(HSTMT &hstmt) {
	// Prepare a statement
	EXECUTE_AND_CHECK("SQLPrepare", SQLPrepare, hstmt, ConvertToSQLCHAR("SELECT id FROM bytea_table WHERE t = ? OR t = ?"), SQL_NTS);
	...
	EXECUTE_AND_CHECK("SQLBindParameter", SQLBindParameter, hstmt, 1, SQL_PARAM_INPUT, SQL_C_BINARY, SQL_VARCHAR, param_1_bytes, 0, ConvertToSQLPOINTER(1), 0, &param_1_len);

	SQLLEN param_2_len = SQL_DATA_AT_EXEC;
	EXECUTE_AND_CHECK("SQLBindParameter", SQLBindParameter, hstmt, 2, SQL_PARAM_INPUT, SQL_C_BINARY, SQL_VARCHAR, 6, 0, ConvertToSQLPOINTER(2), 0, &param_2_len);
...
static void ArrayBindingDataAtExecution(HSTMT &hstmt) {
	...
	EXECUTE_AND_CHECK("SQLSetStmtAttr(SQL_ATTR_PARAM_STATUS_PTR)", SQLSetStmtAttr, hstmt, SQL_ATTR_PARAM_STATUS_PTR, status, 0);
	EXECUTE_AND_CHECK("SQLSetStmtAttr(SQL_ATTR_PARAMS_PROCESSED_PTR)", SQLSetStmtAttr, hstmt, SQL_ATTR_PARAMS_PROCESSED_PTR, &num_processed, 0);
	EXECUTE_AND_CHECK("SQLSetStmtAttr(SQL_ATTR_PARAMSET_SIZE)", SQLSetStmtAttr, hstmt, SQL_ATTR_PARAMSET_SIZE, ConvertToSQLPOINTER(2), 0);

	// Bind the array
	EXECUTE_AND_CHECK("SQLBindParameter", SQLBindParameter, hstmt, 1, SQL_PARAM_INPUT, SQL_C_BINARY, SQL_VARBINARY, 5, 0, ConvertToSQLPOINTER(1), 0, str_ind);
