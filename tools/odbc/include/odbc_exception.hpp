...
public:
	explicit OdbcException(const std::string &msg);
	explicit OdbcException(const std::string &msg, const std::string &sqlstate="", const std::string &server_name="", SQLINTEGER col_number=SQL_NO_COLUMN_NUMBER, SQLINTEGER sql_native=0, SQLLEN row_number=SQL_NO_ROW_NUMBER);
