...
SQLRETURN OdbcUtils::SetStringAndLength(vector<string> &error_messages, const string &val_str, SQLPOINTER target_value_ptr, SQLSMALLINT buffer_length, SQLSMALLINT *str_len_or_ind_ptr) {
	if (!target_value_ptr) {
...
string OdbcUtils::ParseStringFilter(const string filter_name, const string filter_value, SQLUINTEGER sql_attr_metadata_id, const string coalesce_str) {
	string filter;
...
string OdbcUtils::GetQueryDuckdbColumns(const string catalog_filter, const string schema_filter, const string table_filter, const string column_filter) {
	string sql_duckdb_columns = "SELECT "
									"NULL \"TABLE_CAT\", "
	...
	sql_duckdb_columns += " WHERE " + catalog_filter + " AND " + schema_filter + " AND " + table_filter + " AND " + column_filter;
	sql_duckdb_columns += " ORDER BY \"TABLE_CAT\", \"TABLE_SCHEM\", \"TABLE_NAME\", \"ORDINAL_POSITION\"";
	return sql_duckdb_columns;
}

string OdbcUtils::GetQueryDuckdbTables(const string schema_filter, const string table_filter, const string table_type_filter) {
	string sql_duckdb_tables = "SELECT "
								...
								"WHERE " + schema_filter + " AND " + table_filter + " AND table_type IN (" + table_type_filter + ") "
