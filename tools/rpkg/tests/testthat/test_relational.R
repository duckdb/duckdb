...
#     select j, i, sum(i) over (partition by j) from a order by 1,2
    ...
    window_function <- rel_window(rel=rel_a, window_function="sum", window_alias="a_sum", partitions=partitions, children=sum)
    sum2 <- list(expr_reference("a", window_function))
...
#     select a, b, count(b) over (partition by a) from a order by a
    rel_a <- rel_from_df(con, data.frame(a=c(1:8),b=c(1, 1, 2, 2, 3, 3, 4, 4)))
    count <- list(expr_reference("a"))
    partitions <- list(expr_reference("b"))
    window_function <- rel_window(rel=rel_a, window_function="count", window_alias="count", children=count, partitions=partitions)
    a_ref <- list(expr_reference("a", window_function))
    window_order_by_a <- rapi_rel_order(window_function, a_ref)
    expected_result <- data.frame(a=c(1:8),b=c(1, 1, 2, 2, 3, 3, 4, 4), count=c(2, 2, 2, 2, 2, 2, 2, 2))
    res = rel_to_altrep(window_order_by_a)
    expect_equal(res, expected_result)
})
...
#     select a, b, avg(b) over (partition by a) from a order by a
    rel_a <- rel_from_df(con, data.frame(a=c(1:8),b=c(1, 1, 2, 2, 3, 3, 4, 4)))
    avg <- list(expr_reference("a"))
    partitions <- list(expr_reference("b"))
    window_function <- rel_window(rel=rel_a, window_function="avg", window_alias="a_avg", children=avg, partitions=partitions)
    a_ref <- list(expr_reference("a", window_function))
    window_order_by_a <- rapi_rel_order(window_function, a_ref)
...
#   select a, b, avg(a) FILTER (WHERE x % 2 = 0) over (partition by b)
    ...
    filter_expr <- list(rel_filter(rel_a, list(expr_function("eq", list(zero, mod_function)))))
    window_function <- rel_window(rel=rel_a, window_function="avg", window_alias="a_avg", children=avg, partitions=partitions, filter_expr=filter_expr)
    a_ref <- list(expr_reference("a", window_function))
    order_over_window <- rapi_rel_order(window_function, a_ref)
...
test_that("You can perform the window function dense_rank", {
    ...
    order_by_a_output <- list(expr_reference("a", window_function))
...
test_that("You can perform the window function cume_dist", {
    ...
    order_over_window <- list(expr_reference("a", window_function))
