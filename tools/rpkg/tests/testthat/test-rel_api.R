...
test_that("relational count() order-preserving", {
  ...
  con <- dbConnect(duckdb())
  experimental <- FALSE
  invisible(dbExecute(con, "CREATE MACRO \"n\"() AS (COUNT(*))"))
  df1 <- data.frame(a = c(1, 2, 3, 4, 5, 6), b = c(2, 2, 2, 2, 2, 2), g = c(1L, 2L, 2L, 3L, 3L, 3L))
...
test_that("relational count(a) order-preserving", {
  ...
  expect_equal(
    out,
    data.frame(a = c(1, 2, 3, 4, 5, 6), n = c(1L, 1L, 1L, 1L, 1L, 1L))
  )
  dbDisconnect(con, shutdown = TRUE)
})
...
test_that("relational count(g, a) order-preserving", {
  ...
  rel2 <- rel_aggregate(
    rel1,
    groups = list(expr_reference("g"), expr_reference("a")),
    aggregates = list({
      tmp_expr <- expr_function("n", list())
      expr_set_alias(tmp_expr, "n")
      tmp_expr
...
test_that("relational count(b, g) order-preserving", {
  ...
  rel2 <- rel_aggregate(
    rel1,
    groups = list(expr_reference("b"), expr_reference("g")),
    aggregates = list({
      tmp_expr <- expr_function("n", list())
      expr_set_alias(tmp_expr, "n")
      tmp_expr
  ...
  expect_equal(
    out,
    data.frame(b = c(2, 2, 2), g = 1:3, n = 1:3)
  )
  dbDisconnect(con, shutdown = TRUE)
})
...
test_that("relational count(a) order-enforcing", {
  ...
  rel3 <- rel_order(rel2, list(expr_reference("a"), expr_reference("n")))
  rel3
  out <- rel_to_altrep(rel3)
  expect_equal(
    out,
...
test_that("relational count(b) order-enforcing", {
  ...
  rel3 <- rel_order(rel2, list(expr_reference("b"), expr_reference("n")))
  rel3
  out <- rel_to_altrep(rel3)
  expect_equal(
    out,
...
test_that("relational full_join(join_by(a)) order-preserving", {
  ...
  invisible(dbExecute(con, "CREATE MACRO \"___coalesce\"(a, b) AS COALESCE(a, b)"))
  df1 <- data.frame(a = 1:4, b = c(2, 2, 2, 2))
...
test_that("relational relocate(g) order-preserving", {
  ...
  expect_equal(
    out,
    data.frame(g = c(1L, 2L, 2L, 3L, 3L, 3L), a = c(1, 2, 3, 4, 5, 6), b = c(2, 2, 2, 2, 2, 2))
  )
  dbDisconnect(con, shutdown = TRUE)
})
...
test_that("relational relocate(g, .before = b) order-preserving", {
  ...
  expect_equal(
    out,
    data.frame(a = c(1, 2, 3, 4, 5, 6), g = c(1L, 2L, 2L, 3L, 3L, 3L), b = c(2, 2, 2, 2, 2, 2))
  )
  dbDisconnect(con, shutdown = TRUE)
})
