...
[[cpp11::register]] SEXP rapi_expr_constant(sexp val, bool experimental) {
	if (LENGTH(val) != 1) {
...
//	auto expr = make_external_prot<ConstantExpression>("duckdb_expr", val, RApiTypes::SexpToValue(val, 0, experimental));
	auto expr = make_external<ConstantExpression>("duckdb_expr", RApiTypes::SexpToValue(val, 0, experimental));
	return expr;
}
