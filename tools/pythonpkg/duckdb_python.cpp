...
static void InitializeConnectionMethods(py::module_ &m) {
	...
	m.def(
	    ...
	    py::arg("prefer_nullable_dtypes") = false, py::arg("connection") = py::none());
	m.def(
	    "fetch_df",
	    [](bool date_as_object, bool prefer_nullable_dtypes, shared_ptr<DuckDBPyConnection> conn = nullptr) {
		    if (!conn) {
	...
	m.def(
	    "fetch_df_chunk",
	    [](const idx_t vectors_per_chunk = 1, bool date_as_object = false, bool prefer_nullable_dtypes = false,
	       shared_ptr<DuckDBPyConnection> conn = nullptr) {
		    if (!conn) {
			    conn = DuckDBPyConnection::DefaultConnection();
		    }
