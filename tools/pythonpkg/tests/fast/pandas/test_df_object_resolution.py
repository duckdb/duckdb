...
class TestResolveObjectColumns(object):
    ...
    def test_map_duplicate(self, duckdb_cursor):
        ...
        with pytest.raises(RuntimeError, match="Dict->Map conversion failed because 'key' list contains duplicates"):
            converted_col = duckdb.query_df(x, "x", "select * from x").df()
    ...
    def test_map_nullkey(self, duckdb_cursor):
        ...
        with pytest.raises(RuntimeError, match="Dict->Map conversion failed because 'key' list contains None"):
            converted_col = duckdb.query_df(x, "x", "select * from x").df()
    ...
    def test_analyze_sample_too_small(self):
        ...
        with pytest.raises(RuntimeError, match="Unimplemented type for cast"):
            res = duckdb.query_df(x, "x", "select * from x").df()
