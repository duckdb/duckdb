...
class TestValue(object):
    ...
    @pytest.mark.parametrize('item', [
            ...
            (duckdb.decimal_type(12, 8), DecimalValue(decimal.Decimal('1234.12345678'), 12, 8), decimal.Decimal('1234.12345678')),
            (VARCHAR, StringValue('this is a long string'), 'this is a long string'),
            (UUID, UUIDValue(uuid.UUID('ffffffff-ffff-ffff-ffff-ffffffffffff')), uuid.UUID('ffffffff-ffff-ffff-ffff-ffffffffffff')),
            ...
            (TIMESTAMP, TimestampValue(datetime.datetime(1970, 3, 21, 12, 5, 43, 120)), datetime.datetime(1970, 3, 21, 12, 5, 43, 120)),
    ...
    def test_timestamp_sec_not_supported(self, value):
        con = duckdb.connect()
        with pytest.raises(duckdb.NotImplementedException, match="Conversion from 'datetime' to type .* is not implemented yet"):
            con.execute('select $1', [value]).fetchall()
