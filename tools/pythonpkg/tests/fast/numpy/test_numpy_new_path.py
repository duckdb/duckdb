...
class TestScanNumpy(object):
    def test_scan_numpy(self, duckdb_cursor):
        ...
        z = np.array(z)
        res = duckdb.sql("select * from z").fetchall()
        assert res == [({'key': ['3'], 'value': [0]},), ({'key': ['2'], 'value': [1]},), ({'key': ['1'], 'value': [2]},)]
        ...
        res = duckdb.sql("select * from z").fetchall()
        assert res == [(timedelta(days=64, seconds=29156, microseconds=10),), (timedelta(days=19, seconds=29156, microseconds=10),)]
