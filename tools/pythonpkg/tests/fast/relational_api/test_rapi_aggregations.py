...
def aggregation_generic(aggregation_function,assertion_answers):
    ...
    if len(assertion_answers) < 3:
        ...
        with pytest.raises(ValueError, match='No function matches the given name'):
    ...
    # Check empty
    with pytest.raises(ValueError, match='incompatible function arguments'):
    ...
    # Check broken
    with pytest.raises(ValueError, match='Referenced column "nonexistant" not found'):
...
class TestRAPIAggregations(object):
    ...
    def test_value_counts(self, duckdb_cursor):
        ...
        with pytest.raises(ValueError, match='Only one column is accepted'):
