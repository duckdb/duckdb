...
class TestMultipleColumnsSameName(object):
    ...
    def test_multiple_columns_with_same_name(self, duckdb_cursor):
        ...
        original_df = pd.DataFrame({'a': [1, 2, 3, 4], 'b': [5, 6, 7, 8], 'd': [9, 10, 11, 12]})
        original_df = df.rename(columns={ df.columns[1]: "a" })
        ...
        assert con.execute("select * from df_view;").fetchall() == [(5,), (6,), (7,), (8,)]
