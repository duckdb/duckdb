...
static bool ConvertColumn(idx_t target_offset, data_ptr_t target_data, bool *target_mask, CanonicalFormat &idata,
                          idx_t count) {
...
template <class DUCKDB_T, class NUMPY_T>
static bool ConvertColumnCategoricalTemplate(idx_t target_offset, data_ptr_t target_data, CanonicalFormat &idata,
                                             idx_t count) {
	auto src_ptr = (DUCKDB_T *)idata.data;
	auto out_ptr = (NUMPY_T *)target_data;
	if (!idata.validity.AllValid()) {
...
static bool ConvertNested(idx_t target_offset, data_ptr_t target_data, bool *target_mask, Vector &input,
                          CanonicalFormat &idata, idx_t count) {
	auto out_ptr = (NUMPY_T *)target_data;
	if (!idata.validity.AllValid()) {
		for (idx_t i = 0; i < count; i++) {
			idx_t src_idx = idata.sel->get_index(i);
...
template <class NUMPY_T>
static bool ConvertColumnCategorical(idx_t target_offset, data_ptr_t target_data, CanonicalFormat &idata, idx_t count,
...
template <class T>
static bool ConvertColumnRegular(idx_t target_offset, data_ptr_t target_data, bool *target_mask, CanonicalFormat &idata,
                                 idx_t count) {
...
static bool ConvertDecimalInternal(idx_t target_offset, data_ptr_t target_data, bool *target_mask,
                                   CanonicalFormat &idata, idx_t count, double division) {
	auto src_ptr = (DUCKDB_T *)idata.data;
...
static bool ConvertDecimal(const LogicalType &decimal_type, idx_t target_offset, data_ptr_t target_data,
                           bool *target_mask, CanonicalFormat &idata, idx_t count) {
