...
public:
	...
	static unique_ptr<DuckDBPyRelation>
	ReadJSON(const py::object &filename, shared_ptr<DuckDBPyConnection> conn = nullptr,
	         const Optional<py::object> &columns = py::none(), const Optional<py::object> &sample_size = py::none(),
	         const Optional<py::object> &maximum_depth = py::none(), const Optional<py::str> &records = py::none(),
