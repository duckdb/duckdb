...
public:
	...
	duckdb::pyarrow::Table FetchArrow(idx_t approx_rows_per_batch);
	PolarsDataFrame FetchPolars(idx_t approx_rows_per_batch);

	py::dict FetchPyTorch();

	py::dict FetchTF();

	duckdb::pyarrow::RecordBatchReader FetchRecordBatchReader(const idx_t approx_rows_per_batch) const;
