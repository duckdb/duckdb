...
DUCKDB_EXTENSION_EXTERN // NOLINT(cppcoreguidelines-avoid-non-const-global-variables)
#include <type_traits>
	...
	static_assert(static_cast<uint8_t>(OptionTargetScope::LOCAL_ONLY) == DUCKDB_V2_OPTION_TARGET_SCOPE_LOCAL_ONLY,
	              "OptionTargetScope must mirror DUCKDB_V2_OPTION_TARGET_SCOPE");
	static_assert(static_cast<uint8_t>(OptionTargetScope::GLOBAL_DEFAULT) ==
	                  DUCKDB_V2_OPTION_TARGET_SCOPE_GLOBAL_DEFAULT,
	              "OptionTargetScope must mirror DUCKDB_V2_OPTION_TARGET_SCOPE");
	...
	auto Database::GetOption(std::string_view name) const -> DatabaseOption {
		duckdb_v2_option_handle option = nullptr;
		CheckedAPICall(duckdb_v2_database_option_get, handle(), duckdb_v2_identifier_t {name.data(), name.size()},
		               &option);
		return detail::Factory::Make<DatabaseOption>(option);
	}
	...
	auto Connection::CreateType(const QualifiedName &name, const std::vector<TypeParam> &params) -> LogicalType {
		...
		duckdb_v2_logical_type_handle type = nullptr;
		CheckedAPICall(duckdb_v2_connection_create_type_from_name, handle(), name.handle(), split.names(),
	...
	auto Connection::Execute(const SqlStatement &statement, const std::vector<NamedParam> &parameters) -> QueryResult {
		...
		duckdb_v2_result_handle result = nullptr;
		CheckedAPICall(duckdb_v2_statement_execute, handle(), statement.handle(),
	...
	auto PreparedStatement::Execute(const Value *parameters, idx_t parameter_count) -> QueryResult {
		...
		duckdb_v2_result_handle result = nullptr;
		CheckedAPICall(duckdb_v2_prepared_statement_execute, handle(), nullptr,
		               parameter_count ? values.data() : nullptr, parameter_count, &result);
		return detail::Factory::Make<QueryResult>(result);
	}
	...
	auto Connection::GetQueryProgress() const -> Connection::QueryProgress {
		...
		try {
			...
			CheckedAPICall(duckdb_v2_query_progress_get_total_rows_to_process, snapshot,
	...
	auto RunExtensionEntry(void (*body)(Extension &, Context &), void *extension, void *context, void *err) -> void {
		...
		// The guard is the only thing standing between a throwing extension body and unwinding through DuckDB's C
		// frame.
	...
	auto Context::ParseType(std::string_view text) const -> LogicalType {
		duckdb_v2_logical_type_handle type = nullptr;
		CheckedAPICall(duckdb_v2_context_create_type_from_text, handle(), duckdb_v2_str {text.data(), text.size()},
		               &type);
		return detail::Factory::Make<LogicalType>(type);
	}
...
#define MAKE_VALUE_IMPL(ctx, name, value)                                                                              \
	...
	auto bignum_t::Decode() const -> Decoded {
		...
		Decoded out {};
		idx_t length = 0;
		CheckedAPICall(duckdb_v2_bignum_decode, bytes, static_cast<idx_t>(size()), nullptr, static_cast<idx_t>(0),
		               &length, &out.is_negative);
	...
	auto Value::CreateMap(Connection &conn, KeyValueList values) -> Value {
		...
		duckdb_v2_value_handle out = nullptr;
		CheckedAPICall(duckdb_v2_value_create_map_with_connection, conn.handle(), nullptr, nullptr,
	...
	auto Value::CreateMap(Connection &conn, const LogicalType &key_type, const LogicalType &value_type) -> Value {
		duckdb_v2_value_handle out = nullptr;
		CheckedAPICall(duckdb_v2_value_create_map_with_connection, conn.handle(), key_type.handle(),
		               value_type.handle(), nullptr, nullptr, static_cast<idx_t>(0), &out);
		return FromHandle(out);
	}
	...
	DataChunk::DataChunk(const Context &ctx, const std::vector<LogicalType> &types) {
		const auto type_pointers = ExtractTypeHandles(types);
		duckdb_v2_data_chunk_handle chunk = nullptr;
		CheckedAPICall(duckdb_v2_data_chunk_create_with_context, ctx.handle(), type_pointers.data(),
		               type_pointers.size(), &chunk);
	...
	auto ColumnDataCollection::Combine(
	    ColumnDataCollection &&source) // NOLINT(cppcoreguidelines-rvalue-reference-param-not-moved)
	    -> void {
		auto source_handle = source.handle();
		CheckedAPICall(duckdb_v2_column_data_collection_combine, handle(), &source_handle);
		// The C API consumed the source; detach the wrapper so its destructor does not double-free. A refused merge
		// threw above and leaves the source intact.
		source.release();
	}
	...
	auto ColumnDataCollection::Scan(SharedScanState &shared, WorkerScanState &worker, DataChunk &chunk) const -> bool {
		...
		CheckedAPICall(duckdb_v2_column_data_collection_scan, handle(), shared.handle(), worker.handle(),
		               chunk.handle(), &did_produce);
		return did_produce;
	}
...
#undef DUCKDB_CPP_ASSERT_STATEMENT_TYPE
	...
	struct AggregateFunctionInfo {
		...
		AggregateFunctionInfo(AggregateFunction::BindCallback bind_callback,
		                      AggregateFunction::SizeCallback size_callback,
		    ...
		    : bind_callback(bind_callback), size_callback(size_callback), init_callback(init_callback),
		      update_callback(update_callback), combine_callback(combine_callback),
		      finalize_callback(finalize_callback), destroy_callback(destroy_callback),
		      user_data(std::move(user_data)) {
		}
	...
	auto AggregateFunction::SetBindCallback(BindCallback callback) & -> AggregateFunction & {
		...
		// The C-side callback is one shared trampoline; the user's callback is looked
		// up through the info table riding the user_data slot (set by Register).
		static auto trampoline = [](duckdb_v2_aggregate_function_bind_info_handle info,
	...
	auto AggregateFunction::SetSizeCallback(SizeCallback callback) & -> AggregateFunction & {
		...
		static auto trampoline = [](duckdb_v2_aggregate_function_size_info_handle info,
		                            duckdb_v2_error_info_handle *err) {
			WithExceptionGuard(err, [&]() {
				void *user_data = nullptr;
	...
	auto AggregateFunction::SetInitCallback(InitCallback callback) & -> AggregateFunction & {
		...
		static auto trampoline = [](duckdb_v2_aggregate_function_init_info_handle info,
		                            duckdb_v2_error_info_handle *err) {
			WithExceptionGuard(err, [&]() {
				void *user_data = nullptr;
	...
	struct TableFunctionInfo {
		...
		TableFunctionInfo(TableFunction::BindCallback bind_callback,
		                  TableFunction::InitGlobalCallback init_global_callback,
		                  TableFunction::InitLocalCallback init_local_callback,
		                  TableFunction::ExecCallback exec_callback, TableFunction::ProgressCallback progress_callback,
		    ...
		    : bind_callback(bind_callback), init_global_callback(init_global_callback),
		      ...
		      progress_callback(progress_callback), filter_pushdown_callback(filter_pushdown_callback),
		      user_data(std::move(user_data)) {
		}
	...
	auto TableFunction::SetInitGlobalCallback(InitGlobalCallback callback) & -> TableFunction & {
		...
		static auto trampoline = [](duckdb_v2_table_function_init_global_info_handle info,
		                            duckdb_v2_context_handle context, duckdb_v2_error_info_handle *err) {
			WithExceptionGuard(err, [&]() {
				void *user_data = nullptr;
	...
	auto TableFunction::SetInitLocalCallback(InitLocalCallback callback) & -> TableFunction & {
		...
		static auto trampoline = [](duckdb_v2_table_function_init_local_info_handle info,
		                            duckdb_v2_context_handle context, duckdb_v2_error_info_handle *err) {
			WithExceptionGuard(err, [&]() {
				void *user_data = nullptr;
				...
				auto input =
				    detail::Factory::Make<InitLocalInput>(static_cast<void *>(info), static_cast<void *>(context));
	...
	auto TableFunction::SetProgressCallback(ProgressCallback callback) & -> TableFunction & {
		...
		static auto trampoline = [](duckdb_v2_table_function_progress_info_handle info,
		                            duckdb_v2_context_handle context, duckdb_v2_error_info_handle *err) {
			WithExceptionGuard(err, [&]() {
				void *user_data = nullptr;
				...
				auto input =
				    detail::Factory::Make<ProgressInput>(static_cast<void *>(info), static_cast<void *>(context));
	...
	struct CopyFunctionInfo {
		...
		CopyFunctionInfo(CopyFunction::CopyToBindCallback copy_to_bind_callback,
		    ...
		    : copy_to_bind_callback(copy_to_bind_callback), copy_to_batch_size_callback(copy_to_batch_size_callback),
		      ...
		      copy_from_exec_callback(copy_from_exec_callback),
		      copy_from_progress_callback(copy_from_progress_callback), user_data(std::move(user_data)) {
		}
	...
	void *RequireCopyBatchData(void *ptr) {
		if (!ptr) {
			throw InvalidInputException(
			    "no batch data was set; call CopyToBatchInput::SetBatchData in the batch callback");
		}
		return ptr;
	}
	...
	auto CopyFunction::SetCopyToBatchSizeCallback(CopyToBatchSizeCallback callback) & -> CopyFunction & {
		...
		static auto trampoline = [](duckdb_v2_copy_to_batch_size_info_handle info, duckdb_v2_context_handle context,
			...
			WithExceptionGuard(err, [&]() {
				...
				auto input = detail::Factory::Make<CopyToBatchSizeInput>(static_cast<void *>(info),
	...
	auto CopyFunction::SetCopyFromInitGlobalCallback(CopyFromInitGlobalCallback callback) & -> CopyFunction & {
		...
		static auto trampoline = [](duckdb_v2_copy_from_init_global_info_handle info, duckdb_v2_context_handle context,
			...
			WithExceptionGuard(err, [&]() {
				...
				auto input = detail::Factory::Make<CopyFromInitGlobalInput>(static_cast<void *>(info),
				                                                            static_cast<void *>(context));
	...
	auto CopyFunction::SetCopyFromInitLocalCallback(CopyFromInitLocalCallback callback) & -> CopyFunction & {
		...
		static auto trampoline = [](duckdb_v2_copy_from_init_local_info_handle info, duckdb_v2_context_handle context,
			...
			WithExceptionGuard(err, [&]() {
				...
				auto input = detail::Factory::Make<CopyFromInitLocalInput>(static_cast<void *>(info),
				                                                           static_cast<void *>(context));
	...
	auto CopyFunction::SetCopyFromProgressCallback(CopyFromProgressCallback callback) & -> CopyFunction & {
		...
		static auto trampoline = [](duckdb_v2_copy_from_progress_info_handle info, duckdb_v2_context_handle context,
			...
			WithExceptionGuard(err, [&]() {
				...
				auto input = detail::Factory::Make<CopyFromProgressInput>(static_cast<void *>(info),
				                                                          static_cast<void *>(context));
	...
	auto CopyFunction::Register() -> void {
		...
		duckdb_v2_opaque opaque {info.get(), detail::TypedDelete<CopyFunctionInfo>,
		                         detail::TypedEquals<CopyFunctionInfo>};
	...
	auto CopyFunction::CopyToBindInput::GetFilePath() const -> std::string {
		...
		CheckedAPICall(duckdb_v2_copy_to_bind_get_file_path, static_cast<duckdb_v2_copy_to_bind_info_handle>(args),
		               &path);
	...
	auto CopyFunction::CopyToBindInput::GetColumnName(idx_t index) const -> std::string {
		duckdb_v2_identifier_t name = {nullptr, 0};
		CheckedAPICall(duckdb_v2_copy_to_bind_get_column_name, static_cast<duckdb_v2_copy_to_bind_info_handle>(args),
		               index, &name);
		return std::string(FromStr(name));
	}
	...
	auto CopyFunction::CopyToBindInput::GetColumnType(idx_t index) const -> LogicalType {
		duckdb_v2_logical_type_handle type = nullptr;
		CheckedAPICall(duckdb_v2_copy_to_bind_get_column_type, static_cast<duckdb_v2_copy_to_bind_info_handle>(args),
		               index, &type);
		return detail::Factory::Make<LogicalType>(type);
	}
	...
	auto CopyFunction::CopyToBindInput::GetOptionName(idx_t index) const -> std::string {
		duckdb_v2_identifier_t name = {nullptr, 0};
		CheckedAPICall(duckdb_v2_copy_to_bind_get_option_name, static_cast<duckdb_v2_copy_to_bind_info_handle>(args),
		               index, &name);
		return std::string(FromStr(name));
	}
	...
	auto CopyFunction::CopyToBatchSizeInput::SetTarget(idx_t rows) -> void {
		CheckedAPICall(duckdb_v2_copy_to_batch_size_set_target,
		               static_cast<duckdb_v2_copy_to_batch_size_info_handle>(args), rows);
	}
	...
	void *CopyFunction::CopyToInitInput::GetBindDataInternal() const {
		void *data = nullptr;
		CheckedAPICall(duckdb_v2_copy_to_init_get_bind_data, static_cast<duckdb_v2_copy_to_init_info_handle>(args),
		               &data);
		return RequireCopyBindData(data);
	}
	...
	auto CopyFunction::CopyToInitInput::GetFilePath() const -> std::string {
		duckdb_v2_str path = {nullptr, 0};
		CheckedAPICall(duckdb_v2_copy_to_init_get_file_path, static_cast<duckdb_v2_copy_to_init_info_handle>(args),
		               &path);
		return std::string(FromStr(path));
	}
	...
	void *CopyFunction::CopyToFinalizeInput::GetBindDataInternal() const {
		void *data = nullptr;
		CheckedAPICall(duckdb_v2_copy_to_finalize_get_bind_data,
		               static_cast<duckdb_v2_copy_to_finalize_info_handle>(args), &data);
		return RequireCopyBindData(data);
	}

	void *CopyFunction::CopyToFinalizeInput::GetInitDataInternal() const {
		void *data = nullptr;
		CheckedAPICall(duckdb_v2_copy_to_finalize_get_init_data,
		               static_cast<duckdb_v2_copy_to_finalize_info_handle>(args), &data);
		return RequireCopyInitData(data);
	}
	...
	void *CopyFunction::CopyToFinalizeInput::GetUserDataInternal() const {
		void *user_data = nullptr;
		CheckedAPICall(duckdb_v2_copy_to_finalize_get_user_data,
		               static_cast<duckdb_v2_copy_to_finalize_info_handle>(args), &user_data);
		const auto &function = *static_cast<const CopyFunctionInfo *>(user_data);
		return RequireCopyUserData(function.user_data);
	}
	...
	auto CopyFunction::CopyFromBindInput::GetColumnCount() const -> idx_t {
		idx_t count = 0;
		CheckedAPICall(duckdb_v2_copy_from_bind_get_column_count,
		               static_cast<duckdb_v2_copy_from_bind_info_handle>(args), &count);
		return count;
	}

	auto CopyFunction::CopyFromBindInput::GetColumnName(idx_t index) const -> std::string {
		duckdb_v2_identifier_t name = {nullptr, 0};
		CheckedAPICall(duckdb_v2_copy_from_bind_get_column_name,
		               static_cast<duckdb_v2_copy_from_bind_info_handle>(args), index, &name);
		return std::string(FromStr(name));
	}

	auto CopyFunction::CopyFromBindInput::GetColumnType(idx_t index) const -> LogicalType {
		duckdb_v2_logical_type_handle type = nullptr;
		CheckedAPICall(duckdb_v2_copy_from_bind_get_column_type,
		               static_cast<duckdb_v2_copy_from_bind_info_handle>(args), index, &type);
		return detail::Factory::Make<LogicalType>(type);
	}

	auto CopyFunction::CopyFromBindInput::GetOptionCount() const -> idx_t {
		idx_t count = 0;
		CheckedAPICall(duckdb_v2_copy_from_bind_get_option_count,
		               static_cast<duckdb_v2_copy_from_bind_info_handle>(args), &count);
		return count;
	}
	...
	auto CopyFunction::CopyFromBindInput::GetOptionName(idx_t index) const -> std::string {
		duckdb_v2_identifier_t name = {nullptr, 0};
		CheckedAPICall(duckdb_v2_copy_from_bind_get_option_name,
		               static_cast<duckdb_v2_copy_from_bind_info_handle>(args), index, &name);
		return std::string(FromStr(name));
	}
	...
	auto CopyFunction::CopyFromBindInput::GetOptionValue(idx_t index) const -> Value {
		duckdb_v2_value_handle value = nullptr;
		CheckedAPICall(duckdb_v2_copy_from_bind_get_option_value,
		               static_cast<duckdb_v2_copy_from_bind_info_handle>(args), index, &value);
		return detail::Factory::Make<Value>(value);
	}

	auto CopyFunction::CopyFromBindInput::SetCardinality(idx_t cardinality, bool is_exact) -> void {
		CheckedAPICall(duckdb_v2_copy_from_bind_set_cardinality,
		               static_cast<duckdb_v2_copy_from_bind_info_handle>(args), cardinality, is_exact);
	}
	...
	void *CopyFunction::CopyFromExecInput::GetGlobalStateInternal() const {
		void *data = nullptr;
		CheckedAPICall(duckdb_v2_copy_from_exec_get_global_state,
		               static_cast<duckdb_v2_copy_from_exec_info_handle>(args), &data);
		return RequireCopyGlobalState(data);
	}

	void *CopyFunction::CopyFromExecInput::GetLocalStateInternal() const {
		void *data = nullptr;
		CheckedAPICall(duckdb_v2_copy_from_exec_get_local_state,
		               static_cast<duckdb_v2_copy_from_exec_info_handle>(args), &data);
	...
	auto CopyFunction::CopyFromExecInput::GetOutputChunk() const -> DataChunk {
		duckdb_v2_data_chunk_handle chunk = nullptr;
		CheckedAPICall(duckdb_v2_copy_from_exec_get_output_chunk,
		               static_cast<duckdb_v2_copy_from_exec_info_handle>(args), &chunk);
		// Borrowed: the engine owns the chunk and reuses it across invocations.
		return detail::Factory::Make<DataChunk>(chunk, false);
	}
	...
	auto CastFunction::Register() -> void {
		...
		duckdb_v2_opaque opaque {info.get(), detail::TypedDelete<CastFunctionInfo>,
		                         detail::TypedEquals<CastFunctionInfo>};
	...
	auto CastFunction::ExecInput::GetInput() const -> Vector {
		duckdb_v2_vector_handle vector = nullptr;
		CheckedAPICall(duckdb_v2_cast_function_exec_get_input,
		               static_cast<duckdb_v2_cast_function_exec_info_handle>(args), &vector);
		return detail::Factory::Make<Vector>(vector);
	}

	auto CastFunction::ExecInput::GetOutput() const -> Vector {
		duckdb_v2_vector_handle vector = nullptr;
		CheckedAPICall(duckdb_v2_cast_function_exec_get_output,
		               static_cast<duckdb_v2_cast_function_exec_info_handle>(args), &vector);
		return detail::Factory::Make<Vector>(vector);
	}
	...
	auto CastFunction::ExecInput::GetMode() const -> CastMode {
		auto mode = DUCKDB_V2_CAST_MODE_NORMAL;
		CheckedAPICall(duckdb_v2_cast_function_exec_get_mode,
		               static_cast<duckdb_v2_cast_function_exec_info_handle>(args), &mode);
		return static_cast<CastMode>(mode);
	}
	...
	void *ReplacementScan::Input::GetUserDataInternal() const {
		void *user_data = nullptr;
		CheckedAPICall(duckdb_v2_replacement_scan_get_user_data,
		               static_cast<duckdb_v2_replacement_scan_info_handle>(args), &user_data);
	...
	auto ReplacementScan::Input::AddArgument(const Value &value) -> void {
		CheckedAPICall(duckdb_v2_replacement_scan_add_argument,
		               static_cast<duckdb_v2_replacement_scan_info_handle>(args), value.handle());
	}
	...
	auto ReplacementScan::Input::SetCollection(const ColumnDataCollection &collection,
		...
		CheckedAPICall(duckdb_v2_replacement_scan_set_collection,
		               static_cast<duckdb_v2_replacement_scan_info_handle>(args), collection.handle(),
		               names.empty() ? nullptr : names.data(), names.size());
	}

	auto ReplacementScan::Input::SetSubquery(std::string_view sql) -> void {
		CheckedAPICall(duckdb_v2_replacement_scan_set_subquery,
		               static_cast<duckdb_v2_replacement_scan_info_handle>(args), ToStr(sql));
	}
	...
	void Appender::Initialize(Connection &conn, const std::string &query, std::vector<LogicalType> column_types,
		...
		// The scan makes the buffer visible to the statement under its name. It is connection-scoped, so it is
		// invisible to every other connection, and it holds the buffer by shared_ptr because it outlives this object.
	...
	void Appender::Flush() {
		...
		try {
			...
			// A busy connection or an interrupted run keeps the rows so the flush can be retried; any other failure
			// drops them, so a retry does not re-run the same failing statement over the same rows.
