...
# e.g. generate_results('duckdb')
def generate_results(cli_binary, target_file_name='expected_output.txt', sources = ['lineitem', 'all_types'], modes = ['ascii', 'box', 'column', 'csv', 'duckbox', 'html', 'insert', 'json', 'jsonlines', 'latex', 'line', 'list', 'markdown', 'quote', 'table', 'tabs', 'tcl', 'trash']):
    import subprocess
    with open(target_file_name, 'w+') as f:
        for source in sources:
            ...
            for mode in modes:
                result =subprocess.run([cli_binary, '-c', f'.mode {mode}', '-c', query], capture_output=True, text=True)
                f.write(f"expected_results[{source}_source]['{mode}'] = '''")
