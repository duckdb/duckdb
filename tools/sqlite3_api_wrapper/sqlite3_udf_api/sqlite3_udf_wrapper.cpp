...
scalar_function_t SQLiteUDFWrapper::CreateSQLiteScalarFunction(scalar_sqlite_udf_t sqlite_udf, sqlite3 *db_sqlite3, void *pApp) {
	scalar_function_t udf_function = [=](DataChunk &args, ExpressionState &state, Vector &result) -> void {
		...
		SQLiteTypeValue res_sqlite_value_type = SQLiteTypeValue::NULL_VALUE; //to hold the last sqlite value type set by UDF
