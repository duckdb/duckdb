...
public class DuckDBArrowResult {
	...
	public Object Fetch() throws Exception {
		...
		if (!result_set.arrowFetch((Long) c_array_class.getMethod("memoryAddress").invoke(c_array))) {
		    return null;
		}
		Object vector_schema_root = vector_root_class.getMethod("create", schema_class, buffer_allocator_class)
				.invoke(null, schema, allocator);
