...
JNIEXPORT jobject JNICALL Java_nl_cwi_da_duckdb_DuckDBNative_duckdb_1jdbc_1execute(
	...
	auto stmt_ref = (StatementHolder*) env->GetDirectBufferAddress(
			stmt_ref_buf);
	if (!stmt_ref) {
		jclass Exception = env->FindClass("java/sql/SQLException");
...
JNIEXPORT jobject JNICALL Java_nl_cwi_da_duckdb_DuckDBNative_duckdb_1jdbc_1meta(
	...
	jmethodID meta_construct = env->GetMethodID(meta, "<init>",
			"(I[Ljava/lang/String;[Ljava/lang/String;)V");

	auto column_count = res_ref->res->names.size();

	auto name_array = (jobjectArray) env->NewObjectArray(column_count,
			env->FindClass("java/lang/String"), nullptr);
	auto type_array = (jobjectArray) env->NewObjectArray(column_count,
			env->FindClass("java/lang/String"), nullptr);

	for (idx_t col_idx = 0; col_idx < column_count; col_idx++) {
		env->SetObjectArrayElement(name_array, col_idx, env->NewStringUTF(res_ref->res->names[col_idx].c_str()));
		env->SetObjectArrayElement(type_array, col_idx, env->NewStringUTF(SQLTypeToString(res_ref->res->sql_types[col_idx]).c_str()));
	}
