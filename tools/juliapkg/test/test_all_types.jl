...
@testset "Test All Types" begin
	...
	@test isequal(df.hugeint, [-170141183460469231731687303715884105727, 170141183460469231731687303715884105727, missing])
	...
	@test isequal(df.dec38_10, [-9999999999999999999999999999.9999999999, 9999999999999999999999999999.9999999999, missing])
	@test isequal(df.dec38_10, [-9999999999999999999999999999.9999999999, 9999999999999999999999999999.9999999999, missing])
	@test isequal(df.dec38_10, [-9999999999999999999999999999.9999999999, 9999999999999999999999999999.9999999999, missing])
	...
	@test isequal(df.timestamp, [Dates.DateTime(-290308, 12, 22, 0, 0, 0), Dates.DateTime(294247, 1, 10, 4, 0, 54, 775), missing])
	@test isequal(df.timestamp_tz, [Dates.DateTime(-290308, 12, 22, 0, 0, 0), Dates.DateTime(294247, 1, 10, 4, 0, 54, 775), missing])
	@test isequal(df.timestamp_s, [Dates.DateTime(-290308, 12, 22, 0, 0, 0), Dates.DateTime(294247, 1, 10, 4, 0, 54, 0), missing])
	@test isequal(df.timestamp_ms, [Dates.DateTime(-290308, 12, 22, 0, 0, 0), Dates.DateTime(294247, 1, 10, 4, 0, 54, 775), missing])
	@test isequal(df.timestamp_ns, [Dates.DateTime(1677, 9, 21, 0, 12, 43, 146), Dates.DateTime(2262, 4, 11, 23, 47, 16, 854), missing])
	@test isequal(df.interval, [Dates.CompoundPeriod(
			Dates.Month(0),
			Dates.Day(0),
			Dates.Microsecond(0),
			...
			Dates.Month(999),
			Dates.Day(999),
			Dates.Microsecond(999999999),
		), missing])
	@test isequal(df.varchar, ["🦆🦆🦆🦆🦆🦆", "goose", missing])
	@test isequal(df.blob, [UInt8[0x74, 0x68, 0x69, 0x73, 0x69, 0x73, 0x61, 0x6c, 0x6f, 0x6e, 0x67, 0x62, 0x6c, 0x6f, 0x62, 0x00, 0x77, 0x69, 0x74, 0x68, 0x6e, 0x75, 0x6c, 0x6c, 0x62, 0x79, 0x74, 0x65, 0x73], UInt8[0x5c, 0x78, 0x30, 0x30, 0x5c, 0x78, 0x30, 0x30, 0x5c, 0x78, 0x30, 0x30, 0x61], missing])
	...
	@test isequal(df.nested_int_array, [[], [[], [42, 999, missing, missing, -42], missing, [], [42, 999, missing, missing, -42]], missing])
	@test isequal(df.struct, [(a=missing, b=missing), (a=42, b="🦆🦆🦆🦆🦆🦆"), missing])
	@test isequal(df.struct_of_arrays, [(a=missing, b=missing), (a=[42, 999, missing, missing, -42], b=["🦆🦆🦆🦆🦆🦆", "goose", missing, ""]), missing])
