...
@testset "Issue #207: 32 bit integers" begin
	setup_clean_test_db() do db
		ds =
			DBInterface.execute(db, "SELECT 42::INT64 a FROM Track LIMIT 1") |>
			columntable
...
@testset "Prepared Statements" begin
	setup_clean_test_db() do db
		...
		r =
			DBInterface.execute(db, "SELECT * FROM temp WHERE Title ILIKE ?", ["%time%"]) |>
			columntable
		@test r[1] == [76, 111, 187]
		DBInterface.execute(
			db,
		...
		r =
			DBInterface.execute(
				db,
				"SELECT * FROM temp WHERE Title ILIKE ?",
				(word = "%time%",),
			) |> columntable
		@test r[1] == [76, 111, 187]
