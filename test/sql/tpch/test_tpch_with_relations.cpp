...
TEST_CASE("Test TPC-H SF0.01 with relations", "[tpch][.]") {
	...
	// Q01
	result = lineitem->Filter("l_shipdate <= DATE '1998-09-02'")->Aggregate({
		"l_returnflag",
		"l_linestatus",
	...
	})->Order("l_returnflag, l_linestatus")->Execute();
	COMPARE_CSV(result, tpch::get_answer(sf, 1), true);

	// Q02
	auto partsupp_region = partsupp->Join(
		supplier, "s_suppkey=ps_suppkey")->Join(
		nation, "s_nationkey=n_nationkey")->Join(
	...
	result = part_join->Filter({
		"p_size=15",
		"p_type LIKE '%BRASS'",
		"r_name='EUROPE'",
		"ps_supplycost = ( SELECT min(ps_supplycost) FROM partsupp_region WHERE p_partkey = ps_partkey AND r_name = 'EUROPE')"
	})->Project({
		"s_acctbal",
		"s_name",
		"n_name",
	...
	result = cust_join->Filter({
		...
		"o_orderdate",
		"o_shippriority"
	})->Order("revenue DESC, o_orderdate")->Limit(10)->Execute();
	...
	result = lineitem->Filter({
		...
		"l_quantity < 24;"
	})->Aggregate("sum(l_extendedprice * l_discount) AS revenue")->Execute();
	COMPARE_CSV(result, tpch::get_answer(sf, 6), true);

	// Q12
	result = lineitem->Join(orders, "l_orderkey=o_orderkey")->Filter({
		"l_shipmode IN ('MAIL', 'SHIP')",
		"l_commitdate < l_receiptdate",
		"l_shipdate < l_commitdate",
		"l_receiptdate >= cast('1994-01-01' AS date)",
		"l_receiptdate < cast('1995-01-01' AS date)"
	})->Aggregate({
		"l_shipmode",
		"sum(CASE WHEN o_orderpriority = '1-URGENT' OR o_orderpriority = '2-HIGH' THEN 1 ELSE 0 END) AS high_line_count",
		"sum(CASE WHEN o_orderpriority <> '1-URGENT' AND o_orderpriority <> '2-HIGH' THEN 1 ELSE 0 END) AS low_line_count"
	})->Order("l_shipmode")->Execute();
