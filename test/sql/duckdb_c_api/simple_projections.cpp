...
TEST_CASE("Test simple projection statements", "[simpleprojection]") {
	...
	// create table
	REQUIRE(duckdb_query(connection, "CREATE TABLE a (i integer, j integer);", NULL) == DuckDBSuccess);
	// insertion: 1 affected row
	REQUIRE(duckdb_query(connection, "INSERT INTO a VALUES (42, 84);", &result) == DuckDBSuccess);
	REQUIRE(CHECK_NUMERIC(result, 0, 0, 1));
	duckdb_destroy_result(result);

	REQUIRE(duckdb_query(connection, "SELECT * FROM a;", &result) == DuckDBSuccess);
	...
	REQUIRE(duckdb_query(connection, "SELECT a, b FROM test;", &result) == DuckDBSuccess);
	...
	REQUIRE(duckdb_query(connection, "SELECT a + 2, b FROM test WHERE a = 11;", &result) == DuckDBSuccess);
	REQUIRE(CHECK_NUMERIC(result, 0, 0, 13));
	REQUIRE(CHECK_NUMERIC(result, 0, 1, 22));
	duckdb_destroy_result(result);
	
	REQUIRE(duckdb_query(connection, "SELECT a + 2, b FROM test WHERE a = 12;", &result) == DuckDBSuccess);
	REQUIRE(CHECK_NUMERIC(result, 0, 0, 14));
	REQUIRE(CHECK_NUMERIC(result, 0, 1, 21));
	duckdb_destroy_result(result);

	// select unknown column
	REQUIRE(duckdb_query(connection, "SELECT c FROM test;", &result) != DuckDBSuccess);
