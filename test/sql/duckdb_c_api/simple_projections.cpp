...
TEST_CASE("Test scalar queries", "[scalarquery]") {
	...
	REQUIRE(duckdb_query(connection, "SELECT 42 + 1", &result) == DuckDBSuccess);
	REQUIRE(CHECK_NUMERIC(result, 0, 0, 43));
	duckdb_destroy_result(result);

	REQUIRE(duckdb_query(connection, "SELECT 2 * (42 + 1), 35 - 2", &result) == DuckDBSuccess);
	...
	REQUIRE(duckdb_query(connection, "SELECT cast('3' AS INTEGER)", &result) == DuckDBSuccess);
