...
static bool CHECK_NUMERIC(duckdb_result result, duckdb_oid_t row, duckdb_oid_t column, int64_t value) {
...
static bool CHECK_STRING(duckdb_result result, duckdb_oid_t row, duckdb_oid_t column, std::string value) {
	if (result.column_count < column || result.row_count < row) {
		// out of bounds
		return false;
	}
