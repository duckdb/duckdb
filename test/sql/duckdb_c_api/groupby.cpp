...
TEST_CASE("Test aggregation/group by by statements", "[aggregations]") {
	...
	REQUIRE(duckdb_query(connection, "SELECT SUM(41), COUNT(*);", &result) == DuckDBSuccess);
	REQUIRE(CHECK_NUMERIC_COLUMN(result, 0, {41}));
	REQUIRE(CHECK_NUMERIC_COLUMN(result, 1, {1}));
	duckdb_destroy_result(result);

	REQUIRE(duckdb_query(connection, "SELECT SUM(a), COUNT(*) FROM test;", &result) == DuckDBSuccess);
	REQUIRE(CHECK_NUMERIC_COLUMN(result, 0, {36}));
	REQUIRE(CHECK_NUMERIC_COLUMN(result, 1, {3}));
	duckdb_destroy_result(result);

	REQUIRE(duckdb_query(connection, "SELECT SUM(a), COUNT(*) FROM test WHERE a = 11;", &result) == DuckDBSuccess);
	REQUIRE(CHECK_NUMERIC_COLUMN(result, 0, {11}));
	REQUIRE(CHECK_NUMERIC_COLUMN(result, 1, {1}));
	duckdb_destroy_result(result);

	REQUIRE(duckdb_query(connection, "SELECT SUM(a), SUM(b), SUM(a) + SUM (b) FROM test;", &result) == DuckDBSuccess);
	REQUIRE(CHECK_NUMERIC_COLUMN(result, 0, {36}));
	REQUIRE(CHECK_NUMERIC_COLUMN(result, 1, {65}));
	REQUIRE(CHECK_NUMERIC_COLUMN(result, 2, {101}));
	duckdb_destroy_result(result);

	REQUIRE(duckdb_query(connection, "SELECT SUM(a+2), SUM(a) + 2 * COUNT(*) FROM test;", &result) == DuckDBSuccess);
	REQUIRE(CHECK_NUMERIC_COLUMN(result, 0, {42}));
	REQUIRE(CHECK_NUMERIC_COLUMN(result, 1, {42}));
	duckdb_destroy_result(result);

	// aggregations with group by
	REQUIRE(duckdb_query(connection, "SELECT b, SUM(a), SUM(a+2) FROM test GROUP BY b ORDER BY b;", &result) == DuckDBSuccess);
	...
	REQUIRE(duckdb_query(connection, "SELECT b, SUM(a), COUNT(*), SUM(a+2) FROM test GROUP BY b ORDER BY b;", &result) == DuckDBSuccess);
	REQUIRE(CHECK_NUMERIC_COLUMN(result, 0, {21, 22}));
	REQUIRE(CHECK_NUMERIC_COLUMN(result, 1, {12, 24}));
	...
	// group by alias
	REQUIRE(duckdb_query(connection, "SELECT b % 2 AS f, SUM(a) FROM test GROUP BY f;", &result) == DuckDBSuccess);
	REQUIRE(CHECK_NUMERIC_COLUMN(result, 0, {0, 1}));
	REQUIRE(CHECK_NUMERIC_COLUMN(result, 1, {24, 12}));
	duckdb_destroy_result(result);


	//REQUIRE(duckdb_query(connection, "SELECT b, AVG(a) FROM test GROUP BY b;", &result) == DuckDBSuccess);
