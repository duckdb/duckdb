...
TEST_CASE("Test order by statements", "[orderby]") {
	...
	REQUIRE(duckdb_query(connection, "SELECT a, b FROM test ORDER BY a;",
	                     &result) == DuckDBSuccess);
	REQUIRE(CHECK_NUMERIC_COLUMN(result, 0, {11, 12, 13}));
	REQUIRE(CHECK_NUMERIC_COLUMN(result, 1, {22, 21, 22}));
	duckdb_destroy_result(result);

	REQUIRE(duckdb_query(connection, "SELECT a, b FROM test ORDER BY a DESC;",
	                     &result) == DuckDBSuccess);
	REQUIRE(CHECK_NUMERIC_COLUMN(result, 0, {13, 12, 11}));
	REQUIRE(CHECK_NUMERIC_COLUMN(result, 1, {22, 21, 22}));
	duckdb_destroy_result(result);

	REQUIRE(duckdb_query(connection, "SELECT a, b FROM test ORDER BY b, a;",
	                     &result) == DuckDBSuccess);
	...
	// REQUIRE(duckdb_query(connection, "SELECT b % 2 AS f, SUM(a) FROM test
	// GROUP BY f ORDER BY f;", &result) == DuckDBSuccess);
	// REQUIRE(CHECK_NUMERIC_COLUMN(result, 0, {0, 1}));
	// REQUIRE(CHECK_NUMERIC_COLUMN(result, 1, {24, 12}));
	// duckdb_destroy_result(result);

	// REQUIRE(duckdb_query(connection, "SELECT b % 2 AS f, SUM(a) FROM test
	// GROUP BY f ORDER BY b % 2;", &result) == DuckDBSuccess);
	// REQUIRE(CHECK_NUMERIC_COLUMN(result, 0, {0, 1}));
	// REQUIRE(CHECK_NUMERIC_COLUMN(result, 1, {24, 12}));
	// duckdb_destroy_result(result);
