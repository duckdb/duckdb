...
TEST_CASE("Test TPC-H Q1", "[tpch]") {
	...
	// TPC-H
	REQUIRE(duckdb_query(connection, "create table lineitem ( l_orderkey INTEGER NOT NULL, l_partkey "
	     ...
	     "l_shipmode CHAR(10) NOT NULL, l_comment VARCHAR(44) NOT NULL);", NULL) == DuckDBSuccess);

	REQUIRE(duckdb_query(connection, "insert into lineitem values ('1', '155190', '7706', '1', '17', "
	     "'21168.23', '0.04', '0.02', 'N', 'O', '1996-03-13', '1996-02-12', "
	     "'1996-03-22', 'DELIVER IN PERSON', 'TRUCK', 'egular courts above "
	     "the')", NULL) == DuckDBSuccess);

	// TPC-H Q1
	REQUIRE(duckdb_query(connection, "select l_returnflag, l_linestatus, sum(l_quantity) as sum_qty, sum(l_extendedprice) as sum_base_price, sum(l_extendedprice * (1 - l_discount)) as sum_disc_price, sum(l_extendedprice * (1 - l_discount) * (1 + l_tax)) as sum_charge, avg(l_quantity) as avg_qty, avg(l_extendedprice) as avg_price, avg(l_discount) as avg_disc, count(*) as count_order"
	     " from lineitem where l_shipdate <= "
	     "cast('1998-09-02' as date) group by l_returnflag, l_linestatus order "
	     "by l_returnflag, l_linestatus;", &result) == DuckDBSuccess);
	...
	//N|O|17|21168.23|20727.930816|17.0|21168.23|0.04|1
	...
	// REQUIRE(duckdb_query(connection, "select l_returnflag, l_linestatus, sum(l_quantity) as sum_qty, "
