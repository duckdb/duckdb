...
TEST_CASE("Test simple parallelism", "[parallelism]") {
	...
	REQUIRE_NO_FAIL(con.Query("CREATE VIEW v1(i) AS SELECT SUM(i) FROM integers UNION ALL SELECT AVG(i) FROM integers UNION ALL SELECT MIN(i) FROM integers UNION ALL SELECT MAX(i) FROM integers;"));
