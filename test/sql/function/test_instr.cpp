...
TEST_CASE("Instr KMP test", "[function]") {
    unique_ptr<QueryResult> result;
    DuckDB db(nullptr);
    Connection con(db);
    con.EnableQueryVerification();
    ...
    // Test first letter
    result = con.Query("SELECT instr_kmp(s,'h') FROM strings");
    REQUIRE(CHECK_COLUMN(result, 0, {1, 0, 0, Value()}));

    // Test second letter
    result = con.Query("SELECT instr_kmp(s,'e') FROM strings");
    REQUIRE(CHECK_COLUMN(result, 0, {2, 0, 0, Value()}));

    // Test last letter
    result = con.Query("SELECT instr_kmp(s,'d') FROM strings");
    REQUIRE(CHECK_COLUMN(result, 0, {0, 5, 0, Value()}));

    // Test multiple letters
    result = con.Query("SELECT instr_kmp(s,'he') FROM strings");
    REQUIRE(CHECK_COLUMN(result, 0, {1, 0, 0, Value()}));

    // Test multiple letters in the middle
    result = con.Query("SELECT instr_kmp(s,'ello') FROM strings");
    REQUIRE(CHECK_COLUMN(result, 0, {2, 0, 0, Value()}));

    // Test multiple letters at the end
    result = con.Query("SELECT instr_kmp(s,'lo') FROM strings");
    REQUIRE(CHECK_COLUMN(result, 0, {4, 0, 0, Value()}));

    // Test no match
    result = con.Query("SELECT instr_kmp(s,'he-man') FROM strings");
    REQUIRE(CHECK_COLUMN(result, 0, {0, 0, 0, Value()}));

    // Test matching needle in multiple rows
    result = con.Query("SELECT instr_kmp(s,'o'),s FROM strings");
    REQUIRE(CHECK_COLUMN(result, 0, {5, 2, 0, Value()}));
    REQUIRE(CHECK_COLUMN(result, 1, {"hello", "world", "b", Value()}));

    // Test NULL constant in different places
    result = con.Query("SELECT instr_kmp(NULL,'o') FROM strings");
    REQUIRE(CHECK_COLUMN(result, 0, {Value(), Value(), Value(), Value()}));
    result = con.Query("SELECT instr_kmp(s,NULL) FROM strings");
    REQUIRE(CHECK_COLUMN(result, 0, {Value(), Value(), Value(), Value()}));
    result = con.Query("SELECT instr_kmp(NULL,NULL) FROM strings");
    REQUIRE(CHECK_COLUMN(result, 0, {Value(), Value(), Value(), Value()}));
}

TEST_CASE("Instr KMP test with UTF8", "[function]") {
    unique_ptr<QueryResult> result;
    DuckDB db(nullptr);
    Connection con(db);
    con.EnableQueryVerification();
    ...
    // Test one matching UTF8 letter
    result = con.Query("SELECT instr_kmp(s,'\xc3\xa1') FROM strings");
    REQUIRE(CHECK_COLUMN(result, 0, {1, 3, 0, 0}));

    // Test a sentence with an UTF-8
    result = con.Query("SELECT instr_kmp(s,'ol\xc3\xa1 mundo') FROM strings");
    REQUIRE(CHECK_COLUMN(result, 0, {0, 1, 0, 0}));

    // Test an entire UTF-8 word
    result = con.Query("SELECT instr_kmp(s,'\xe4\xbd\xa0\xe5\xa5\xbd\xe4\xb8\x96\xe7\x95\x8c') FROM strings");
    REQUIRE(CHECK_COLUMN(result, 0, {0, 0, 1, 0}));

    // Test a substring of the haystack from the beginning
    result = con.Query("SELECT instr_kmp(s,'two \xc3\xb1 thr') FROM strings");
    REQUIRE(CHECK_COLUMN(result, 0, {0, 0, 0, 1}));

    // Test a single UTF8 substring of the haystack in the middle
    result = con.Query("SELECT instr_kmp(s,'\xc3\xb1') FROM strings");
    REQUIRE(CHECK_COLUMN(result, 0, {0, 0, 0, 5}));

    // Test a multiple UTF8 substring of the haystack in the middle
    result = con.Query("SELECT instr_kmp(s,'\xE2\x82\xA1 four \xF0\x9F\xA6\x86 e') FROM strings");
    REQUIRE(CHECK_COLUMN(result, 0, {0, 0, 0, 14}));

    // Test a substring of the haystack from the middle to the end
    result = con.Query("SELECT instr_kmp(s,'\xF0\x9F\xA6\x86 end') FROM strings");
    REQUIRE(CHECK_COLUMN(result, 0, {0, 0, 0, 23}));
}


TEST_CASE("Instr BM test", "[function]") {
    unique_ptr<QueryResult> result;
    DuckDB db(nullptr);
    Connection con(db);
    con.EnableQueryVerification();
    ...
    // Test first letter
    result = con.Query("SELECT instr_bm(s,'h') FROM strings");
    REQUIRE(CHECK_COLUMN(result, 0, {1, 0, 0, Value()}));

    // Test second letter
    result = con.Query("SELECT instr_bm(s,'e') FROM strings");
    REQUIRE(CHECK_COLUMN(result, 0, {2, 0, 0, Value()}));

    // Test last letter
    result = con.Query("SELECT instr_bm(s,'d') FROM strings");
    REQUIRE(CHECK_COLUMN(result, 0, {0, 5, 0, Value()}));

    // Test multiple letters
    result = con.Query("SELECT instr_bm(s,'he') FROM strings");
    REQUIRE(CHECK_COLUMN(result, 0, {1, 0, 0, Value()}));

    // Test multiple letters in the middle
    result = con.Query("SELECT instr_bm(s,'ello') FROM strings");
    REQUIRE(CHECK_COLUMN(result, 0, {2, 0, 0, Value()}));

    // Test multiple letters at the end
    result = con.Query("SELECT instr_bm(s,'lo') FROM strings");
    REQUIRE(CHECK_COLUMN(result, 0, {4, 0, 0, Value()}));

    // Test no match
    result = con.Query("SELECT instr_bm(s,'he-man') FROM strings");
    REQUIRE(CHECK_COLUMN(result, 0, {0, 0, 0, Value()}));

    // Test matching needle in multiple rows
    result = con.Query("SELECT instr_bm(s,'o'),s FROM strings");
    REQUIRE(CHECK_COLUMN(result, 0, {5, 2, 0, Value()}));
    REQUIRE(CHECK_COLUMN(result, 1, {"hello", "world", "b", Value()}));

    // Test NULL constant in different places
    result = con.Query("SELECT instr_bm(NULL,'o') FROM strings");
    REQUIRE(CHECK_COLUMN(result, 0, {Value(), Value(), Value(), Value()}));
    result = con.Query("SELECT instr_bm(s,NULL) FROM strings");
    REQUIRE(CHECK_COLUMN(result, 0, {Value(), Value(), Value(), Value()}));
    result = con.Query("SELECT instr_bm(NULL,NULL) FROM strings");
    REQUIRE(CHECK_COLUMN(result, 0, {Value(), Value(), Value(), Value()}));
}

TEST_CASE("Instr BM test with UTF8", "[function]") {
    unique_ptr<QueryResult> result;
    DuckDB db(nullptr);
    Connection con(db);
    con.EnableQueryVerification();
    ...
    // Test one matching UTF8 letter
    result = con.Query("SELECT instr_bm(s,'\xc3\xa1') FROM strings");
    REQUIRE(CHECK_COLUMN(result, 0, {1, 3, 0, 0}));

    // Test a sentence with an UTF-8
    result = con.Query("SELECT instr_bm(s,'ol\xc3\xa1 mundo') FROM strings");
    REQUIRE(CHECK_COLUMN(result, 0, {0, 1, 0, 0}));

    // Test an entire UTF-8 word
    result = con.Query("SELECT instr_bm(s,'\xe4\xbd\xa0\xe5\xa5\xbd\xe4\xb8\x96\xe7\x95\x8c') FROM strings");
    REQUIRE(CHECK_COLUMN(result, 0, {0, 0, 1, 0}));

    // Test a substring of the haystack from the beginning
    result = con.Query("SELECT instr_bm(s,'two \xc3\xb1 thr') FROM strings");
    REQUIRE(CHECK_COLUMN(result, 0, {0, 0, 0, 1}));

    // Test a single UTF8 substring of the haystack in the middle
    result = con.Query("SELECT instr_bm(s,'\xc3\xb1') FROM strings");
    REQUIRE(CHECK_COLUMN(result, 0, {0, 0, 0, 5}));

    // Test a multiple UTF8 substring of the haystack in the middle
    result = con.Query("SELECT instr_bm(s,'\xE2\x82\xA1 four \xF0\x9F\xA6\x86 e') FROM strings");
    REQUIRE(CHECK_COLUMN(result, 0, {0, 0, 0, 14}));

    // Test a substring of the haystack from the middle to the end
    result = con.Query("SELECT instr_bm(s,'\xF0\x9F\xA6\x86 end') FROM strings");
    REQUIRE(CHECK_COLUMN(result, 0, {0, 0, 0, 23}));
}
