...
TEST_CASE("Test interval addition/subtraction", "[interval]") {
	...
	REQUIRE(CHECK_COLUMN(
	    result, 0,
	    {Value::INTERVAL(
	        23, 30, 22 * Interval::MSECS_PER_HOUR + 59 * Interval::MSECS_PER_MINUTE + 30 * Interval::MSECS_PER_SEC)}));
}
...
TEST_CASE("Test storage for interval type", "[interval]") {
	...
	for (idx_t i = 0; i < 2; i++) {
		...
		REQUIRE(CHECK_COLUMN(result, 0, {Value(), Value::INTERVAL(0, 1, 0), Value::INTERVAL(3, 2, 5000)}));
...
TEST_CASE("Test various ops involving intervals", "[interval]") {
	...
	REQUIRE_NO_FAIL(con.Query("CREATE TABLE interval (t INTERVAL);"));
	REQUIRE_NO_FAIL(
	    con.Query("INSERT INTO interval VALUES (INTERVAL '20' DAY), (INTERVAL '1' YEAR), (INTERVAL '1' MONTH);"));
	...
	REQUIRE(CHECK_COLUMN(result, 0,
	                     {Value::INTERVAL(1, 0, 0), Value::INTERVAL(1, 0, 0), Value::INTERVAL(1, 0, 0),
	                      Value::INTERVAL(1, 0, 0), Value::INTERVAL(12, 0, 0)}));
	result = con.Query("SELECT * FROM interval i1 JOIN interval i2 ON (i1.t <> i2.t) ORDER BY 1");
	REQUIRE(CHECK_COLUMN(
	    result, 0,
	    {Value::INTERVAL(1, 0, 0), Value::INTERVAL(1, 0, 0), Value::INTERVAL(12, 0, 0), Value::INTERVAL(12, 0, 0)}));
	REQUIRE(CHECK_COLUMN(
	    result, 1,
	    {Value::INTERVAL(12, 0, 0), Value::INTERVAL(12, 0, 0), Value::INTERVAL(1, 0, 0), Value::INTERVAL(1, 0, 0)}));
	result = con.Query("SELECT * FROM interval i1 JOIN interval i2 ON (i1.t > i2.t) ORDER BY 1");
	...
	result = con.Query("with recursive t as (select timestamp '2020-05-02 10:00:00' as x union select x+interval '15' "
	                   "minute from t where x < timestamp '2020-05-02 12:00:00') select x::string from t order by x");
	REQUIRE(CHECK_COLUMN(result, 0,
	                     {"2020-05-02 10:00:00", "2020-05-02 10:15:00", "2020-05-02 10:30:00", "2020-05-02 10:45:00",
	                      "2020-05-02 11:00:00", "2020-05-02 11:15:00", "2020-05-02 11:30:00", "2020-05-02 11:45:00",
	                      "2020-05-02 12:00:00"}));
}
