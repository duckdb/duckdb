...
TEST_CASE("Test subqueries", "[subqueries]") {
	...
	CHECK_COLUMN(result, 0, {11 * 42});

	// operations on subquery
	result = con.Query("SELECT a*(SELECT 42) FROM test");
	CHECK_COLUMN(result, 0, {11 * 42, 12 * 42, 13 * 42});

	result = con.Query("CREATE TABLE t1(a INTEGER, b INTEGER, c INTEGER, d INTEGER, e INTEGER)");
	...
	CHECK_COLUMN(result, 0, {102 - 208, 106 - 208});

	result = con.Query("SELECT CASE WHEN c>(SELECT sum(c)/count(*) FROM t1) THEN a*2 ELSE b*10 END FROM t1");
	CHECK_COLUMN(result, 0, {1000, 214});
	// correlated subqueries
	result = con.Query("SELECT a, (SELECT SUM(b) FROM test tsub WHERE test.a=tsub.a) FROM test");
	CHECK_COLUMN(result, 0, {11, 12, 13});
	CHECK_COLUMN(result, 1, {22, 21, 22});

	result = con.Query("SELECT a, (SELECT CASE WHEN test.a=11 THEN 22 ELSE NULL END) FROM test");
	CHECK_COLUMN(result, 0, {11, 12, 13});
	CHECK_COLUMN(result, 1, {22, Value(), Value()});

	result = con.Query("SELECT a, (SELECT CASE WHEN test.a=11 THEN b ELSE NULL END FROM test tsub) FROM test");
	CHECK_COLUMN(result, 0, {11, 12, 13});
	CHECK_COLUMN(result, 1, {22, Value(), Value()});

	result = con.Query("SELECT * from test where a=(SELECT MIN(a) FROM test t WHERE t.b=test.b)");
	CHECK_COLUMN(result, 0, {11, 12});
	CHECK_COLUMN(result, 1, {22, 21});

	// exists / in / any subqueries
	result = con.Query("SELECT * FROM test WHERE EXISTS (SELECT a FROM test ts WHERE ts.a = test.a AND b>21)");
	CHECK_COLUMN(result, 0, {11, 13});
	CHECK_COLUMN(result, 1, {22, 22});
}
