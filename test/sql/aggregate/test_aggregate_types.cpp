...
TEST_CASE("Test aggregates with many different types", "[aggregate]") {
	...
	result = con.Query("SELECT g, COUNT(*), COUNT(s), MIN(s), MAX(s), STRING_AGG(s, ' ') FROM strings GROUP BY g ORDER BY g");
	...
	// empty group
	result = con.Query("SELECT g, COUNT(*), COUNT(s), MIN(s), MAX(s), STRING_AGG(s, ' ') FROM strings WHERE s IS NULL OR s <> 'hello' GROUP BY g ORDER BY g");
	REQUIRE(CHECK_COLUMN(result, 0, {0, 1}));
	...
	result = con.Query("SELECT g, COUNT(*), COUNT(b), MIN(b), MAX(b) FROM booleans WHERE b IS NULL OR b=true GROUP BY g ORDER BY g");
	REQUIRE(CHECK_COLUMN(result, 0, {0, 1}));
	...
	// more complex agg with empty groups
	result = con.Query("SELECT g, COUNT(*), COUNT(i), MIN(i), MAX(i), SUM(i) FROM integers WHERE i IS NULL OR i > 15 GROUP BY g ORDER BY g");
	REQUIRE(CHECK_COLUMN(result, 0, {0, 1}));
	REQUIRE(CHECK_COLUMN(result, 1, {1, 1}));
	REQUIRE(CHECK_COLUMN(result, 2, {0, 1}));
