...
TEST_CASE("Test union all with allow_stream_result", "[storage][.]") {
	...
	DeleteDatabase(storage_database);
	{
		...
		REQUIRE_NO_FAIL(con.Query("INSERT INTO test SELECT row_number() over () AS z, gen_random_uuid() id FROM generate_series(0, 160000)"));
		REQUIRE_NO_FAIL(con.Query("create table test_2 as from test;"));

		REQUIRE_NO_FAIL(con.Query("SET GLOBAL memory_limit='128MB';"));
		auto res = con.context->Query("SELECT distinct(test.a) as b FROM test left join test_2 on test.a = test_2.a UNION ALL SELECT a FROM test UNION ALL SELECT distinct(a) as b FROM test", 
