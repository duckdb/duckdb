...
VALUES (1.0, 3.22, 10420942.4567, 12908124908190481290481.11),
    (NULL, NULL, NULL, NULL);
...
VALUES ('hello world', '\xAA\xFF\xAA'),
    (NULL, NULL);
...
VALUES (date '1992-01-01', time '12:00:03', timestamp '1992-09-20 10:00:03'),
    (NULL, NULL, NULL);
CREATE TABLE uuid_values(u uuid);
...
CREATE VIEW cv6 AS
SELECT *
FROM generate_series(0, 6, 1) tbl(i)
INTERSECT
SELECT *
FROM generate_series(0, 4, 1) tbl(i)
EXCEPT (
        SELECT 1
        UNION ALL
        SELECT 3
...
-- v29: IGNORE NULLS
CREATE FUNCTION V29(x) AS LAST_VALUE(x IGNORE NULLS) OVER (
