...
TEST_CASE("Vectorized UDF functions using arguments", "[udf_function]") {
	...
	for(SQLType sql_type: all_sql_types) {
		...
		{
			con.CreateVectorizedFunction(func_name + "_1", {SQLType::BOOLEAN}, SQLType::BOOLEAN, &udf_unary_function<bool>);

			con.CreateVectorizedFunction(func_name + "_2", {SQLType::BOOLEAN, SQLType::BOOLEAN}, SQLType::BOOLEAN,
										 &udf_binary_function<bool>);

			con.CreateVectorizedFunction(func_name + "_3", {SQLType::BOOLEAN, SQLType::BOOLEAN, SQLType::BOOLEAN},
										 SQLType::BOOLEAN, &udf_ternary_function<bool>);
			break;
		}
		case SQLTypeId::TINYINT:
		{
			con.CreateVectorizedFunction(func_name + "_1", {SQLType::TINYINT}, SQLType::TINYINT, &udf_unary_function<int8_t>);

			con.CreateVectorizedFunction(func_name + "_2", {SQLType::TINYINT, SQLType::TINYINT}, SQLType::TINYINT,
										 &udf_binary_function<int8_t>);

			con.CreateVectorizedFunction(func_name + "_3", {SQLType::TINYINT, SQLType::TINYINT, SQLType::TINYINT},
                                         SQLType::TINYINT, &udf_ternary_function<int8_t>);
			break;
		}
		case SQLTypeId::SMALLINT:
		{
			con.CreateVectorizedFunction(func_name + "_1", {SQLType::SMALLINT}, SQLType::SMALLINT, &udf_unary_function<int16_t>);

			con.CreateVectorizedFunction(func_name + "_2", {SQLType::SMALLINT, SQLType::SMALLINT}, SQLType::SMALLINT,
										 &udf_binary_function<int16_t>);

			con.CreateVectorizedFunction(func_name + "_3", {SQLType::SMALLINT, SQLType::SMALLINT, SQLType::SMALLINT},
										 SQLType::SMALLINT, &udf_ternary_function<int16_t>);
			break;
		}
		case SQLTypeId::DATE:
		{
			con.CreateVectorizedFunction(func_name + "_1", {SQLType::DATE}, SQLType::DATE, &udf_unary_function<date_t>);

			con.CreateVectorizedFunction(func_name + "_2", {SQLType::DATE, SQLType::DATE}, SQLType::DATE,
										 &udf_binary_function<date_t>);

			con.CreateVectorizedFunction(func_name + "_3", {SQLType::DATE, SQLType::DATE, SQLType::DATE},
										 SQLType::DATE, &udf_ternary_function<date_t>);
			break;
		}
		case SQLTypeId::TIME:
		{
			con.CreateVectorizedFunction(func_name + "_1", {SQLType::TIME}, SQLType::TIME, &udf_unary_function<dtime_t>);

			con.CreateVectorizedFunction(func_name + "_2", {SQLType::TIME, SQLType::TIME}, SQLType::TIME,
										 &udf_binary_function<dtime_t>);

			con.CreateVectorizedFunction(func_name + "_3", {SQLType::TIME, SQLType::TIME, SQLType::TIME},
										 SQLType::TIME, &udf_ternary_function<dtime_t>);
			break;
		}
		case SQLTypeId::INTEGER:
		{
			con.CreateVectorizedFunction(func_name + "_1", {SQLType::INTEGER}, SQLType::INTEGER, &udf_unary_function<int32_t>);

			con.CreateVectorizedFunction(func_name + "_2", {SQLType::INTEGER, SQLType::INTEGER}, SQLType::INTEGER,
										 &udf_binary_function<int32_t>);

			con.CreateVectorizedFunction(func_name + "_3", {SQLType::INTEGER, SQLType::INTEGER, SQLType::INTEGER},
										 SQLType::INTEGER, &udf_ternary_function<int32_t>);
			break;
		}
		case SQLTypeId::BIGINT:
		{
			con.CreateVectorizedFunction(func_name + "_1", {SQLType::BIGINT}, SQLType::BIGINT, &udf_unary_function<int64_t>);

			con.CreateVectorizedFunction(func_name + "_2", {SQLType::BIGINT, SQLType::BIGINT}, SQLType::BIGINT,
										 &udf_binary_function<int64_t>);

			con.CreateVectorizedFunction(func_name + "_3", {SQLType::BIGINT, SQLType::BIGINT, SQLType::BIGINT},
										 SQLType::BIGINT, &udf_ternary_function<int64_t>);
			break;
		}
		case SQLTypeId::TIMESTAMP:
		{
			con.CreateVectorizedFunction(func_name + "_1", {SQLType::TIMESTAMP}, SQLType::TIMESTAMP, &udf_unary_function<timestamp_t>);

			con.CreateVectorizedFunction(func_name + "_2", {SQLType::TIMESTAMP, SQLType::TIMESTAMP}, SQLType::TIMESTAMP,
										 &udf_binary_function<timestamp_t>);

			con.CreateVectorizedFunction(func_name + "_3", {SQLType::TIMESTAMP, SQLType::TIMESTAMP, SQLType::TIMESTAMP},
										 SQLType::TIMESTAMP, &udf_ternary_function<timestamp_t>);
			break;
		}
		...
		{ 	con.CreateVectorizedFunction(func_name + "_1", {SQLType::VARCHAR}, SQLType::VARCHAR, &udf_unary_function<char *>);
			...
			con.CreateVectorizedFunction(func_name + "_2", {SQLType::VARCHAR, SQLType::VARCHAR}, SQLType::VARCHAR,
										 &udf_binary_function<char *>);

			con.CreateVectorizedFunction(func_name + "_3", {SQLType::VARCHAR, SQLType::VARCHAR, SQLType::VARCHAR},
									     SQLType::VARCHAR, &udf_ternary_function<char *>);
			break;
		}
		case SQLTypeId::BLOB:
		{
			con.CreateVectorizedFunction(func_name + "_1", {SQLType::BLOB}, SQLType::BLOB, &udf_unary_function<char *>);

			con.CreateVectorizedFunction(func_name + "_2", {SQLType::BLOB, SQLType::BLOB}, SQLType::BLOB,
										 &udf_binary_function<char *>);

			con.CreateVectorizedFunction(func_name + "_3", {SQLType::BLOB, SQLType::BLOB, SQLType::BLOB},
										 SQLType::BLOB, &udf_ternary_function<char *>);
			break;
		}
		default:
			break;
	...
	SECTION("Cheking Vectorized UDF functions with several input columns") {
		vector<SQLType> sql_args = {SQLType::INTEGER, SQLType::INTEGER, SQLType::INTEGER, SQLType::INTEGER};
		// UDF with 4 input ints, return the last one
		con.CreateVectorizedFunction("udf_four_ints", sql_args, SQLType::INTEGER, &udf_several_constant_input<int, 4>);
		result = con.Query("SELECT udf_four_ints(1, 2, 3, 4)");
		REQUIRE(CHECK_COLUMN(result, 0, {4}));

		// UDF with 5 input ints, return the last one
		sql_args.push_back(SQLType::INTEGER);
		con.CreateVectorizedFunction("udf_five_ints", sql_args, SQLType::INTEGER, &udf_several_constant_input<int, 5>);
		...
		con.CreateVectorizedFunction("udf_ten_ints", sql_args, SQLType::INTEGER, &udf_several_constant_input<int, 10>);
	...
	SECTION("Cheking Vectorized UDF functions with varargs and constant values") {
		...
		con.CreateVectorizedFunction("udf_const_max_int", {SQLType::INTEGER}, SQLType::INTEGER, &udf_max_constant<int>, SQLType::INTEGER);
		...
		con.CreateVectorizedFunction("udf_const_max_double", {SQLType::DOUBLE}, SQLType::DOUBLE, &udf_max_constant<double>, SQLType::DOUBLE);
	...
	SECTION("Cheking Vectorized UDF functions with varargs and input columns") {
		...
		con.CreateVectorizedFunction("udf_flat_max_int", {SQLType::INTEGER}, SQLType::INTEGER, &udf_max_flat<int>, SQLType::INTEGER);
		...
		con.CreateVectorizedFunction("udf_flat_max_double", {SQLType::DOUBLE}, SQLType::DOUBLE, &udf_max_flat<double>, SQLType::DOUBLE);
