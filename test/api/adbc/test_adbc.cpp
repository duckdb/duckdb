...
TEST_CASE("ADBC Main Driver Manager", "[adbc]") {
	...
	REQUIRE(AdbcDatabaseSetOption(&adbc_database, "driver", "../../build/debug/src/libduckdb.dylib", &adbc_error) == ADBC_STATUS_OK);
	REQUIRE(AdbcDatabaseSetOption(&adbc_database, "entrypoint", "duckdb_adbc_init", &adbc_error) == ADBC_STATUS_OK);
	REQUIRE(AdbcDatabaseInit(&adbc_database, &adbc_error) == ADBC_STATUS_OK);
