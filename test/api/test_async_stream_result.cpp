...
//! Install a default notifier that also counts the notifications it delivers
void SetCountingNotifier(Connection &con, NotifyListener &listener, std::atomic<idx_t> &count) {
	auto &config = ClientConfig::GetConfig(*con.context);
	config.default_notify_callback = [&listener, &count]() {
		count++;
		listener.Notify();
	};
}
...
//! inside an engine call. Wrap engine calls in in_call to arm the check.
struct StackCheckListener {
	...
	//! Install as the connection default notifier; records the calling thread as the consumer
	void Install(Connection &con) {
		consumer_thread = std::this_thread::get_id();
		auto &config = ClientConfig::GetConfig(*con.context);
		config.default_notify_callback = [this]() {
			notification_count++;
...
// Wake-ups: a parked consumer is always woken, and never on its own stack

TEST_CASE("Async stream result wakes a parked consumer", "[api][async_stream]") {
	DuckDB db(nullptr);
	Connection con(db);
	REQUIRE_NO_FAIL(con.Query("SET streaming_buffer_size='100KB'"));
...
TEST_CASE("Async batched stream result wakes a parked consumer", "[api][async_stream]") {
	...
	REQUIRE(state.notification_count.load() >= (ROW_COUNT / STANDARD_VECTOR_SIZE) / 2);
}
...
TEST_CASE("Async stream result wakes a parked consumer on interrupt", "[api][async_stream]") {
	DuckDB db(nullptr);
	Connection con(db);
	REQUIRE_NO_FAIL(con.Query("SET streaming_buffer_size='100KB'"));
...
TEST_CASE("The connection default notifier observes completion without fetching", "[api][async_stream]") {
	DuckDB db(nullptr);
	Connection con(db);
...
TEST_CASE("Async stream results refuse a single-threaded database", "[api][async_stream]") {
	DuckDB db(nullptr);
	Connection con(db);
...
TEST_CASE("Async stream results refuse an external-threads-only scheduler", "[api][async_stream]") {
	DuckDB db(nullptr);
	Connection con(db);
...
TEST_CASE("Async stream results require a streaming result", "[api][async_stream]") {
	DuckDB db(nullptr);
	Connection con(db);
...
TEST_CASE("Sync queries ignore the connection default notifier", "[api][async_stream]") {
	DuckDB db(nullptr);
	Connection con(db);
...
TEST_CASE("Async stream results refuse a threadless build", "[api][async_stream]") {
	DuckDB db(nullptr);
	Connection con(db);
