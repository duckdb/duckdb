...
TEST_CASE("Test external threads", "[api]") {
	...
	REQUIRE(DBConfig::GetSetting<ExternalThreadsSetting>(config) == 13);
	REQUIRE(db.NumberOfThreads() == 13);

	con.Query("SET external_threads=0");
	REQUIRE(DBConfig::GetSetting<ExternalThreadsSetting>(config) == 0);
	REQUIRE(db.NumberOfThreads() == 13);
	...
	REQUIRE(DBConfig::GetSetting<ExternalThreadsSetting>(config) == 5);
	REQUIRE(db.NumberOfThreads() == 13);

	con.Query("RESET external_threads");
	REQUIRE(DBConfig::GetSetting<ExternalThreadsSetting>(config) == 1);
	REQUIRE(db.NumberOfThreads() == 13);
