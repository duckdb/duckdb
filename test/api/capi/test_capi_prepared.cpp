...
TEST_CASE("Test prepared statements in C API", "[capi]") {
	...
	status = duckdb_execute_prepared(stmt, &res);
	REQUIRE(status == DuckDBSuccess);
	//! Have to use 'as_properties' because the result is not of type DECIMAL
	duckdb_decimal result_decimal = duckdb_value_decimal_as_properties(&res, 0, 0, 7, 4);
	REQUIRE(result_decimal.scale == 4);
	REQUIRE(result_decimal.width == 7);
	REQUIRE(result_decimal.value.upper == 0);
	REQUIRE(result_decimal.value.lower == 6340000);
	duckdb_destroy_result(&res);
