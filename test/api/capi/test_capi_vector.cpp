...
void copy_data_chunk_using_vector_copy_sel(duckdb_data_chunk src, duckdb_data_chunk dst) {
	...
	duckdb_selection_vector incr_sel_vector = duckdb_create_selection_vector(src_size);
...
TEST_CASE("Test duckdb_vector_copy_sel to copy data_chunk", "[capi]") {
	CAPITester tester;
...
void reference_data_chunk_using_vector_reference_vector(duckdb_data_chunk src, duckdb_data_chunk dst,
	...
	for (idx_t i = 0; i < ref_len; i++) {
		...
		duckdb_vector src_vector = duckdb_data_chunk_get_vector(src, idx);
		duckdb_vector dst_vector = duckdb_data_chunk_get_vector(dst, i);
...
TEST_CASE("Test duckdb_data_chunk_reference_columns using duckdb_vector_reference_vector", "[capi]") {
	CAPITester tester;
	duckdb::unique_ptr<CAPIResult> result;
	REQUIRE(tester.OpenDatabase(nullptr));
