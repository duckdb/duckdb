...
void AddNumbersTogether(duckdb_function_info info, duckdb_data_chunk input, duckdb_vector output) {
	...
	// extract all input vectors and have fun
	auto result_data = (int64_t *)duckdb_vector_get_data(output);
	for (idx_t row = 0; row < input_size; row++) {
		result_data[row] = duckdb_data_chunk_get_column_count(input);
	}
...
//	duckdb_scalar_function_add_parameter(function, type);
	...
	duckdb_logical_type varargs_type = duckdb_create_logical_type(DUCKDB_TYPE_ANY);
	duckdb_scalar_function_set_varargs(function, varargs_type);
	duckdb_destroy_logical_type(&varargs_type);
...
//	CAPIRegisterAddition(tester.connection, "my_addition", DuckDBError);
	...
	// now call it
	result = tester.Query("SELECT my_addition(40, 'hello', 42, 3)");
