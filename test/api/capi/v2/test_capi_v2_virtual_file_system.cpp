...
void MemCanHandle(duckdb_v2_virtual_file_system_info_handle, duckdb_v2_str path, bool *can_handle,
                  duckdb_v2_error_info_handle *) {
	auto p = Convert(path);
	*can_handle = p.rfind(SCHEME, 0) == 0;
}
...
struct MemFsOptions {
	...
	bool route_by_callback = false; // can_handle instead of a prefix
};
...
void RegisterMemFs(duckdb_v2_connection_handle conn, MemFs &fs, const MemFsOptions &options = MemFsOptions(),
	...
	if (options.route_by_callback) {
		REQUIRE(duckdb_v2_virtual_file_system_set_can_handle_callback(vfs, MemCanHandle, nullptr) ==
		        DUCKDB_V2_ERROR_NONE);
	} else {
...
TEST_CASE("V2 virtual file system: registration requires a name, routing and the core callbacks", "[capi_v2][vfs]") {
	...
	expect_failure("prefix or a can handle callback");
...
TEST_CASE("V2 virtual file system: a can handle callback routes paths without a prefix", "[capi_v2][vfs]") {
	EnvFixture fx;
	MemFs mem;
...
// ---------------------------------------------------------------------------
// Delegation: an overlay that forwards to whatever file system claims the path underneath
// ---------------------------------------------------------------------------
...
// An overlay that claims the very paths of what it wraps, and reaches it through the file system beneath.
void ShadowOpen(duckdb_v2_virtual_file_system_info_handle info, duckdb_v2_virtual_file_open_info_handle open_info,
                duckdb_v2_error_info_handle *err) {
	auto &overlay = OverlayOf(info, err);
	duckdb_v2_str path {nullptr, 0};
	duckdb_v2_virtual_file_open_info_get_path(open_info, &path, err);
	duckdb_v2_file_system_handle beneath = nullptr;
	if (duckdb_v2_virtual_file_open_info_get_file_system_beneath(open_info, &beneath, err) != DUCKDB_V2_ERROR_NONE) {
		return;
	}
	duckdb_v2_file_open_options_handle options = nullptr;
	if (duckdb_v2_virtual_file_open_info_get_options(open_info, &options, err) != DUCKDB_V2_ERROR_NONE) {
	...
	duckdb_v2_file_handle file = nullptr;
	auto rc = duckdb_v2_file_system_open(beneath, path, options, &file, err);
	duckdb_v2_file_open_options_destroy(&options);
	if (rc != DUCKDB_V2_ERROR_NONE) {
		return;
	}
...
void ShadowStatPath(duckdb_v2_virtual_file_system_info_handle info, duckdb_v2_virtual_file_path_info_handle path_info,
                    duckdb_v2_file_stat_handle stat, duckdb_v2_error_info_handle *err) {
	OverlayOf(info, err).stats++;
	duckdb_v2_str path {nullptr, 0};
	duckdb_v2_virtual_file_path_info_get_path(path_info, &path, err);
	duckdb_v2_file_system_handle beneath = nullptr;
	if (duckdb_v2_virtual_file_path_info_get_file_system_beneath(path_info, &beneath, err) != DUCKDB_V2_ERROR_NONE) {
		return;
	}
	duckdb_v2_file_stat_handle underneath = nullptr;
	if (duckdb_v2_file_system_stat(beneath, path, &underneath, err) != DUCKDB_V2_ERROR_NONE) {
		return;
	}
	DUCKDB_V2_FILE_TYPE type = DUCKDB_V2_FILE_TYPE_INVALID;
	duckdb_v2_file_stat_get_type(underneath, &type, err);
	duckdb_v2_file_stat_set_type(stat, type, err);
	// What the file system beneath knew by path comes through.
	idx_t size = 0;
	bool known = false;
	duckdb_v2_file_stat_get_size(underneath, &size, &known, err);
	if (known) {
...
void ShadowList(duckdb_v2_virtual_file_system_info_handle info, duckdb_v2_virtual_file_path_info_handle path_info,
                duckdb_v2_file_listing_handle list, duckdb_v2_error_info_handle *err) {
	OverlayOf(info, err).lists++;
	duckdb_v2_str path {nullptr, 0};
	duckdb_v2_virtual_file_path_info_get_path(path_info, &path, err);
	...
	duckdb_v2_file_listing_handle underneath = nullptr;
	if (duckdb_v2_file_system_list(beneath, path, &underneath, err) != DUCKDB_V2_ERROR_NONE) {
		return;
	}
	idx_t count = 0;
	duckdb_v2_file_listing_get_entry_count(underneath, &count, err);
...
void RegisterShadow(duckdb_v2_connection_handle conn, Overlay &overlay) {
	duckdb_v2_virtual_file_system_handle vfs = nullptr;
	REQUIRE(duckdb_v2_virtual_file_system_create_with_connection(conn, &vfs, nullptr) == DUCKDB_V2_ERROR_NONE);
	REQUIRE(duckdb_v2_virtual_file_system_set_name(vfs, Convert("shadow"), nullptr) == DUCKDB_V2_ERROR_NONE);
	duckdb_v2_opaque user_data {&overlay, nullptr, nullptr};
	REQUIRE(duckdb_v2_virtual_file_system_set_user_data(vfs, &user_data, nullptr) == DUCKDB_V2_ERROR_NONE);
	// The same prefix as the in-memory file system: this one shadows it.
	REQUIRE(duckdb_v2_virtual_file_system_add_prefix(vfs, Convert(SCHEME), nullptr) == DUCKDB_V2_ERROR_NONE);
	REQUIRE(duckdb_v2_virtual_file_system_set_open_callback(vfs, ShadowOpen, nullptr) == DUCKDB_V2_ERROR_NONE);
	REQUIRE(duckdb_v2_virtual_file_system_set_read_at_callback(vfs, OverlayReadAt, nullptr) == DUCKDB_V2_ERROR_NONE);
	REQUIRE(duckdb_v2_virtual_file_system_set_stat_callback(vfs, OverlayStat, nullptr) == DUCKDB_V2_ERROR_NONE);
	REQUIRE(duckdb_v2_virtual_file_system_set_stat_path_callback(vfs, ShadowStatPath, nullptr) == DUCKDB_V2_ERROR_NONE);
	REQUIRE(duckdb_v2_virtual_file_system_set_list_callback(vfs, ShadowList, nullptr) == DUCKDB_V2_ERROR_NONE);
	REQUIRE(duckdb_v2_virtual_file_system_register(vfs, nullptr) == DUCKDB_V2_ERROR_NONE);
	duckdb_v2_virtual_file_system_destroy(&vfs);
}
...
TEST_CASE("V2 virtual file system: an in-place overlay reaches the file system it shadows", "[capi_v2][vfs]") {
	EnvFixture fx;
	MemFs mem;
	mem.files["mem://data/a.csv"] = CSV_A;
	mem.files["mem://data/b.csv"] = CSV_B;
	RegisterMemFs(fx.conn, mem);
	Overlay overlay;
	RegisterShadow(fx.conn, overlay);

	// The same path as before now goes through the overlay first, and through the in-memory file system beneath.
	auto rows = VfsQueryStrings(fx.conn, "SELECT s FROM read_csv('mem://data/a.csv') ORDER BY i");
	REQUIRE(rows == std::vector<std::string> {"one", "two"});
	REQUIRE(overlay.opens > 0);
	REQUIRE(mem.opens > 0);
	...
	// Consumer-side stat routes to the overlay, whose stat passes the size the file system beneath knew by path.
	auto fs = VfsEngineFs(fx.conn);
	duckdb_v2_file_stat_handle stat = nullptr;
	REQUIRE(duckdb_v2_file_system_stat(fs, Convert("mem://data/b.csv"), &stat, nullptr) == DUCKDB_V2_ERROR_NONE);
	idx_t size = 0;
	bool known = false;
...
TEST_CASE("V2 file system: path operations on the local file system", "[capi_v2][vfs][file_system]") {
	...
	// Stat by path sees a regular file.
