...
TEST_CASE("UDF functions with arguments", "[udf_function]") {
	...
	for (LogicalType sql_type : all_sql_types) {
		...
		case LogicalTypeId::BLOB: {
			con.CreateScalarFunction<string_t, string_t>(func_name + "_1", {LogicalType::BLOB}, LogicalType::BLOB,
			                                             &udf_varchar);

			con.CreateScalarFunction<string_t, string_t, string_t>(
			    func_name + "_2", {LogicalType::BLOB, LogicalType::BLOB}, LogicalType::BLOB, &udf_varchar);

			con.CreateScalarFunction<string_t, string_t, string_t, string_t>(
			    func_name + "_3", {LogicalType::BLOB, LogicalType::BLOB, LogicalType::BLOB}, LogicalType::BLOB,
			    &udf_varchar);
			break;
		}
