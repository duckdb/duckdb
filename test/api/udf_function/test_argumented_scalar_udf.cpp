...
TEST_CASE("UDF functions with arguments", "[udf_function]") {
	...
	const vector<LogicalTypeId> all_sql_types = {LogicalTypeId::BOOLEAN, LogicalTypeId::TINYINT, LogicalTypeId::SMALLINT,
											 LogicalTypeId::DATE, LogicalTypeId::TIME, LogicalTypeId::INTEGER,
											 LogicalTypeId::BIGINT, LogicalTypeId::TIMESTAMP, LogicalTypeId::FLOAT,
											 LogicalTypeId::DOUBLE, LogicalTypeId::DECIMAL, LogicalTypeId::VARCHAR,
											 LogicalTypeId::BLOB};
	...
	for(LogicalType sql_type: all_sql_types) {
		...
		case LogicalTypeId::DATE:
		{
			...
			con.CreateScalarFunction<date_t, date_t, date_t, date_t>(func_name + "_3",
																   {LogicalType::DATE, LogicalType::DATE, LogicalType::DATE},
																   LogicalType::DATE,
																   &udf_date);
			break;
		}
		case LogicalTypeId::TIME:
		{
			...
			con.CreateScalarFunction<dtime_t, dtime_t, dtime_t, dtime_t>(func_name + "_3",
																   {LogicalType::TIME, LogicalType::TIME, LogicalType::TIME},
																   LogicalType::TIME,
																   &udf_time);
			break;
		}
...
//		}
		...
		case LogicalTypeId::BLOB:
		{
			...
			con.CreateScalarFunction<string_t, string_t, string_t, string_t>(func_name + "_3",
																	   {LogicalType::BLOB, LogicalType::BLOB, LogicalType::BLOB},
																	   LogicalType::BLOB,
																	   &udf_varchar);
			break;
		}
