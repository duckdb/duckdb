...
public:
	static constexpr const ResultUnitType TYPE = ResultUnitType::EXTENSION;

public:
	TestUnit() : ResultUnit(ResultUnitType::EXTENSION) {
	}
...
public:
	...
	void Append(ResultFormatGlobalState &gstate, ResultFormatLocalState &lstate_p, DataChunk &chunk) override {
		...
		if (!lstate.unit) {
			lstate.unit = make_uniq<TestUnit>();
			lstate.unit->producer = std::this_thread::get_id();
		}
		auto copy = BufferedData::CopyForBuffering(chunk);
		lstate.rows += copy->size();
		lstate.bytes += copy->GetDataSize();
		lstate.unit->chunks.push_back(std::move(copy));
	}
	...
	unique_ptr<ResultUnit> Finish(ResultFormatGlobalState &gstate, ResultFormatLocalState &lstate_p) override {
		...
		auto unit = std::move(lstate.unit);
		unit->row_count = lstate.rows;
		unit->byte_size = lstate.bytes;
...
//! A second extension format, so a mismatch between two formats that share ResultUnitType::EXTENSION
//! can be tested. It never produces a unit
