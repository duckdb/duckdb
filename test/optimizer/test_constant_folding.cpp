...
TEST_CASE("Constant folding does something", "[optimizer]") {
	...
	unique_ptr<AbstractExpression> root = make_unique<OperatorExpression>(ExpressionType::OPERATOR_ADD, TypeId::INTEGER, move(left), move(right));
...
TEST_CASE("Constant folding finishes in fixpoint", "[optimizer]") {
	...
	unique_ptr<AbstractExpression> il = make_unique<OperatorExpression>(ExpressionType::OPERATOR_ADD, TypeId::INTEGER, move(ll), move(lr));
	auto ir = make_unique<ConstantExpression>(Value(10));
	unique_ptr<AbstractExpression> root = make_unique<OperatorExpression>(ExpressionType::OPERATOR_ADD, TypeId::INTEGER, move(il), move(ir));
