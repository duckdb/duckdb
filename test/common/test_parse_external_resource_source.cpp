...
TEST_CASE("Parse ATTACH TO NEW TEMPORARY EXTERNAL RESOURCE + ToString roundtrip", "[parse_external_resource]") {
	Parser parser(TestParserOptions());
	parser.ParseQuery(
	...
	Parser reparser(TestParserOptions());
...
TEST_CASE("ATTACH TO EXTERNAL RESOURCE ToString stays reparseable", "[parse_external_resource]") {
	...
	Parser ignore_parser(TestParserOptions());
	...
	Parser replace_parser(TestParserOptions());
...
TEST_CASE("CONNECT TO EXTERNAL RESOURCE option lists", "[parse_external_resource]") {
	...
	Parser provision(TestParserOptions());
	...
	Parser borrow(TestParserOptions());
	...
	Parser borrow_again(TestParserOptions());
	...
	Parser rejected(TestParserOptions());
	REQUIRE_THROWS(
