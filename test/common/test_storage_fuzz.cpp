...
// Regression test: RevertCommit hits D_ASSERT(IsMainTable()) in RemoveFromIndexes.
//
// Scenario:
//   Tx A executes, in order:
//     1. DELETE from table t1 (has PRIMARY KEY) → DELETE_TUPLE undo entry on old t1
//     2. CREATE OR REPLACE TABLE t1            → CATALOG_ENTRY undo entries
//     3. DELETE from table t2 (has PRIMARY KEY) → DELETE_TUPLE undo entry on old t2
//   Then, Tx B executes CREATE OR REPLACE TABLE t2, dropping old t2.
//
//   During CommitEntry processing of Tx A:
//     - Entry 1 (DELETE_TUPLE on old t1): succeeds, t1 is still MAIN_TABLE
//     - Entry 2 (CATALOG_ENTRY):         CommitDrop → SetAsDropped() marks old t1 as DROPPED
//     - Entry 3 (DELETE_TUPLE on old t2): !IsMainTable() → throws TransactionException
//
//   RevertCommit then processes entries 1 and 2:
//     - Entry 1 (DELETE_TUPLE on old t1): calls CommitDelete → RemoveFromIndexes
//       But old t1 is now DROPPED (from entry 2) → D_ASSERT(IsMainTable()) fails.
//
// The fix: IndexDataRemover::PushDelete skips index removal for dropped/altered tables.
TEST_CASE("revert commit index entries after CREATE OR REPLACE", "[storage][.]") {
	duckdb::DBConfig config;

	auto path = TestCreatePath("revert_commit_index_replace");
	duckdb::DeleteDatabase(path);
	duckdb::DuckDB db(path, &config);

	// Create two tables with PRIMARY KEY indexes.
	{
		duckdb::Connection con(db);
		REQUIRE_NO_FAIL(con.Query("CREATE TABLE t1 (id INTEGER PRIMARY KEY, val INTEGER)"));
		REQUIRE_NO_FAIL(con.Query("INSERT INTO t1 SELECT i, i * 10 FROM range(1000) tbl(i)"));
		REQUIRE_NO_FAIL(con.Query("CREATE TABLE t2 (id INTEGER PRIMARY KEY, val INTEGER)"));
		REQUIRE_NO_FAIL(con.Query("INSERT INTO t2 SELECT i, i * 10 FROM range(1000) tbl(i)"));
	}

	// Tx A: begin transaction, do DELETE + CREATE OR REPLACE on t1, then DELETE from t2.
	// DELETE on existing main-table rows creates DELETE_TUPLE directly in the undo buffer
	// (no local storage involved), ensuring undo_buffer.Commit() processes these entries.
	{
		duckdb::Connection con_a(db);
		REQUIRE_NO_FAIL(con_a.Query("BEGIN TRANSACTION"));

		// Step 1: DELETE from t1 → DELETE_TUPLE on old t1 DataTable (still MAIN_TABLE).
		REQUIRE_NO_FAIL(con_a.Query("DELETE FROM t1 WHERE id < 500"));

		// Step 2: CREATE OR REPLACE t1 → CATALOG_ENTRY entries that will SetAsDropped() old t1 at commit.
		REQUIRE_NO_FAIL(con_a.Query("CREATE OR REPLACE TABLE t1 (id INTEGER PRIMARY KEY, val INTEGER)"));

		// Step 3: DELETE from t2 → DELETE_TUPLE on old t2 DataTable (t2 is still MAIN_TABLE).
		REQUIRE_NO_FAIL(con_a.Query("DELETE FROM t2 WHERE id < 500"));

		// Tx B: replace t2 so old t2 DataTable is marked DROPPED.
		{
			duckdb::Connection con_b(db);
			REQUIRE_NO_FAIL(con_b.Query("CREATE OR REPLACE TABLE t2 (id INTEGER PRIMARY KEY, val INTEGER)"));
		}

		// Tx A tries to commit. Undo buffer processing (in order):
		//   1. DELETE_TUPLE on old t1 → CommitEntry succeeds (old t1 is still MAIN_TABLE)
		//   2. CATALOG_ENTRY entries for CREATE OR REPLACE t1:
		//      - The DROP entry calls CommitDrop → SetAsDropped() on old t1
		//   3. DELETE_TUPLE on old t2 → !IsMainTable() (Tx B dropped it) → TransactionException
		//
		// RevertCommit processes entries 1 and 2:
		//   Entry 1: DELETE_TUPLE on old t1 → CommitDelete → RemoveFromIndexes
		//   But old t1 is now DROPPED (from entry 2) → would crash without the fix.
		auto result = con_a.Query("COMMIT");
		// The commit fails because old t2 was dropped by Tx B.
