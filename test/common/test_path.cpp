...
TEST_CASE("Path::Equals / EqualsCI / EqualsCanonical", "[path]") {
	// clang-format off

	// Equals: field-by-field, case-sensitive — file: variants are NOT equal
	...
	// EqualsCI: case-insensitive (for Windows local + UNC)
	...
	// EqualsCanonical: collapses file: variants, then compares — all local forms are equal
	CHECK( Path::FromString("/a/b").EqualsCanonical(Path::FromString("file:/a/b")));
	CHECK( Path::FromString("/a/b").EqualsCanonical(Path::FromString("file:///a/b")));
	CHECK( Path::FromString("/a/b").EqualsCanonical(Path::FromString("file://localhost/a/b")));
	CHECK( Path::FromString("file:/a/b").EqualsCanonical(Path::FromString("file:///a/b")));
	CHECK(!Path::FromString("/a/b").EqualsCanonical(Path::FromString("/a/c")));
	CHECK(!Path::FromString("/a/b").EqualsCanonical(Path::FromString("s3://bucket/a/b"))); // remote != local

	// EqualsCanonical: relative paths throw
	CHECK_THROWS(Path::FromString("a/b").ToCanonical());

	// FromString with canonicalize=true
	CHECK(Path::FromString("file:/a/b",           true).ToString() == "/a/b");
	CHECK(Path::FromString("file:///a/b",         true).ToString() == "/a/b");
	CHECK(Path::FromString("file://localhost/a/b",true).ToString() == "/a/b");
	CHECK(Path::FromString("/a/b",                true).ToString() == "/a/b"); // no-op
	CHECK(Path::FromString("s3://bucket/a/b",     true).ToString() == "s3://bucket/a/b"); // remote: no-op
	CHECK_THROWS(Path::FromString("a/b",          true));                                 // relative: throws

#if defined(_WIN32)
	// \\?\ extended-length local → strip prefix
	CHECK(Path::FromString(R"(\\?\C:\foo\bar)",         true).ToString() == R"(C:\foo\bar)");
	// \\?\UNC\ extended UNC → regular UNC
	CHECK(Path::FromString(R"(\\?\UNC\server\share\p)", true).ToString() == R"(\\server\share\p)");
	// regular UNC and drive paths are already canonical
	CHECK(Path::FromString(R"(\\server\share\p)",       true).ToString() == R"(\\server\share\p)");
	CHECK(Path::FromString(R"(C:\foo\bar)",             true).ToString() == R"(C:\foo\bar)");
#endif
	// clang-format on
}
