...
TEST_CASE("Date parsing works", "[date]") {
	...
	REQUIRE(Date::ToString(Date::FromString(Date::Format(1992, 1, 1))) ==
	        Date::Format(1992, 1, 1));
	REQUIRE(Date::ToString(Date::FromString(Date::Format(1992, 10, 10))) ==
	        Date::Format(1992, 10, 10));
	REQUIRE(Date::ToString(Date::FromString(Date::Format(1992, 9, 20))) ==
	        Date::Format(1992, 9, 20));
	REQUIRE(Date::ToString(Date::FromString(Date::Format(1992, 12, 31))) ==
		...
		for (int month = 1; month <= 12; month++) {
			for (int day = 1; day <= 31; day++) {
				if (Date::IsValidDay(year, month, day)) {
					REQUIRE(Date::ToString(Date::FromString(
