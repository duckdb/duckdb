...
TEST_CASE("Arrow filter pushdown - setting enables pushdown with view types", "[arrow]") {
	DuckDB db;
	Connection con(db);

	REQUIRE(!con.Query("CREATE TABLE src AS SELECT i AS id, i::VARCHAR AS name FROM range(10) tbl(i)")->HasError());

	SECTION("Default (false): pushdown disabled for view types") {
		auto factory = MakeArrowFactory(con, "SELECT * FROM src", true);
		auto explain_str = GetExplainForFilter(con, *factory, "id > 5");
		REQUIRE(StandaloneFilter(explain_str));
		REQUIRE(!FilterInScan(explain_str));
	...
	SECTION("Setting true: pushdown enabled even with view types") {
		REQUIRE(!con.Query("SET arrow_filter_pushdown_with_view_types = true")->HasError());
		auto factory = MakeArrowFactory(con, "SELECT * FROM src", true);
		auto explain_str = GetExplainForFilter(con, *factory, "id > 5");
		// With the setting enabled, filter should be pushed into the scan
		REQUIRE(!StandaloneFilter(explain_str));
		REQUIRE(FilterInScan(explain_str));
	}

	SECTION("Toggle back to false: pushdown disabled again") {
		REQUIRE(!con.Query("SET arrow_filter_pushdown_with_view_types = true")->HasError());
		REQUIRE(!con.Query("SET arrow_filter_pushdown_with_view_types = false")->HasError());
		auto factory = MakeArrowFactory(con, "SELECT * FROM src", true);
		auto explain_str = GetExplainForFilter(con, *factory, "id > 5");
		REQUIRE(StandaloneFilter(explain_str));
		REQUIRE(!FilterInScan(explain_str));
...
TEST_CASE("Arrow filter pushdown - setting management", "[arrow]") {
	DuckDB db;
	Connection con(db);

	auto result = con.Query("SELECT current_setting('arrow_filter_pushdown_with_view_types')");
	REQUIRE(!result->HasError());
	REQUIRE((result->GetValue(0, 0).ToString() == "false"));

	REQUIRE(!con.Query("SET arrow_filter_pushdown_with_view_types = true")->HasError());
	result = con.Query("SELECT current_setting('arrow_filter_pushdown_with_view_types')");
	REQUIRE(!result->HasError());
	REQUIRE((result->GetValue(0, 0).ToString() == "true"));
