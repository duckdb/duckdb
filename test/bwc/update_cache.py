...
parser.add_argument('--run-id', dest='run_id', required=True,
                    help='GitHub Actions run ID to download cache artifacts from')
parser.add_argument('--test-utils-dir', dest='test_utils_dir', required=True,
                    help='Path to the test-utils repository')
parser.add_argument('--version', dest='version',
                    help='Update cache for a specific version only')
parser.add_argument('--repo', dest='repo', default='duckdb/duckdb',
                    help='GitHub repository (default: duckdb/duckdb)')
parser.add_argument('--commit', dest='commit', action='store_true',
                    help='Commit the updated cache files in test-utils')
...
if __name__ == '__main__':
    ...
    if args.commit:
        ...
        subprocess.run(
            ['git', '-C', args.test_utils_dir, 'commit', '-m', f'Update BWC cache for {version_str}'],
            check=True
        )
