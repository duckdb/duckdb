...
bool TestResultHelper::CheckStatementResult(const Statement &statement, ExecuteContext &context,
		...
		if (result.HasError() && !statement.expected_error.empty()) {
			if (!StringUtil::Contains(result.GetError(), statement.expected_error)) {
				...
				if (!success) {
					...
					// `./build/debug/test/unittest --on-init "SET max_memory='400kb';" test/fuzzer/pedro/concurrent_catalog_usage.test`
	...
	if (error) {
		if (expected_result == ExpectedResult::RESULT_SUCCESS && SkipErrorMessage(result.GetError()) && SkipLoggingSameError(statement.file_name)) {
			runner.finished_processing_file = true;
			return true;
		}
