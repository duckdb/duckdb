...
int main(int argc, char **argv) {
	...
	*/
	...
	if (pEngine == 0) {
		...
		for (i = 0; i < nEngine; i++)
			fprintf(stdout, " %s", apEngine[i]->zName);
		fprintf(stdout, "\n");
		exit(1);
	}
	...
	** once. */
			...
			** from the set "TIR". */
			for (k = 0; (c = sScript.azToken[1][k]) != 0; k++) {
				if (c != 'T' && c != 'I' && c != 'R') {
					fprintf(
					    stderr,
					    "%s:%d: unknown type character '%c' in type string\n",
	...
	if (verifyMode || nErr || nSkipped) {
		fprintf(stderr, "%d errors out of %d tests in %s - %d skipped.", nErr,
		        nCmd, zScriptFile, nSkipped);
		if (zParam)
			fprintf(stderr, " [%s]", zParam);
		fprintf(stderr, "\n");
	}
	free(zScript);
	return nErr;
