...
FROM (
    ...
    INNER JOIN main.lineitem AS ref_7 ON (ref_6.o_totalprice = ref_7.l_extendedprice)
    ON ((ref_6.o_orderstatus IS NULL)
            OR (1))
        INNER JOIN main.partsupp AS ref_8 ON (ref_2.n_nationkey = ref_8.ps_partkey)
        ON ((((1)
                        OR (0))
                    AND (((1)
