INSERT INTO main.customer
    VALUES (14, DEFAULT, DEFAULT, 76, CAST(coalesce(CAST(NULL AS VARCHAR), CAST(NULL AS VARCHAR)) AS VARCHAR), DEFAULT, CASE WHEN EXISTS (
                SELECT
                    ref_0.c_phone AS c0, ref_0.c_address AS c1, CASE WHEN ref_0.c_address IS NOT NULL THEN
                        ref_0.c_phone
                    ELSE
                        ref_0.c_phone
                    END AS c2, ref_0.c_comment AS c3
                FROM
            ...
            OR ((EXISTS (
                        SELECT
                            ref_2.n_nationkey AS c0, ref_0.c_custkey AS c1, ref_0.c_nationkey AS c2
                        FROM
