...
DuckDBLoaderFactory::Create${TABLENAME}Loader() {
	auto table = context->db.catalog.GetTable(context->ActiveTransaction(), schema, "${TABLEINDB}" + suffix);
	return new DuckDB${TABLENAME}Load(table, context);
}
""".replace("${TABLENAME}", get_tablename(table)).replace("${ROW_TYPE}", table.upper().replace(' ', '_') + '_ROW').replace("${TABLEINDB}", table.replace(' ', '')))
