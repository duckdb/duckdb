...
CDM::CDM( CDMSUTInterface *pSUT, CBaseLogger *pLogger, const DataFileManager &dfm, TIdent iConfiguredCustomerCount, TIdent iActiveCustomerCount, INT32 iScaleFactor, INT32 iDaysOfInitialTrades, UINT32 UniqueId )
    ...
    , m_Securities(dfm.SecurityFile())
    , m_Companies(dfm.CompanyFile())
...
CDM::CDM( CDMSUTInterface *pSUT, CBaseLogger *pLogger, const DataFileManager &dfm, TIdent iConfiguredCustomerCount, TIdent iActiveCustomerCount, INT32 iScaleFactor, INT32 iDaysOfInitialTrades, UINT32 UniqueId, RNGSEED RNGSeed )
    : m_DriverGlobalSettings ( iConfiguredCustomerCount, iActiveCustomerCount, iScaleFactor, iDaysOfInitialTrades )
...
void CDM::DoTxn( void )
{
    ...
    case 4: // CUSTOMER_TAXRATE
        ...
        break;
    case 5: // DAILY_MARKET
        m_Securities.CreateSymbol( GenerateRandomSecurityId(), m_TxnInput.symbol, static_cast<int>(sizeof( m_TxnInput.symbol )));
...
void CDM::DoCleanupTxn( void )
{
    ...
    m_CleanupTxnInput.start_trade_id = (TTrade)(( m_DriverGlobalSettings.cur.iDaysOfInitialTrades * (TTrade)HoursPerWorkDay * (TTrade)SecondsPerHour * ( m_DriverGlobalSettings.cur.iConfiguredCustomerCount / m_DriverGlobalSettings.cur.iScaleFactor )) * iAbortTrade / 100 ) + 1 + iTTradeShift;  // 1.01 to account for rollbacks, +1 to get first runtime trade
