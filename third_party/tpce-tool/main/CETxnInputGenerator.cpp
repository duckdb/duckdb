...
*           not applicable.
*/
CCETxnInputGenerator::CCETxnInputGenerator( const DataFileManager &dfm,
    ...
    , m_pCompanies(dfm.CompanyFile())
    , m_pSecurities(dfm.SecurityFile())
    , m_pIndustries(dfm.IndustryDataFile())
    , m_pSectors(dfm.SectorDataFile())
...
void CCETxnInputGenerator::GenerateBrokerVolumeInput(TBrokerVolumeTxnInput &TxnReq)
{
    ...
    do
    {
        ...
        if (i == iCount)    //make sure brokers are distinct
        {
            ...
            m_Brokers.GenerateBrokerName(B_ID[iCount], TxnReq.broker_list[iCount], static_cast<int>(sizeof(TxnReq.broker_list[iCount])));
            ++iCount;
        }
    ...
    strncpy(TxnReq.sector_name,
        m_pSectors[iSectorIndex].SC_NAME_CSTR(),
...
void CCETxnInputGenerator::GenerateMarketWatchInput(TMarketWatchTxnInput &TxnReq)
{
    ...
    if (iThreshold <= m_pDriverCETxnSettings->MW_settings.cur.by_industry)
    {
        ...
        strncpy(TxnReq.industry_name,
            m_pIndustries[m_rnd.RndIntRange(0, m_iIndustryCount-1)].IN_NAME_CSTR(),
            sizeof(TxnReq.industry_name));

        if( iBaseCompanyCount < m_iActiveCompanyCount )
        {
            TxnReq.starting_co_id = m_rnd.RndInt64Range( m_iStartFromCompany,
                                                        m_iStartFromCompany +
...
void CCETxnInputGenerator::GenerateTradeOrderInput(TTradeOrderTxnInput &TxnReq, INT32 &iTradeType, bool &bExecutorIsAccountOwner)
{
    ...
    else
    {
        ...
        m_pCompanies.CreateName( m_pSecurities.GetCompanyIndex( iFlatFileSymbIndex ), TxnReq.co_name, static_cast<int>(sizeof( TxnReq.co_name )));

        strncpy(TxnReq.issue, m_pSecurities.GetRecord(iFlatFileSymbIndex).S_ISSUE_CSTR(),
                sizeof(TxnReq.issue));
    }
