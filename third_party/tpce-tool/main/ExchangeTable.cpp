...
void CExchangeTable::ComputeNumSecurities(TIdent iCustomerCount) {
	...
	const int iSecurityCounts[4][11] = {
	...
	for (int i = 0; i < 4; i++) {
		securityCount[i] = iSecurityCounts[i][10] * numLU_Tens +
		                   iSecurityCounts[i][numLU_Ones];
	}
}

CExchangeTable::CExchangeTable(const ExchangeDataFile_t &dataFile,
...
void CExchangeTable::LoadTableRow() {
	...
	strncpy(tableRow.EX_DESC, dataRecord.EX_DESC_CSTR(),
	        sizeof(tableRow.EX_DESC));
	strncpy(tableRow.EX_ID, dataRecord.EX_ID_CSTR(), sizeof(tableRow.EX_ID));
	strncpy(tableRow.EX_NAME, dataRecord.EX_NAME_CSTR(),
	        sizeof(tableRow.EX_NAME));
