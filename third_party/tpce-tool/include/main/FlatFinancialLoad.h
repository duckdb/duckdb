  ...
  public:
	CFlatFinancialLoad(char *szFileName, FlatFileOutputModes FlatFileOutputMode)
	    : CFlatFileLoader<FINANCIAL_ROW>(szFileName, FlatFileOutputMode),
	      FinancialRowFmt(
	          "%" PRId64
	          "|%d|%d|%s|%.2f|%.2f|%.2f|%.2f|%.2f|%.2f|%.2f|%.2f|%" PRId64
	          "|%" PRId64 "\n"){};
	...
	void WriteNextRecord(const FINANCIAL_ROW &next_record) {
		...
		int rc =
		    fprintf(hOutFile, FinancialRowFmt.c_str(), next_record.FI_CO_ID,
		            next_record.FI_YEAR, next_record.FI_QTR,
		            Flat_FI_QTR_START_DATE.ToStr(FlatFileDateFormat),
		            next_record.FI_REVENUE, next_record.FI_NET_EARN,
		            next_record.FI_BASIC_EPS, next_record.FI_DILUT_EPS,
		            next_record.FI_MARGIN, next_record.FI_INVENTORY,
		            next_record.FI_ASSETS, next_record.FI_LIABILITY,
		            next_record.FI_OUT_BASIC, next_record.FI_OUT_DILUT);
		if (rc < 0) {
