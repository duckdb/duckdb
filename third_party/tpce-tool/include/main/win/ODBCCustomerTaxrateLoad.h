...
class CODBCCustomerTaxRateLoad : public CDBLoader<CUSTOMER_TAXRATE_ROW> {
  public:
	CODBCCustomerTaxRateLoad(char *szServer, char *szDatabase,
	                         char *szLoaderParams,
	                         char *szTable = "CUSTOMER_TAXRATE")
	    : CDBLoader<CUSTOMER_TAXRATE_ROW>(szServer, szDatabase, szLoaderParams,
	                                      szTable){};

	virtual void BindColumns() {
		// Binding function we have to implement.
		int i = 0;
		if (bcp_bind(m_hdbc, (BYTE *)&m_row.CX_C_ID, 0, SQL_VARLEN_DATA, NULL,
		             0, IDENT_BIND, ++i) != SUCCEED ||
		    bcp_bind(m_hdbc, (BYTE *)&m_row.CX_TX_ID, 0, SQL_VARLEN_DATA,
