...
class CODBCSettlementLoad : public CDBLoader<SETTLEMENT_ROW> {
  ...
  public:
	CODBCSettlementLoad(char *szServer, char *szDatabase, char *szLoaderParams,
	                    char *szTable = "SETTLEMENT")
	    : CDBLoader<SETTLEMENT_ROW>(szServer, szDatabase, szLoaderParams,
	                                szTable){};

	void BindColumns() {
		// Binding function we have to implement.
		int i = 0;
		if (bcp_bind(m_hdbc, (BYTE *)&m_row.SE_T_ID, 0, SQL_VARLEN_DATA, NULL,
		             0, IDENT_BIND, ++i) != SUCCEED ||
		    bcp_bind(m_hdbc, (BYTE *)&m_row.SE_CASH_TYPE, 0, SQL_VARLEN_DATA,
		             (BYTE *)"", 1, SQLCHARACTER, ++i) != SUCCEED ||
		    bcp_bind(m_hdbc, (BYTE *)&ODBC_SE_CASH_DUE_DATE, 0, SQL_VARLEN_DATA,
		             NULL, 0, SQLDATETIME, ++i) != SUCCEED ||
		    bcp_bind(m_hdbc, (BYTE *)&m_row.SE_AMT, 0, SQL_VARLEN_DATA, NULL, 0,
		             SQLFLT8, ++i) != SUCCEED)
