...
class CODBCNewsXRefLoad : public CDBLoader<NEWS_XREF_ROW> {
  public:
	CODBCNewsXRefLoad(char *szServer, char *szDatabase, char *szLoaderParams,
	                  char *szTable = "NEWS_XREF")
	    : CDBLoader<NEWS_XREF_ROW>(szServer, szDatabase, szLoaderParams,
	                               szTable){};

	virtual void BindColumns() {
		// Binding function we have to implement.
		int i = 0;
		if (bcp_bind(m_hdbc, (BYTE *)&m_row.NX_NI_ID, 0, SQL_VARLEN_DATA, NULL,
		             0, IDENT_BIND, ++i) != SUCCEED ||
		    bcp_bind(m_hdbc, (BYTE *)&m_row.NX_CO_ID, 0, SQL_VARLEN_DATA, NULL,
		             0, IDENT_BIND, ++i) != SUCCEED)
