...
class CODBCCashTransactionLoad : public CDBLoader<CASH_TRANSACTION_ROW> {
  ...
  public:
	CODBCCashTransactionLoad(char *szServer, char *szDatabase,
	                         char *szLoaderParams,
	                         char *szTable = "CASH_TRANSACTION")
	    : CDBLoader<CASH_TRANSACTION_ROW>(szServer, szDatabase, szLoaderParams,
	                                      szTable){};

	virtual void BindColumns() {
		// Binding function we have to implement.
		int i = 0;
		if (bcp_bind(m_hdbc, (BYTE *)&m_row.CT_T_ID, 0, SQL_VARLEN_DATA, NULL,
		             0, IDENT_BIND, ++i) != SUCCEED ||
		    bcp_bind(m_hdbc, (BYTE *)&ODBC_CT_DTS, 0, SQL_VARLEN_DATA, NULL, 0,
		             SQLDATETIME, ++i) != SUCCEED ||
		    bcp_bind(m_hdbc, (BYTE *)&m_row.CT_AMT, 0, SQL_VARLEN_DATA, NULL, 0,
		             SQLFLT8, ++i) != SUCCEED ||
		    bcp_bind(m_hdbc, (BYTE *)&m_row.CT_NAME, 0, SQL_VARLEN_DATA,
