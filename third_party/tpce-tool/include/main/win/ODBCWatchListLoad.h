...
class CODBCWatchListLoad : public CDBLoader<WATCH_LIST_ROW> {
  public:
	CODBCWatchListLoad(char *szServer, char *szDatabase, char *szLoaderParams,
	                   char *szTable = "WATCH_LIST")
	    : CDBLoader<WATCH_LIST_ROW>(szServer, szDatabase, szLoaderParams,
	                                szTable){};

	virtual void BindColumns() {
		// Binding function we have to implement.
		int i = 0;
		if (bcp_bind(m_hdbc, (BYTE *)&m_row.WL_ID, 0, SQL_VARLEN_DATA, NULL, 0,
		             IDENT_BIND, ++i) != SUCCEED ||
		    bcp_bind(m_hdbc, (BYTE *)&m_row.WL_C_ID, 0, SQL_VARLEN_DATA, NULL,
