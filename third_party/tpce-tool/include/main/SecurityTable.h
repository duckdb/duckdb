...
class CSecurityTable : public TableTemplate<SECURITY_ROW> {
	...
	void GenerateSecurityRow() {
		...
		strncpy(m_row.S_ST_ID,
		        m_SecurityFile.GetRecord(m_iLastRowNumber).S_ST_ID_CSTR(),
		        sizeof(m_row.S_ST_ID));
		strncpy(m_row.S_ISSUE,
		        m_SecurityFile.GetRecord(m_iLastRowNumber).S_ISSUE_CSTR(),
		        sizeof(m_row.S_ISSUE));

		CreateName(m_iLastRowNumber, m_row.S_NAME,
		           static_cast<int>(sizeof(m_row.S_NAME)));

		m_SecurityFile.CreateSymbol(m_iLastRowNumber, m_row.S_SYMB,
		                            static_cast<int>(sizeof(m_row.S_SYMB)));
		strncpy(m_row.S_EX_ID,
		        m_SecurityFile.GetRecord(m_iLastRowNumber).S_EX_ID_CSTR(),
		        sizeof(m_row.S_EX_ID));
		...
		iStartDayNo = m_rnd.RndIntRange(
		    m_iJan1_1900DayNo, m_iJan2_2000DayNo); // generate random date
		...
		m_row.S_52WK_HIGH = (float)m_rnd.RndDoubleIncrRange(
		    fMinSecPrice + ((fMaxSecPrice - fMinSecPrice) / 2), fMaxSecPrice,
		    0.01);
		i52HighDayNo = m_rnd.RndIntRange(
		    m_iCurrentDayNo - iDaysPerWeek * iWeeksPerYear, m_iCurrentDayNo);
		m_row.S_52WK_HIGH_DATE.Set(i52HighDayNo);

		// 52 week low - selected from the minimum security price up to the 52wk
		// high
		m_row.S_52WK_LOW = (float)m_rnd.RndDoubleIncrRange(
		    fMinSecPrice, m_row.S_52WK_HIGH, 0.01);
		i52LowDayNo = m_rnd.RndIntRange(
		    m_iCurrentDayNo - iDaysPerWeek * iWeeksPerYear, m_iCurrentDayNo);
		m_row.S_52WK_LOW_DATE.Set(i52LowDayNo);

		if (m_rnd.RndPercent(iPercentCompaniesWithNonZeroDividend)) {
			m_row.S_YIELD =
			    m_rnd.RndDoubleIncrRange(fS_YIELDNonZeroMin, fS_YIELDMax, 0.01);
			m_row.S_DIVIDEND = m_rnd.RndDoubleIncrRange(
			    (m_row.S_YIELD * 0.20), (m_row.S_YIELD * 0.30), 0.01);
		} else {
	...
	void InitNextLoadUnit() {
		m_rnd.SetSeed(m_rnd.RndNthElement(RNGSeedTableDefault,
		                                  (RNGSEED)m_iLastRowNumber *
		                                      iRNGSkipOneRowSecurity));

		ClearRecord(); // this is needed for EGenTest to work
	}

  public:
	CSecurityTable(const DataFileManager &dfm, TIdent iCustomerCount,
	               TIdent iStartFromCustomer)
	    : TableTemplate<SECURITY_ROW>(), m_CompanyFile(dfm.CompanyFile()),
	      m_SecurityFile(dfm.SecurityFile()) {
		m_iCurrentDayNo = CDateTime::YMDtoDayno(
		    InitialTradePopulationBaseYear, InitialTradePopulationBaseMonth,
		    InitialTradePopulationBaseDay); // last initial trading date
		m_iJan1_1900DayNo = CDateTime::YMDtoDayno(
		    1900, 1, 1); // find out number of days for Jan-1-1900
		m_iJan2_2000DayNo = CDateTime::YMDtoDayno(
		    2000, 1, 2); // find out number of days for Jan-2-2000
	...
	void
	CreateName(TIdent iIndex,     // row number
	           char *szOutput,    // output buffer
	           size_t iOutputLen) // size of the output buffer (including null)
	{
		char CompanyName[cCO_NAME_len + 1];

		m_CompanyFile.CreateName(m_SecurityFile.GetCompanyIndex(iIndex),
		                         CompanyName,
		                         static_cast<int>(sizeof(CompanyName)));
		snprintf(szOutput, iOutputLen, "%s of %s",
		         m_SecurityFile.GetRecord(iIndex).S_ISSUE_CSTR(), CompanyName);
	}
};
