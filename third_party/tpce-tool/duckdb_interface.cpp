...
// see: http://hstore.cs.brown.edu/wordpress/wp-content/uploads/2011/05/TPCE-v0.32.2g.pdf

void CTradeOrderDBInterface::DoTradeOrderFrame1(const TTradeOrderFrame1Input *pIn, TTradeOrderFrame1Output *pOut) {
	auto id = pIn->acct_id;

	con.Query("BEGIN TRANSACTION");
	auto result = con.Query("select CA_NAME, CA_B_ID, CA_C_ID, CA_TAX_ST, C_F_NAME, C_L_NAME, C_TIER, C_TAX_ID, B_NAME from CUSTOMER_ACCOUNT, CUSTOMER, BROKER where CA_C_ID=C_ID AND CA_B_ID=B_ID AND CA_ID = " + to_string(id));
...
void CTradeOrderDBInterface::DoTradeOrderFrame2(const TTradeOrderFrame2Input *pIn, TTradeOrderFrame2Output *pOut) {
	auto result = con.Query("select AP_ACL from ACCOUNT_PERMISSION where AP_CA_ID = " + to_string(pIn->acct_id) + " and AP_F_NAME = " + string(pIn->exec_f_name) + " and AP_L_NAME = " + string(pIn->exec_l_name) + " and AP_TAX_ID = " + string(pIn->exec_tax_id));
	if (result->size() > 0) {
...
void CTradeOrderDBInterface::DoTradeOrderFrame3(const TTradeOrderFrame3Input *pIn, TTradeOrderFrame3Output *pOut) {
	...
	if (pIn->symbol[0] == '\0') {
		// empty symbol
		auto result = con.Query("select S_EX_ID, S_NAME, S_SYMB from COMPANY, SECURITY where S_CO_ID=CO_ID AND CO_NAME = " + string(pIn->co_name) + " AND S_ISSUE=" + string(pIn->issue));
		assert(result->size() == 1);
		...
		auto result = con.Query("select CO_NAME, S_EX_ID, S_NAME from SECURITY, COMPANY where CO_ID = S_CO_ID AND S_SYMB = " + symbol);
		assert(result->size() == 1);
	...
	// if this this a market-order, then we need to set the requested_price to the current market price.
    ...
    result = con.Query("select HS_QTY from HOLDING_SUMMARY where HS_CA_ID = " + to_string(pIn->acct_id) + " and HS_S_SYMB = " + symbol);
    ...
    if (pOut->type_is_sell) {
    	...
    	if (hs_qty > 0) {
    		if (pIn->is_lifo) {
				...
				result = con.Query("select H_QTY, H_PRICE from HOLDING where H_CA_ID = " + to_string(pIn->acct_id) + " and H_S_SYMB = " + symbol + " order by H_DTS desc");
    		} else {
    			// Estimates will be based on closing oldest holdings
    			// Could return 0, 1 or many rows
    			result = con.Query("select H_QTY, H_PRICE from HOLDING where H_CA_ID = "+ to_string(pIn->acct_id) + " and H_S_SYMB = " + symbol + " order by H_DTS asc");
    		}
    	...
    	if (hs_qty < 0) {
			...
			// (if any) and then open a new long position for the remaining balance // of this transaction.
    	}
    }
	...
	if ((sell_value > buy_value) &&
		...
		auto result = con.Query("select sum(TX_RATE) from TAXRATE where TX_ID in ( select CX_TX_ID from CUSTOMER_TAXRATE where CX_C_ID = " + to_string(pIn->cust_id) + ")");
		pOut->tax_amount = (sell_value - buy_value) * result->GetValue<double>(0, 0);
	}
	// Get administrative fees (e.g. trading charge, commision rate)
	result = con.Query("select CR_RATE from COMMISSION_RATE where CR_C_TIER = " + to_string(pIn->cust_tier) + " and CR_TT_ID = " + string(pIn->trade_type_id) + " and CR_EX_ID = " + to_string(exch_id) + " and CR_FROM_QTY <= " + to_string(pIn->trade_qty) + " and CR_TO_QTY >= " + to_string(pIn->trade_qty));
	pOut->comm_rate = result->GetValue<double>(0, 0);

	result = con.Query("select CH_CHRG from CHARGE where CH_C_TIER = " + to_string(pIn->cust_tier) + " and CH_TT_ID = " + string(pIn->trade_type_id));
	...
	if (pIn->type_is_margin) {
		...
		result = con.Query("select sum(HS_QTY * LT_PRICE) from HOLDING_SUMMARY, LAST_TRADE where HS_CA_ID = acct_id and LT_S_SYMB = HS_S_SYMB");
...
void CTradeOrderDBInterface::DoTradeOrderFrame4(const TTradeOrderFrame4Input *pIn, TTradeOrderFrame4Output *pOut) {
	...
	con.Query("insert into TRADE (T_DTS, T_ST_ID, T_TT_ID, T_IS_CASH, T_S_SYMB, T_QTY, T_BID_PRICE, T_CA_ID, T_EXEC_NAME, T_TRADE_PRICE, T_CHRG, T_COMM, T_TAX, T_LIFO) VALUES (NOW(), " + string(pIn->status_id) + ", " + string(pIn->trade_type_id) + ", " + to_string(pIn->is_cash) + ", " + string(pIn->symbol) + ", " + to_string(pIn->trade_qty) + ", " + to_string(pIn->requested_price) + ", " + to_string(pIn->acct_id) + ", " + string(pIn->exec_name) + ", NULL, " + to_string(pIn->charge_amount) + ", " + to_string(pIn->comm_amount) + ", 0, " + to_string(pIn->is_lifo) + ");");
	if (!pIn->type_is_market) {
		//con.Query("INSERT INTO TRADE_REQUEST (TR_T_ID, TR_TT_ID, TR_S_SYMB, TR_QTY, TR_BID_PRICE, TR_CA_ID) VALUES ()");
