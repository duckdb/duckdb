...
public:
	...
	static inline CHIMP_TYPE Load(uint8_t flag, uint8_t leading_zeros[], uint32_t &leading_zero_index,
	...
	static inline CHIMP_TYPE DecompressValue(uint8_t flag, uint8_t leading_zeros[],
	   ...
	   DecompressState& state) {
		...
		//static thread_local CHIMP_TYPE flag_count[4] = {0};
		//flag_count[flag]++;
		//printf("FLAG[%u] - total_count: %llu\n", (uint32_t)flag, (CHIMP_TYPE)flag_count[flag]);
		...
		case TRAILING_EXCEEDS_THRESHOLD: {
			...
			//static thread_local CHIMP_TYPE total_bitcount_read = 0;
			//static thread_local CHIMP_TYPE bits_to_read_count[65] = {0};
			//static thread_local CHIMP_TYPE total_read_count = 0;
