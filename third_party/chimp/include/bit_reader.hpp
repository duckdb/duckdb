...
public:
	...
	inline uint8_t InnerReadByte(const uint8_t& offset) {
		uint8_t result = input[ByteIndex() + offset] << BitIndex() | ((input[ByteIndex() + offset + 1] & remainder_masks[8 + BitIndex()]) >> (8 - BitIndex()));
		return result;
	}
	...
	inline uint8_t InnerRead(const uint8_t &size, const uint8_t &offset) {
		...
		// We bit-wise AND these together (no need to shift anything because the index is essentially zero for this new byte)
