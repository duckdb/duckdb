...
int64 SecondsSinceCommonEra(const DateTime &time) {
	...
	assert(time.day >= 1 &&
	       time.day <= (month == 2 && IsLeapYear(year) ? kDaysInMonth[month] + 1 : kDaysInMonth[month]));
...
std::string FormatTime(int64 seconds, int32 nanos) {
	...
	std::string result = StringPrintf("%04d-%02d-%02dT%02d:%02d:%02d", time.year, time.month, time.day, time.hour,
