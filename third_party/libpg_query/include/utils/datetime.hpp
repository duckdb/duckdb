...
typedef struct PGTimeZoneAbbrevTable
{
	PGSize		tblsize;		/* size in bytes of PGTimeZoneAbbrevTable */
	int			numabbrevs;		/* number of entries in abbrevs[] array */
	PGdatetkn		abbrevs[FLEXIBLE_ARRAY_MEMBER];
	/* DynamicZoneAbbrev(s) may follow the abbrevs[] array */
} PGTimeZoneAbbrevTable;

/* auxiliary data for a dynamic time zone abbreviation (non-fixed-offset) */
typedef struct PGDynamicZoneAbbrev
{
...
/* TMODULO()
 * Like FMODULO(), but work on the timestamp datatype (either int64_t or float8).
...
extern void GetCurrentTimeUsec(struct pg_tm * tm, pg_fsec_t *fsec, int *tzp);
...
extern int DecodeDateTime(char **field, int *ftype,
			   int nf, int *dtype,
			   struct pg_tm * tm, pg_fsec_t *fsec, int *tzp);
extern int	DecodeTimezone(char *str, int *tzp);
extern int DecodeTimeOnly(char **field, int *ftype,
			   int nf, int *dtype,
			   struct pg_tm * tm, pg_fsec_t *fsec, int *tzp);
extern int DecodeInterval(char **field, int *ftype, int nf, int range,
			   int *dtype, struct pg_tm * tm, pg_fsec_t *fsec);
...
extern int DetermineTimeZoneAbbrevOffsetTS(PGTimestampTz ts, const char *abbr,
								pg_tz *tzp, int *isdst);

extern void EncodeDateOnly(struct pg_tm * tm, int style, char *str);
extern void EncodeTimeOnly(struct pg_tm * tm, pg_fsec_t fsec, bool print_tz, int tz, int style, char *str);
extern void EncodeDateTime(struct pg_tm * tm, pg_fsec_t fsec, bool print_tz, int tz, const char *tzn, int style, char *str);
extern void EncodeInterval(struct pg_tm * tm, pg_fsec_t fsec, int style, char *str);
extern void EncodeSpecialTimestamp(PGTimestamp dt, char *str);
...
extern PGNode *TemporalTransform(int32_t max_precis, PGNode *node);

extern bool CheckDateTokenTables(void);

extern PGTimeZoneAbbrevTable *ConvertTimeZoneAbbrevs(struct pg_tzEntry *abbrevs,
					   int n);
extern void InstallTimeZoneAbbrevs(PGTimeZoneAbbrevTable *tbl);

extern PGDatum pg_timezone_abbrevs(PG_FUNCTION_ARGS);
extern PGDatum pg_timezone_names(PG_FUNCTION_ARGS);
