...
int mk_s_web_order_lineitem(void *pDest, ds_key_t kIndex) {
	...
	if (!bInit) {
		memset(&g_s_web_order_lineitem, 0,
		       sizeof(struct S_WEB_ORDER_LINEITEM_TBL));
		strtodec(&dMin, "1.00");
 ...
 * TODO: None
 */
int vld_s_web_order_lineitem(int nTable, ds_key_t kRow, int *Permutation) {
	...
	CS_PRICING_QUANTITY); for (i = 1; i < nLineitem; i++)
	 {
	     mk_detail(NULL, 0);
	 }
	mk_detail(NULL, 1);
