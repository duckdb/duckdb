 ...
 **/
static void mk_detail(int i, int bPrint) {
	int nTemp;
...
static void mk_master(void *pDest, ds_key_t kIndex) {
	...
	if (nGiftPct > WS_GIFT_PCT)
		r->kShipCustomerID = mk_join(S_WORD_SHIP_CUSTOMER_ID, CUSTOMER, 2);
	else
	...
	gen_text(&r->szComment[0], (int)(RS_S_WORD_COMMENT * 0.6),
	         RS_S_WORD_COMMENT, S_WORD_COMMENT);
...
int vld_s_web_order(int nTable, ds_key_t kRow, int *bPermutation) {
	int nLineitem, i;
