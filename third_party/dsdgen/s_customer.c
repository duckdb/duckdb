...
int mk_s_customer(void *pDest, ds_key_t kIndex) {
	...
	static date_t dtMin, dtMax, dtBirthMin, dtBirthMax, dtToday, dt1YearAgo,
	    dt10YearsAgo;
	...
	bitmap_to_dist(&r->pEducation, "education", &kTempDemographic, 1,
	               CUSTOMER_DEMOGRAPHICS);
	bitmap_to_dist(&r->nPurchaseEstimate, "purchase_band", &kTempDemographic, 1,
	               CUSTOMER_DEMOGRAPHICS);
	...
	r->bPreferredFlag = (genrand_integer(NULL, DIST_UNIFORM, 1, 100, 0,
	                                     S_CUST_PREFERRED_FLAG) > 50)
	                        ? 1
	                        : 0;
...
int pr_s_customer(void *pSrc) {
	...
	print_varchar(S_CUST_ADDRESS_CITY, g_w_customer_address.ca_address.city, 1);
	sprintf(szTemp, "%05d", g_w_customer_address.ca_address.zip);
	print_varchar(S_CUST_ADDRESS_ZIP, szTemp, 1);
	print_varchar(S_CUST_ADDRESS_COUNTY, g_w_customer_address.ca_address.county,
	              1);
