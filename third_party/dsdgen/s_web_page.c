 ...
 * TODO: None
 */
int mk_s_web_page(void *row, ds_key_t index) {
	static int bInit = 0;
	...
	if (!bInit) {
		pPermutation =
		    makePermutation(NULL, (int)getIDCount(WEB_PAGE), S_WPAG_PERMUTE);
		bInit = 1;
	}
...
int vld_s_web_page(int nTable, ds_key_t kRow, int *Permutation) {
	static int bInit = 0;
	static int *pPermutation;
