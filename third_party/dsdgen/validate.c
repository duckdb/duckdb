...
int validateSCD(int nTable, ds_key_t kRow, int *Permutation) {
	ds_key_t kStart, kEnd;
	char szID[RS_BKEY + 1];
	int nColumn, nOffset = 0, nSKColumn, nID,
	             nSkipTable = 0; /* some source schema tales are generated using
	                                the warehouse routine */
	table_func_t *pTF = getTdefFunctionsByNumber(nTable);
	table_func_t *pTS;
