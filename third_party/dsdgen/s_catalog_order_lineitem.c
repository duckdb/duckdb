...
int mk_s_catalog_order_lineitem(void *pDest, ds_key_t kIndex) {
	...
	if (!bInit) {
		memset(&g_s_catalog_order_lineitem, 0,
		       sizeof(struct S_CATALOG_ORDER_LINEITEM_TBL));
		strtodec(&dMin, "1.00");
		strtodec(&dMax, "1000.00");
		pItemPermutation =
		    makePermutation(NULL, (int)getIDCount(ITEM), S_CLIN_PERMUTE);
	...
	r->kCatalogPage = mk_join(S_CLIN_CATALOG_PAGE_ID, CATALOG_PAGE,
	                          g_s_catalog_order.dtOrderDate.julian);
	r->kCatalogID = getCatalogNumberFromPage(r->kCatalogPage);
	// genrand_integer(&r->Pricing.quantity, DIST_UNIFORM, 1, 100, 0,
	// S_CLIN_QUANTITY);
