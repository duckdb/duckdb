...
int mk_s_pline(void *pDest, ds_key_t kIndex) {
	...
	static int *pItemPermutation, nItemIDCount;
	...
	if (!bInit) {
		...
		/*
		 * need to assure that a given item only appears in a single lineitem
		 * within an order use adjacent orders from within a permutation of
		 * possible values; since item is an SCD, use the item count
		 */
	...
	genrand_integer(&r->Pricing.quantity, DIST_UNIFORM, PLINE_MIN_QUANTITY,
	                PLINE_MAX_QUANTITY, 0, S_PLINE_QUANTITY);
