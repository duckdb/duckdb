 ...
 * TODO: None
 */
static int mk_master(void *pDest, ds_key_t kIndex) {
	static int bInit = 0;
	...
	genrand_integer(&r->nOrderTime, DIST_UNIFORM, 0, (24 * 3600) - 1, 0,
	                S_CORD_ORDER_TIME);
	...
	if (nGiftPct > WS_GIFT_PCT)
		r->kShipCustomerID = mk_join(S_CORD_SHIP_CUSTOMER_ID, CUSTOMER, 2);
	else
		r->kShipCustomerID = r->kBillCustomerID;
	r->kShipModeID = mk_join(S_CORD_SHIP_MODE_ID, SHIP_MODE, 1);
	gen_text(&r->szComment[0], (int)(RS_S_CATALOG_ORDER_COMMENT * 0.6),
	         RS_S_CATALOG_ORDER_COMMENT, S_CORD_COMMENT);

	return (0);
}

static int mk_detail(void *pDest, int nLine, int bPrint) {
	int nTemp;
	...
	if ((nTemp < S_CATALOG_RETURN_PCT) &&
	    (g_s_catalog_order_lineitem.dtShipDate.julian != -1)) {
...
int vld_s_catalog_order(int nTable, ds_key_t kRow, int *bPermutation) {
	int nLineitem, i;
