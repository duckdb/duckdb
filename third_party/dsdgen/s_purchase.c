 ...
 * TODO: None
 */
int mk_master(void *pDest, ds_key_t kIndex) {
	static int bInit = 0;
	...
	gen_text(&r->szComment[0], (int)(RS_S_PURCHASE_COMMENT * 0.6),
	         RS_S_PURCHASE_COMMENT, S_PURCHASE_COMMENT);

	return (0);
}

int mk_detail(int i, int bPrint) {
	int nTemp;
...
int mk_s_purchase(void *pDest, ds_key_t kIndex) {
	...
	genrand_integer(&nItemIndex, DIST_UNIFORM, 1, (int)getIDCount(ITEM), 0,
	                S_PLINE_ITEM_ID);
	for (i = 1; i <= 12; i++) {
		mk_detail(i, 1);
	}
...
int vld_s_purchase(int nTable, ds_key_t kRow, int *bPermutation) {
	int nLineitem, i;
