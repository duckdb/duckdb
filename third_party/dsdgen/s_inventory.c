...
int mk_s_inventory(void *pDest, ds_key_t kIndex) {
	...
	/*
	 * the generation of SCD ids in the warehouse is tied to the monotonically
	 * increasing surrogate key. this isn't ideal, but changing the data set on
	 * the warehouse side is a problem at this late date, so we need to mimic
	 * the behavior.
	 */
	r->invn_date.julian = getUpdateDate(S_INVENTORY, kIndex);
	r->item_id = getFirstSK(kItem);
	genrand_integer(&r->quantity, DIST_UNIFORM, INVN_MIN_QUANTITY,
	                INVN_MAX_QUANTITY, 0, S_INVN_QUANTITY);
