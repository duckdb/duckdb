...
int mk_s_store_returns(void *pDest, ds_key_t kIndex) {
	...
	if (g_s_purchase.dtPurchaseDate.julian == -1)
		...
		jtodt(&r->dtReturnDate,
		      g_s_purchase.dtPurchaseDate.julian + nReturnLag);
	...
	if (g_s_purchase_lineitem.Pricing.quantity != -1)
		genrand_integer(&r->Pricing.quantity, DIST_UNIFORM, 1,
		                g_s_purchase_lineitem.Pricing.quantity, 0,
		                S_SRET_PRICING);
	else
