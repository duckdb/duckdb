 ...
 * TODO: None
 */
int mk_s_call_center(void *row, ds_key_t index) {
	static int bInit = 0;
	static int *pPermutation;
	ds_key_t kIndex;

	if (!bInit) {
		pPermutation = makePermutation(NULL, (int)getIDCount(CALL_CENTER),
		                               S_CALL_CENTER_ID);
		bInit = 1;
	}
