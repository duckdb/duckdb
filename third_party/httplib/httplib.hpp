...
typename std::enable_if<!std::is_array<T>::value, duckdb_wrapped::std::unique_ptr<T>>::type
...
make_unique(std::size_t n) {
  ...
  return duckdb_wrapped::std::unique_ptr<T>(new RT[n]);
}
...
private:
  ...
  static duckdb_wrapped::std::unique_ptr<detail::MatcherBase>
...
public:
  Result() = default;
  Result(duckdb_wrapped::std::unique_ptr<Response> &&res, Error err,
...
private:
  duckdb_wrapped::std::unique_ptr<Response> res_;
...
private:
  ...
  bool handle_request(Stream &strm, Request &req, Response &res,
                      bool close_connection, Error &error);
  duckdb_wrapped::std::unique_ptr<Response> send_with_content_provider(
...
private:
  duckdb_wrapped::std::unique_ptr<ClientImpl> cli_;
...
bool prepare_content_receiver(T &x, int &status,
  ...
  if (decompress) {
    ...
    duckdb_wrapped::std::unique_ptr<decompressor> decompressor;
...
inline std::string message_digest(const std::string &s, const EVP_MD *algo) {
  auto context = duckdb_wrapped::std::unique_ptr<EVP_MD_CTX, decltype(&EVP_MD_CTX_free)>(
...
template <typename T>
using CFObjectPtr =
    duckdb_wrapped::std::unique_ptr<typename std::remove_pointer<T>::type, void (*)(CFTypeRef)>;
...
inline duckdb_wrapped::std::unique_ptr<detail::MatcherBase>
...
Server::write_content_with_provider(Stream &strm, const Request &req,
  ...
  if (res.content_length_ > 0) {
    ...
    if (res.is_chunked_content_provider_) {
      ...
      duckdb_wrapped::std::unique_ptr<detail::compressor> compressor;
    ...
    if (!req.path.compare(0, entry.mount_point.size(), entry.mount_point)) {
      ...
      if (detail::is_valid_path(sub_path)) {
        ...
        if (detail::is_file(path)) {
          ...
          auto mm = duckdb_wrapped::std::make_shared<detail::mmap>(path.c_str());
...
inline bool Server::listen_internal() {
  ...
  {
    duckdb_wrapped::std::unique_ptr<TaskQueue> task_queue(new_task_queue());
...
inline duckdb_wrapped::std::unique_ptr<Response> ClientImpl::send_with_content_provider(
    Request &req, const char *body, size_t content_length,
    ContentProvider content_provider,
    ContentProviderWithoutLength content_provider_without_length,
    const std::string &content_type, Error &error) {
