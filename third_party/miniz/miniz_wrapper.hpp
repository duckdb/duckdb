...
public:
	...
	void Decompress(const char *compressed_data, size_t compressed_size, char *out_data, size_t out_size) {
		...
		stream.avail_in = UnsafeNumericCast<unsigned int >(compressed_size - GZIP_HEADER_MINSIZE);
		stream.next_out = (unsigned char *)out_data;
		stream.avail_out =  UnsafeNumericCast<unsigned int >(out_size);
	...
	void Compress(const char *uncompressed_data, size_t uncompressed_size, char *out_data, size_t *out_size) {
		...
		stream.avail_in =  UnsafeNumericCast<unsigned int >(uncompressed_size);
		stream.next_out = gzip_body;
		stream.avail_out =  UnsafeNumericCast<unsigned int >(*out_size - GZIP_HEADER_MINSIZE);
