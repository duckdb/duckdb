...
* blocked_Mo5.h++
*
* interface for BlockQuicksort with median-of-five
*
******************************************************************************
...
namespace blocked_mo5 {
	template<typename iter, typename Compare>
	void sort(iter begin, iter end, Compare less) {
		quicksort::qsort<partition::Hoare_block_partition_Mo5>(begin, end, less);
	}
	template<typename T>
	void sort(std::vector<T> &v) {
		typename std::vector<T>::iterator begin = v.begin();
		typename std::vector<T>::iterator end = v.end();
		quicksort::qsort<partition::Hoare_block_partition_Mo5>(begin, end, std::less<T>());
	}
}