...
cmake_config = os.path.join('extension_external', 'external_extension_config.cmake')
basedir = os.getcwd()
def init_cmake_config():
    os.makedirs(os.path.dirname(cmake_config), exist_ok=True)
...
for task in tasks:
    ...
    if build_extension(task):
        clonedir = os.path.join("extension_external",  task['name'])
