...
def load_selected_grammar_rules(file_rules):
    rules = {}
    for gram_file, rule_names in file_rules.items():
        gram_path = statements_dir / gram_file
        try:
            parsed_rules = parse_peg_grammar(gram_path.read_text())
        except Exception as e:
        ...
        for rule_name in rule_names:
            if rule_name not in parsed_rules:
                raise Exception(f"{gram_file}: missing dependency rule '{rule_name}'")
            if rule_name in rules:
                raise Exception(f"duplicate dependency grammar rule '{rule_name}' in {gram_file}")
            rules[rule_name] = parsed_rules[rule_name]
    return rules
...
def main():
    ...
    use_default_grammar_files = args.grammar is None
    grammar_files = sorted(set(args.grammar or DEFAULT_GRAMMAR_FILES))
    rules = load_grammar_rules(grammar_files)
    if use_default_grammar_files:
        for rule_name, rule in load_selected_grammar_rules(DEFAULT_EXTRA_RULES).items():
            if rule_name in rules:
                raise Exception(f"duplicate dependency grammar rule '{rule_name}'")
            rules[rule_name] = rule
    ...
    if args.write:
        if grammar_files != sorted(set(DEFAULT_GRAMMAR_FILES)):
