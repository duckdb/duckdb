...
# ---------------------------------------------------------------------------
# Terminal rule overrides registered in compiled_grammar.cpp.
...
matcher_cpp_path = peg_dir / 'matcher_factory.cpp'
matcher_override_cpp_path = peg_dir / 'compiled_grammar.cpp'
...
def _matcher_override_expr(rule_name, override):
    ...
    if matcher == "identifier":
        if suggestion:
            return f"make_uniq<IdentifierMatcher>(SuggestionState::{suggestion}, keyword_helper)"
    if matcher == "reserved_identifier":
        if suggestion:
            return f"make_uniq<ReservedIdentifierMatcher>(SuggestionState::{suggestion}, keyword_helper)"
    if matcher == "identifier_string":
        return "make_uniq<ReservedIdentifierMatcher>(SuggestionState::SUGGEST_VARIABLE, keyword_helper)"
    if matcher == "number_literal":
        return "make_uniq<NumberLiteralMatcher>()"
...
def write_matcher_rule_overrides(matcher_overrides):
    content = matcher_override_cpp_path.read_text()
    start_idx = content.find(_MATCHER_START_BLOCK)
    if start_idx == -1:
        raise RuntimeError(f"Could not find START GENERATED RULE OVERRIDES marker in {matcher_override_cpp_path}")
    end_idx = content.find(_MATCHER_END_BLOCK, start_idx + len(_MATCHER_START_BLOCK))
    if end_idx == -1:
        raise RuntimeError(f"Could not find END GENERATED RULE OVERRIDES marker in {matcher_override_cpp_path}")
    ...
    for rule_name, override in matcher_overrides.items():
        ...
        line = f'\tAddTerminalRuleOverride(overrides, "{rule_name}", {expr});\n'
        if len(line.rstrip()) <= 120:
            lines.append(line)
        else:
            lines.append(f'\tAddTerminalRuleOverride(overrides, "{rule_name}",\n')
            lines.append(f'\t                        {expr});\n')
    ...
    matcher_override_cpp_path.write_text(new_content)
