...
def write_keyword_map(keyword_sets):
    # The keyword lists are emitted as static tables filled by a loop rather than as one
    # `map.insert("literal")` call site per keyword: the call-site form compiled to ~42 KB
    # of code for ~530 keywords.
    output_path = peg_dir / "keyword_map.cpp"
    with open(output_path, "w") as f:
        f.write("/* THIS FILE WAS AUTOMATICALLY GENERATED BY inline_grammar.py */\n")
        f.write("#include \"duckdb/parser/peg/keyword_helper.hpp\"\n\n")
        ...
        for category, keywords in keyword_sets.items():
            ...
            table_name = cpp_map_name.upper().replace("_MAP", "S")
            table_names[category] = table_name
            f.write(f"static const char *const {table_name}[] = {{\n")
        ...
        for category in keyword_sets:
            cpp_map_name = CPP_MAP_NAMES[category]
            f.write(f"\tfor (auto keyword : {table_names[category]}) {{\n")
            f.write(f"\t\t{cpp_map_name}.insert(keyword);\n")
            f.write("\t}\n")
        f.write("}\n\n")
        f.write("} // namespace duckdb\n")
...
def load_all_rules():
    """Load and validate all grammar rules from statements/ and keywords/. Returns (rules, contents)."""
    ...
    return rules, contents, keyword_sets
...
def main():
    ...
    write_keyword_map(keyword_sets)
