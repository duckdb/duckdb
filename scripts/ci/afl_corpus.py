...
class CorpusConfig:
    ...
    afl_cmin_bin: str = DEFAULT_AFL_CMIN_BIN
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=f"afl-cmin executable path (default: {DEFAULT_AFL_CMIN_BIN})",
    )
...
def run_afl_cmin(task: GroupTask, afl_cmin_bin: str, target: Path) -> tuple[str, int, str, str]:
    cmd = [
        afl_cmin_bin,
        "-i",
        str(task.raw_dir),
...
def validate_environment(afl_cmin_bin: str, target: Path) -> None:
...
def run(config: CorpusConfig) -> int:
    ...
    try:
        validate_environment(config.afl_cmin_bin, config.target)
    ...
    with tempfile.TemporaryDirectory(prefix="corpus_work_") as workdir:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=config.jobs) as executor:
            futures = [executor.submit(run_afl_cmin, task, config.afl_cmin_bin, config.target) for task in tasks]
