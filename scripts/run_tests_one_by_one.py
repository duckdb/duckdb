...
parser.add_argument(
    '--rss-output',
    action='store',
    help='Write peak subprocess RSS measurements (bytes) to this JSON file',
    default=None,
)
...
def run_subprocess_and_measure_rss(test_cmd, unittest_stdout, unittest_stderr, timeout, env):
    ...
    with tempfile.TemporaryFile() as stdout_tmp, tempfile.TemporaryFile() as stderr_tmp:
        ...
        return subprocess.CompletedProcess(test_cmd, returncode, stdout_data, stderr_data), ru_maxrss_to_bytes(
            wait_rusage.ru_maxrss
        )
