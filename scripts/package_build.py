...
def build_package(
    ...
    include_files += [os.path.join('src', 'include', 'duckdb', 'main', 'extension', 'linked_extension_registry.hpp')]
    # include the separate extensions, and generate the object that registers the linked ones with the
    # engine. Every source is compiled straight into the package, so the registrars run before main
    # like any other static initializer and no link root is needed.
    ext_loader_defines = ''
    ext_headers = ''
    ...
    for ext in extensions:
        ...
        if ext_kind == 'CAPI':
            ...
            load_body = f'db.LoadStaticCAPIExtension("{ext}", {ext}_init_c_api);'
            ...
            load_body = f'db.LoadStaticCAPIExtensionV2("{ext}", {ext}_init_c_api_v2);'
        else:
            ext_headers += f'#if {ext_linked_define}\n#include "{ext}_extension.hpp"\n#endif\n'
            ext_name_camelcase = ext.replace('_', ' ').title().replace(' ', '')
            load_body = f'db.LoadStaticExtension<{ext_name_camelcase}Extension>();'
    ...
    # every known root, as the weak no-op the engine archive would supply: the package links objects, so one
    # file serves (see scripts/generate_autolink.py)
    known_extensions = []
    with open(os.path.join('extension', 'known_extensions.txt')) as f:
        for line in f:
            line = line.strip()
