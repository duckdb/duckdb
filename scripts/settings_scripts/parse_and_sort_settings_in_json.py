...
def add_all_settings_to_global_list():
    valid_entries = ['name', 'description', 'return_type', 'sql_type', 'scope', 'on_callbacks', 'custom_implementation', 'aliases']
    ...
    for entry in json_data:
        for field_entry in entry:
            if field_entry not in valid_entries:
                raise ValueError(f"Found entry unexpected entry {field_entry} in setting")
        setting = Setting(
            name=entry['name'],
