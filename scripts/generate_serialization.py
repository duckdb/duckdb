...
class {writer_name} {{
public:
\texplicit {writer_name}(MessageSerializer &serializer) : serializer(serializer) {{
\t}}
...
WRITER_CHECK_ORDER_FORMAT = """
void {writer_name}::CheckFieldOrder(const field_id_t field_id) {{
#ifdef DEBUG
\tif (last_field_id != DConstants::INVALID_INDEX && field_id <= last_field_id) {{
\t\tthrow InternalException("{writer_name} wrote field %d after field %llu, but a message must be written in "
\t\t                        "schema order",
...
# The unsigned type a slot is sized for, and the value it reads back as until it is patched. Zero is
# wrong as a placeholder for a type whose zero is a real value.
BACKPATCH_SLOT_TYPES = {
    "uint8_t": ("uint8_t", None),
    "uint16_t": ("uint16_t", None),
    "uint32_t": ("uint32_t", None),
    "uint64_t": ("uint64_t", None),
    "idx_t": ("idx_t", None),
    "optional_idx": ("idx_t", "DConstants::INVALID_INDEX"),
}

# An incremental writer builds one message field by field. A key that needs the whole object, or that
# rewrites the field list, has no meaning here, so the script stops instead of guessing.
INCREMENTAL_WRITER_UNSUPPORTED_CLASS_KEYS = [
    "base",
    "class_type",
    "constructor",
    "constructor_method",
...
INCREMENTAL_WRITER_UNSUPPORTED_MEMBER_KEYS = [
    "base",
...
def container_element_type(type):
    # vector<unique_ptr<X>> -> unique_ptr<X>
    inner = type[type.index("<") + 1 : type.rindex(">")]
    return inner.strip()


def append_parameter_type(element_type):
    # A pointer element reaches Serializer::WriteValue(const T *) either way. Take the plain pointer,
    # so the caller does not have to make an allocation.
    match = re.match(r"^(?:unique_ptr|shared_ptr|optionally_owned_ptr)<(.+)>$", element_type)
    if match:
        return f"const {match.group(1)} *"
    if element_type.endswith("*"):
        return f"const {element_type[:-1].strip()} *"
    return f"const {element_type} &"


def write_parameter_type(type):
    if is_zeroable(type) or type in ["float", "double", "hugeint_t", "uhugeint_t", "char"]:
        return f"{type} "
    return f"const {type} &"
...
class MemberVariable:
    def __init__(self, entry):
        ...
        self.backpatch = entry.get("backpatch", False)
        self.entry = entry
...
class SerializableClass:
    def __init__(self, entry):
        ...
        self.incremental_writer = entry.get("incremental_writer", False)
        ...
        if self.incremental_writer:
            self.verify_incremental_writer(entry)

    def verify_incremental_writer(self, entry):
        for key in INCREMENTAL_WRITER_UNSUPPORTED_CLASS_KEYS:
            if key in entry:
                print(f'Class "{self.name}" sets "incremental_writer", which does not support "{key}"')
                exit(1)
        if not self.members:
            print(f'Class "{self.name}" sets "incremental_writer" but has no members')
            exit(1)
        for member in self.members:
            for key in INCREMENTAL_WRITER_UNSUPPORTED_MEMBER_KEYS:
                if key in member.entry:
                    print(
                        f'Member "{member.name}" of "{self.name}" uses "{key}", which an incremental writer '
                        f"does not support"
                    )
                    exit(1)
            if member.backpatch:
                if member.type not in BACKPATCH_SLOT_TYPES:
                    valid = ", ".join(BACKPATCH_SLOT_TYPES.keys())
                    print(
                        f'Member "{member.name}" of "{self.name}" is "backpatch" but has type "{member.type}"; '
                        f"a patched slot is a padded varint, so its type must be one of: {valid}"
                    )
                    exit(1)
                if is_container(member.type):
                    print(f'Member "{member.name}" of "{self.name}" cannot be both a container and "backpatch"')
                    exit(1)
    ...
    def get_serialize_element(self, entry: MemberVariable, value_expression: Optional[str] = None):
        # An incremental writer takes the value as an argument. The rest of the statement stays the
        # same, so the writer and the one-shot Serialize() cannot disagree.
        property_name = entry.serialize_property if value_expression is None else value_expression
        property_id = entry.id
        property_key = entry.name
        property_type = replace_pointer(entry.type)
...
def writer_include_path(source_path):
    # Below an include root the path after "include/" is what a consumer writes, outside one the file
    # name is enough
    directory = os.path.dirname(source_path).replace(os.path.sep, "/") + "/"
    basename = os.path.basename(source_path).replace(".json", "") + "_writer.hpp"
    if "include/" in directory:
        return directory.split("include/")[-1] + basename
    return basename


def generate_writer_class_code(class_entry: SerializableClass):
    writer_name = class_entry.name + "Writer"
    declarations = ""
    bodies = ""
    slots = ""

    for entry in class_entry.members:
        name = to_pascal_case(entry.name)
        property_type = replace_pointer(entry.type)

        if entry.backpatch:
            slot_type, initial_value = BACKPATCH_SLOT_TYPES[entry.type]
            arguments = f'{entry.id}, "{entry.name}"'
            if initial_value is not None:
                arguments += f", {initial_value}"
            declarations += f"\tMessageSerializer::ReservedSlot Reserve{name}();\n"
            bodies += (
                f"\nMessageSerializer::ReservedSlot {writer_name}::Reserve{name}() {{\n"
                f"\tCheckFieldOrder({entry.id});\n"
                f"\treturn serializer.ReserveProperty<{slot_type}>({arguments});\n"
                f"}}\n"
            )
            continue

        if is_container(entry.type):
            element_type = replace_pointer(container_element_type(entry.type))
            parameter = append_parameter_type(element_type)
            slot_name = entry.name + "_slot"
            declarations += f"\tvoid Write{name}(const {property_type} &value);\n"
            declarations += f"\tvoid Begin{name}();\n"
            declarations += f"\tvoid Append{name}({parameter}value);\n"
            declarations += f"\tvoid End{name}();\n"
            slots += f"\tMessageSerializer::ReservedSlot {slot_name};\n"
            bodies += (
                f"\nvoid {writer_name}::Write{name}(const {property_type} &value) {{\n"
                f"\tCheckFieldOrder({entry.id});\n"
                f"{class_entry.get_serialize_element(entry, 'value')}"
                f"}}\n"
                f"\nvoid {writer_name}::Begin{name}() {{\n"
                f"\tCheckFieldOrder({entry.id});\n"
                f'\t{slot_name} = serializer.BeginDeferredList({entry.id}, "{entry.name}");\n'
                f"}}\n"
                f"\nvoid {writer_name}::Append{name}({parameter}value) {{\n"
                f"\tserializer.AppendElement({slot_name}, value);\n"
                f"}}\n"
                f"\nvoid {writer_name}::End{name}() {{\n"
                f"\tserializer.EndDeferredList({slot_name});\n"
                f"}}\n"
            )
            continue

        parameter = write_parameter_type(property_type)
        declarations += f"\tvoid Write{name}({parameter}value);\n"
        bodies += (
            f"\nvoid {writer_name}::Write{name}({parameter}value) {{\n"
            f"\tCheckFieldOrder({entry.id});\n"
            f"{class_entry.get_serialize_element(entry, 'value')}"
            f"}}\n"
        )

    bodies += WRITER_CHECK_ORDER_FORMAT.format(writer_name=writer_name)
    declaration = WRITER_CLASS_FORMAT.format(writer_name=writer_name, declarations=declarations, slots=slots)
    return declaration, bodies


def generate_writer_files(source_path, target_path, classes, include_list):
    writer_classes = sorted([x for x in classes if x.incremental_writer], key=lambda x: x.name)
    if not writer_classes:
        return
    base_name = os.path.basename(source_path).replace(".json", "")
    header_path = os.path.join(os.path.dirname(source_path), base_name + "_writer.hpp")
    source_target_path = os.path.join(os.path.dirname(target_path), "serialize_" + base_name + "_writer.cpp")

    declarations = ""
    bodies = ""
    for class_entry in writer_classes:
        declaration, body = generate_writer_class_code(class_entry)
        declarations += declaration
        bodies += body

    writer_includes = [x for x in include_list if x not in DEFAULT_INCLUDE_LIST]
    with open(header_path, "w+") as f:
        f.write(
            WRITER_HEADER_FORMAT.format(
                include_list="".join([INCLUDE_FORMAT.format(filename=x) for x in writer_includes]),
                classes=declarations,
            )
        )
    with open(source_target_path, "w+") as f:
        f.write(WRITER_SOURCE_FORMAT.format(header_path=writer_include_path(source_path), bodies=bodies))
...
for entry in file_list:
    ...
    include_list = list(DEFAULT_INCLUDE_LIST)
    ...
    generate_writer_files(source_path, target_path, classes, include_list)
