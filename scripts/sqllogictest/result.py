...
class QueryResult:
    def __init__(self, result: List[Tuple[Any]]):
        self._result = result
...
def check_query_result(runner: SQLLogicRunner, query: Query, context, result: QueryResult):
    ...
    logger = SQLLogicTestLogger(context, query)

    # If the result has an error, log it
    if result.has_error:
        logger.unexpected_failure(result)
        if skip_error_message(result.get_error()):
    ...
    if len(values) == 1 and result_is_file(values[0]):
        fname = SQLLogicTestRunner.loop_replacement(values[0], context.running_loops)
        csv_error = ""
    ...
    if not compare_hash:
        ...
        if query_has_label:
            ...
            if entry is None:
                ...
                runner.result_label_map[query_label] = result
            else:
