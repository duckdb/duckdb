...
docker run -i --rm -v $(pwd):/duckdb --workdir /duckdb alpine:latest <<< "apk add g++ git make cmake ninja python3 && cmake -Bbuild . && cmake --build build && cmake --install build && g++ -std=c++17 examples/embedded-c++/main.cpp"
...
docker run -i --rm -v $(pwd):/duckdb --workdir /duckdb ubuntu:24.04 <<< "apt-get update && export DEBIAN_FRONTEND=noninteractive && apt-get install g++-riscv64-linux-gnu git make cmake ninja-build -y && export LD_LIBRARY_PATH=/usr/riscv64-linux-gnu/lib:/usr/riscv64-linux-gnu/lib64 && ln -s /usr/riscv64-linux-gnu/lib/ld-linux-riscv64-lp64d.so.1 /lib/ld-linux-riscv64-lp64d.so.1 && GEN=ninja CC=riscv64-linux-gnu-gcc CXX=riscv64-linux-gnu-g++ DUCKDB_PLATFORM=linux_riscv64 make && $TEST" 2>&1
docker run -i --rm -v $(pwd):/duckdb --workdir /duckdb ubuntu:24.04 <<< "apt-get update && export DEBIAN_FRONTEND=noninteractive && apt-get install g++-13-loongarch64-linux-gnu git make cmake ninja-build -y && export LD_LIBRARY_PATH=/usr/loongarch64-linux-gnu/lib:/usr/loongarch64-linux-gnu/lib64 && ln -s /usr/loongarch64-linux-gnu/lib/ld-linux-loongarch-lp64d.so.1 /lib64/ld-linux-loongarch-lp64d.so.1 && GEN=ninja CC=loongarch64-linux-gnu-gcc-13 CXX=loongarch64-linux-gnu-g++-13 DUCKDB_PLATFORM=linux_loong64 make && $TEST" 2>&1