SELECT Route.id AS route, Sensor.id AS sensor, SwitchPosition.id AS swP, Switch.id AS sw
FROM Sensor
...
LEFT OUTER JOIN requires ON requires.Route_id = Route.id AND requires.Sensor_id = Sensor.id
