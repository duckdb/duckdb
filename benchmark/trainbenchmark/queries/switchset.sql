SELECT
    ...
    Switch.currentPosition AS currentPosition -- (route)
FROM Route -- (route)-[:follows]->(swP)
INNER JOIN SwitchPosition ON Route.id = SwitchPosition.route -- the "SwitchPosition.route" attribute is the inverse of the "Route.follows" edge
-- (swP)-[:target]->(sw)
