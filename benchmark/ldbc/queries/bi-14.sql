...
WITH person1_list AS (
        SELECT p.p_personid AS personid, ci.pl_placeid AS cityid
...
,
        ...
        WHERE 1 = 1 -- join
          ...
          AND ci.pl_placeid = p.p_placeid -- filter
...
,
    ...
    case1 AS (
        SELECT DISTINCT p1.personid AS person1id, p2.personid AS person2id, 4 AS score
        FROM person1_list p1, person2_list p2, message m -- message by p2
,
    ...
    case2 AS (
        SELECT DISTINCT p1.personid AS person1id, p2.personid AS person2id, 1 AS score
        FROM person1_list p1, person2_list p2, message m -- message by p1
,
...
p1.personid AS person1id,
            ...
            15 AS score
        FROM person1_list p1, person2_list p2, knows k
        WHERE 1 = 1 -- join
          AND p1.personid = k.k_person1id
          AND p2.personid = k.k_person2id
    ),
    case4 AS (
        SELECT DISTINCT p1.personid AS person1id, p2.personid AS person2id, 10 AS score
        FROM person1_list p1, person2_list p2, message m -- message by p2
,
            likes l
        WHERE 1 = 1 -- join
    ...
    score_ranks AS (
        SELECT
            ...
            row_number() OVER (
                ...
                ORDER BY s.score DESC NULLS LAST, s.person1id, s.person2id
            ) AS rn
        FROM place co -- country
...
SELECT s.person1id AS "person1.id", s.person2id AS "person2.id", s.cityName AS "city1.name", s.score
FROM score_ranks s
WHERE 1 = 1 -- filter
  AND s.rn = 1
