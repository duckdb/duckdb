SELECT t2.t_name AS "relatedTag.name", count(*) AS count
FROM tag t
INNER JOIN
    message_tag pt ON (t.t_tagid = pt.mt_tagid) -- as an optimization, we don't need message here as it's ID is in message_tag pt
...
LEFT JOIN
    message_tag nt ON (c.m_messageid = nt.mt_messageid
            AND nt.mt_tagid = pt.mt_tagid)
WHERE 1 = 1 -- join
