...
LEFT OUTER JOIN (
        SELECT
            sum(creditCard_cvv) AS t1rp1,
            c.customer_id AS t1pk
        FROM CustomerView c
        LEFT OUTER JOIN
            CreditCardView cc ON c.customer_id = cc.creditCard_customerId
        GROUP BY c.customer_id
    ) t1 ON c.customer_id = t1.t1pk
LEFT OUTER JOIN (
        SELECT
            min(p.product_likes) AS t2rp1,
...
LEFT OUTER JOIN (
        SELECT
            max(o.order_subShipments) AS t5rp1,
            c.customer_id AS t5pk
        FROM CustomerView c
        LEFT OUTER JOIN
            OrderView o ON c.customer_id = o.order_customerId
        GROUP BY c.customer_id
...
LEFT OUTER JOIN (
        SELECT
            max(coalesce(oi.orderItem_weight, 1)) AS t6rp1,
            c.customer_id AS t6pk
        FROM CustomerView c
        LEFT OUTER JOIN
            OrderView o ON c.customer_id = o.order_customerId
        LEFT OUTER JOIN
...
LEFT OUTER JOIN (
        SELECT
            count(ca.category_seasonal) AS t7rp1,
            c.customer_id AS t7pk
        FROM CustomerView c
        LEFT OUTER JOIN
            OrderView o ON c.customer_id = o.order_customerId
        LEFT OUTER JOIN
