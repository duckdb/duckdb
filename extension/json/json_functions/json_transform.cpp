...
void JSONFunctions::RegisterJSONTransformCastFunctions(CastFunctionSet &casts) {
	...
	const auto array_type = LogicalType::ARRAY(LogicalType::ANY);
	auto json_to_array_cost = casts.ImplicitCastCost(LogicalType::VARCHAR, array_type) - 2;
	casts.RegisterCastFunction(JSONCommon::JSONType(), array_type, JSONToAnyCastBind, json_to_array_cost);
