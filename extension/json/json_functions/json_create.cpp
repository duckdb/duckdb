...
void JSONFunctions::RegisterJSONCreateCastFunctions(CastFunctionSet &casts) {
	...
	auto array_to_json_cost = casts.ImplicitCastCost(array_type, LogicalType::VARCHAR) - 2;
	casts.RegisterCastFunction(array_type, JSONCommon::JSONType(), AnyToJSONCastBind, array_to_json_cost);
