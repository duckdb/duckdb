	...
	while(!stack.empty()) {
		...
		// Copy orig keys not in patch or where patch value is null
		{
			...
			yyjson_mut_obj_foreach(nodes.orig_node, idx, max, key, orig_val) {
				auto patch_val = yyjson_mut_obj_getn(nodes.patch_node, unsafe_yyjson_get_str(key), unsafe_yyjson_get_len(key));
				if (!patch_val) {
		...
		// Merge non-null items from patch
		{
			...
			yyjson_mut_obj_foreach(nodes.patch_node, idx, max, key, patch_val) {
				auto orig_val = yyjson_mut_obj_getn(nodes.orig_node, unsafe_yyjson_get_str(key), unsafe_yyjson_get_len(key));
				auto mut_key = yyjson_mut_val_mut_copy(doc, key);
				
				// at least one of (patch_val, orig_val) is not an object, then we copy the patch if it's not null, and the original otherwise
