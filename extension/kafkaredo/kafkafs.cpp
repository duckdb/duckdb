...
namespace duckdb {
KafkaFileHandle::KafkaFileHandle(FileSystem &fs, string bootstrap_server, string topic, FileOpenFlags flags)
    : FileHandle(fs, bootstrap_server, flags) {
...
unique_ptr<FileHandle> KafkaFileSystem::OpenFile(const string &path, FileOpenFlags flags, optional_ptr<FileOpener> opener) {
