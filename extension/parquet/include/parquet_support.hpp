...
public:
    ...
    void next(
        ...
        DATALIB_CHECK(false)
            << "next() is only defined in SelectiveStructColumnReader";
    }
    ...
    inline void addNull() {
        DATALIB_DCHECK(
            rawResultNulls_ && rawValues_ &&
            (numValues_ + 1) * sizeof(T) < rawSize_);
    ...
    FOLLY_ALWAYS_INLINE vector_size_t checkAndSkipNulls(
        ...
        if (isDense) {
            ...
            for (;;) {
                ...
                if ((nullWord & (1UL << nullBit)) == 0) {
                    toSkip +=
                        __builtin_popcountll(~nullWord & f4d::bits::lowMask(nullBit));
                    current = testRow;
                    return toSkip;
                }
                if (++rowIndex_ >= numRows_) {
                    ...
                    toSkip += __builtin_popcountll(
                        ~nullWord & f4d::bits::lowMask(testRow - rowOfNullWord));
                    atEnd = true;
                    return toSkip;
                }
            }
