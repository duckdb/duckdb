...
struct ParquetReaderScanState {
	...
	duckdb::unique_ptr<ColumnReader> root_reader;
	duckdb::unique_ptr<duckdb_apache::thrift::protocol::TProtocol> thrift_file_proto;
...
class ParquetReader {
public:
	ParquetReader(Allocator &allocator, duckdb::unique_ptr<FileHandle> file_handle_p,
...
public:
	...
	ReadStatistics(ParquetReader &reader, LogicalType &type, column_t column_index,
...
private:
	...
	duckdb::unique_ptr<ColumnReader> CreateReader(const duckdb_parquet::format::FileMetaData *file_meta_data);

	duckdb::unique_ptr<ColumnReader> CreateReaderRecursive(const duckdb_parquet::format::FileMetaData *file_meta_data,
	...
	void RearrangeChildReaders(duckdb::unique_ptr<duckdb::ColumnReader> &root_reader, vector<column_t> &column_ids);
