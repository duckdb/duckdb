...
public:
	DuckdbFileTransport(unique_ptr<FileHandle> handle_p) : handle(move(handle_p)), location(0) {
	}
...
public:
	DuckdbBufferTransport(duckdb::ResizeableBuffer &buffer_p) : buffer(buffer_p) {
	}
...
template <class T>
static idx_t thrift_unpack_file(apache::thrift::protocol::TProtocol &protocol, idx_t starting_pos,
                                T *deserialized_msg) {
	((DuckdbFileTransport *)protocol.getTransport().get())->SetLocation(starting_pos);

	try {
		return deserialized_msg->read(&protocol);
...
template <class T> static void thrift_unpack(const uint8_t *buf, uint32_t *len, T *deserialized_msg) {
	...
	shared_ptr<apache::thrift::protocol::TProtocol> tproto(
	    new apache::thrift::protocol::TCompactProtocolT<apache::thrift::transport::TMemoryBuffer>(tmem_transport));

	((apache::thrift::transport::TMemoryBuffer *)tproto->getTransport().get())
	    ->resetBuffer(const_cast<uint8_t *>(buf), *len);
