...
static void DuckDBVariantToParquetVariant(Vector &input, DataChunk &result, idx_t count) {
	// DuckDB Variant:
	// - keys = VARCHAR[]
...
void VariantColumnWriter::Write(ColumnWriterState &state_p, Vector &input, idx_t count) {
	...
	chunk_types.emplace_back(LogicalType::BLOB);
	chunk_types.emplace_back(LogicalType::BLOB);

	DataChunk result;
	result.Initialize(context, chunk_types, count);

	DuckDBVariantToParquetVariant(input, result, count);
	...
	metadata_writer.Write(metadata_state, result.data[0], count);
	value_writer.Write(value_state, result.data[1], count);
}
