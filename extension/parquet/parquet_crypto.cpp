...
uint32_t ParquetCrypto::Read(TBase &object, TProtocol &iprot, const string &key,
	...
	auto dprot = tproto_factory.getProtocol(
	    duckdb_wrapped::std::make_shared<DecryptionTransport>(iprot, key, encryption_util_p));
	...
	auto simple_prot = tsimple_proto_factory.getProtocol(
	    duckdb_wrapped::std::make_shared<SimpleReadTransport>(all.get(), all.GetSize()));
...
uint32_t ParquetCrypto::Write(const TBase &object, TProtocol &oprot, const string &key,
	...
	auto eprot = tproto_factory.getProtocol(
	    duckdb_wrapped::std::make_shared<EncryptionTransport>(oprot, key, encryption_util_p));
