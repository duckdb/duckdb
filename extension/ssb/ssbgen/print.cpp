 ...
 * Function Prototypes
 */
FILE *print_prep PROTO((int table, int upSSB_DATE));
int pr_drange PROTO((int tbl, long min, long cnt, long num));

FILE *print_prep(int table, int upSSB_DATE) {
	char upath[128];
	FILE *res;
	...
	if (updates) {
		if (updates > 0)
			if (insert_segments) {
				...
				sprintf(upath, "%s%c%s.u%d.%d", env_config(PATH_TAG, PATH_DFLT), PATH_SEP, tdefs[table].name,
				        upSSB_DATE % 10000, this_segment);
			} else {
				sprintf(upath, "%s%c%s.u%d", env_config(PATH_TAG, PATH_DFLT), PATH_SEP, tdefs[table].name, upSSB_DATE);
			}
		else /* deletes */
			if (delete_segments) {
				++delete_segment;
				sprintf(upath, "%s%cdelete.u%d.%d", env_config(PATH_TAG, PATH_DFLT), PATH_SEP, -upSSB_DATE % 10000,
				        delete_segment);
			} else {
				sprintf(upath, "%s%cdelete.%d", env_config(PATH_TAG, PATH_DFLT), PATH_SEP, -upSSB_DATE);
			}
...
int dbg_print(int format, FILE *target, void *data, int len, int sep) {
	...
	case DT_CHR:
		if (columnar)
			fprintf(target, "%c ", static_cast<char>(reinterpret_cast<uintptr_t>(data)));
		else
			fprintf(target, "%c", static_cast<char>(reinterpret_cast<uintptr_t>(data)));
		break;
	}
...
int pr_cust(customer_t *c, int mode) {
	...
	PR_VSTR(fp, c->address, (columnar) ? (long)(ceil(C_ADDR_LEN * V_STR_HGH)) : c->alen);
...
int pr_supp(supplier_t *supp, int mode) {
	...
	PR_VSTR(fp, supp->address, (columnar) ? (long)(ceil(S_ADDR_LEN * V_STR_HGH)) : supp->alen);
 ...
 * would require ~660 upSSB_DATE iterations, this seems an acceptable
 * oversight
 */
int pr_drange(int tbl, long min, long cnt, long num) {
	...
	for (child = min; cnt > 0; child++, cnt--) {
		new_t = MK_SPARSE(child, (num - 1) / (10000 / refresh));
		if (gen_rng == 1 && new_t - last == 1) {
			last = new_t;
			continue;
		}
		if (gen_sql) {
			fprintf(dfp, "delete from %s where %s between %ld and %ld;\n", tdefs[ORDER].name, "o_orderkey", start,
			        last);
...
int vrf_date(ssb_date_t *d, int mode) {
	VRF_STRT(SSB_DATE);
	VRF_STR(SSB_DATE, d->datekey);
	VRF_STR(SSB_DATE, d->date);
	VRF_STR(SSB_DATE, d->dayofweek);
	VRF_STR(SSB_DATE, d->month);
	VRF_INT(SSB_DATE, d->year);
	VRF_INT(SSB_DATE, d->yearmonthnum);
	VRF_STR(SSB_DATE, d->yearmonth);
	VRF_INT(SSB_DATE, d->daynuminweek);
	VRF_INT(SSB_DATE, d->daynuminmonth);
	VRF_INT(SSB_DATE, d->daynuminyear);
	VRF_INT(SSB_DATE, d->monthnuminyear);
	VRF_INT(SSB_DATE, d->weeknuminyear);
	VRF_STR(SSB_DATE, d->sellingseason);
	VRF_STR(SSB_DATE, d->lastdayinweekfl);
	VRF_STR(SSB_DATE, d->lastdayinmonthfl);
	VRF_STR(SSB_DATE, d->weekdayfl);
	VRF_END(SSB_DATE);
	return (0);
}
