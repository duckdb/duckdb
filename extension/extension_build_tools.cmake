...
function(duckdb_link_root TARGET SYMBOL)
    if(MSVC)
        ...
        target_link_options(${TARGET} PRIVATE "/INCLUDE:${SYMBOL}")
    elseif(APPLE)
        target_link_options(${TARGET} PRIVATE "-Wl,-u,_${SYMBOL}")
    elseif(EMSCRIPTEN)
        target_link_options(${TARGET} PRIVATE "-Wl,--undefined=${SYMBOL}")
    else()
        if(MINGW AND CMAKE_SIZEOF_VOID_P EQUAL 4)
            set(SYMBOL "_${SYMBOL}")
        endif()
        target_link_options(${TARGET} PRIVATE "-Wl,-u,${SYMBOL}")
    endif()
endfunction()
...
function(build_static_extension NAME PARAMETERS)
    ...
    set(FILES "${ARGV}")
    list(REMOVE_AT FILES 0)
    duckdb_extension_link_member(${NAME} CPP LINK_FILE)
...
# Adds the root member to an extension archive created without build_static_extension.
function(duckdb_add_missing_extension_root NAME)
    ...
    duckdb_extension_link_member(${NAME} CPP LINK_FILE)
    target_sources(${NAME}_extension PRIVATE ${LINK_FILE})
    set_property(TARGET ${NAME}_extension PROPERTY DUCKDB_EXTENSION_KIND "CPP")
endfunction()
...
function(build_static_extension_capi_internal NAME KIND FILES)
    duckdb_extension_link_member(${NAME} ${KIND} LINK_FILE)
    add_library(${NAME}_extension STATIC ${FILES} ${LINK_FILE})
    target_link_libraries(${NAME}_extension duckdb_static)
