...
CouponHashSet<A> *HllSketchImplFactory<A>::promoteListToSet(const CouponList<A> &list) {
	...
	CouponHashSet<A> *chSet = new (ChsAlloc(list.getAllocator()).allocate(1))
	    CouponHashSet<A>(list.getLgConfigK(), list.getTgtHllType(), list.getAllocator());
...
HllArray<A> *HllSketchImplFactory<A>::promoteListOrSetToHll(const CouponList<A> &src) {
	HllArray<A> *tgtHllArr =
	    HllSketchImplFactory<A>::newHll(src.getLgConfigK(), src.getTgtHllType(), false, src.getAllocator());
...
template <typename A>
HllArray<A> *HllSketchImplFactory<A>::newHll(uint8_t lgConfigK, target_hll_type tgtHllType, bool startFullSize,
                                             const A &allocator) {
	switch (tgtHllType) {
	case HLL_8:
...
HllSketchImpl<A> *HllSketchImplFactory<A>::reset(HllSketchImpl<A> *impl, bool startFullSize) {
	if (startFullSize) {
		...
		CouponList<A> *cl = new (ClAlloc(impl->getAllocator()).allocate(1))
		    CouponList<A>(impl->getLgConfigK(), impl->getTgtHllType(), hll_mode::LIST, impl->getAllocator());
		impl->get_deleter()(impl);
		return cl;
	}
}
