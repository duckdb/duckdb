...
CouponHashSet<A> *CouponHashSet<A>::newSet(const void *bytes, size_t len, const A &allocator) {
	...
	if (compactFlag) {
		...
		std::memcpy(sketch->coupons_.data(), data + hll_constants::HASH_SET_INT_ARR_START,
...
CouponHashSet<A> *CouponHashSet<A>::newSet(std::istream &is, const A &allocator) {
	...
	const bool compactFlag =
	    ((listHeader[hll_constants::FLAGS_BYTE] & hll_constants::COMPACT_FLAG_MASK) ? true : false);
...
bool CouponHashSet<A>::checkGrowOrPromote() {
	if (static_cast<size_t>(hll_constants::RESIZE_DENOM * this->couponCount_) >
	    (hll_constants::RESIZE_NUMER * this->coupons_.size())) {
		const uint8_t lgCouponArrInts = count_trailing_zeros_in_u32(static_cast<uint32_t>(this->coupons_.size()));
