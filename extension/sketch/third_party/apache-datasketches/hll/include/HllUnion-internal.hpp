...
hll_union_alloc<A>::hll_union_alloc(uint8_t lg_max_k, const A &allocator)
    : lg_max_k_(HllUtil<A>::checkLgK(lg_max_k)), gadget_(lg_max_k, target_hll_type::HLL_8, false, allocator) {
}
...
void hll_union_alloc<A>::coupon_update(uint32_t coupon) {
	if (coupon == HllUtil<A>::EMPTY) {
		return;
	}
	HllSketchImpl<A> *result = gadget_.sketch_impl->coupon_update(coupon);
	if (result != gadget_.sketch_impl) {
		if (gadget_.sketch_impl != nullptr) {
			gadget_.sketch_impl->get_deleter()(gadget_.sketch_impl);
		}
...
template <typename A>
double hll_union_alloc<A>::get_rel_err(bool upper_bound, bool unioned, uint8_t lg_config_k, uint8_t num_std_dev) {
...
HllSketchImpl<A> *hll_union_alloc<A>::copy_or_downsample(const HllSketchImpl<A> *src_impl, uint8_t tgt_lg_k) {
	...
	Hll8Array<A> *tgtHllArr =
	    new (hll8Alloc(src->getAllocator()).allocate(1)) Hll8Array<A>(tgt_lg_k, false, src->getAllocator());
