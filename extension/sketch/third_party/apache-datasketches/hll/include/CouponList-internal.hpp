...
CouponList<A>::CouponList(uint8_t lgConfigK, target_hll_type tgtHllType, hll_mode mode, const A &allocator)
    : HllSketchImpl<A>(lgConfigK, tgtHllType, mode, false), couponCount_(0), oooFlag_(false),
      coupons_(1ULL << (mode == hll_mode::LIST ? hll_constants::LG_INIT_LIST_SIZE : hll_constants::LG_INIT_SET_SIZE), 0,
               allocator) {
}

template <typename A>
CouponList<A>::CouponList(const CouponList &that, const target_hll_type tgtHllType)
    : HllSketchImpl<A>(that.lgConfigK_, tgtHllType, that.mode_, false), couponCount_(that.couponCount_),
      oooFlag_(that.oooFlag_), coupons_(that.coupons_) {
}
...
CouponList<A> *CouponList<A>::newList(const void *bytes, size_t len, const A &allocator) {
	...
	const uint32_t couponsInArray =
	    (compact ? couponCount : (1 << HllUtil<A>::computeLgArrInts(LIST, couponCount, lgK)));
...
CouponList<A> *CouponList<A>::newList(std::istream &is, const A &allocator) {
	...
	const bool oooFlag =
	    ((listHeader[hll_constants::FLAGS_BYTE] & hll_constants::OUT_OF_ORDER_FLAG_MASK) ? true : false);
...
vector_u8<A> CouponList<A>::serialize(bool compact, unsigned header_size_bytes) const {
	const size_t sketchSizeBytes =
	    (compact ? getCompactSerializationBytes() : getUpdatableSerializationBytes()) + header_size_bytes;
...
template <typename A>
double CouponList<A>::getCompositeEstimate() const {
	return getEstimate();
}
...
template <typename A>
bool CouponList<A>::isEmpty() const {
...
template <typename A>
bool CouponList<A>::isCompact() const {
	return false;
}

template <typename A>
bool CouponList<A>::isOutOfOrderFlag() const {
	return oooFlag_;
}
