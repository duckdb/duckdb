...
public:
	const_iterator(const uint8_t *array, uint32_t array_slze, uint32_t index, target_hll_type hll_type,
	               const AuxHashMap<A> *exceptions, uint8_t offset, bool all);
...
private:
	...
	static inline uint8_t get_value(const uint8_t *array, uint32_t index, target_hll_type hll_type,
