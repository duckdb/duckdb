...
template <typename A>
inline double HllUtil<A>::getRelErr(bool upperBound, bool unioned, uint8_t lgConfigK, uint8_t numStdDev) {
...
inline uint8_t HllUtil<A>::computeLgArrInts(hll_mode mode, uint32_t count, uint8_t lgConfigK) {
	...
	if ((hll_constants::RESIZE_DENOM * count) > (hll_constants::RESIZE_NUMER * ceilPwr2)) {
		ceilPwr2 <<= 1;
	}
	if (mode == SET) {
