...
Hll4Array<A>::Hll4Array(uint8_t lgConfigK, bool startFullSize, const A &allocator)
    : HllArray<A>(lgConfigK, target_hll_type::HLL_4, startFullSize, allocator), auxHashMap_(nullptr) {
	const uint32_t numBytes = this->hll4ArrBytes(lgConfigK);
	this->hllByteArr_.resize(numBytes, 0);
}

template <typename A>
Hll4Array<A>::Hll4Array(const Hll4Array<A> &that) : HllArray<A>(that) {
...
uint8_t Hll4Array<A>::get_value(uint32_t index) const {
	...
	if (value != hll_constants::AUX_TOKEN)
...
void Hll4Array<A>::internalHll4Update(uint32_t slotNo, uint8_t newVal) {
	...
	if (newVal > lbOnOldValue) { // 842
		...
		const uint8_t actualOldValue =
		    (rawStoredOldValue < hll_constants::AUX_TOKEN) ? (lbOnOldValue) : (auxHashMap_->mustFindValueFor(slotNo));
