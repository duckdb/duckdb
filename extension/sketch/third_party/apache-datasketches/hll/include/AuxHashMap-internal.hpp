...
AuxHashMap<A>::AuxHashMap(uint8_t lgAuxArrInts, uint8_t lgConfigK, const A &allocator)
    : lgConfigK(lgConfigK), lgAuxArrInts(lgAuxArrInts), auxCount(0), entries(1ULL << lgAuxArrInts, 0, allocator) {
}
...
template <typename A>
AuxHashMap<A> *AuxHashMap<A>::deserialize(const void *bytes, size_t len, uint8_t lgConfigK, uint32_t auxCount,
                                          uint8_t lgAuxArrInts, bool srcCompact, const A &allocator) {
...
template <typename A>
AuxHashMap<A> *AuxHashMap<A>::deserialize(std::istream &is, uint8_t lgConfigK, uint32_t auxCount, uint8_t lgAuxArrInts,
...
void AuxHashMap<A>::mustAdd(uint32_t slotNo, uint8_t value) {
	...
	if (index >= 0) {
		throw std::invalid_argument("Found a slotNo that should not be there: SlotNo: " + std::to_string(slotNo) +
