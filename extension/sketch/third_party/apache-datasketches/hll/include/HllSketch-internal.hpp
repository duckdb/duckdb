...
template <typename A>
hll_sketch_alloc<A>::hll_sketch_alloc(uint8_t lg_config_k, target_hll_type tgt_type, bool start_full_size,
                                      const A &allocator) {
...
template <typename A>
hll_sketch_alloc<A>::hll_sketch_alloc(const hll_sketch_alloc<A> &that) : sketch_impl(that.sketch_impl->copy()) {
}
...
template <typename A>
hll_sketch_alloc<A>::hll_sketch_alloc(hll_sketch_alloc<A> &&that) noexcept : sketch_impl(nullptr) {
	std::swap(sketch_impl, that.sketch_impl);
}

template <typename A>
hll_sketch_alloc<A>::hll_sketch_alloc(HllSketchImpl<A> *that) : sketch_impl(that) {
}
...
template <typename A>
string<A> hll_sketch_alloc<A>::to_string(const bool summary, const bool detail, const bool aux_detail,
	...
	if (summary) {
		...
		if (get_current_mode() == HLL) {
			...
			os << "  Coupon count   : " << std::to_string(((CouponList<A> *)sketch_impl)->getCouponCount())
			   << std::endl;
		}
...
template <typename A>
double hll_sketch_alloc<A>::get_rel_err(bool upperBound, bool unioned, uint8_t lg_config_k, uint8_t numStdDev) {
