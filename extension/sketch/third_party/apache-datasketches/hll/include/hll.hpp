 ...
 * storage space consumed is CPC (Compressed Probabilistic Counting). For large enough counts, this HLL version (with
 * HLL_4) can be 2 to 16 times smaller than the Theta sketch family for the same accuracy.
 *
...
public:
	 ...
	 */
	explicit hll_sketch_alloc(uint8_t lg_config_k, target_hll_type tgt_type = HLL_4, bool start_full_size = false,
	                          const A &allocator = A());
