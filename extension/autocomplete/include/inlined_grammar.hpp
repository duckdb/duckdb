...
const char INLINED_PEG_GRAMMAR[] = {
	...
	"ColumnAliasPrefixClause <- ColIdOrString ColumnAliases ':'\n"
	"DistinctClause <- DistinctOn / DistinctAll\n"
	"DistinctAll <- 'ALL'\n"
	"DistinctOn <- 'DISTINCT' DistinctOnTargets?\n"
	"DistinctOnTargets <- 'ON' Parens(List(Expression))\n"
	"InnerTableRef <- ColumnAliasPrefixTableRef / ValuesRef / TableFunction / TableSubquery / BaseTableRef / ParensTableRef\n"
	"ColumnAliasPrefixTableRef <- ColumnAliasPrefixClause ColumnAliasPrefixTarget\n"
	"ColumnAliasPrefixTarget <- ColumnAliasPrefixFunction / ColumnAliasPrefixBaseTable / ColumnAliasPrefixSubquery / ColumnAliasPrefixParensTable\n"
	"ColumnAliasPrefixFunction <- QualifiedTableFunction TableFunctionArguments WithOrdinality? SampleClause?\n"
	"ColumnAliasPrefixBaseTable <- BaseTableName AtClause? SampleClause?\n"
	"ColumnAliasPrefixSubquery <- SubqueryReference SampleClause?\n"
	"ColumnAliasPrefixParensTable <- Parens(TableRef) SampleClause?\n"
	"TableRef <- InnerTableRef JoinOrPivot* TableAlias?\n"
	"TableSubquery <- TableSubqueryLateralOpt / TableSubqueryAliasColon\n"
	"TableSubqueryLateralOpt <- Lateral? SubqueryReference TableAlias?\n"
	"TableSubqueryAliasColon <- TableAliasColon SubqueryReference SampleClause?\n"
