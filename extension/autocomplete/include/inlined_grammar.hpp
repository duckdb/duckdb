...
const char INLINED_PEG_GRAMMAR[] = {
	...
	"MergeIntoStatement <- WithClause? 'MERGE'i 'INTO'i TargetOptAlias MergeIntoUsingClause List(MergeMatch)\n"
	"MergeIntoUsingClause <- ('USING'i TableRef)*\n"
	...
	"InsertMatchClause <- 'INSERT'i ((InsertColumnList? 'VALUES'i List(Expression)) / (ByNameOrPosition? '*'?) / DefaultValues)\n"
	...
	"Lateral <- 'LATERAL'i\n"
	"BaseTableName <- (CatalogQualification ReservedSchemaQualification ReservedTableName) / (SchemaQualification ReservedTableName) / TableName\n"
