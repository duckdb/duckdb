...
const char INLINED_PEG_GRAMMAR[] = {
	...
	"CastOperator <- '::' Type\n"
	"DotOperator <- '.' (MethodCall / ColLabel)\n"
	"MethodCall <- ColLabel Parens(DistinctOrAll? List(FunctionArgument)? OrderByClause? IgnoreNulls?)\n"
	...
	"SelectSetOpChain <- SelectAtom (SetOpClause SelectAtom)*\n"
	"# Crucial: This must NOT include WithClause or ResultModifiers directly.\n"
	"# It can only be a raw SelectStatementType or a fully parenthesized statement.\n"
	...
	"LimitOffset <- LimitOffsetClause / OffsetLimitClause / LimitClause / OffsetClause\n"
	...
	"Recursive <- 'RECURSIVE'\n"
	"SelectClause <- ('SELECT' / 'QUACK') DistinctClause? TargetList\n"
