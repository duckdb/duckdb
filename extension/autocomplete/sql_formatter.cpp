...
//! Fills compound_text with the full keyword string (e.g. "GROUP BY").
...
static string FormatMultiline(const vector<MatcherToken> &tokens, const FormatterConfig &config) {
	...
	// Stack: did the matching open-paren introduce clause-level content?
	vector<bool> paren_has_clauses;
	...
	auto clause_indent = [&]() -> int32_t { return paren_depth * indent_size; };
	...
	auto emit_clause = [&](const string &kw_text) {
		...
		if (!paren_has_clauses.empty()) {
			paren_has_clauses.back() = true;
		}
	};
	...
	for (idx_t i = 0; i < tokens.size();) {
		...
		if (tok.type == TokenType::OPERATOR && tok.text == "(") {
			...
			at_line_start = false;
			paren_depth++;
			paren_has_clauses.push_back(false);
			i++;
			continue;
		}
		...
		if (tok.type == TokenType::OPERATOR && tok.text == ")") {
			after_clause = false;
			prev_was_keyword = false;
			bool had_clauses = !paren_has_clauses.empty() && paren_has_clauses.back();
			if (!paren_has_clauses.empty()) {
				paren_has_clauses.pop_back();
			}
			paren_depth--;
		...
		if (tok.type == TokenType::KEYWORD) {
			...
			idx_t extra = DetectCompoundClause(tokens, i, compound_text);

			if (extra != static_cast<idx_t>(-1)) {
				emit_clause(compound_text);
				i += 1 + extra;
				continue;
			}
			...
			result += ShouldUppercase(upper) ? upper : tok.text;
			at_line_start = false;
			prev_was_keyword = true;
			i++;
			continue;
...
string FormatSQL(const string &sql, const FormatterConfig &config) {
	...
	return MergeShortClauses(multiline, config);
}
