...
unique_ptr<ResponseWrapper> HTTPFileSystem::Request(FileHandle &handle, string url, string method, HeaderMap header_map,
	...
	auto headers = make_unique<duckdb_httplib::Headers>();
	...
	duckdb_httplib::Client cli(proto_host_port.c_str());
...
ResponseWrapper::ResponseWrapper(duckdb_httplib::Response &res) {
	code = res.status;
	error = res.reason;
