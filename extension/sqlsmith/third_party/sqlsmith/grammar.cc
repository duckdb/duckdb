...
shared_ptr<table_ref> table_ref::factory(prod *p) {
	try {
		if (p->level < 3 + d6()) {
			if (d6() > 3 && p->level < d6())
				return std::make_shared<table_subquery>(p);
			if (d6() > 3)
				return std::make_shared<joined_table>(p);
		}
		if (d6() > 3)
...
table_or_query_name::table_or_query_name(prod *p) : table_ref(p) {
	...
	refs.push_back(std::make_shared<aliased_relation>(scope->stmt_uid("ref"), t));
}
...
target_table::target_table(prod *p, table *victim) : table_ref(p) {
	...
	victim_ = victim;
	refs.push_back(std::make_shared<aliased_relation>(scope->stmt_uid("target"), victim));
}
...
table_sample::table_sample(prod *p) : table_ref(p) {
	...
	refs.push_back(std::make_shared<aliased_relation>(scope->stmt_uid("sample"), t));
	percent = 0.1 * d100();
...
table_subquery::table_subquery(prod *p, bool lateral) : table_ref(p), is_lateral(lateral) {
	query = std::make_shared<query_spec>(this, scope, lateral);
	string alias = scope->stmt_uid("subq");
	relation *aliased_rel = &query->select_list->derived_table;
	refs.push_back(std::make_shared<aliased_relation>(alias, aliased_rel));
}
...
from_clause::from_clause(prod *p) : prod(p) {
	...
	while (d6() > 5) {
		...
		if (!impedance::matched(typeid(lateral_subquery)))
			break;
		reflist.push_back(std::make_shared<lateral_subquery>(this));
		for (auto r : reflist.back()->refs)
			scope->refs.push_back(&*r);
	}
}
...
query_spec::query_spec(prod *p, struct scope *s, bool lateral) : prod(p), myscope(s) {
	...
	from_clause = std::make_shared<struct from_clause>(this);
	select_list = std::make_shared<struct select_list>(this);
...
shared_ptr<prod> statement_factory(struct scope *s) {
	try {
		s->new_stmt();
		if (d42() == 1)
			return std::make_shared<merge_stmt>((struct prod *)0, s);
		if (d42() == 1)
			return std::make_shared<insert_stmt>((struct prod *)0, s);
		else if (d42() == 1)
			return std::make_shared<delete_returning>((struct prod *)0, s);
		else if (d42() == 1) {
			return std::make_shared<upsert_stmt>((struct prod *)0, s);
		} else if (d42() == 1)
			return std::make_shared<update_returning>((struct prod *)0, s);
		else if (d6() > 4)
			return std::make_shared<select_for_update>((struct prod *)0, s);
		else if (d6() > 5)
...
common_table_expression::common_table_expression(prod *parent, struct scope *s) : prod(parent), myscope(s) {
	scope = &myscope;
	do {
		shared_ptr<query_spec> query = std::make_shared<query_spec>(this, s);
		with_queries.push_back(query);
		string alias = scope->stmt_uid("jennifer");
		relation *relation = &query->select_list->derived_table;
		auto aliased_rel = std::make_shared<aliased_relation>(alias, relation);
...
merge_stmt::merge_stmt(prod *p, struct scope *s, table *v) : modifying_stmt(p, s, v) {
	match();
	target_table_ = std::make_shared<target_table>(this, victim);
	data_source = table_ref::factory(this);
	//   join_condition = join_cond::factory(this, *target_table_,
	//   *data_source);
	join_condition = std::make_shared<simple_join_cond>(this, *target_table_, *data_source);
...
shared_ptr<when_clause> when_clause::factory(struct merge_stmt *p) {
	try {
		...
		case 1:
		case 2:
			return std::make_shared<when_clause_insert>(p);
		case 3:
		case 4:
			return std::make_shared<when_clause_update>(p);
		default:
