...
int32_t run_sqlsmith(duckdb::DatabaseInstance &database, SQLSmithOptions opt) {
	...
	try {
		...
		schema = make_refcounted<schema_duckdb>(database, opt.exclude_catalog, opt.verbose_output);
		...
		loggers.push_back(make_refcounted<impedance_feedback>());

		if (opt.verbose_output) {
			auto l = make_refcounted<cerr_logger>();
		...
		if (opt.dump_all_graphs)
			loggers.push_back(make_refcounted<ast_logger>());

		if (opt.dump_all_queries)
			loggers.push_back(make_refcounted<query_dumper>());
		...
		dut = make_refcounted<dut_duckdb>(database);
