...
int mk_w_date(void *info_arr, ds_key_t index) {
	...
	append_key(info, r->d_date_sk, D_DATE_SK);
	append_varchar(info, r->d_date_id, D_DATE_ID);
	append_date(info, r->d_date_sk, D_DATE);
	append_integer(info, r->d_month_seq, D_MONTH_SEQ);
	append_integer(info, r->d_week_seq, D_WEEK_SEQ);
	append_integer(info, r->d_quarter_seq, D_QUARTER_SEQ);
	append_integer(info, r->d_year, D_YEAR);
	append_integer(info, r->d_dow, D_DOW);
	append_integer(info, r->d_moy, D_MOY);
	append_integer(info, r->d_dom, D_DOM);
	append_integer(info, r->d_qoy, D_QOY);
	append_integer(info, r->d_fy_year, D_FY_YEAR);
	append_integer(info, r->d_fy_quarter_seq, D_FY_QUARTER_SEQ);
	append_integer(info, r->d_fy_week_seq, D_FY_WEEK_SEQ);
	append_varchar(info, r->d_day_name, D_DAY_NAME);
	sprintf(sQuarterName, "%4dQ%d", r->d_year, r->d_qoy);
	append_varchar(info, sQuarterName, D_QUARTER_NAME);
	append_varchar(info, r->d_holiday ? "Y" : "N", D_HOLIDAY);
	append_varchar(info, r->d_weekend ? "Y" : "N", D_WEEKEND);
	append_varchar(info, r->d_following_holiday ? "Y" : "N", D_FOLLOWING_HOLIDAY);
	append_integer(info, r->d_first_dom, D_FIRST_DOM);
	append_integer(info, r->d_last_dom, D_LAST_DOM);
	append_integer(info, r->d_same_day_ly, D_SAME_DAY_LY);
	append_integer(info, r->d_same_day_lq, D_SAME_DAY_LQ);
	append_varchar(info, r->d_current_day ? "Y" : "N", D_CURRENT_DAY);
	append_varchar(info, r->d_current_week ? "Y" : "N", D_CURRENT_WEEK);
	append_varchar(info, r->d_current_month ? "Y" : "N", D_CURRENT_MONTH);
	append_varchar(info, r->d_current_quarter ? "Y" : "N", D_CURRENT_QUARTER);
	append_varchar(info, r->d_current_year ? "Y" : "N", D_CURRENT_YEAR);
