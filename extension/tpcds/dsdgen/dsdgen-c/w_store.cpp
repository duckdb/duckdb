...
int mk_w_store(void *info_arr, ds_key_t index) {
	...
	append_key(info, r->store_sk, W_STORE_SK);
	append_varchar(info, r->store_id, W_STORE_ID);
	append_date(info, r->rec_start_date_id, W_STORE_REC_START_DATE_ID);
	append_date(info, r->rec_end_date_id, W_STORE_REC_END_DATE_ID);
	append_key(info, r->closed_date_id, W_STORE_CLOSED_DATE_ID);
	append_varchar(info, r->store_name, W_STORE_NAME);
	append_integer(info, r->employees, W_STORE_EMPLOYEES);
	append_integer(info, r->floor_space, W_STORE_FLOOR_SPACE);
	append_varchar(info, r->hours, W_STORE_HOURS);
	append_varchar(info, &r->store_manager[0], W_STORE_MANAGER);
	append_integer(info, r->market_id, W_STORE_MARKET_ID);
	append_varchar(info, r->geography_class, W_STORE_GEOGRAPHY_CLASS);
	append_varchar(info, &r->market_desc[0], W_STORE_MARKET_DESC);
	append_varchar(info, &r->market_manager[0], W_STORE_MARKET_MANAGER);
	append_key(info, r->division_id, W_STORE_DIVISION_ID);
	append_varchar(info, r->division_name, W_STORE_DIVISION_NAME);
	append_key(info, r->company_id, W_STORE_COMPANY_ID);
	append_varchar(info, r->company_name, W_STORE_COMPANY_NAME);
	append_integer(info, r->address.street_num, W_STORE_ADDRESS_STREET_NUM);
	if (r->address.street_name2) {
		sprintf(szTemp2, "%s %s", r->address.street_name1, r->address.street_name2);
		append_varchar(info, szTemp2, W_STORE_ADDRESS_STREET_NAME1);
	} else
		append_varchar(info, r->address.street_name1, W_STORE_ADDRESS_STREET_NAME1);
	append_varchar(info, r->address.street_type, W_STORE_ADDRESS_STREET_TYPE);
	append_varchar(info, r->address.suite_num, W_STORE_ADDRESS_SUITE_NUM);
	append_varchar(info, r->address.city, W_STORE_ADDRESS_CITY);
	append_varchar(info, r->address.county, W_STORE_ADDRESS_COUNTY);
	append_varchar(info, r->address.state, W_STORE_ADDRESS_STATE);
	sprintf(szTemp2, "%05d", r->address.zip);
	append_varchar(info, szTemp2, W_STORE_ADDRESS_ZIP);
	append_varchar(info, r->address.country, W_STORE_ADDRESS_COUNTRY);
	append_integer_decimal(info, r->address.gmt_offset, W_STORE_ADDRESS_GMT_OFFSET);
	append_decimal(info, &r->dTaxPercentage, W_STORE_TAX_PERCENTAGE);
