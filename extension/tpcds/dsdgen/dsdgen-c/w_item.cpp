...
int mk_w_item(void *info_arr, ds_key_t index) {
	...
	append_key(info, r->i_item_sk, I_ITEM_SK);
	append_varchar(info, r->i_item_id, I_ITEM_ID);
	append_date(info, r->i_rec_start_date_id, I_REC_START_DATE_ID);
	append_date(info, r->i_rec_end_date_id, I_REC_END_DATE_ID);
	append_varchar(info, r->i_item_desc, I_ITEM_DESC);
	append_decimal(info, &r->i_current_price, I_CURRENT_PRICE);
	append_decimal(info, &r->i_wholesale_cost, I_WHOLESALE_COST);
	append_key(info, r->i_brand_id, I_BRAND_ID);
	append_varchar(info, r->i_brand, I_BRAND);
	append_key(info, r->i_class_id, I_CLASS_ID);
	append_varchar(info, r->i_class, I_CLASS);
	append_key(info, r->i_category_id, I_CATEGORY_ID);
	append_varchar(info, r->i_category, I_CATEGORY);
	append_key(info, r->i_manufact_id, I_MANUFACT_ID);
	append_varchar(info, r->i_manufact, I_MANUFACT);
	append_varchar(info, r->i_size, I_SIZE);
	append_varchar(info, r->i_formulation, I_FORMULATION);
	append_varchar(info, r->i_color, I_COLOR);
	append_varchar(info, r->i_units, I_UNITS);
	append_varchar(info, r->i_container, I_CONTAINER);
	append_key(info, r->i_manager_id, I_MANAGER_ID);
	append_varchar(info, r->i_product_name, I_PRODUCT_NAME);
