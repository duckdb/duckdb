...
string FormatQualifiedTableName(const string &catalog_name, const string &schema_name, const string &table) {
	if (catalog_name.empty() || catalog_name == INVALID_CATALOG) {
...
optional_ptr<Index> FindIndexByName(TableIndexList &index_list, const string &index_name, const string &catalog_name,
                                    const string &schema_name, const string &table) {
	...
	if (!found_index) {
		auto qualified_name = FormatQualifiedTableName(catalog_name, schema_name, table);
		...
		throw CatalogException("index_key: index '%s' was not found on table %s. Available indexes: %s", index_name,
		                       qualified_name, available_list);
	}

	return found_index;
}

void ValidateIndex(const Index &index, const string &catalog_name, const string &schema_name, const string &table,
                   const string &index_name) {
	if (!index.IsBound()) {
		auto qualified_name = FormatQualifiedTableName(catalog_name, schema_name, table);
		throw CatalogException("index_key: index '%s' on table %s is not yet bound", index_name, qualified_name);
	}
...
struct IndexKeyBindData : public FunctionData {
	IndexKeyBindData(optional_ptr<ART> art_index_p, vector<LogicalType> index_types_p, string index_name_p)
	    : art_index(art_index_p), index_types(std::move(index_types_p)), index_name(std::move(index_name_p)) {
	}

	unique_ptr<FunctionData> Copy() const override {
		return make_uniq<IndexKeyBindData>(art_index, index_types, index_name);
	}
...
unique_ptr<FunctionData> IndexKeyBind(ClientContext &context, ScalarFunction &bound_function,
	...
	auto found_index = FindIndexByName(index_list, index_name, path.catalog, path.schema, path.table);
	ValidateIndex(*found_index, path.catalog, path.schema, path.table, index_name);
...
void IndexKeyFunction(DataChunk &args, ExpressionState &state, Vector &result) {
	...
	for (idx_t row = 0; row < count; row++) {
		...
		if (generated_key.Empty()) {
			...
			result_data[row] =
			    StringVector::AddStringOrBlob(result, const_char_ptr_cast(generated_key.data), generated_key.len);
		}
	}
