...
struct BlobLengthOperation {
	static int64_t Operation(const string_t &value) {
		return value.GetSize();
	}
};

struct StringLengthOperation {
	static int64_t Operation(const string_t &value) {
		return LengthFun::Length<string_t, int64_t>(value);
	}
};

struct ListLengthOperation {
	static int64_t Operation(const list_entry_t &value) {
...
template <typename INPUT_TYPE, typename INDEX_TYPE, typename LENGTH_OP>
static bool ClampSlice(const INPUT_TYPE &value, INDEX_TYPE &begin, INDEX_TYPE &end) {
	...
	const auto length = LENGTH_OP::Operation(value);
...
string_t SliceValue(Vector &result, string_t input, int64_t begin, int64_t end) {
	...
	default:
		throw InternalException("Unsupported type for string slice: %s", result.GetType().ToString());
	}
}
...
static void ExecuteConstantSlice(Vector &result, Vector &str_vector, Vector &begin_vector, Vector &end_vector,
	...
	auto end = end_is_empty ? LENGTH_OP::Operation(str) : end_data[0];
	auto step = step_data ? step_data[0] : 1;

	if (step < 0) {
		swap(begin, end);
		begin = end_is_empty ? 0 : begin;
		end = begin_is_empty ? LENGTH_OP::Operation(str) : end;
	}

	// Clamp offsets
	bool clamp_result = false;
	if (step_valid || step == 1) {
		clamp_result = ClampSlice<INPUT_TYPE, INDEX_TYPE, LENGTH_OP>(str, begin, end);
	}
...
static void ExecuteFlatSlice(Vector &result, Vector &list_vector, Vector &begin_vector, Vector &end_vector,
	...
	for (idx_t i = 0; i < count; ++i) {
		...
		auto end = end_is_empty ? LENGTH_OP::Operation(sliced) : reinterpret_cast<INDEX_TYPE *>(end_data.data)[end_idx];
		auto step = step_vector ? reinterpret_cast<INDEX_TYPE *>(step_data.data)[step_idx] : 1;

		if (step < 0) {
			swap(begin, end);
			begin = end_is_empty ? 0 : begin;
			end = begin_is_empty ? LENGTH_OP::Operation(sliced) : end;
		}

		bool clamp_result = false;
		if (step_valid || step == 1) {
			clamp_result = ClampSlice<INPUT_TYPE, INDEX_TYPE, LENGTH_OP>(sliced, begin, end);
		}
...
static void ExecuteSlice(Vector &result, Vector &list_or_str_vector, Vector &begin_vector, Vector &end_vector,
	...
	if (result.GetVectorType() == VectorType::CONSTANT_VECTOR) {
		ExecuteConstantSlice<INPUT_TYPE, INDEX_TYPE, LENGTH_OP>(result, list_or_str_vector, begin_vector, end_vector,
		                                                        step_vector, count, sel, sel_idx, result_child_vector,
		                                                        begin_is_empty, end_is_empty);
	} else {
		ExecuteFlatSlice<INPUT_TYPE, INDEX_TYPE, LENGTH_OP>(result, list_or_str_vector, begin_vector, end_vector,
		                                                    step_vector, count, sel, sel_idx, result_child_vector,
		                                                    begin_is_empty, end_is_empty);
	}
	result.Verify(count);
...
static void ArraySliceFunction(DataChunk &args, ExpressionState &state, Vector &result) {
	...
	case LogicalTypeId::LIST: {
		...
		ExecuteSlice<list_entry_t, int64_t, ListLengthOperation>(result, list_or_str_vector, begin_vector, end_vector,
		                                                         step_vector, count, begin_is_empty, end_is_empty);
		break;
	}
	case LogicalTypeId::BLOB:
		ExecuteSlice<string_t, int64_t, BlobLengthOperation>(result, list_or_str_vector, begin_vector, end_vector,
		                                                     step_vector, count, begin_is_empty, end_is_empty);
		break;
	case LogicalTypeId::VARCHAR:
		ExecuteSlice<string_t, int64_t, StringLengthOperation>(result, list_or_str_vector, begin_vector, end_vector,
		                                                       step_vector, count, begin_is_empty, end_is_empty);
		break;
	default:
