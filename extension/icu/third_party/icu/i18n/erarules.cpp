...
static UBool isValidRuleStartDate(int32_t year, int32_t month, int32_t day) {
	return year >= MIN_ENCODED_START_YEAR && year <= MAX_ENCODED_START_YEAR && month >= 1 && month <= 12 && day >= 1 &&
	       day <= 31;
}
...
EraRules::EraRules(LocalMemory<int32_t> &eraStartDates, int32_t numEras) : numEras(numEras) {
