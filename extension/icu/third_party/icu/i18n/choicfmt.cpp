...
// CHOICE_FORMAT_LESS_EQUAL is generated by toPattern().
#define CHOICE_FORMAT_SINGLE_QUOTE ((UChar)0x0027)   /*'*/
#define CHOICE_FORMAT_LESS_THAN    ((UChar)0x003C)   /*<*/
#define CHOICE_FORMAT_LESS_EQUAL   ((UChar)0x0023)   /*#*/
#define CHOICE_FORMAT_LESS_EQUAL2  ((UChar)0x2264)
#define CHOICE_FORMAT_VERTICAL_BAR ((UChar)0x007C)   /*|*/
#define CHOICE_FORMAT_MINUS        ((UChar)0x002D)   /*-*/

static const UChar LEFT_CURLY_BRACE = 0x7B;     /*{*/
static const UChar RIGHT_CURLY_BRACE = 0x7D;    /*}*/

#ifdef CHOICE_FORMAT_INFINITY
#undef CHOICE_FORMAT_INFINITY
#endif
#define CHOICE_FORMAT_INFINITY     ((UChar)0x221E)

//static const UChar gPositiveInfinity[] = {CHOICE_FORMAT_INFINITY, 0};
//static const UChar gNegativeInfinity[] = {CHOICE_FORMAT_MINUS, CHOICE_FORMAT_INFINITY, 0};
#define CHOICE_FORMAT_POSITIVE_INF_STRLEN 1
#define CHOICE_FORMAT_NEGATIVE_INF_STRLEN 2
...
ChoiceFormat::setChoices(const double* limits,
    ...
    for (int32_t i = 0; i < count; ++i) {
        if (i != 0) {
            result += CHOICE_FORMAT_VERTICAL_BAR;
        }
        UnicodeString buf;
        if (uprv_isPositiveInfinity(limits[i])) {
            result += CHOICE_FORMAT_INFINITY;
        } else if (uprv_isNegativeInfinity(limits[i])) {
            result += CHOICE_FORMAT_MINUS;
            result += CHOICE_FORMAT_INFINITY;
        } else {
            result += dtos(limits[i], buf);
        }
        if (closures != NULL && closures[i]) {
            result += CHOICE_FORMAT_LESS_THAN;
        } else {
            result += CHOICE_FORMAT_LESS_EQUAL;
        }
        ...
        for (int32_t j = 0; j < textLength; ++j) {
            UChar c = text[j];
            if (c == CHOICE_FORMAT_SINGLE_QUOTE && nestingLevel == 0) {
                // Double each top-level apostrophe.
                result.append(c);
            } else if (c == CHOICE_FORMAT_VERTICAL_BAR && nestingLevel == 0) {
                ...
                //   |' -> '|'''
                //   |'' -> '|''''' etc.
                result.append(CHOICE_FORMAT_SINGLE_QUOTE).append(c).append(CHOICE_FORMAT_SINGLE_QUOTE);
...
ChoiceFormat::findSubMessage(const MessagePattern &pattern, int32_t partIndex, double number) {
    ...
    for (;;) {
        ...
        if (boundaryChar == CHOICE_FORMAT_LESS_THAN ? !(number > boundary) : !(number >= boundary)) {
