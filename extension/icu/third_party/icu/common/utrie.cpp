...
static inline UBool
utrie_equal_uint32(const uint32_t *s, const uint32_t *t, int32_t length) {
    while(length>0 && *s==*t) {
        ++s;
        ++t;
        --length;
...
_findSameDataBlock(const uint32_t *data, int32_t dataLength,
    ...
    for(block=0; block<=dataLength; block+=step) {
        if(utrie_equal_uint32(data+block, data+otherBlock, UTRIE_DATA_BLOCK_LENGTH)) {
            return block;
        }
    }
    return -1;
    ...
    for(start=newStart; start<trie->dataLength;) {
        ...
        if(overlap && start>=overlapStart) {
            ...
            for(i=UTRIE_DATA_BLOCK_LENGTH-UTRIE_DATA_GRANULARITY;
                i>0 && !utrie_equal_uint32(trie->data+(newStart-i), trie->data+start, i);
...
static uint32_t U_CALLCONV
utrie_enumSameValue(const void * /*context*/, uint32_t value) {
    return value;
}
