...
U_CAPI UChar *U_EXPORT2 u_strFromUTF32(UChar *dest, int32_t destCapacity, int32_t *pDestLength, const UChar32 *src,
...
U_CAPI UChar32 *U_EXPORT2 u_strToUTF32(UChar32 *dest, int32_t destCapacity, int32_t *pDestLength, const UChar *src,
                                       int32_t srcLength, UErrorCode *pErrorCode) {
			...
			if (U8_IS_SINGLE(c)) {
				*pDest++ = (UChar)c;
				...
				if (/* handle U+0800..U+FFFF inline */
				    (0xe0 <= (c) && (c) < 0xf0) && U8_IS_VALID_LEAD3_AND_T1((c), src[i]) &&
			...
			do {
				...
				if (U8_IS_SINGLE(c)) {
					*pDest++ = (UChar)c;
					...
					uint8_t __t1, __t2;
					if (/* handle U+0800..U+FFFF inline */
					    (0xe0 <= (c) && (c) < 0xf0) && ((i) + 1) < srcLength && U8_IS_VALID_LEAD3_AND_T1((c), src[i]) &&
					    (__t2 = src[(i) + 1] - 0x80) <= 0x3f) {
						*pDest++ = (((c)&0xf) << 12) | ((src[i] & 0x3f) << 6) | __t2;
						i += 2;
					} else if (/* handle U+0080..U+07FF inline */
...
U_CAPI UChar *U_EXPORT2 u_strFromUTF8Lenient(UChar *dest, int32_t destCapacity, int32_t *pDestLength, const char *src,
                                             int32_t srcLength, UErrorCode *pErrorCode) {
	UChar *pDest = dest;
	UChar32 ch;
	int32_t reqLength = 0;
...
U_CAPI char *U_EXPORT2 u_strToUTF8WithSub(char *dest, int32_t destCapacity, int32_t *pDestLength, const UChar *pSrc,
	...
	if ((pSrc == NULL && srcLength != 0) || srcLength < -1 || (destCapacity < 0) ||
...
U_CAPI char *U_EXPORT2 u_strToUTF8(char *dest, int32_t destCapacity, int32_t *pDestLength, const UChar *pSrc,
                                   int32_t srcLength, UErrorCode *pErrorCode) {
	return u_strToUTF8WithSub(dest, destCapacity, pDestLength, pSrc, srcLength, U_SENTINEL, NULL, pErrorCode);
}

U_CAPI UChar *U_EXPORT2 u_strFromJavaModifiedUTF8WithSub(UChar *dest, int32_t destCapacity, int32_t *pDestLength,
				...
				*pDest++ = (UChar)ch;
				...
				if (ch >= 0xe0) {
					if (/* handle U+0000..U+FFFF inline */
					    ch <= 0xef && (t1 = (uint8_t)(src[i] - 0x80)) <= 0x3f &&
	...
	while (i < srcLength && (pDest < pDestLimit)) {
		...
		if (U8_IS_SINGLE(ch)) {
			*pDest++ = (UChar)ch;
			...
			if (ch >= 0xe0) {
				if (/* handle U+0000..U+FFFF inline */
				    ch <= 0xef && (i + 1) < srcLength && (t1 = (uint8_t)(src[i] - 0x80)) <= 0x3f &&
				    (t2 = (uint8_t)(src[i + 1] - 0x80)) <= 0x3f) {
					/* no need for (ch & 0xf) because the upper bits are truncated after <<12 in the cast to (UChar) */
					*pDest++ = (UChar)((ch << 12) | (t1 << 6) | t2);
					i += 2;
	...
	while (i < srcLength) {
		...
		if (U8_IS_SINGLE(ch)) {
			...
			if (ch >= 0xe0) {
				if (/* handle U+0000..U+FFFF inline */
				    ch <= 0xef && (i + 1) < srcLength && (uint8_t)(src[i] - 0x80) <= 0x3f &&
...
U_CAPI char *U_EXPORT2 u_strToJavaModifiedUTF8(char *dest, int32_t destCapacity, int32_t *pDestLength, const UChar *src,
                                               int32_t srcLength, UErrorCode *pErrorCode) {
	int32_t reqLength = 0;
	uint32_t ch = 0;
	uint8_t *pDest = (uint8_t *)dest;
