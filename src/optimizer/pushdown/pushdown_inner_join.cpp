...
unique_ptr<LogicalOperator> FilterPushdown::PushdownInnerJoin(unique_ptr<LogicalOperator> op, unordered_set<idx_t> &left_bindings, unordered_set<idx_t> &right_bindings)
{
	...
	if (op->logical_type == LogicalOperatorType::LOGICAL_ANY_JOIN)
	{
	...
	{
		// comparison join
		D_ASSERT(op->logical_type == LogicalOperatorType::LOGICAL_COMPARISON_JOIN);
