...
static bool ProjectionCastsToLowerType(Expression &proj_expr) {
	bool changes_type = false;
	if (proj_expr.expression_class == ExpressionClass::BOUND_CAST) {
		auto &cast = proj_expr.Cast<BoundCastExpression>();
		const auto return_type = proj_expr.return_type;
		const auto child_type = cast.child->return_type;
	...
	ExpressionIterator::EnumerateChildren(
	    proj_expr, [&](unique_ptr<Expression> &child) { changes_type |= ProjectionCastsToLowerType(*child); });
	return changes_type;
}

static bool ProjectionChangesExpressionType(LogicalProjection &proj, unique_ptr<Expression> expr) {
	auto changes_type = false;
	if (expr->type == ExpressionType::BOUND_COLUMN_REF) {
		auto &colref = expr->Cast<BoundColumnRefExpression>();
		D_ASSERT(colref.binding.table_index == proj.table_index);
		D_ASSERT(colref.binding.column_index < proj.expressions.size());
		...
		if (ProjectionCastsToLowerType(*proj_expr)) {
			return true;
		}
	}
	ExpressionIterator::EnumerateChildren(*expr, [&](unique_ptr<Expression> &child) {
		changes_type |= ProjectionChangesExpressionType(proj, std::move(child));
	});
	return changes_type;
}
...
unique_ptr<LogicalOperator> FilterPushdown::PushdownProjection(unique_ptr<LogicalOperator> op) {
	...
	for (auto &filter : filters) {
		...
		if (is_volatile || ProjectionChangesExpressionType(proj, f.filter->Copy())) {
