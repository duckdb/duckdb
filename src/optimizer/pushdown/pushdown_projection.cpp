...
static bool ProjectionCastsToLowerType(LogicalProjection &proj, unique_ptr<Expression> expr) {
	...
	if (expr->type == ExpressionType::BOUND_COLUMN_REF) {
		...
		if (expr->return_type != proj_expr->return_type &&
		    LogicalType::ForceMaxLogicalType(expr->return_type, proj_expr->return_type) == expr->return_type) {
			changes_type = true;
		}
	}
	ExpressionIterator::EnumerateChildren(*expr, [&](unique_ptr<Expression> &child) {
		changes_type |= ProjectionCastsToLowerType(proj, std::move(child));
	});
	return changes_type;
}
...
unique_ptr<LogicalOperator> FilterPushdown::PushdownProjection(unique_ptr<LogicalOperator> op) {
	...
	for (auto &filter : filters) {
		...
		if (is_volatile || ProjectionCastsToLowerType(proj, f.filter->Copy())) {
