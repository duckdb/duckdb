...
bool IsFalseConstant(Expression &expr) {
	...
	return !value.IsNull() && value.type() == LogicalTypeId::BOOLEAN && !value.GetValue<bool>();
}
...
bool IsCoalesceMarkerFalse(Expression &expr) {
	...
	auto &children = coalesce.GetChildren();
	if (children.size() != 2) {
		return false;
	}
	return IsMarkerRef(*children[0]) && IsFalseConstant(*children[1]);
}
	...
	for (idx_t i = 0; i < filters.size(); i++) {
		...
		if (side == JoinSide::LEFT) {
			...
			if (filters[i]->filter->GetExpressionType() == ExpressionType::OPERATOR_NOT) {
				...
				if (!children.empty() && children[0]->GetExpressionType() == ExpressionType::BOUND_COLUMN_REF) {
