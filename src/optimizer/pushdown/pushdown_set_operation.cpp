...
static void ReplaceSetOpBindings(LogicalSetOperation &setop, Expression& expr, size_t child_index) {
	...
	expr.EnumerateChildren([&](Expression *child) {
		ReplaceSetOpBindings(setop, *child, child_index);
	});
}

unique_ptr<LogicalOperator> FilterPushdown::PushdownSetOperation(unique_ptr<LogicalOperator> op) {
	assert(op->type == LogicalOperatorType::UNION || op->type == LogicalOperatorType::EXCEPT || op->type == LogicalOperatorType::INTERSECT);
	...
	for(size_t i = 0; i < 2; i++) {
		if (op->children[i]->type != LogicalOperatorType::SUBQUERY) {
			op->children[i] = make_unique<LogicalSubquery>(move(op->children[i]), optimizer.binder.GenerateTableIndex());
		}
	}
	assert(op->children[0]->type == LogicalOperatorType::SUBQUERY && op->children[1]->type == LogicalOperatorType::SUBQUERY);
