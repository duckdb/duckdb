...
unique_ptr<LogicalOperator> CountWindowElimination::OptimizeInternal(unique_ptr<LogicalOperator> op, ColumnBindingReplacer &replacer) {
    if (op->type == LogicalOperatorType::LOGICAL_FILTER) {
		...
		if (filter.expressions.size() == 1 && filter.children.size() == 1 &&
            ...
            auto agg_expr = make_uniq<BoundAggregateExpression>(
                std::move(count_func),
                std::move(children),
                nullptr,
                std::move(bind_info),
			...
			auto cnt_ref = make_uniq<BoundColumnRefExpression>(
				agg_op->types[agg_op->groups.size()], 
			...
			for(size_t i=0; i<w_expr.partitions.size(); ++i) {
				...
				cond.right = make_uniq<BoundColumnRefExpression>(
					w_expr.partitions[i]->return_type,
