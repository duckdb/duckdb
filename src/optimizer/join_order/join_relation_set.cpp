...
JoinRelationSet &JoinRelationSetManager::GetJoinRelation(array_ptr<idx_t> relations, idx_t count) {
...
JoinRelationSet &JoinRelationSetManager::GetJoinRelation(idx_t index) {
	...
	auto relations = make_array<idx_t>(1);
	relations[0] = index;
	idx_t count = 1;
...
JoinRelationSet &JoinRelationSetManager::GetJoinRelation(unordered_set<idx_t> &bindings) {
	// create a sorted vector of the relations
	array_ptr<idx_t> relations = bindings.empty() ? nullptr : make_array<idx_t>(bindings.size());
	idx_t count = 0;
...
JoinRelationSet &JoinRelationSetManager::Union(JoinRelationSet &left, JoinRelationSet &right) {
	auto relations = make_array<idx_t>(left.count + right.count);
	idx_t count = 0;
...
// 	auto relations = array_ptr<idx_t>(new idx_t[left->count]);
// 	idx_t count = 0;
