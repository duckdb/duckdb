...
template <class T> void RemoveUnusedColumns::ClearUnusedExpressions(vector<T> &list, idx_t table_idx, bool replace)
{
	idx_t offset = 0;
...
void RemoveUnusedColumns::VisitOperator(LogicalOperator &op)
{
	switch (op.logical_type)
	{
		case LogicalOperatorType::LOGICAL_AGGREGATE_AND_GROUP_BY:
		{
			...
			if (!everything_referenced)
			{
				...
				if (aggr.expressions.empty() && aggr.groups.empty())
				{
					...
					aggr.expressions.push_back(function_binder.BindAggregateFunction(count_star_fun, {}, nullptr, AggregateType::NON_DISTINCT));
				}
			}
			...
			remove.VisitOperator(*((LogicalOperator*)op.children[0].get()));
			return;
		}
		...
		case LogicalOperatorType::LOGICAL_COMPARISON_JOIN:
		{
			if (!everything_referenced)
			{
				...
				for (auto &cond : comp_join.conditions)
				{
					...
					{
						if (cond.left->expression_class == ExpressionClass::BOUND_COLUMN_REF && cond.right->expression_class == ExpressionClass::BOUND_COLUMN_REF)
						{
		...
		case LogicalOperatorType::LOGICAL_UNION:
			if (!everything_referenced)
			{
				...
				if (entries.size() < setop.column_count)
				{
					...
					for (idx_t child_idx = 0; child_idx < op.children.size(); child_idx++)
					{
						...
						((LogicalOperator*)child.get())->ResolveOperatorTypes();
						...
						remove.VisitOperator(*((LogicalOperator*)op.children[child_idx].get()));
					}
					return;
				}
			}
			...
			{
				RemoveUnusedColumns remove(binder, context, true);
				remove.VisitOperator(*((LogicalOperator*)child.get()));
			}
			return;
