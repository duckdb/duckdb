...
unique_ptr<LogicalOperator> RemoveObsoleteFilterRule::Apply(Rewriter &rewriter, LogicalOperator &op_root, vector<AbstractOperator> &bindings,
                                  bool &fixed_point) {
    ...
    if (!TypeIsNumeric(compare_one->children[0]->return_type) || !TypeIsNumeric(compare_two->children[0]->return_type)) {
        return nullptr;
    }
    ...
    auto compare_type_left =  constant_left_side > 0 ? ComparisonExpression::FlipComparisionExpression(compare_one->type) : compare_one->type;
    auto compare_type_right = constant_right_side > 0 ? ComparisonExpression::FlipComparisionExpression(compare_two->type) : compare_two->type;
    int discard = -1;
