...
static optional<TableColumnOrigin> GetTableColumnOrigin(LogicalOperator &op, idx_t output_idx) {
	...
	if (op.type == LogicalOperatorType::LOGICAL_PROJECTION) {
		...
		auto child_idx = GetDirectReferenceIndex(*projection.expressions[output_idx], *projection.children[0]);
		return child_idx.IsValid() ? GetTableColumnOrigin(*projection.children[0], child_idx.GetIndex()) : nullopt;
	}
	...
	if (!table || !get.children.empty() || get.table_filters.HasFilters() ||
	    (get.dynamic_filters && get.dynamic_filters->HasFilters()) || !get.scan_partition_indices.empty()) {
		return nullopt;
	}
...
static optional<TableColumnOrigin> GetTableColumnOrigin(LogicalOperator &op, const Expression &expression) {
	...
	return output_idx.IsValid() ? GetTableColumnOrigin(op, output_idx.GetIndex()) : nullopt;
}
