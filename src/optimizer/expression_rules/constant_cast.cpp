...
ConstantCastRule::ConstantCastRule() {
	root = make_unique_base<AbstractRuleNode, ExpressionNodeType>(ExpressionType::OPERATOR_CAST);
...
unique_ptr<Expression> ConstantCastRule::Apply(Rewriter &rewriter, Expression &root, vector<AbstractOperator> &bindings,
                                               bool &fixed_point) {
	auto &cast_expr = (CastExpression &)root;
	auto const_expr = reinterpret_cast<ConstantExpression *>(root.children[0].get());
