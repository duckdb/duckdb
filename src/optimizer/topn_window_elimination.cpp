		...
		case LogicalOperatorType::LOGICAL_PROJECTION: {
			...
			for (idx_t i = 0; i < projections.size(); i++) {
				...
				if (!IsRecreatableByLateMaterialization(*projection.expressions[projection_idx])) {
					// The payload is derived through a value-transforming expression that the rowid semi-join cannot
					// reconstruct. Fall back to carrying the computed value through the aggregate instead.
					return false;
				}
