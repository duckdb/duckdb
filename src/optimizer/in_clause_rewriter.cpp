...
unique_ptr<Expression> InClauseRewriter::VisitReplace(BoundOperatorExpression &expr, unique_ptr<Expression> *expr_ptr) {
	...
	vector<PhysicalType> types = {in_type};
	...
	unique_ptr<Expression> result =
	    make_unique<BoundColumnRefExpression>("IN (...)", PhysicalType::BOOL, ColumnBinding(chunk_index, 0));
	if (!is_regular_in) {
		// NOT IN: invert
		auto invert = make_unique<BoundOperatorExpression>(ExpressionType::OPERATOR_NOT, PhysicalType::BOOL);
