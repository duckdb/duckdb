...
struct MeasureTypeState {
	...
	const Expression&  MeasureBoundExpression() const {
		return *measure_type.GetAuxInfoShrPtr()->Cast<MeasureTypeInfo>().bound_measure_expression;
	}

	MeasureTypeState(LogicalType measure_type, uint64_t measure_id) : measure_type(measure_type), measure_id(measure_id) {}
	...
	friend std::ostream& operator<<(std::ostream& os, const MeasureTypeState& state) {
		...
		os << "Measure ID " << state.measure_id << " called " << measure_type_info.alias << " with bound expression " << state.MeasureBoundExpression().ToString();
		return os;
	}
};
...
struct MeasureRewriteState : public LogicalOperatorVisitor {
	...
	void VisitExpression(unique_ptr<Expression> *expression) override {
		...
		if (expr.return_type.id() == LogicalTypeId::MEASURE_TYPE) {
			...
			// Analyze the expression to find dependencies on other measures
			FindMeasureDependencies(expr, measure_id);
			
			std::cerr << "MEASURE USAGE found: ID=" << measure_id 
			          << ", Type=" << expr.return_type.ToString() 
			          << ", Expression=" << expr.ToString() 
			          << ", Operator=" << op_stack.top()->GetName() << std::endl;
		}
		...
		if (expr.GetExpressionType() == ExpressionType::BOUND_AGGREGATE) {
			...
			std::cerr << "Unhandled expression type: " << ExpressionTypeToString(expr.GetExpressionType()) << " " << expr.ToString() << std::endl;
		}
		
		VisitExpressionChildren(expr);
	}

	// Find measure dependencies within an expression
	void FindMeasureDependencies(const Expression &expr, uint64_t current_measure_id) {
		// Check if this expression references another measure
		if (expr.return_type.id() == LogicalTypeId::MEASURE_TYPE) {
			// Find the measure ID for this type
			for (uint64_t i = 0; i < measure_type_states.size(); i++) {
				if (measure_type_states[i].measure_type == expr.return_type && i != current_measure_id) {
					// Add dependency: current measure depends on this referenced measure
					measure_type_states[current_measure_id].measure_ids_referenced.push_back(i);
					break;
				}
			}
		}
		...
		ExpressionIterator::EnumerateChildren(expr, [&](const Expression &child) {
			FindMeasureDependencies(child, current_measure_id);
		});
	}
	...
	friend std::ostream& operator<<(std::ostream& os, const MeasureRewriteState& state) {
		...
		for (const auto &measure_state : state.measure_type_states) {
			...
			if (!measure_state.bound_aggregate_invocations.empty()) {
				os << "Bound aggregate expressions: " << measure_state.bound_aggregate_invocations.size() << " found:\n";
				size_t i = 1;
...
struct MeasureSimpleReplacer : public LogicalOperatorVisitor {
	...
	MeasureSimpleReplacer(MeasureTypeState& measure_state) : measure_state(measure_state) {}
	...
	void VisitExpression(unique_ptr<Expression> *expression) override {
		...
		if (measure_state.bound_aggregate_invocations.find(expr) != measure_state.bound_aggregate_invocations.end()) {
			...
			*expression = bound_expression.Copy();
		}
...
void MeasureAggRewriter::RewriteMeasures(unique_ptr<LogicalOperator> &plan) {
	...
	for(;;) {
		...
		MeasureSimpleReplacer measure_simple_replacer(*measure_state);
