...
void FilterCombiner::GenerateFilters(const std::function<void(unique_ptr<Expression> filter)> &callback) {
	...
	for (auto &entry : equivalence_map) {
		...
		// regenerate the comparison connecting each pair: equality by default, or IS NOT DISTINCT FROM when the
		// pair is only connected that way (no explicit equality, no constant to make every member non-NULL)
...
FilterResult FilterCombiner::AddBoundComparisonFilter(Expression &expr) {
	...
	if (left_is_scalar || right_is_scalar) {
		...
		if (expr.GetExpressionType() == ExpressionType::COMPARE_NOT_DISTINCT_FROM) {
			// every member of the left set is connected to every member of the right one: the members inside
			// each set already are, and NDF is transitive
