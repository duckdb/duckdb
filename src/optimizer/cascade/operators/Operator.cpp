...
//
//---------------------------------------------------------------------------
Operator* Operator::PexprRehydrate(CCostContext* pcc, duckdb::vector<Operator*> pdrgpexpr, CDrvdPropCtxtPlan* pdpctxtplan)
{
...
bool Operator::FMatchPattern(CGroupExpression* pgexpr)
{
	...
	else
	{
		...
		if ((this->logical_type == pgexpr->m_pop->logical_type || this->physical_type == pgexpr->m_pop->physical_type) && this->Arity() == pgexpr->Arity())
		{
			return true;
		}
	}
...
duckdb::unique_ptr<Operator> Operator::CopywithNewChilds(CGroupExpression* pgexpr, duckdb::vector<duckdb::unique_ptr<Operator>> pdrgpexpr, double cost)
{
	duckdb::unique_ptr<Operator> result = make_uniq<Operator>();
	result->m_pgexpr = pgexpr;
