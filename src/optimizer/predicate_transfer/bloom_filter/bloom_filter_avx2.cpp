...
inline __m256i BlockedBloomFilter::mask_avx2(__m256i hash) const {
	...
	__m256i rotation =
	    _mm256_and_si256(_mm256_srli_epi64(hash, BloomFilterMasks::kLogNumMasks), _mm256_set1_epi64x(63));

	result = _mm256_or_si256(_mm256_sllv_epi64(result, rotation),
...
int64_t BlockedBloomFilter::Find_avx2(int64_t num_rows, const uint64_t *hashes, uint8_t *result_bit_vector) const {
	...
	for (int64_t i = 0; i < num_rows / unroll; ++i) {
		...
		uint64_t result_bytes =
		    static_cast<uint32_t>(_mm256_movemask_epi8(_mm256_cmpeq_epi64(_mm256_and_si256(block_A, mask_A), mask_A)));
		result_bytes |=
		    static_cast<uint64_t>(_mm256_movemask_epi8(_mm256_cmpeq_epi64(_mm256_and_si256(block_B, mask_B), mask_B)))
		    << 32;
