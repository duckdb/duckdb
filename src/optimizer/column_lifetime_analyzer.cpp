...
void ColumnLifetimeAnalyzer::GenerateProjectionMap(vector<ColumnBinding> bindings, column_binding_set_t &unused_bindings, vector<idx_t> &projection_map)
{
...
void ColumnLifetimeAnalyzer::VisitOperator(LogicalOperator &op)
{
	...
	case LogicalOperatorType::LOGICAL_AGGREGATE_AND_GROUP_BY:
	{
		...
		analyzer.VisitOperator(*((LogicalOperator*)op.children[0].get()));
		return;
	}
	...
	case LogicalOperatorType::LOGICAL_COMPARISON_JOIN:
	{
		...
		if (comp_join.join_type == JoinType::MARK || comp_join.join_type == JoinType::SEMI || comp_join.join_type == JoinType::ANTI)
		{
			break;
		}
	...
	case LogicalOperatorType::LOGICAL_INTERSECT:
		...
		for (auto &child : op.children)
		{
			...
			analyzer.VisitOperator(*((LogicalOperator*)child.get()));
		}
		return;
