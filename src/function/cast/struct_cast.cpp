	...
	for (idx_t r = 0; r < count; r++) {
		...
		for (idx_t c = 0; c < children.size(); c++) {
			...
			if (!is_unnamed) {
				...
				string_length +=
				    VectorCastHelpers::CalculateEscapedStringLength<true>(name.GetName(), key_needs_quotes[c]);
		...
		for (idx_t c = 0; c < children.size(); c++) {
			...
			if (!is_unnamed) {
				...
				// "{<name>: <value>}"
				offset +=
				    VectorCastHelpers::WriteEscapedString<true>(dataptr + offset, name.GetName(), key_needs_quotes[c]);
				dataptr[offset++] = ':';
				dataptr[offset++] = ' ';
			}
			// value
...
static bool StructToMapCast(Vector &source, Vector &result, idx_t count, CastParameters &parameters) {
	...
	for (idx_t field_idx = 0; field_idx < field_count; field_idx++) {
		...
		key_data.WriteValue(field_name.GetName());
	}
