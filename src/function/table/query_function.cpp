...
static unique_ptr<SubqueryRef> ParseSubquery(ClientContext &context, const string &query, const string &err_msg) {
...
static unique_ptr<TableRef> QueryBindReplace(ClientContext &context, TableFunctionBindInput &input) {
	...
	auto subquery_ref = ParseSubquery(context, query, "Expected a single SELECT statement");
...
static unique_ptr<TableRef> TableBindReplace(ClientContext &context, TableFunctionBindInput &input) {
	...
	auto subquery_ref = ParseSubquery(context, query, "Expected a table or a list with tables as input");
	return std::move(subquery_ref);
}
