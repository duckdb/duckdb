...
static void Execute(ClientContext &context, TableFunctionInput &input, DataChunk &output) {
	...
	for (idx_t error_idx = state.cursor; error_idx < next_idx; error_idx++) {
		...
		output.SetValue(2, count, Value(err.column_name));
		output.SetValue(3, count, Value(err.parsed_value));
		output.SetValue(4, count, Value(err.error));
		output.SetValue(5, count, Value(err.file_name));
		count++;
	}
