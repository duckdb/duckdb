...
void TestLoggingFunction(ClientContext &context, TableFunctionInput &data_p, DataChunk &output) {
	...
	if (bind_data.logging_enabled) {
		if (bind_data.log_entries_per_tuple > 1) {
			InnerLogTestLoop<true,true>(data,bind_data, tuples_to_create, output, context);
		} else {
			InnerLogTestLoop<true>(data,bind_data, tuples_to_create, output, context);
		}
	} else {
		InnerLogTestLoop<false>(data, bind_data, tuples_to_create, output, context);
	}
}
