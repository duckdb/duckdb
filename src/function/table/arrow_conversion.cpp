...
static void ArrowToDuckDBList(Vector &vector, ArrowArray &array, ArrowArrayScanState &array_state, idx_t size,
	...
	auto &list_info = arrow_type.GetTypeInfo<ArrowListInfo>();
	ArrowToDuckDBConversion::SetValidityMask(vector, array, scan_state.chunk_offset, size, parent_offset,
	                                         nested_offset);

	auto effective_offset = GetEffectiveOffset(array, parent_offset, scan_state.chunk_offset, nested_offset);
	...
	ArrowToDuckDBConversion::SetValidityMask(child_vector, *array.children[0], scan_state.chunk_offset, list_size,
	                                         array.offset, NumericCast<int64_t>(start_offset));
...
static void ArrowToDuckDBArray(Vector &vector, ArrowArray &array, ArrowArrayScanState &array_state, idx_t size,
	...
	auto child_offset = GetEffectiveOffset(array, parent_offset, scan_state.chunk_offset, nested_offset) * array_size;

	ArrowToDuckDBConversion::SetValidityMask(vector, array, scan_state.chunk_offset, size, parent_offset,
	                                         nested_offset);

	auto &child_vector = ArrayVector::GetEntry(vector);
	ArrowToDuckDBConversion::SetValidityMask(child_vector, *array.children[0], scan_state.chunk_offset, child_count,
	                                         array.offset, NumericCast<int64_t>(child_offset));
...
static void ArrowToDuckDBBlob(Vector &vector, ArrowArray &array, const ArrowScanLocalState &scan_state, idx_t size,
	...
	if (size_type == ArrowVariableSizeType::FIXED_SIZE) {
		...
		idx_t offset = GetEffectiveOffset(array, parent_offset, scan_state.chunk_offset, nested_offset) * fixed_size;
		...
		auto offsets = ArrowBufferData<uint32_t>(array, 1) +
		               GetEffectiveOffset(array, parent_offset, scan_state.chunk_offset, nested_offset);
...
static void DirectConversion(Vector &vector, ArrowArray &array, const ArrowScanLocalState &scan_state,
	...
	auto data_ptr = ArrowBufferData<data_t>(array, 1) +
	                internal_type * GetEffectiveOffset(array, NumericCast<int64_t>(parent_offset),
	                                                   scan_state.chunk_offset, nested_offset);
...
static void ColumnArrowToDuckDBRunEndEncoded(Vector &vector, const ArrowArray &array, ArrowArrayScanState &array_state,
	...
	if (!run_end_encoding.run_ends) {
		...
		auto &values = *run_end_encoding.values;
		ArrowToDuckDBConversion::SetValidityMask(values, values_array, scan_state.chunk_offset, compressed_size,
	...
	idx_t scan_offset =
	    GetEffectiveOffset(array, NumericCast<int64_t>(parent_offset), scan_state.chunk_offset, nested_offset);
...
static void ColumnArrowToDuckDB(Vector &vector, ArrowArray &array, ArrowArrayScanState &array_state, idx_t size,
	...
	case LogicalTypeId::VARCHAR: {
		...
		case ArrowVariableSizeType::SUPER_SIZE: {
			auto cdata = ArrowBufferData<char>(array, 2);
			auto offsets =
			    ArrowBufferData<uint64_t>(array, 1) +
			    GetEffectiveOffset(array, NumericCast<int64_t>(parent_offset), scan_state.chunk_offset, nested_offset);
		...
		case ArrowVariableSizeType::FIXED_SIZE: {
			auto cdata = ArrowBufferData<char>(array, 2);
			auto offsets =
			    ArrowBufferData<uint32_t>(array, 1) +
			    GetEffectiveOffset(array, NumericCast<int64_t>(parent_offset), scan_state.chunk_offset, nested_offset);
			SetVectorString(vector, size, cdata, offsets);
			break;
		}
	...
	case LogicalTypeId::TIME: {
		...
		case ArrowDateTimeType::NANOSECONDS: {
			auto tgt_ptr = FlatVector::GetData<dtime_t>(vector);
			auto src_ptr =
			    ArrowBufferData<int64_t>(array, 1) +
			    GetEffectiveOffset(array, NumericCast<int64_t>(parent_offset), scan_state.chunk_offset, nested_offset);
			for (idx_t row = 0; row < size; row++) {
	...
	case LogicalTypeId::DECIMAL: {
		...
		case DecimalBitWidth::DECIMAL_32: {
			auto src_ptr =
			    ArrowBufferData<int32_t>(array, 1) +
			    GetEffectiveOffset(array, NumericCast<int64_t>(parent_offset), scan_state.chunk_offset, nested_offset);
		...
		case DecimalBitWidth::DECIMAL_128: {
			auto src_ptr =
			    ArrowBufferData<hugeint_t>(array, 1) +
			    GetEffectiveOffset(array, NumericCast<int64_t>(parent_offset), scan_state.chunk_offset, nested_offset);
			ConvertDecimal(src_ptr, vector, array, size, nested_offset, parent_offset, scan_state, val_mask, bit_width);
			break;
		}
	...
	case LogicalTypeId::STRUCT: {
		...
		for (idx_t child_idx = 0; child_idx < NumericCast<idx_t>(array.n_children); child_idx++) {
			...
			ArrowToDuckDBConversion::SetValidityMask(child_entry, child_array, scan_state.chunk_offset, size,
			                                         array.offset, nested_offset);
	...
	case LogicalTypeId::UNION: {
		...
		for (idx_t child_idx = 0; child_idx < NumericCast<idx_t>(array.n_children); child_idx++) {
			...
			ArrowToDuckDBConversion::SetValidityMask(child, child_array, scan_state.chunk_offset, size,
			                                         NumericCast<int64_t>(parent_offset), nested_offset);
			auto array_physical_type = child_type.GetPhysicalType();
...
static void ColumnArrowToDuckDBDictionary(Vector &vector, ArrowArray &array, ArrowArrayScanState &array_state,
	...
	if (array_state.CacheOutdated(array.dictionary)) {
		...
		ArrowToDuckDBConversion::SetValidityMask(*base_vector, *array.dictionary, scan_state.chunk_offset,
	...
	if (has_nulls) {
		...
		GetValidityMask(indices_validity, array, scan_state.chunk_offset, size, NumericCast<int64_t>(parent_offset));
