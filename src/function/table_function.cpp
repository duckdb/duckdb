...
TableFunction::TableFunction(Identifier name, const vector<LogicalType> &arguments, table_function_t function_,
    ...
    : SimpleNamedParameterFunction(std::move(name), arguments), bind(bind), bind_replace(nullptr),
      ...
      pushdown_expression(nullptr), combine_schema(nullptr), get_file_columns(nullptr),
      get_file_partition_stats(nullptr), claim_batch(nullptr), finish_batch(nullptr), supports_read_ahead(nullptr),
      schedule_io(nullptr), prepare_read_ahead(nullptr), to_string(nullptr), table_scan_progress(nullptr),
      ...
      projection_pushdown(false), reuses_local_state(false), supports_cast_map(false), filter_pushdown(false),
