...
TableFunction::TableFunction(Identifier name, const vector<LogicalType> &arguments, table_function_t function_,
    ...
    : SimpleNamedParameterFunction(std::move(name), arguments), bind(bind), bind_replace(nullptr),
      ...
      pushdown_expression(nullptr), combine_schema(nullptr), get_file_columns(nullptr), get_file_partition_stats(nullptr), claim_batch(nullptr), finish_batch(nullptr),
      ...
      projection_pushdown(false), reuses_local_state(false), supports_cast_map(false), filter_pushdown(false), filter_prune(false),
