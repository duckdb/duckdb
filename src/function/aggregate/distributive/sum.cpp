...
AggregateFunction GetSumAggregate(PhysicalType type) {
	...
	case PhysicalType::INT32: {
		auto function = AggregateFunction::UnaryAggregate<sum_state_t<hugeint_t>, int32_t, hugeint_t, SumToHugeintOperation>(
		    LogicalType::INTEGER, LogicalType::HUGEINT);
		function.statistics = sum_propagate_stats;
		return function;
	}
	case PhysicalType::INT64: {
		auto function = AggregateFunction::UnaryAggregate<sum_state_t<hugeint_t>, int64_t, hugeint_t, SumToHugeintOperation>(
		    LogicalType::BIGINT, LogicalType::HUGEINT);
		function.statistics = sum_propagate_stats;
		return function;
	}
