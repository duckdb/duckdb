...
struct CombineState : public FunctionLocalState {
	...
	array_ptr<data_t> state_buffer0, state_buffer1;
	Vector state_vector0, state_vector1;

	explicit CombineState(idx_t state_size_p)
	    : state_size(state_size_p), state_buffer0(make_array<data_t>(state_size_p)),
	      state_buffer1(make_array<data_t>(state_size_p)),
...
struct FinalizeState : public FunctionLocalState {
	idx_t state_size;
	array_ptr<data_t> state_buffer;
	Vector addresses;

	explicit FinalizeState(idx_t state_size_p)
	    : state_size(state_size_p), state_buffer(make_array<data_t>(STANDARD_VECTOR_SIZE * AlignValue(state_size_p))),
