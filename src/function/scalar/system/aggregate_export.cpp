...
static void AggregateStateFinalize(DataChunk &input, ExpressionState &state_p, Vector &result) {
	...
	CanonicalFormat state_data;
	input.data[0].ToCanonical(input.size(), state_data);
...
static void AggregateStateCombine(DataChunk &input, ExpressionState &state_p, Vector &result) {
	...
	CanonicalFormat state0_data, state1_data;
	input.data[0].ToCanonical(input.size(), state0_data);
	input.data[1].ToCanonical(input.size(), state1_data);
