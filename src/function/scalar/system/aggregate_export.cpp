...
static void DeserializeField(const LogicalType &type, const AggregateStateField &field, const Vector &input_vec,
                              idx_t count, data_ptr_t dest_buffer, idx_t stride, idx_t base,
	...
	case AggregateFieldKind::STRUCT: {
		...
		const idx_t new_base = base + field.field_offset;
		for (idx_t field_idx = 0; field_idx < field.children.size(); field_idx++) {
			DeserializeField(child_types[field_idx].second, field.children[field_idx], struct_entries[field_idx],
			                 count, dest_buffer, stride, new_base, allocator);
		}
		break;
	}
	case AggregateFieldKind::PRIMITIVE:
		TemplateDispatch<LoadOp>(type.InternalType(), stride, input_vec, count, dest_buffer,
