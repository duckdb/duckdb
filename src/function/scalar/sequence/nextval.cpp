...
struct CurrentSequenceValueOperator {
	static int64_t Operation(DTransaction &transaction, SequenceCatalogEntry *seq) {
...
static void NextValFunction(DataChunk &args, ExpressionState &state, Vector &result) {
	...
	if (info.sequence) {
		auto &transaction = DTransaction::Get(context, *info.sequence->catalog);
		...
		UnaryExecutor::Execute<string_t, int64_t>(input, result, args.size(), [&](string_t value) {
			...
			auto &transaction = DTransaction::Get(context, *sequence->catalog);
