...
MapInvalidReason CheckMapValidity(Vector &map, idx_t count, const SelectionVector &sel) {
	...
	CanonicalFormat map_vdata;
	map.ToCanonical(count, map_vdata);
	auto &map_validity = map_vdata.validity;

	auto &key_vector = *(StructVector::GetEntries(map)[0]);
	CanonicalFormat key_vdata;
	key_vector.ToCanonical(count, key_vdata);
	...
	CanonicalFormat key_entry_vdata;
	key_entries.ToCanonical(count, key_entry_vdata);
