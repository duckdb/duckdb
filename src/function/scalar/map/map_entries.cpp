...
struct ListStructValuesFunctor {
	static Vector &FetchVector(Vector &list) {
		auto &struct_vec = ListEntryFunctor::FetchVector(list);
		D_ASSERT(struct_vec.GetType().id() == LogicalTypeId::STRUCT);
		return *StructVector::GetEntries(struct_vec)[1];
	}
};

struct ListStructKeysFunctor {
	static Vector &FetchVector(Vector &list) {
		auto &struct_vec = ListEntryFunctor::FetchVector(list);
		D_ASSERT(struct_vec.GetType().id() == LogicalTypeId::STRUCT);
		return *StructVector::GetEntries(struct_vec)[0];
	}
};
...
static void MapEntriesFunction(DataChunk &args, ExpressionState &state, Vector &result) {
	...
	MapFetchInternals<MapValuesFunctor, ListStructValuesFunctor, false>(args, state, result);
	MapFetchInternals<MapKeysFunctor, ListStructKeysFunctor, true>(args, state, result);
