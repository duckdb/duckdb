...
unique_ptr<FunctionData> DecimalDivBind(BindScalarFunctionInput &input) {
	...
	if (Settings::Get<DecimalArithmeticSetting>(input.GetClientContext()) == DecimalArithmetic::DOUBLE) {
		bound_function.GetArguments()[0] = LogicalType::DOUBLE;
		bound_function.GetArguments()[1] = LogicalType::DOUBLE;
		bound_function.SetReturnType(LogicalType::DOUBLE);
		if (Settings::Get<IeeeFloatingPointOpsSetting>(input.GetClientContext())) {
			bound_function.SetFunctionCallback(GetScalarBinaryFunction<DivideOperator>(PhysicalType::DOUBLE));
		} else {
			bound_function.SetFunctionCallback(GetBinaryFunctionIgnoreZero<DivideOperator>(PhysicalType::DOUBLE));
		}
		return nullptr;
	}
