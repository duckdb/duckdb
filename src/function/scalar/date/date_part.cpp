...
template <>
int64_t YearOperator::Operation(dtime_s input) {
...
template <>
int64_t MonthOperator::Operation(dtime_s input) {
...
template <>
int64_t DayOperator::Operation(dtime_s input) {
	throw NotImplementedException("\"time\" units \"month\" not recognized");
}
...
template <>
int64_t DecadeOperator::Operation(dtime_s input) {
...
template <>
int64_t CenturyOperator::Operation(dtime_s input) {
...
template <>
int64_t MilleniumOperator::Operation(dtime_s input) {
...
template <>
int64_t QuarterOperator::Operation(dtime_s input) {
...
template <>
int64_t DayOfWeekOperator::Operation(dtime_s input) {
...
template <>
int64_t ISODayOfWeekOperator::Operation(dtime_s input) {
...
template <>
int64_t DayOfYearOperator::Operation(dtime_s input) {
...
template <>
int64_t WeekOperator::Operation(dtime_s input) {
...
template <>
int64_t MicrosecondsOperator::Operation(dtime_s input) {
	// remove everything but the second & microsecond part
...
template <>
int64_t MillisecondsOperator::Operation(dtime_s input) {
	return MicrosecondsOperator::Operation<dtime_s, int64_t>(input) / Interval::MICROS_PER_MSEC;
}
...
template <>
int64_t SecondsOperator::Operation(dtime_s input) {
	return MicrosecondsOperator::Operation<dtime_s, int64_t>(input) / Interval::MICROS_PER_SEC;
}
...
template <>
int64_t MinutesOperator::Operation(dtime_s input) {
	// remove the hour part, and truncate to minutes
...
template <>
int64_t HoursOperator::Operation(dtime_s input) {
...
template <>
int64_t EpochOperator::Operation(dtime_s input) {
	return SecondsOperator::Operation<dtime_s, int64_t>(input);
}

template <>
unique_ptr<BaseStatistics>
EpochOperator::PropagateStatistics<dtime_s>(ClientContext &context, BoundFunctionExpression &expr,
...
static void AddTimePartOperator(BuiltinFunctions &set, string name) {
	AddGenericTimePartOperator(
	    ...
	    ScalarFunction::UnaryFunction<dtime_s, int64_t, OP>, OP::template PropagateStatistics<date_t>,
	    OP::template PropagateStatistics<timestamp_t>, OP::template PropagateStatistics<dtime_s>);
}
...
void DatePartFun::RegisterFunction(BuiltinFunctions &set) {
	...
	date_part.AddFunction(
	    ScalarFunction({LogicalType::VARCHAR, LogicalType::TIME}, LogicalType::BIGINT,
	                   ScalarFunction::BinaryFunction<string_t, dtime_s, int64_t, DatePartOperator, true>));
	date_part.AddFunction(
