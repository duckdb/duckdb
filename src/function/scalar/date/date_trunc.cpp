...
// TODO date_trunc function should also handle interval data type when it is implemented. See https://www.postgresql.org/docs/9.1/functions-datetime.html
...
struct MinuteTruncOperator {
    template <class TA, class TR> static inline TR Operation(TA input) {
        date_t date = Timestamp::GetDate(input);
        return Timestamp::FromDatetime(date, Time::FromTime(Timestamp::GetHours(input), Timestamp::GetMinutes(input), 0, 0));
    }
};
...
struct SecondsTruncOperator {
    template <class TA, class TR> static inline TR Operation(TA input) {
        date_t date = Timestamp::GetDate(input);
        return Timestamp::FromDatetime(date, Time::FromTime(Timestamp::GetHours(input), Timestamp::GetMinutes(input), Timestamp::GetSeconds(input), 0));
    }
};
