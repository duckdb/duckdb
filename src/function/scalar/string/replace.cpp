...
static idx_t
next_needle(const char* input_haystack, idx_t size_haystack, const char* input_needle, const idx_t size_needle)
{
...
static string_t replace_scalar_function(const string_t& haystack, const string_t& needle, const string_t& thread, vector<char>& result) {
...
static void replace_function(DataChunk &args, ExpressionState &state, Vector &result) {
    assert(args.column_count() == 3 &&
           args.data[0].type == TypeId::VARCHAR &&
           args.data[1].type == TypeId::VARCHAR &&
    ...
    TernaryExecutor::Execute<string_t, string_t, string_t, string_t>(
        ...
        [&](string_t input_string, string_t needle_string, string_t thread_string) {
            return StringVector::AddString(
                result, replace_scalar_function(input_string, needle_string, thread_string, buffer));
        });
}
