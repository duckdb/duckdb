...
template <bool IS_UPPER> static string_t strcase_ascii(Vector &result, const char *input_data, idx_t input_length) {
	...
	for (idx_t i = 0; i < input_length; i++) {
		result_data[i] = IS_UPPER ? ascii_to_upper_map[uint8_t(input_data[i])] : ascii_to_lower_map[uint8_t(input_data[i])];
	}
	result_str.Finalize();
	return result_str;
}
...
template <bool IS_UPPER> static string_t strcase_unicode(Vector &result, const char *input_data, idx_t input_length) {
	...
	for (idx_t i = 0; i < input_length;) {
		if (input_data[i] & 0x80) {
			...
			// ascii
			*result_data = IS_UPPER ? ascii_to_upper_map[uint8_t(input_data[i])] : ascii_to_lower_map[uint8_t(input_data[i])];
			result_data++;
			i++;
		}
	}
...
template<bool IS_UPPER>
static unique_ptr<BaseStatistics> caseconvert_propagate_stats(
...
ScalarFunction LowerFun::GetFunction() {
	return ScalarFunction({LogicalType::VARCHAR}, LogicalType::VARCHAR,
		caseconvert_function<false>, false, nullptr, nullptr, caseconvert_propagate_stats<false>);
}
...
void UpperFun::RegisterFunction(BuiltinFunctions &set) {
	set.AddFunction({"upper", "ucase"},
	                ScalarFunction({LogicalType::VARCHAR}, LogicalType::VARCHAR, caseconvert_function<true>, false, nullptr, nullptr, caseconvert_propagate_stats<true>));
}
