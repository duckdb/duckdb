...
static void bitwise_lshift_function(ExpressionExecutor &exec, Vector inputs[], index_t input_count, BoundFunctionExpression &expr,
                  Vector &result) {
	result.Initialize(inputs[0].type);
	VectorOperations::BitwiseShiftLeft(inputs[0], inputs[1], result);
}

void LeftShift::RegisterFunction(BuiltinFunctions &set) {
	FunctionSet functions("bitwise_lshift");
