...
public:
	TypeTypeParameter(LogicalType type_p) : TypeParameter(TypeParameterKind::TYPE), type(std::move(type_p)) {
	}

	unique_ptr<TypeParameter> Copy() const override {
		return make_uniq<TypeTypeParameter>(type);
	}
...
public:
	ExpressionTypeParameter(unique_ptr<ParsedExpression> expr_p)
	    : TypeParameter(TypeParameterKind::EXPRESSION), expr(std::move(expr_p)) {
	}

	unique_ptr<TypeParameter> Copy() const override {
		return make_uniq<ExpressionTypeParameter>(expr->Copy());
	}
...
unique_ptr<TypeParameter> TypeParameter::TYPE(LogicalType type) {
	return make_uniq<TypeTypeParameter>(std::move(type));
}

unique_ptr<TypeParameter> TypeParameter::EXPRESSION(unique_ptr<ParsedExpression> expr) {
	return make_uniq<ExpressionTypeParameter>(std::move(expr));
}
