...
template <>
struct RowMatchOP<DistinctFrom> {
	template <class T>
	static bool Operation(const T &left, const T &right, bool left_null, bool right_null) {
...
template <>
struct RowMatchOP<NotDistinctFrom> {
	template <class T>
	static bool Operation(const T &left, const T &right, bool left_null, bool right_null) {
...
template <class T, class OP, bool NO_MATCH_SEL, bool COMPARE_NULL>
static void TemplatedMatchType(UnifiedVectorFormat &col, Vector &rows, SelectionVector &sel, idx_t &count,
	...
	if (!col.validity.AllValid()) {
		for (idx_t i = 0; i < count; i++) {
			...
			auto lhs_idx = col.sel->get_index(idx);
			const auto &rhs_row = ptrs[idx];
			ValidityBytes row_mask(rhs_row);

			auto lhs_is_null = !col.validity.RowIsValid(lhs_idx);
			auto rhs_is_null = !row_mask.RowIsValid(row_mask.GetValidityEntry(entry_idx), idx_in_entry);

			const auto &lhs_value = data[lhs_idx];
			auto rhs_value = Load<T>(rhs_row + col_offset);

			if (!lhs_is_null && !rhs_is_null) {
				// normal comparison
				if (RowMatchOP<OP>::template Operation<T>(lhs_value, rhs_value, false, false)) {
			...
			// special null comparison for (NOT) DISTINCT FROM
			if (RowMatchOP<OP>::template Operation<T>(lhs_value, rhs_value, lhs_is_null, rhs_is_null)) {
				// match: move to next value to compare
				sel.set_index(match_count++, idx);
			} else {
				if (NO_MATCH_SEL) {
	...
	// all LHS values are valid (no NULLs)
	for (idx_t i = 0; i < count; i++) {
		auto idx = sel.get_index(i);
		...
		if (!rhs_is_null && RowMatchOP<OP>::template Operation<T>(data[col_idx], value, false, rhs_is_null)) {
			sel.set_index(match_count++, idx);
		} else {
			if (NO_MATCH_SEL) {
				no_match->set_index(no_match_count++, idx);
...
template <class OP, bool NO_MATCH_SEL, bool COMPARE_NULL>
...
static void TemplatedMatchStruct(Vector &vec, UnifiedVectorFormat &col, const TupleDataLayout &layout, Vector &rows,
	...
	for (idx_t struct_col_no = 0; struct_col_no < struct_layout.ColumnCount(); struct_col_no++) {
		...
		TemplatedMatchOp<OP, NO_MATCH_SEL, COMPARE_NULL>(struct_vec, struct_col, struct_layout, struct_rows, sel, count,
...
template <class OP, bool NO_MATCH_SEL, bool COMPARE_NULL = false>
static void TemplatedMatchOp(Vector &vec, UnifiedVectorFormat &col, const TupleDataLayout &layout, Vector &rows,
                             SelectionVector &sel, idx_t &count, idx_t col_no, SelectionVector *no_match,
                             idx_t &no_match_count, const idx_t original_count) {
	if (count == 0) {
	...
	case PhysicalType::INT8:
		TemplatedMatchType<int8_t, OP, NO_MATCH_SEL, COMPARE_NULL>(col, rows, sel, count, col_offset, col_no, no_match,
		                                                           no_match_count);
		break;
	case PhysicalType::INT16:
		TemplatedMatchType<int16_t, OP, NO_MATCH_SEL, COMPARE_NULL>(col, rows, sel, count, col_offset, col_no, no_match,
		                                                            no_match_count);
		break;
	case PhysicalType::INT32:
		TemplatedMatchType<int32_t, OP, NO_MATCH_SEL, COMPARE_NULL>(col, rows, sel, count, col_offset, col_no, no_match,
		                                                            no_match_count);
		break;
	case PhysicalType::INT64:
		TemplatedMatchType<int64_t, OP, NO_MATCH_SEL, COMPARE_NULL>(col, rows, sel, count, col_offset, col_no, no_match,
		                                                            no_match_count);
		break;
	case PhysicalType::UINT8:
		TemplatedMatchType<uint8_t, OP, NO_MATCH_SEL, COMPARE_NULL>(col, rows, sel, count, col_offset, col_no, no_match,
		                                                            no_match_count);
		break;
	case PhysicalType::UINT16:
		TemplatedMatchType<uint16_t, OP, NO_MATCH_SEL, COMPARE_NULL>(col, rows, sel, count, col_offset, col_no,
		                                                             no_match, no_match_count);
		break;
	case PhysicalType::UINT32:
		TemplatedMatchType<uint32_t, OP, NO_MATCH_SEL, COMPARE_NULL>(col, rows, sel, count, col_offset, col_no,
		                                                             no_match, no_match_count);
		break;
	case PhysicalType::UINT64:
		TemplatedMatchType<uint64_t, OP, NO_MATCH_SEL, COMPARE_NULL>(col, rows, sel, count, col_offset, col_no,
		                                                             no_match, no_match_count);
		break;
	case PhysicalType::INT128:
		TemplatedMatchType<hugeint_t, OP, NO_MATCH_SEL, COMPARE_NULL>(col, rows, sel, count, col_offset, col_no,
		                                                              no_match, no_match_count);
		break;
	case PhysicalType::FLOAT:
		TemplatedMatchType<float, OP, NO_MATCH_SEL, COMPARE_NULL>(col, rows, sel, count, col_offset, col_no, no_match,
		                                                          no_match_count);
		break;
	case PhysicalType::DOUBLE:
		TemplatedMatchType<double, OP, NO_MATCH_SEL, COMPARE_NULL>(col, rows, sel, count, col_offset, col_no, no_match,
		                                                           no_match_count);
		break;
	case PhysicalType::INTERVAL:
		TemplatedMatchType<interval_t, OP, NO_MATCH_SEL, COMPARE_NULL>(col, rows, sel, count, col_offset, col_no,
		                                                               no_match, no_match_count);
		break;
	case PhysicalType::VARCHAR:
		TemplatedMatchType<string_t, OP, NO_MATCH_SEL, COMPARE_NULL>(col, rows, sel, count, col_offset, col_no,
		                                                             no_match, no_match_count);
		break;
	case PhysicalType::STRUCT:
		TemplatedMatchStruct<OP, NO_MATCH_SEL, COMPARE_NULL>(vec, col, layout, rows, sel, count, col_no, no_match,
		                                                     no_match_count, original_count);
		break;
...
static void TemplatedMatch(DataChunk &columns, UnifiedVectorFormat col_data[], const TupleDataLayout &layout,
	...
	for (idx_t col_no = 0; col_no < predicates.size(); ++col_no) {
		auto predicate = predicates[col_no];
		auto type = layout.GetTypes()[col_no].InternalType();
		if (type == PhysicalType::LIST || type == PhysicalType::STRUCT) {
			if (predicate == ExpressionType::COMPARE_DISTINCT_FROM) {
				predicate = ExpressionType::COMPARE_NOTEQUAL;
			} else if (predicate == ExpressionType::COMPARE_NOT_DISTINCT_FROM) {
				predicate = ExpressionType::COMPARE_EQUAL;
			}
		}
		...
		case ExpressionType::COMPARE_NOT_DISTINCT_FROM:
			TemplatedMatchOp<NotDistinctFrom, NO_MATCH_SEL, true>(vec, col, layout, rows, sel, count, col_no, no_match,
		...
		case ExpressionType::COMPARE_DISTINCT_FROM:
			TemplatedMatchOp<DistinctFrom, NO_MATCH_SEL, true>(vec, col, layout, rows, sel, count, col_no, no_match,
			                                                   no_match_count, count);
			break;
