		...
		if (layout.AllConstant() || !state.external) {
			...
			if (!l_done && !r_done) {
				...
				for (idx_t i = 0; i < merged; i++) {
					...
					memcpy(result_heap_ptr, reinterpret_cast<data_ptr_t>(l_smaller * CastPointerToValue(l_heap_ptr) + r_smaller * CastPointerToValue(r_heap_ptr)),
					       entry_size);
...
void MergeSorter::MergeRows(data_ptr_t &l_ptr, idx_t &l_entry_idx, const idx_t &l_count, data_ptr_t &r_ptr,
	...
	for (i = 0; i < next && l_entry_idx < l_count && r_entry_idx < r_count; i++) {
		...
		FastMemcpy(target_ptr, reinterpret_cast<data_ptr_t>(l_smaller * CastPointerToValue(l_ptr) + r_smaller * CastPointerToValue(r_ptr)), entry_size);
