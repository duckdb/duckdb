...
void SortedData::CreateBlock() {
	auto capacity = MaxValue((buffer_manager.GetBlockSize() + layout.GetRowWidth() - 1) / layout.GetRowWidth(),
	...
	if (!layout.AllConstant() && state.external) {
		heap_blocks.push_back(
		    make_uniq<RowDataBlock>(MemoryTag::ORDER_BY, buffer_manager, buffer_manager.GetBlockSize(), 1U));
...
void SortedBlock::CreateBlock() {
	auto capacity = MaxValue((buffer_manager.GetBlockSize() + sort_layout.entry_size - 1) / sort_layout.entry_size,
	                         state.block_capacity);
...
PayloadScanner::PayloadScanner(SortedData &sorted_data, GlobalSortState &global_sort_state, bool flush_p) {
	...
	rows = make_uniq<RowDataCollection>(global_sort_state.buffer_manager,
	                                    global_sort_state.buffer_manager.GetBlockSize(), 1U);
	rows->count = count;

	heap = make_uniq<RowDataCollection>(global_sort_state.buffer_manager,
	                                    global_sort_state.buffer_manager.GetBlockSize(), 1U);
