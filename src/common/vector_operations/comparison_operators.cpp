...
inline void COMPARISON_TYPE_CHECK(Vector &left, Vector &right, Vector &result) {
	...
	if (result.type != TypeId::BOOLEAN) {
		throw InvalidTypeException(result.type,
		                           "result of comparison must be boolean");
	}
...
template <class OP>
static void templated_boolean_operation(Vector &left, Vector &right,
                                        Vector &result) {
	...
	case TypeId::BIGINT:
		...
		break;
	case TypeId::POINTER:
		templated_binary_loop<uint64_t, uint64_t, bool, OP>(left, right,
		                                                    result);
		break;
	case TypeId::DECIMAL:
