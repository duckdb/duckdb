...
void templated_cast_loop(Vector &source, Vector &result, SQLType source_type, SQLType target_type) {
	...
	if (!IGNORE_NULL || !result.nullmask.any()) {
		VectorOperations::Exec(source, [&](index_t i, index_t k) {
			result_data[i] = OP::template Operation<SRC_TYPE, DST_TYPE>(ldata[i], target_type);
		});
	} else {
		VectorOperations::Exec(source, [&](index_t i, index_t k) {
...
static void result_cast_switch(Vector &source, Vector &result, SQLType source_type, SQLType target_type) {
	...
	case SQLTypeId::BOOLEAN:
		...
		templated_cast_loop<SRC, bool, OP, IGNORE_NULL>(source, result, source_type, target_type);
		break;
	case SQLTypeId::TINYINT:
		assert(result.type == TypeId::TINYINT);
		templated_cast_loop<SRC, int8_t, OP, IGNORE_NULL>(source, result, source_type, target_type);
		break;
	case SQLTypeId::SMALLINT:
		assert(result.type == TypeId::SMALLINT);
		templated_cast_loop<SRC, int16_t, OP, IGNORE_NULL>(source, result, source_type, target_type);
		break;
	case SQLTypeId::INTEGER:
		assert(result.type == TypeId::INTEGER);
		templated_cast_loop<SRC, int32_t, OP, IGNORE_NULL>(source, result, source_type, target_type);
		break;
	case SQLTypeId::BIGINT:
		assert(result.type == TypeId::BIGINT);
		templated_cast_loop<SRC, int64_t, OP, IGNORE_NULL>(source, result, source_type, target_type);
		break;
	case SQLTypeId::FLOAT:
		assert(result.type == TypeId::FLOAT);
		templated_cast_loop<SRC, float, OP, IGNORE_NULL>(source, result, source_type, target_type);
		break;
	case SQLTypeId::DECIMAL:
	case SQLTypeId::DOUBLE:
		assert(result.type == TypeId::DOUBLE);
		templated_cast_loop<SRC, double, OP, IGNORE_NULL>(source, result, source_type, target_type);
		break;
	case SQLTypeId::DATE:
		assert(result.type == TypeId::INTEGER);
		templated_cast_loop<SRC, int32_t, duckdb::CastToDate, true>(source, result, source_type, target_type);
		break;
	case SQLTypeId::TIME:
		assert(result.type == TypeId::INTEGER);
		templated_cast_loop<SRC, int32_t, duckdb::CastToTime, true>(source, result, source_type, target_type);
		break;
	case SQLTypeId::TIMESTAMP:
		assert(result.type == TypeId::BIGINT);
		templated_cast_loop<SRC, int64_t, duckdb::CastToTimestamp, true>(source, result, source_type, target_type);
		break;
	case SQLTypeId::VARCHAR: {
		...
		VectorOperations::Exec(source, [&](index_t i, index_t k) {
			if (source.nullmask[i]) {
				...
				auto str = OP::template Operation<SRC, string>(ldata[i], target_type);
    ...
    case SQLTypeId::INTERVAL:
        ...
        templated_cast_loop<SRC, Interval, duckdb::CastToInterval, true>(source, result, source_type, target_type);
        break;
	default:
