...
void LogicalType::SetAlias(string alias) {
	if (!type_info_) {
		type_info_ = make_refcounted<ExtraTypeInfo>(ExtraTypeInfoType::GENERIC_TYPE_INFO, std::move(alias));
	} else {
...
LogicalType LogicalType::DECIMAL(uint8_t width, uint8_t scale) {
	...
	auto type_info = make_refcounted<DecimalTypeInfo>(width, scale);
...
LogicalType LogicalType::VARCHAR_COLLATION(string collation) { // NOLINT
	auto string_info = make_refcounted<StringTypeInfo>(std::move(collation));
...
LogicalType LogicalType::LIST(const LogicalType &child) {
	auto info = make_refcounted<ListTypeInfo>(child);
...
LogicalType LogicalType::STRUCT(child_list_t<LogicalType> children) {
	auto info = make_refcounted<StructTypeInfo>(std::move(children));
...
LogicalType LogicalType::AGGREGATE_STATE(aggregate_state_t state_type) { // NOLINT
	auto info = make_refcounted<AggregateStateTypeInfo>(std::move(state_type));
...
LogicalType LogicalType::UNION(child_list_t<LogicalType> members) {
	...
	auto info = make_refcounted<StructTypeInfo>(std::move(members));
...
LogicalType LogicalType::USER(const string &user_type_name) {
	auto info = make_refcounted<UserTypeInfo>(user_type_name);
	return LogicalType(LogicalTypeId::USER, std::move(info));
}

LogicalType LogicalType::USER(string catalog, string schema, string name) {
	auto info = make_refcounted<UserTypeInfo>(std::move(catalog), std::move(schema), std::move(name));
	return LogicalType(LogicalTypeId::USER, std::move(info));
}
...
LogicalType LogicalType::ANY_PARAMS(LogicalType target, idx_t cast_score) { // NOLINT
	auto type_info = make_refcounted<AnyTypeInfo>(std::move(target), cast_score);
...
LogicalType LogicalType::INTEGER_LITERAL(const Value &constant) { // NOLINT
	...
	auto type_info = make_refcounted<IntegerLiteralTypeInfo>(constant);
