...
void VectorStructBuffer::SetVectorSize(idx_t new_size) {
	...
	for (auto &child : children) {
		...
		if (vtype == VectorType::FLAT_VECTOR || vtype == VectorType::CONSTANT_VECTOR) {
			...
			// Non-flat children (e.g., DICTIONARY from Slice) can only be shrunk, not grown.
			// Just update v_size without touching the underlying buffer structure.
			child.GetBufferRef()->SetVectorSizeOnly(new_size);
		}
	}
}
