...
static void MallocTrim(idx_t pad) {
	...
	static atomic<int64_t> LAST_TRIM_TIMESTAMP_MS {Timestamp::GetMonotonicTimestamp().value /
	                                               Interval::MICROS_PER_MSEC};

	int64_t last_trim_timestamp_ms = LAST_TRIM_TIMESTAMP_MS.load();
	auto current_timestamp_ms = Timestamp::GetMonotonicTimestamp().value / Interval::MICROS_PER_MSEC;
