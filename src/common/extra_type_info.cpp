...
MeasureTypeInfo::MeasureTypeInfo(LogicalType measure_output_type_p, std::string alias, unique_ptr<Expression> bound_measure_expression)
    : ExtraTypeInfo(ExtraTypeInfoType::MEASURE_TYPE_INFO), measure_output_type(std::move(measure_output_type_p)), measure_alias(std::move(alias)), bound_measure_expression(std::move(bound_measure_expression)) {
}
...
shared_ptr<ExtraTypeInfo> MeasureTypeInfo::Copy() const {
	return make_shared_ptr<MeasureTypeInfo>(measure_output_type, measure_alias, bound_measure_expression ? bound_measure_expression->Copy() : nullptr);
}

shared_ptr<ExtraTypeInfo> MeasureTypeInfo::DeepCopy() const {
	return make_shared_ptr<MeasureTypeInfo>(measure_output_type.DeepCopy(), measure_alias, bound_measure_expression ? bound_measure_expression->Copy() : nullptr);
}
