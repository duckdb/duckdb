...
public:
	explicit VectorCacheBuffer(Allocator &allocator, const LogicalType &type_p, idx_t capacity_p = STANDARD_VECTOR_SIZE)
		...
		case PhysicalType::LIST: {
			...
			auxiliary = make_refcounted<VectorListBuffer>(std::move(child_vector));
			break;
		}
		case PhysicalType::ARRAY: {
			...
			auxiliary = make_refcounted<VectorArrayBuffer>(std::move(child_vector), array_size, capacity);
			break;
		}
		case PhysicalType::STRUCT: {
			...
			auto struct_buffer = make_refcounted<VectorStructBuffer>(type);
