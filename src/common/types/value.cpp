...
Value::Value(string val) : type_(LogicalType::VARCHAR), is_null(false) {
	...
	value_info_ = make_refcounted<StringValueInfo>(std::move(val));
}
...
Value Value::STRUCT(const LogicalType &type, vector<Value> struct_values) {
	...
	result.value_info_ = make_refcounted<NestedValueInfo>(std::move(struct_values));
	result.type_ = type;
	result.is_null = false;
	return result;
}
...
Value Value::MAP(const LogicalType &key_type, const LogicalType &value_type, vector<Value> keys, vector<Value> values) {
	...
	result.value_info_ = make_refcounted<NestedValueInfo>(std::move(values));
	return result;
}
...
Value Value::UNION(child_list_t<LogicalType> members, uint8_t tag, Value value) {
	...
	result.value_info_ = make_refcounted<NestedValueInfo>(std::move(union_values));
...
Value Value::EMPTYLIST(const LogicalType &child_type) {
	...
	result.value_info_ = make_refcounted<NestedValueInfo>();
	result.is_null = false;
	return result;
}
...
Value Value::BLOB(const_data_ptr_t data, idx_t len) {
	...
	result.is_null = false;
	result.value_info_ = make_refcounted<StringValueInfo>(string(const_char_ptr_cast(data), len));
	return result;
}
...
Value Value::BLOB(const string &data) {
	Value result(LogicalType::BLOB);
	result.is_null = false;
	result.value_info_ = make_refcounted<StringValueInfo>(Blob::ToBlob(string_t(data)));
	return result;
}
...
Value Value::BIT(const string &data) {
	Value result(LogicalType::BIT);
	result.is_null = false;
	result.value_info_ = make_refcounted<StringValueInfo>(Bit::ToBit(string_t(data)));
	return result;
}
...
Value Value::Deserialize(Deserializer &deserializer) {
	...
	case PhysicalType::VARCHAR: {
		...
		if (type.id() == LogicalTypeId::BLOB) {
			new_value.value_info_ = make_refcounted<StringValueInfo>(Blob::ToBlob(str));
		} else {
			new_value.value_info_ = make_refcounted<StringValueInfo>(str);
		}
	} break;
	case PhysicalType::LIST: {
		deserializer.ReadObject(102, "value", [&](Deserializer &obj) {
			auto children = obj.ReadProperty<vector<Value>>(100, "children");
			new_value.value_info_ = make_refcounted<NestedValueInfo>(children);
		});
	} break;
	case PhysicalType::STRUCT: {
		deserializer.ReadObject(102, "value", [&](Deserializer &obj) {
