...
void ParquetVariantConversion::ToParquetVariant(const Vector &variant_vec, idx_t count, Vector &result) {
	// DuckDB Variant:
	// - keys = VARCHAR[]
	...
	// Parquet VARIANT:
	// - metadata = BLOB
	// - value = BLOB (+ typed_value when 'result' declares a shredded layout)
