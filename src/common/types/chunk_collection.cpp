...
void ChunkCollection::Append(DataChunk &new_chunk) {
	...
	// we have to ensure that every chunk in the ChunkCollection is completely filled, otherwise our O(1) lookup in GetValue and SetValue does not work
	...
	if (chunks.size() == 0) {
		...
		size_t added_data = std::min(remaining_data, STANDARD_VECTOR_SIZE - last_chunk.count);
		if (added_data > 0) {
