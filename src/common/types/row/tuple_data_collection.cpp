...
static void InitializeVectorFormat(vector<TupleDataVectorFormat> &vector_data, const vector<LogicalType> &types) {
	...
	for (idx_t col_idx = 0; col_idx < types.size(); col_idx++) {
		...
		case PhysicalType::ARRAY:
			InitializeVectorFormat(vector_data[col_idx].child_formats, {ArrayType::GetChildType(type)});
			break;
		default:
			break;
		}
...
static inline void ToUnifiedFormatInternal(TupleDataVectorFormat &format, Vector &vector, const idx_t count) {
	...
	case PhysicalType::ARRAY: {
		...
		format.data.data = reinterpret_cast<data_ptr_t>(format.array_list_entries.get());

		// Set the array size in the child format
		format.child_formats[0].parent_array_size = array_size;

		ToUnifiedFormatInternal(format.child_formats[0], ArrayVector::GetEntry(vector),
