...
void StringVector::MoveStringsToHeap(Vector &vector, idx_t count, StringHeap &heap, SelectionVector &sel) {
...
void StringVector::MoveStringsToHeap(Vector &vector, idx_t count, StringHeap &heap) {
	assert(vector.type == TypeId::VARCHAR);
	...
	case VectorType::CONSTANT_VECTOR: {
		...
		*target_strings = heap.AddString(*source_strings);
		break;
	}
	case VectorType::FLAT_VECTOR: {
		auto source_strings = FlatVector::GetData<string_t>(vector);
