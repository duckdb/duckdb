...
void Vector::Slice(const SelectionVector &sel, idx_t count) {
	...
	if (buffer->vector_type == VectorType::DICTIONARY_VECTOR) {
		...
		buffer = make_buffer<DictionaryBuffer>(move(sliced_dictionary), buffer->type, buffer->vector_type);
		return;
	}
	auto child_ref = make_buffer<VectorChildBuffer>();
	child_ref->data.Reference(*this);

	auto dict_buffer = make_buffer<DictionaryBuffer>(sel, buffer->type, VectorType::DICTIONARY_VECTOR);
...
void Vector::Slice(const SelectionVector &sel, idx_t count, SelCache &cache) {
	if (buffer->vector_type == VectorType::DICTIONARY_VECTOR) {
		...
		if (entry != cache.cache.end()) {
			...
			auto type_tmp = buffer->type;
			this->buffer = entry->second;
			buffer->type = type_tmp;
		} else {
			Slice(sel, count);
...
void Vector::Initialize(const LogicalType &new_type, bool zero_data) {
	if (new_type.id() != LogicalTypeId::INVALID) {
		buffer->type = new_type;
	}
	auxiliary.reset();
	nullmask.reset();
	if (GetTypeIdSize(buffer->type.InternalType()) > 0) {
...
void Vector::SetValue(idx_t index, const Value &val) {
	...
	case LogicalTypeId::STRUCT: {
		...
		for (size_t i = 0; i < val.struct_value.size(); i++) {
			auto &struct_child = val.struct_value[i];
			D_ASSERT(buffer->vector_type == VectorType::CONSTANT_VECTOR ||
			         buffer->vector_type == VectorType::FLAT_VECTOR);
...
string Vector::ToString(idx_t count) const {
	string retval =
	    VectorTypeToString(buffer->vector_type) + " " + buffer->type.ToString() + ": " + to_string(count) + " = [ ";
	switch (buffer->vector_type) {
	case VectorType::FLAT_VECTOR:
	case VectorType::DICTIONARY_VECTOR:
...
void Vector::Normalify(idx_t count) {
	...
	case VectorType::CONSTANT_VECTOR: {
		...
		auto old_data = data;
		buffer = VectorBuffer::CreateStandardVector(VectorType::FLAT_VECTOR, old_buffer->type,
		                                            old_buffer->type.InternalType());
		data = buffer->GetData();
		if (nullmask[0]) {
...
void Vector::Verify(const SelectionVector &sel, idx_t count) {
	...
	if (buffer->type.InternalType() == PhysicalType::STRUCT) {
		D_ASSERT(buffer->type.child_types().size() > 0);
	...
	if (buffer->type.InternalType() == PhysicalType::LIST) {
		D_ASSERT(buffer->type.child_types().size() == 1);
		if (buffer->vector_type == VectorType::CONSTANT_VECTOR) {
			if (!ConstantVector::IsNull(*this)) {
...
string_t StringVector::AddString(Vector &vector, string_t data) {
	...
	D_ASSERT(vector.auxiliary->buffer_type == VectorBufferType::STRING_BUFFER);
...
void StringVector::AddHeapReference(Vector &vector, Vector &other) {
	...
	D_ASSERT(vector.auxiliary->buffer_type == VectorBufferType::STRING_BUFFER);
	D_ASSERT(other.auxiliary->buffer_type == VectorBufferType::STRING_BUFFER);
	auto &string_buffer = (VectorStringBuffer &)*vector.auxiliary;
...
bool StructVector::HasEntries(const Vector &vector) {
	...
	D_ASSERT(vector.buffer->vector_type == VectorType::FLAT_VECTOR ||
	         vector.buffer->vector_type == VectorType::CONSTANT_VECTOR);
	D_ASSERT(vector.auxiliary == nullptr || vector.auxiliary->buffer_type == VectorBufferType::STRUCT_BUFFER);
...
child_list_t<unique_ptr<Vector>> &StructVector::GetEntries(const Vector &vector) {
	...
	D_ASSERT(vector.auxiliary->buffer_type == VectorBufferType::STRUCT_BUFFER);
...
ChunkCollection &ListVector::GetEntry(const Vector &vector) {
	...
	D_ASSERT(vector.buffer->vector_type == VectorType::FLAT_VECTOR ||
	         vector.buffer->vector_type == VectorType::CONSTANT_VECTOR);
	D_ASSERT(vector.auxiliary);
	D_ASSERT(vector.auxiliary->buffer_type == VectorBufferType::LIST_BUFFER);
