...
int32_t Date::YearDays(int32_t year) {
...
void Date::ExtractYearOffset(int32_t &n, int32_t &year, int32_t &year_offset) {
	...
	idx_t start;
	if (n < Date::CumulativeYearDays[200]) {
		if (n < Date::CumulativeYearDays[100]) {
			if (n < Date::CumulativeYearDays[50]) {
				if (n < Date::CumulativeYearDays[25]) {
					start = 0;
				} else {
					start = 25;
				}
			} else {
				if (n < Date::CumulativeYearDays[75]) {
					start = 50;
				} else {
					start = 75;
				}
			...
			if (n < Date::CumulativeYearDays[150]) {
				if (n < Date::CumulativeYearDays[125]) {
					start = 100;
				} else {
					start = 125;
				}
			} else {
				if (n < Date::CumulativeYearDays[175]) {
					start = 150;
				} else {
					start = 175;
				}
		...
		if (n < Date::CumulativeYearDays[300]) {
			if (n < Date::CumulativeYearDays[250]) {
				if (n < Date::CumulativeYearDays[225]) {
					start = 200;
				} else {
					start = 225;
				}
			} else {
				if (n < Date::CumulativeYearDays[275]) {
					start = 250;
				} else {
					start = 275;
				}
			...
			if (n < Date::CumulativeYearDays[350]) {
				if (n < Date::CumulativeYearDays[325]) {
					start = 300;
				} else {
					start = 325;
				}
			} else {
				if (n < Date::CumulativeYearDays[375]) {
					start = 350;
				} else {
					start = 375;
				}
	...
	for(idx_t i = 0; i < 25; i++) {
		year_offset += n >= Date::CumulativeYearDays[start + i + 1];
	}
	year += year_offset;
}
...
void Date::ExtractYearDay(int32_t n, int32_t &year, int32_t &day_of_year) {
	...
	day_of_year = n - Date::CumulativeYearDays[year_offset];
}
...
void Date::Convert(int32_t n, int32_t &year, int32_t &month, int32_t &day) {
	...
	if (Date::IsLeapYear(year)) {
		for(idx_t i = 1; i < 12; i++) {
			month += day > Date::CumulativeLeapDays[i];
		}
		day -= Date::CumulativeLeapDays[month - 1];
		D_ASSERT(day > 0 && day <= Date::LeapDays[month]);
	} else {
		for(idx_t i = 1; i < 12; i++) {
			month += day > Date::CumulativeDays[i];
		}
...
int32_t Date::ExtractDayOfTheYear(date_t date) {
	int32_t year, day_of_year;
	Date::ExtractYearDay(date, year, day_of_year);
