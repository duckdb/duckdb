...
unique_ptr<RenderTreeNode> TreeRenderer::CreateNode(const ProfilingNode &op) {
	...
	if (op_node.profiling_info.Enabled(MetricsType::EXTRA_INFO)) {
		extra_info = op_node.profiling_info.metrics.extra_info;
	}
	auto result = TreeRenderer::CreateRenderNode(op_node.name, extra_info);
	result->extra_text += "\n[INFOSEPARATOR]";
	result->extra_text += "\n" + to_string(op_node.profiling_info.metrics.operator_cardinality);
	string timing = StringUtil::Format("%.2f", op_node.profiling_info.metrics.operator_timing);
	result->extra_text += "\n(" + timing + "s)";
	return result;
}
