...
public:
	...
	array_ptr<TupleDataParallelScanState> ht_scan_states;
...
SourceResultType RadixPartitionedHashTable::GetData(ExecutionContext &context, DataChunk &chunk,
	...
	if (gstate.is_empty && grouping_set.empty()) {
		...
		for (idx_t i = 0; i < op.aggregates.size(); i++) {
			...
			auto aggr_state = make_array<data_t>(aggr.function.state_size());
	...
	if (!state.initialized) {
		...
		if (!state.initialized) {
			...
			state.ht_scan_states = make_array<TupleDataParallelScanState>(finalized_hts.size());
