...
unique_ptr<PhysicalOperator> PhysicalPlanGenerator::PlanTableInOutFunction(LogicalGet &op) {
	...
	if (child_node->types.size() < op.input_table_types.size()) {
		throw InternalException("Mismatch between input table types and child node types - expected %llu but got %llu",
...
unique_ptr<PhysicalOperator> PhysicalPlanGenerator::PlanTableScan(LogicalGet &op,
                                                                  unique_ptr<TableFilterSet> table_filters) {
	auto &column_ids = op.GetColumnIds();
	// function does not support projection pushdown
	auto node = make_uniq<PhysicalTableScan>(op.returned_types, op.function, std::move(op.bind_data), op.returned_types,
	                                         column_ids, vector<column_t>(), op.names, std::move(table_filters),
...
unique_ptr<PhysicalOperator> PhysicalPlanGenerator::CreatePlan(LogicalGet &op) {
	...
	if (!op.children.empty()) {
		plan = PlanTableInOutFunction(op);
	} else {
		...
		if (!op.function.projection_pushdown) {
			plan = PlanTableScan(op, std::move(table_filters));
		} else {
			auto &column_ids = op.GetColumnIds();
	...
	if (!op.user_provided_types.empty()) {
