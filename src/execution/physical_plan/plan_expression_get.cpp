...
unique_ptr<PhysicalOperator> PhysicalPlanGenerator::CreatePlan(LogicalExpressionGet &op) {
	...
	if (plan->type != PhysicalOperatorType::DUMMY_SCAN) {
		expr_scan->children.push_back(move(plan));
		return move(expr_scan);
	} else {
		// no children: constant expression scan (i.e. no subqueries to evaluate)
		// we can evaluate all the expressions right now
		auto chunk_scan =
