...
void ExpressionExecutor::Execute(BoundOperatorExpression &expr, ExpressionState *state, Vector &result) {
	...
	if (expr.type == ExpressionType::COMPARE_IN || expr.type == ExpressionType::COMPARE_NOT_IN) {
		...
		Vector left(GetCardinality(), expr.children[0]->return_type);
		// eval left side
		...
		for (idx_t child = 1; child < expr.children.size(); child++) {
			...
			if (child == 1) {
				...
				Vector new_result(GetCardinality(), TypeId::BOOL, true, false);
		...
		Vector child(GetCardinality(), expr.children[0]->return_type);
