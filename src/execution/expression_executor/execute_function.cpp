...
// #ifdef DEBUG
		...
		if (expr.function.errors == FunctionErrors::CANNOT_ERROR) {
			ErrorData err(ex);
			throw InternalException("Error thrown in function \"%s\" - but function is labeled as not being able to throw errors\n\nError thrown: %s", expr.function.name, err.Message());
		}
		throw;
	}
// #endif
