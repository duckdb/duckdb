...
SuperLargeHashTable::SuperLargeHashTable(index_t initial_capacity, vector<TypeId> group_types,
	...
	for (index_t i = 0; i < aggregates.size(); i++) {
		payload_width += aggregates[i]->bound_aggregate->function.state_size(aggregates[i]->return_type);
	}
	...
	for (index_t i = 0; i < aggregates.size(); i++) {
		...
		aggr->bound_aggregate->function.initialize(pointer, return_type);
		pointer += aggr->bound_aggregate->function.state_size(return_type);
	}
...
void SuperLargeHashTable::AddChunk(DataChunk &groups, DataChunk &payload) {
	...
	for (index_t aggr_idx = 0; aggr_idx < aggregates.size(); aggr_idx++) {
		...
		if (aggr->distinct) {
			...
			aggr->bound_aggregate->function.update(&distinct_payload, 1, distinct_addresses);
			payload_idx++;
		}

		else {
			auto input_count = max(size_t(1), aggr->children.size());
			aggr->bound_aggregate->function.update(&payload.data[payload_idx], input_count, addresses);
			payload_idx += input_count;
		}

		// move to the next aggregate
		VectorOperations::AddInPlace(addresses, aggr->bound_aggregate->function.state_size(aggr->return_type));
	}
}
...
void SuperLargeHashTable::FetchAggregates(DataChunk &groups, DataChunk &result) {
	...
	for (index_t aggr_idx = 0; aggr_idx < aggregates.size(); aggr_idx++) {
		...
		VectorOperations::AddInPlace(
		    addresses, aggregates[aggr_idx]->bound_aggregate->function.state_size(aggregates[aggr_idx]->return_type));
	}
}
...
index_t SuperLargeHashTable::Scan(index_t &scan_position, DataChunk &groups, DataChunk &result) {
	...
	for (index_t i = 0; i < aggregates.size(); i++) {
		...
		target.count = entry;
		auto aggr = aggregates[i];
		aggr->bound_aggregate->function.finalize(addresses, target);

		VectorOperations::AddInPlace(addresses, aggr->bound_aggregate->function.state_size(target.type));
	}
