...
Vector::Vector(TypeId type, oid_t max_elements) : 
	type(type), count(0), sel_vector(nullptr), data(nullptr), owns_data(false) {
	if (max_elements > 0) {
		owns_data = true;
...
Vector::Vector(Value value) : 
	type(value.type), count(1), sel_vector(nullptr) {
	owns_data = true;
...
Vector::Vector() : 
	...
	owns_data(false), sel_vector(nullptr) {
...
void Vector::Append(Vector& other) {
	...
	memcpy(data + count * GetTypeIdSize(type), other.data, other.count * GetTypeIdSize(type));
	count += other.count;
}
