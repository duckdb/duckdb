		...
		if (existing_db) {
			...
			if (info->on_conflict == OnCreateConflict::REPLACE_ON_CONFLICT) {
				...
				if (existing_db->GetCatalog().GetDBPath() == path &&
					existing_db->GetCatalog().GetCatalogType() == db_type && options.options.empty()) {
			...
			// ATTACH OR REPLACE or ATTACH IF NOT EXISTS without conflicting database name. Treat it as ERROR_ON_CONFLICT
