...
PhysicalCreateIndexMaterialized::PhysicalCreateIndexMaterialized(
    PhysicalPlan &physical_plan, LogicalOperator &op, TableCatalogEntry &table_p, const vector<column_t> &column_ids,
    unique_ptr<CreateIndexInfo> info, vector<unique_ptr<Expression>> unbound_expressions_p, idx_t estimated_cardinality,
    IndexType index_type, unique_ptr<IndexBuildBindData> bind_data, unique_ptr<AlterTableInfo> alter_table_info)
...
// Sink
//---------------------------------------------------------------------------------------------------------------------
class CreateIndexMaterializedGlobalSinkState : public GlobalSinkState {
public:
...
class CreateIndexMaterializedLocalSinkState : public LocalSinkState {
public:
...
SinkResultType PhysicalCreateIndexMaterialized::Sink(ExecutionContext &context, DataChunk &chunk,
	...
	auto &gstate = input.global_state.Cast<CreateIndexMaterializedGlobalSinkState>();
	auto &lstate = input.local_state.Cast<CreateIndexMaterializedLocalSinkState>();
