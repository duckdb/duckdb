...
namespace duckdb
{
PhysicalUnion::PhysicalUnion(vector<LogicalType> types, unique_ptr<PhysicalOperator> top, unique_ptr<PhysicalOperator> bottom, idx_t estimated_cardinality)
...
void PhysicalUnion::BuildPipelines(Pipeline &current, MetaPipeline &meta_pipeline)
{
	...
	((PhysicalOperator*)children[1].get())->BuildPipelines(*union_pipeline, meta_pipeline);
...
vector<const_reference<PhysicalOperator>> PhysicalUnion::GetSources() const
{
	...
	for (auto &child : children) {
		auto child_sources = ((PhysicalOperator*)child.get())->GetSources();
