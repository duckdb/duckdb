...
public:
	LimitPercentOperatorState(const PhysicalLimitPercent &op) : limit(DConstants::INVALID_INDEX), current_offset(0) {
...
void PhysicalLimitPercent::GetData(ExecutionContext &context, DataChunk &chunk, GlobalSourceState &gstate_p,
	...
	if (gstate.is_limit_percent_delimited && limit == DConstants::INVALID_INDEX) {
		...
		limit = MinValue((idx_t)(percent_limit / 100 * count), count);
		if (limit == 0) {
			return;
		}
	}
