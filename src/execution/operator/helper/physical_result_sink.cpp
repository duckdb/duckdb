...
public:
	...
	vector<unique_ptr<ResultUnit>> units DUCKDB_GUARDED_BY(glock);
};
...
unique_ptr<ResultUnit> PhysicalResultSink::FinishUnit(ResultSinkGlobalState &gstate,
	...
	auto unit = gstate.buffered_data->Format().Finish(gstate.buffered_data->FormatState(), *lstate.format_state);
	if (unit) {
		unit->batch_index = BatchOrdered() ? lstate.current_batch : DConstants::INVALID_INDEX;
	}
	return unit;
}
...
SinkResultType PhysicalResultSink::SinkRetainedFormatted(ResultSinkGlobalState &gstate, ResultSinkLocalState &lstate,
	...
	if (auto unit = AppendToUnit(gstate, lstate, chunk)) {
		lstate.units.push_back(std::move(unit));
	}
	return SinkResultType::NEED_MORE_INPUT;
}

bool PhysicalResultSink::HandOver(ResultSinkGlobalState &gstate, unique_ptr<ResultUnit> unit,
                                  const InterruptState &interrupt) const {
	if (BatchOrdered()) {
		return gstate.buffered_data->Cast<BatchedBufferedData>().AppendOrBlock(std::move(unit), interrupt);
	}
...
SinkResultType PhysicalResultSink::SinkDraining(ResultSinkGlobalState &gstate, ResultSinkLocalState &lstate,
	...
	if (HandOver(gstate, std::move(unit), input.interrupt_state)) {
...
bool PhysicalResultSink::FlushPartialUnit(ResultSinkGlobalState &gstate, ResultSinkLocalState &lstate,
	...
	return HandOver(gstate, std::move(unit), interrupt);
}
...
unique_ptr<QueryResult> PhysicalResultSink::GetFormattedResult(ResultSinkGlobalState &gstate,
	...
	{
		annotated_lock_guard<annotated_mutex> l(gstate.glock);
		units = std::move(gstate.units);
	}
	if (BatchOrdered()) {
		// Stable, so the units a producer contributed to one batch keep their production order
		std::stable_sort(units.begin(), units.end(),
		                 [](const unique_ptr<ResultUnit> &lhs, const unique_ptr<ResultUnit> &rhs) {
