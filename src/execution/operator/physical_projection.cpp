...
void PhysicalProjection::GetChunk(DataChunk& chunk, PhysicalOperatorState* state_) {
	...
	if (children.size() > 0) {
		...
		// no FROM clause, set a simple marker to ensure the projection is only executed once
		if (state->executed) {
			return;
		}
		state->executed = true;
...
unique_ptr<PhysicalOperatorState> PhysicalProjection::GetOperatorState() {
	return make_unique<PhysicalProjectionOperatorState>(children.size() == 0 ? nullptr : children[0].get());
}
