...
static void UpdateWindowBoundaries(BoundWindowExpression *wexpr, ChunkCollection &input, idx_t input_size,
	...
	case WindowBoundary::EXPR_PRECEDING: {
		D_ASSERT(boundary_start_collection.column_count() > 0);
		bounds.window_start =
		    (int64_t)row_idx -
	...
	case WindowBoundary::UNBOUNDED_FOLLOWING:
		...
		break;
	case WindowBoundary::EXPR_PRECEDING:
		D_ASSERT(boundary_end_collection.column_count() > 0);
		bounds.window_end =
		    (int64_t)row_idx -
...
static void ComputeWindowExpression(BoundWindowExpression *wexpr, ChunkCollection &input, ChunkCollection &output,
	...
	if (wexpr->aggregate) {
		segment_tree = make_unique<WindowSegmentTree>(*(wexpr->aggregate), wexpr->bind_info.get(), wexpr->return_type, &payload_collection);
	}
