...
// class PhysicalIndexJoinOperatorState : public OperatorState {
// public:
...
PhysicalIndexJoin::PhysicalIndexJoin(LogicalOperator &op, unique_ptr<PhysicalOperator> left,
	...
	throw InternalException("FIXME: index join");
}

void PhysicalIndexJoin::Output(ExecutionContext &context, DataChunk &chunk, OperatorState *state_p) const {
...
void PhysicalIndexJoin::GetRHSMatches(ExecutionContext &context, OperatorState *state_p) const {
	// auto state = reinterpret_cast<PhysicalIndexJoinOperatorState *>(state_p);
	// auto &art = (ART &)*index;
	// auto &transaction = Transaction::GetTransaction(context.client);
...
// unique_ptr<OperatorState> PhysicalIndexJoin::GetOperatorState() {
...
// 	return move(state);
// }
// void PhysicalIndexJoin::FinalizeOperatorState(OperatorState &state, ExecutionContext &context) {
