...
PhysicalNestedLoopJoinInner::PhysicalNestedLoopJoinInner(LogicalOperator &op, unique_ptr<PhysicalOperator> left,
...
static size_t templated_nested_loop_join(Vector &left, Vector &right, size_t &lpos, size_t &rpos, sel_t lvector[],
                                         sel_t rvector[]) {
	...
	left_count = Vector::NotNullSelVector(left, left_null_vector, left_sel_vector);
	right_count = Vector::NotNullSelVector(right, right_null_vector, right_sel_vector);
...
static size_t nested_loop_join_operator(Vector &left, Vector &right, size_t &lpos, size_t &rpos, sel_t lvector[],
	...
	case TypeId::BOOLEAN:
	case TypeId::TINYINT:
		return templated_nested_loop_join<int8_t, OP>(left, right, lpos, rpos, lvector, rvector);
	case TypeId::SMALLINT:
		return templated_nested_loop_join<int16_t, OP>(left, right, lpos, rpos, lvector, rvector);
	case TypeId::DATE:
	case TypeId::INTEGER:
		return templated_nested_loop_join<int32_t, OP>(left, right, lpos, rpos, lvector, rvector);
	case TypeId::TIMESTAMP:
	case TypeId::BIGINT:
		return templated_nested_loop_join<int64_t, OP>(left, right, lpos, rpos, lvector, rvector);
	case TypeId::DECIMAL:
		return templated_nested_loop_join<double, OP>(left, right, lpos, rpos, lvector, rvector);
	case TypeId::POINTER:
		return templated_nested_loop_join<uint64_t, OP>(left, right, lpos, rpos, lvector, rvector);
	case TypeId::VARCHAR:
		return templated_nested_loop_join<const char *, OP>(left, right, lpos, rpos, lvector, rvector);
	default:
...
size_t nested_loop_join(ExpressionType op, Vector &left, Vector &right, size_t &lpos, size_t &rpos, sel_t lvector[],
	...
	case ExpressionType::COMPARE_EQUAL:
		return nested_loop_join_operator<operators::Equals>(left, right, lpos, rpos, lvector, rvector);
	case ExpressionType::COMPARE_NOTEQUAL:
		return nested_loop_join_operator<operators::NotEquals>(left, right, lpos, rpos, lvector, rvector);
	case ExpressionType::COMPARE_LESSTHAN:
		return nested_loop_join_operator<operators::LessThan>(left, right, lpos, rpos, lvector, rvector);
	case ExpressionType::COMPARE_GREATERTHAN:
		return nested_loop_join_operator<operators::GreaterThan>(left, right, lpos, rpos, lvector, rvector);
	case ExpressionType::COMPARE_LESSTHANOREQUALTO:
		return nested_loop_join_operator<operators::LessThanEquals>(left, right, lpos, rpos, lvector, rvector);
	case ExpressionType::COMPARE_GREATERTHANOREQUALTO:
		return nested_loop_join_operator<operators::GreaterThanEquals>(left, right, lpos, rpos, lvector, rvector);
	default:
...
template <class T, class OP>
static size_t templated_nested_loop_comparison(Vector &left, Vector &right, sel_t lvector[], sel_t rvector[],
                                               size_t count) {
	assert(count > 0);
...
template <class OP>
static size_t nested_loop_comparison_operator(Vector &l, Vector &r, sel_t lvector[], sel_t rvector[], size_t count) {
	assert(l.type == r.type);
	switch (l.type) {
	case TypeId::BOOLEAN:
	case TypeId::TINYINT:
		return templated_nested_loop_comparison<int8_t, OP>(l, r, lvector, rvector, count);
	case TypeId::SMALLINT:
		return templated_nested_loop_comparison<int16_t, OP>(l, r, lvector, rvector, count);
	case TypeId::DATE:
	case TypeId::INTEGER:
		return templated_nested_loop_comparison<int32_t, OP>(l, r, lvector, rvector, count);
	case TypeId::TIMESTAMP:
	case TypeId::BIGINT:
		return templated_nested_loop_comparison<int64_t, OP>(l, r, lvector, rvector, count);
	case TypeId::DECIMAL:
		return templated_nested_loop_comparison<double, OP>(l, r, lvector, rvector, count);
	case TypeId::POINTER:
		return templated_nested_loop_comparison<uint64_t, OP>(l, r, lvector, rvector, count);
	case TypeId::VARCHAR:
		return templated_nested_loop_comparison<const char *, OP>(l, r, lvector, rvector, count);
	default:
		throw NotImplementedException("Unimplemented type for join!");
	}
}

size_t nested_loop_comparison(ExpressionType op, Vector &l, Vector &r, sel_t lvector[], sel_t rvector[], size_t count) {
	switch (op) {
	case ExpressionType::COMPARE_EQUAL:
		return nested_loop_comparison_operator<operators::Equals>(l, r, lvector, rvector, count);
	case ExpressionType::COMPARE_NOTEQUAL:
		return nested_loop_comparison_operator<operators::NotEquals>(l, r, lvector, rvector, count);
	case ExpressionType::COMPARE_LESSTHAN:
		return nested_loop_comparison_operator<operators::LessThan>(l, r, lvector, rvector, count);
	case ExpressionType::COMPARE_GREATERTHAN:
		return nested_loop_comparison_operator<operators::GreaterThan>(l, r, lvector, rvector, count);
	case ExpressionType::COMPARE_LESSTHANOREQUALTO:
		return nested_loop_comparison_operator<operators::LessThanEquals>(l, r, lvector, rvector, count);
	case ExpressionType::COMPARE_GREATERTHANOREQUALTO:
		return nested_loop_comparison_operator<operators::GreaterThanEquals>(l, r, lvector, rvector, count);
	default:
		throw NotImplementedException("Unimplemented comparison type for join!");
	}
}

void PhysicalNestedLoopJoinInner::_GetChunk(ClientContext &context, DataChunk &chunk, PhysicalOperatorState *state_) {
	assert(type == JoinType::INNER);
	auto state = reinterpret_cast<PhysicalNestedLoopJoinInnerOperatorState *>(state_);
	...
	if (state->right_chunks.column_count() == 0) {
		...
		do {
			...
			for (size_t i = 0; i < conditions.size(); i++) {
				executor.ExecuteExpression(conditions[i].right.get(), right_condition.data[i]);
			}
	...
	do {
		...
		match_count = nested_loop_join(conditions[0].comparison, state->left_join_condition.data[0],
		                               right_chunk.data[0], state->left_tuple, state->right_tuple, lvector, rvector);
		...
		for (size_t i = 1; i < conditions.size(); i++) {
			...
			match_count = nested_loop_comparison(conditions[i].comparison, l, r, lvector, rvector, match_count);
		}
...
unique_ptr<PhysicalOperatorState> PhysicalNestedLoopJoinInner::GetOperatorState(ExpressionExecutor *parent_executor) {
	return make_unique<PhysicalNestedLoopJoinInnerOperatorState>(children[0].get(), children[1].get(), parent_executor);
}
