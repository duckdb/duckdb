...
PhysicalNestedLoopJoinSemi::PhysicalNestedLoopJoinSemi(LogicalOperator &op, unique_ptr<PhysicalOperator> left,
...
void PhysicalNestedLoopJoinSemi::_GetChunk(ClientContext &context, DataChunk &chunk, PhysicalOperatorState *state_) {
	assert(type == JoinType::SEMI || type == JoinType::ANTI);
	auto state = reinterpret_cast<PhysicalNestedLoopJoinSemiOperatorState *>(state_);
	...
	do {
		...
		for (size_t r = 0; r < state->right_chunk.chunks.size(); r++) {
			...
			do {
				...
				match_count = nested_loop_join(conditions[0].comparison, left_condition, right_chunk.data[0], left_pos,
				                               right_pos, lvector, rvector);
				if (match_count == 0) {
					break;
				}
...
unique_ptr<PhysicalOperatorState> PhysicalNestedLoopJoinSemi::GetOperatorState(ExpressionExecutor *parent_executor) {
	return make_unique<PhysicalNestedLoopJoinSemiOperatorState>(children[0].get(), children[1].get(), parent_executor);
}
