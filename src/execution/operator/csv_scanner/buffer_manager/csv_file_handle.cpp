...
idx_t CSVFileHandle::Read(void *buffer, idx_t nr_bytes) {
	...
	// the data that is written while reading the file. This can be useful, for example when reading a duckdb log file
	// in csv format while logging is enabled
	if (file_handle->GetFileCompressionType() == FileCompressionType::UNCOMPRESSED && encoder.encoding_name == "utf-8") {
