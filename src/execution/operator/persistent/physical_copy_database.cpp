...
SourceResultType PhysicalCopyDatabase::GetDataInternal(ExecutionContext &context, DataChunk &chunk,
	...
	for (auto &create_info : info->entries) {
		...
		auto &table_entry = catalog.GetEntry<TableCatalogEntry>(context.client, create_index_info.schema.GetName(),
		                                                        create_index_info.table.GetName());
		auto &data_table = table_entry.GetStorage();

		IndexStorageInfo storage_info(create_index_info.index_name.GetName());
