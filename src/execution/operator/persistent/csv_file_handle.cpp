...
idx_t CSVFileHandle::Read(void *buffer, idx_t nr_bytes) {
	...
	auto bytes_read = file_handle->Read(data_ptr_cast<char>(buffer) + result_offset, nr_bytes - result_offset);
	file_size = file_handle->GetFileSize();
	...
	if (reset_enabled) {
		...
		memcpy(cached_buffer.get() + buffer_size, data_ptr_cast<char>(buffer) + result_offset, bytes_read);
