...
void BufferedCSVReaderOptions::SetSQLType(const LogicalType &type) {
...
void BufferedCSVReaderOptions::FromNamedParameters(named_parameter_map_t &in, ClientContext &context,
...
void BufferedCSVReaderOptions::ToNamedParameters(named_parameter_map_t &named_params) {
	...
	if (skip_rows_set) {
		named_params["skip"] = Value::BIGINT(skip_rows);
	}
