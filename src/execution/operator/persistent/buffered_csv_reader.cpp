...
static bool ParseBoolean(vector<Value> &set, const string &loption) {
	if (set.empty()) {
...
static string ParseString(vector<Value> &set, const string &loption) {
	if (set.size() != 1) {
...
static int64_t ParseInteger(vector<Value> &set, const string &loption) {
	if (set.size() != 1) {
		// no option specified or multiple options specified
...
static vector<bool> ParseColumnList(vector<Value> &set, vector<string> &names, const string &loption) {
	vector<bool> result;
	if (set.empty()) {
...
void BufferedCSVReaderOptions::SetReadOption(const string &loption, const Value &value,
	...
	SetReadOption(loption, set, expected_names);
}

void BufferedCSVReaderOptions::SetReadOption(const string &loption, vector<Value> &set,
                                             vector<string> &expected_names) {
	if (SetBaseOption(loption, set)) {
		return;
	}
	if (loption == "auto_detect") {
		auto_detect = ParseBoolean(set, loption);
	} else if (loption == "sample_size") {
		int64_t sample_size = ParseInteger(set, loption);
		...
		skip_rows = ParseInteger(set, loption);
	} else if (loption == "max_line_size" || loption == "maximum_line_size") {
		maximum_line_size = ParseInteger(set, loption);
	} else if (loption == "sample_chunk_size") {
		sample_chunk_size = ParseInteger(set, loption);
		...
		sample_chunks = ParseInteger(set, loption);
		if (sample_chunks < 1) {
			throw BinderException("Unsupported parameter for SAMPLE_CHUNKS: cannot be smaller than 1");
		}
	} else if (loption == "force_not_null") {
		force_not_null = ParseColumnList(set, expected_names, loption);
	} else if (loption == "date_format" || loption == "dateformat") {
		string format = ParseString(set, loption);
...
void BufferedCSVReaderOptions::SetWriteOption(const string &loption, vector<Value> &set) {
	if (SetBaseOption(loption, set)) {
		return;
	}

	if (loption == "force_quote") {
		force_quote = ParseColumnList(set, names, loption);
	} else {
		throw BinderException("Unrecognized option CSV writer \"%s\"", loption);
	}
}

bool BufferedCSVReaderOptions::SetBaseOption(const string &loption, vector<Value> &set) {
	...
	if (StringUtil::StartsWith(loption, "delim") || StringUtil::StartsWith(loption, "sep")) {
		SetDelimiter(ParseString(set, loption));
	} else if (loption == "quote") {
		quote = ParseString(set, loption);
		has_quote = true;
	} else if (loption == "escape") {
		escape = ParseString(set, loption);
		has_escape = true;
	} else if (loption == "header") {
		header = ParseBoolean(set, loption);
		has_header = true;
	} else if (loption == "null" || loption == "nullstr") {
		null_str = ParseString(set, loption);
	} else if (loption == "encoding") {
		auto encoding = StringUtil::Lower(ParseString(set, loption));
		if (encoding != "utf8" && encoding != "utf-8") {
			throw BinderException("Copy is only supported for UTF-8 encoded files, ENCODING 'UTF-8'");
		}
	} else if (loption == "compression") {
		compression = FileCompressionTypeFromString(ParseString(set, loption));
	} else {
