...
FixedSizeAllocatorInfo FixedSizeAllocatorInfo::Deserialize(Deserializer &source) {

deserializer.ReadProperty<idx_t>(1001, "segment_size_");
}
...
FixedSizeAllocator FixedSizeAllocator::Deserialize(Deserializer &deserializer) {
	...
	deserializer.ReadList(1002, "buffer_ids", [&](Deserializer::List &list, idx_t i) {
		buffer_ids.push_back(list.ReadElement<idx_t>());
	});

	deserializer.ReadList(1003, "buffer_block_pointers", [&](Deserializer::List &list, idx_t i) {
		buffer_block_pointers.push_back(list.ReadElement<BlockPointer>());
	});

	deserializer.ReadList(1004, "buffer_segment_counts", [&](Deserializer::List &list, idx_t i) {
		buffer_segment_counts.push_back(list.ReadElement<idx_t>());
	});

	deserializer.ReadList(1005, "buffer_allocation_sizes", [&](Deserializer::List &list, idx_t i) {
		buffer_allocation_sizes.push_back(list.ReadElement<idx_t>());
	});

	for (idx_t i = 0; i < buffer_ids.size(); i++) {
		FixedSizeBuffer new_buffer(block_manager, buffer_segment_counts[i],
		                           buffer_allocation_sizes[i], buffer_block_pointers[i]);
		buffers.insert(make_pair(buffer_ids[i], std::move(new_buffer)));
		total_segment_count += buffer_segment_counts[i];
	}

	deserializer.ReadList(1006, "buffers_with_free_space_vec", [&](Deserializer::List &list, idx_t i) {
		buffers_with_free_space.insert(list.ReadElement<idx_t>());
	});
