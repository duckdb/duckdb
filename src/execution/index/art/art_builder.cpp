...
ARTConflictType ARTBuilder::Build() {
	while (!s.empty()) {
		...
		if (start.len == entry.depth) {
			...
			SlotHandle slot(entry.node);
			auto count = UnsafeNumericCast<idx_t>(start.len - prefix_depth);
			Prefix::New(art, slot, start, prefix_depth, count);

			// Inline the row ID.
			if (row_id_count == 1) {
				Leaf::New(slot.Ref(), row_ids[entry.start].GetRowId());
				continue;
			}
			...
			for (idx_t i = entry.start; i < entry.start + row_id_count; i++) {
				ARTOperator::Insert(arena, art, slot.Ref(), row_ids[i], 0, row_ids[i], GateStatus::GATE_SET,
				                    DeleteIndexInfo(), IndexAppendMode::DEFAULT);
			}
			slot.Ref().SetGateStatus(GateStatus::GATE_SET);
			continue;
		}
		...
		Prefix::New(art, slot, start, prefix_depth, prefix_length);
		...
		NodePtr::New(art, slot.Ref(), NodePtr::GetInternalNodeType(child_offsets.size()));
		...
		for (idx_t i = 1; i <= child_offsets.size(); i++) {
			...
			NodePtr::InsertChild(art, slot.Ref(), child_byte);
			auto child = slot.Ref().NodePtr::GetChildMutable(art, child_byte, true);
