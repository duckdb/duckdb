...
ARTConflictType ARTBuilder::Build() {
	while (!s.empty()) {
		...
		if (start.len == entry.depth) {
			...
			reference<NodePtr> ref(entry.node);
		...
		NodePtr::New(art, ref, NodePtr::GetNodeType(child_offsets.size()));
		...
		for (idx_t i = 1; i <= child_offsets.size(); i++) {
			...
			NodePtr::InsertChild(art, ref, child_byte);
			auto child = ref.get().NodePtr::GetChildMutable(art, child_byte, true);
