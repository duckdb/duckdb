...
void ART::Vacuum(IndexLock &state) {
	...
	auto preorder_handler = [&](NodePtr &child) -> ARTScanStep {
...
void ART::InitializeMerge(NodePtr &other_tree, unsafe_vector<idx_t> &upper_bounds) {
	...
	auto filter = [](NodePtr node) -> ScanNodeResult {
		D_ASSERT(node.HasMetadata());
		return ScanNodeResult::SCAN_CHILDREN;
	};

	auto pre_handler = [&](NodePtr &child) -> ARTScanStep {
