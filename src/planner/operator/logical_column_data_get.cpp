...
LogicalColumnDataGet::LogicalColumnDataGet(idx_t table_index, vector<LogicalType> types, unique_ptr<ColumnDataCollection> collection)
    : LogicalOperator(LogicalOperatorType::LOGICAL_CHUNK_GET), table_index(table_index), collection(std::move(collection))
{
...
CKeyCollection* LogicalColumnDataGet::DeriveKeyCollection(CExpressionHandle &exprhdl)
{
...
//
//---------------------------------------------------------------------------
CPropConstraint* LogicalColumnDataGet::DerivePropertyConstraint(CExpressionHandle &exprhdl)
{
	return nullptr;
...
Operator* LogicalColumnDataGet::SelfRehydrate(CCostContext* pcc, duckdb::vector<Operator*> pdrgpexpr, CDrvdPropCtxtPlan* pdpctxtplan)
{
...
//
//---------------------------------------------------------------------------
CXformSet* LogicalColumnDataGet::PxfsCandidates() const
{
