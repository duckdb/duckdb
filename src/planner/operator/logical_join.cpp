...
JoinSide LogicalJoin::GetJoinSide(std::unique_ptr<AbstractExpression>& expr) {
	if (expr->type == ExpressionType::COLUMN_REF) {
		...
		if (children[0]->referenced_tables.find(colref->binding.table_index) != children[0]->referenced_tables.end()) {
			return JoinSide::LEFT;
		} else {
			assert(children[1]->referenced_tables.find(colref->binding.table_index) != children[1]->referenced_tables.end());
...
void LogicalJoin::SetJoinCondition(std::unique_ptr<AbstractExpression> condition) {
	...
	if (condition->GetExpressionType() == ExpressionType::CONJUNCTION_AND) {
		...
		condition->GetExpressionType() <= ExpressionType::COMPARE_GREATERTHANOREQUALTO) {
		// logical comparison
		// figure out which side belongs to the left and which side belongs to the right
		...
		} else if (left_side == JoinSide::RIGHT && left_side == JoinSide::LEFT) {
			// left is right right is left
			join_condition.left = move(condition->children[1]);
			join_condition.right = move(condition->children[0]);
		} else {
			throw Exception("FIXME: Join condition is not comparison between left and right sides");
		}
