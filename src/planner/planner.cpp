...
void Planner::CreatePlan(SQLStatement &statement) {
	...
	for (auto &expr : bound_parameters) {
		...
		if (entry == value_map.end()) {
			...
			throw BinderException("Inconsistent types found for parameter with index %llu, current type %s, new type %s",
					      expr->parameter_nr, entry->second.back()->type().ToString().c_str(),
