...
static unique_ptr<Expression> PlanUncorrelatedSubquery(Binder &binder, BoundSubqueryExpression &expr,
	...
	case SubqueryType::EXISTS: {
		...
		auto count_func = binder.context.catalog.GetAggregateFunction(binder.context.ActiveTransaction(), DEFAULT_SCHEMA, "count_star");
		auto count_star =
		    make_unique<BoundAggregateExpression>(TypeId::BIGINT, nullptr, count_func, false);
	...
	case SubqueryType::SCALAR: {
		...
		auto first_agg =
		    make_unique<BoundAggregateExpression>(expr.return_type, move(bound), first_func, false);
