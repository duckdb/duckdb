...
bool PushVariantCollation(ClientContext &context, unique_ptr<Expression> &source, const LogicalType &sql_type,
	...
	auto &catalog = Catalog::GetSystemCatalog(context);
	auto &function_entry =
	    catalog.GetEntry<ScalarFunctionCatalogEntry>(context, Identifier::DefaultSchema(), "variant_sort_key");
	if (function_entry.functions.Size() != 1) {
		throw InternalException("variant_sort_key should only have a single overload");
	}
