...
BindResult OrderBinder::CreateProjectionReference(Expression &expr, size_t index) {
	return BindResult(make_unique<BoundColumnRefExpression>(expr, TypeId::INVALID, ColumnBinding(node.binding.projection_index, index)));
}

BindResult OrderBinder::BindExpression(unique_ptr<Expression> expr) {
	// in the ORDER BY clause we do not bind children
	// we bind ONLY to the select list
	// if there is no matching entry in the SELECT list already, we add the expression to the SELECT list and refer the new expression
	// the new entry will then be bound later during the binding of the SELECT list
	// we also don't do type resolution here: this only happens after the SELECT list has been bound
