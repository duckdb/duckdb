...
BindResult ExpressionBinder::BindExpression(MeasureDefinitionExpression &expr, idx_t depth) {
	if (!expr.measure_expression) {
		return BindResult(BinderException::Unsupported(expr, "MEASURE_DEFINITION expression must have a measure expression"));
	}
	BindResult result = BindExpression(expr.measure_expression, depth, false);
	if (result.HasError()) {
		return result;
	}
	auto measure_type = LogicalType::MEASURE_TYPE(result.expression->return_type, expr.alias, result.expression->Copy());
