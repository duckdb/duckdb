	...
	for (idx_t i = 1; i < children.size(); i++) {
		...
		if (is_in_operator) {
			...
			if (!BoundComparisonExpression::TryBindComparison(max_type, child_return, max_type)) {
				throw BinderException(binder.FormatError(op.query_location, "Cannot mix values of type %s and %s in %s clause - an explicit cast is required",
			...
			if (!LogicalType::TryGetMaxLogicalType(max_type, child_return, max_type)) {
				throw BinderException(binder.FormatError(op.query_location, "Cannot mix values of type %s and %s in COALESCE operator - an explicit cast is required",
														 max_type.ToString(), child_return.ToString()));
			}
		}
	}
