...
unique_ptr<BoundTableRef> Binder::BindBoundPivot(PivotRef &ref) {
	...
	if (aggregates.size() != ref.bound_aggregate_names.size()) {
		throw BinderException("Pivot aggregate count mismatch. Expected %llu aggregates but found %llu. Are all pivot expressions aggregate functions?", ref.bound_aggregate_names.size(), aggregates.size());
	}
