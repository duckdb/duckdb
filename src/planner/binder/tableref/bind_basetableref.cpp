...
unique_ptr<BoundTableRef> Binder::Bind(BaseTableRef &expr) {
	...
	if (cte) {
		...
		if (ctebinding == nullptr) {
			...
			bind_context.AddGenericBinding(index, expr.alias.empty() ? expr.table_name : expr.alias, b->names,
			                               b->types);
	...
	auto table_or_view =
	    Catalog::GetCatalog(context).GetEntry(context, CatalogType::TABLE, expr.schema_name, expr.table_name);
