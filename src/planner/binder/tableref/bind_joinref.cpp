...
unique_ptr<LogicalOperator> Binder::Bind(JoinRef &ref) {
	// first plan the left and right side of the join
	auto left = Bind(*ref.left);
	auto right = Bind(*ref.right);

	// we insert any columns specified in the USING clause as hidden
	// this is to avoid expanding them in a * statement
	for (auto &hidden_column : ref.hidden_columns) {
		bind_context.hidden_columns.insert(hidden_column);
	}

	// bind the join condition
	WhereBinder binder(*this, context);
	auto condition = binder.Bind(ref.condition);

	// now we create the actual logical join node
	...
	if (join->type == LogicalOperatorType::COMPARISON_JOIN) {
		...
		if (any_join.condition->HasSubquery()) {
			throw NotImplementedException("Cannot perform non-comparison join on subquery!");
		}
	}
	return result;
}
