...
unique_ptr<LogicalOperator> Binder::CastLogicalOperatorToTypes(vector<LogicalType> &source_types, vector<LogicalType> &target_types, unique_ptr<LogicalOperator> op)
{
	D_ASSERT(op);
	...
	// otherwise add casts
	auto node = op.get();
	if (node->logical_type == LogicalOperatorType::LOGICAL_PROJECTION)
	{
		...
		for (idx_t i = 0; i < target_types.size(); i++) {
			if (source_types[i] != target_types[i])
			{
				...
				node->expressions[i] = BoundCastExpression::AddCastToType(context, std::move(node->expressions[i]), target_types[i]);
