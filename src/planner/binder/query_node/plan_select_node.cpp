...
unique_ptr<LogicalOperator> Binder::PlanFilter(unique_ptr<Expression> condition, unique_ptr<LogicalOperator> root)
{
	...
	filter->AddChild(unique_ptr_cast<LogicalOperator, Operator>(std::move(root)));
...
unique_ptr<LogicalOperator> Binder::CreatePlan(BoundSelectNode &statement)
{
	...
	if (!statement.aggregates.empty() || !statement.groups.group_expressions.empty())
	{
		...
		auto aggregate = make_uniq<LogicalAggregate>(statement.group_index, statement.aggregate_index, std::move(statement.aggregates));
		...
		aggregate->AddChild(unique_ptr_cast<LogicalOperator, Operator>(std::move(root)));
	...
	if (statement.having)
	{
		...
		having->AddChild(unique_ptr_cast<LogicalOperator, Operator>(std::move(root)));
	...
	if (!statement.windows.empty())
	{
		...
		win->AddChild(unique_ptr_cast<LogicalOperator, Operator>(std::move(root)));
		root = std::move(win);
	}
	if (statement.qualify)
	{
		...
		qualify->AddChild(unique_ptr_cast<LogicalOperator, Operator>(std::move(root)));
	...
	for (idx_t i = statement.unnests.size(); i > 0; i--)
	{
		...
		unnest->AddChild(unique_ptr_cast<LogicalOperator, Operator>(std::move(root)));
	...
	proj->AddChild(unique_ptr_cast<LogicalOperator, Operator>(std::move(root)));
	root = std::move(proj);
	...
	if (statement.need_prune)
	{
		...
		{
			prune_expressions.push_back(make_uniq<BoundColumnRefExpression>(projection.expressions[i]->return_type, ColumnBinding(statement.projection_index, i)));
		}
		auto prune = make_uniq<LogicalProjection>(statement.prune_index, std::move(prune_expressions));
		prune->AddChild(unique_ptr_cast<LogicalOperator, Operator>(std::move(root)));
		root = std::move(prune);
	}
	return root;
}
