...
unique_ptr<LogicalOperator> Binder::VisitQueryNode(BoundQueryNode &node, unique_ptr<LogicalOperator> root)
{
	...
	for (auto &mod : node.modifiers)
	{
		...
		case ResultModifierType::ORDER_MODIFIER:
		{
			...
			if (root->logical_type == LogicalOperatorType::LOGICAL_DISTINCT)
			{
		...
		case ResultModifierType::LIMIT_MODIFIER:
		{
			...
			auto limit = make_uniq<LogicalLimit>(bound.limit_val, bound.offset_val, std::move(bound.limit), std::move(bound.offset));
		...
		case ResultModifierType::LIMIT_PERCENT_MODIFIER:
		{
			...
			auto limit = make_uniq<LogicalLimitPercent>(bound.limit_percent, bound.offset_val, std::move(bound.limit), std::move(bound.offset));
			limit->AddChild(std::move(root));
			root = std::move(limit);
			break;
		}
