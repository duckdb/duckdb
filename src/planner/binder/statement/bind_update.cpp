...
static void BindUpdateConstraints(TableCatalogEntry &table, Binder &binder, ClientContext &context, BoundUpdateStatement &result) {
	// check the constraints of the table to see if we need to project any additional columns
	// we do this for CHECK constraints in the UPDATE clause
	...
	// (FIXME: we probably want to do this for UNIQUE with multiple columns as well)
