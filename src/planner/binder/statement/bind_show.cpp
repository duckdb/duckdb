...
BoundStatement Binder::Bind(ShowStatement &stmt)
{
	...
	auto show = make_uniq<LogicalShow>(unique_ptr_cast<Operator, LogicalOperator>(std::move(plan.plan)));
	...
	result.types = {LogicalType::VARCHAR, LogicalType::VARCHAR, LogicalType::VARCHAR, LogicalType::VARCHAR, LogicalType::VARCHAR, LogicalType::VARCHAR};
