...
BoundStatement Binder::Bind(DeleteStatement &stmt)
{
	...
	D_ASSERT(root->logical_type == LogicalOperatorType::LOGICAL_GET);
	if (!table.temporary)
	{
	...
	del->expressions.push_back(make_uniq<BoundColumnRefExpression>(LogicalType::ROW_TYPE, ColumnBinding(get.table_index, get.column_ids.size())));
	...
	if (!stmt.returning_list.empty())
	{
		...
		return BindReturning(std::move(stmt.returning_list), table, stmt.table->alias, update_table_index, std::move(del_as_logicaloperator), std::move(result));
	}
