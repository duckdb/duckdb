...
string BaseTableRef::ToString() const {
	string result;
	result += catalog_name.empty() ? "" : (SQLIdentifier(catalog_name.GetName()) + ".");
	result += schema_name.empty() ? "" : (SQLIdentifier(schema_name.GetName()) + ".");
	result += SQLIdentifier(table_name.GetName());
