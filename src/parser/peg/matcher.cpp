...
struct PEGParserProfileStats {
	string name;
	MatcherType type;
	idx_t calls = 0;
	idx_t success = 0;
	idx_t fail = 0;
	idx_t repeated_start_calls = 0;
...
struct PEGParserProfileKey {
	...
	bool operator==(const PEGParserProfileKey &other) const {
		return matcher == other.matcher && token_index == other.token_index;
	}
};

struct PEGParserProfileKeyHash {
	size_t operator()(const PEGParserProfileKey &key) const {
		return std::hash<const Matcher *>()(key.matcher) ^ (std::hash<idx_t>()(key.token_index) << 1);
	}
};
...
public:
	static bool Enabled() {
		auto env = std::getenv("DUCKDB_PEG_PROFILE");
		return env && env[0] != '\0' && env[0] != '0';
	}

	void Reset(idx_t token_count_p) {
		stats.clear();
		seen_starts.clear();
		depth = 0;
	...
	optional_ptr<ParseResult> Profile(const Matcher &matcher, MatchState &state) {
		auto &entry = GetStats(matcher);
		auto start_token_index = state.token_index;
		auto key = PEGParserProfileKey {&matcher, start_token_index};
		auto seen_entry = seen_starts.find(key);
		if (seen_entry == seen_starts.end()) {
			seen_starts.insert(make_pair(key, 1));
		} else {
			seen_entry->second++;
			entry.repeated_start_calls++;
		}
		...
		if (result) {
			entry.success++;
			auto consumed = state.token_index >= start_token_index ? state.token_index - start_token_index : 0;
			entry.total_consumed += consumed;
			entry.max_consumed = MaxValue(entry.max_consumed, consumed);
		} else {
			entry.fail++;
		}
		depth--;
	...
	void Print() {
		...
		idx_t total_calls = 0;
		idx_t total_repeated_start_calls = 0;
		...
		for (idx_t i = 0; i < limit; i++) {
			...
			std::fprintf(stderr, "%-36s %10llu %10llu %10llu %10llu %7.2f%% %10.3f %10.3f %10llu %8llu\n",
			             ...
			             static_cast<unsigned long long>(entry.repeated_start_calls), hit_rate, time_ms, avg_us,
			             static_cast<unsigned long long>(entry.max_consumed),
			             static_cast<unsigned long long>(entry.max_depth));
		}
	}
...
private:
	...
	PEGParserProfileStats &GetStats(const Matcher &matcher) {
		auto entry = stats.find(&matcher);
		if (entry != stats.end()) {
			return entry->second;
		}
		PEGParserProfileStats new_stats;
		new_stats.name = matcher.GetName();
		new_stats.type = matcher.Type();
		auto result = stats.insert(make_pair(&matcher, std::move(new_stats)));
		return result.first->second;
	}

private:
	std::unordered_map<const Matcher *, PEGParserProfileStats> stats;
	std::unordered_map<PEGParserProfileKey, idx_t, PEGParserProfileKeyHash> seen_starts;
