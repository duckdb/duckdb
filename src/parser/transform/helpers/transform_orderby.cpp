...
bool Transformer::TransformOrderBy(postgres::PGList *order, vector<OrderByNode> &result) {
	if (!order) {
		return false;
	}

	for (auto node = order->head; node != nullptr; node = node->next) {
		auto temp = reinterpret_cast<postgres::PGNode *>(node->data.ptr_value);
		if (temp->type == postgres::T_PGSortBy) {
			OrderByNode ordernode;
			auto sort = reinterpret_cast<postgres::PGSortBy *>(temp);
			auto target = sort->node;
			if (sort->sortby_dir == postgres::PG_SORTBY_ASC || sort->sortby_dir == postgres::PG_SORTBY_DEFAULT) {
				ordernode.type = OrderType::ASCENDING;
			} else if (sort->sortby_dir == postgres::PG_SORTBY_DESC) {
