...
unique_ptr<TableRef> Transformer::TransformJoin(postgres::PGJoinExpr *root) {
	auto result = make_unique<JoinRef>();
	switch (root->jointype) {
	case postgres::PG_JOIN_INNER: {
		result->type = JoinType::INNER;
		break;
	}
	case postgres::PG_JOIN_LEFT: {
		result->type = JoinType::LEFT;
		break;
	}
	case postgres::PG_JOIN_FULL: {
		result->type = JoinType::OUTER;
		break;
	}
	case postgres::PG_JOIN_RIGHT: {
		result->type = JoinType::RIGHT;
		break;
	}
	case postgres::PG_JOIN_SEMI: {
	...
	if (root->usingClause && root->usingClause->length > 0) {
		...
		for (auto node = root->usingClause->head; node != nullptr; node = node->next) {
			...
			assert(target->type == postgres::T_PGString);
			auto column_name = string(reinterpret_cast<postgres::PGValue *>(target)->val.str);
