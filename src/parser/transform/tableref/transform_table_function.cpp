...
unique_ptr<TableRef> Transformer::TransformRangeFunction(postgres::PGRangeFunction *root) {
	if (root->lateral) {
	...
	auto function_sublist = (postgres::PGList *)root->functions->head->data.ptr_value;
	assert(function_sublist->length == 2);
	auto call_tree = (postgres::PGNode *)function_sublist->head->data.ptr_value;
	auto coldef = function_sublist->head->next->data.ptr_value;

	assert(call_tree->type == postgres::T_PGFuncCall);
	if (coldef) {
		throw NotImplementedException("Explicit column definition not supported yet");
	}
	// transform the function call
	auto result = make_unique<TableFunctionRef>();
	result->function = TransformFuncCall((postgres::PGFuncCall *)call_tree);
