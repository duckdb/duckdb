...
unique_ptr<ParsedExpression> Transformer::TransformCase(postgres::PGCaseExpr *root) {
	if (!root) {
		return nullptr;
	}
	...
	if (root->defresult) {
		def_res = TransformExpression(reinterpret_cast<postgres::PGNode *>(root->defresult));
	} else {
	...
	for (auto cell = root->args->head; cell != nullptr; cell = cell->next) {
		auto w = reinterpret_cast<postgres::PGCaseWhen *>(cell->data.ptr_value);
		auto test_raw = TransformExpression(reinterpret_cast<postgres::PGNode *>(w->expr));
		unique_ptr<ParsedExpression> test;
		auto arg = TransformExpression(reinterpret_cast<postgres::PGNode *>(root->arg));
		if (arg) {
		...
		cur_root->result_if_true = TransformExpression(reinterpret_cast<postgres::PGNode *>(w->result));
