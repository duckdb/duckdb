...
unique_ptr<ParsedExpression> Transformer::TransformSubquery(postgres::PGSubLink *root) {
	if (!root) {
		return nullptr;
	}
	auto subquery_expr = make_unique<SubqueryExpression>();
	subquery_expr->subquery = TransformSelectNode((postgres::PGSelectStmt *)root->subselect);
	...
	case postgres::PG_EXISTS_SUBLINK: {
		subquery_expr->subquery_type = SubqueryType::EXISTS;
		break;
	}
	case postgres::PG_ANY_SUBLINK:
	case postgres::PG_ALL_SUBLINK: {
		...
		if (!root->operName) {
			...
			auto operator_name =
			    string((reinterpret_cast<postgres::PGValue *>(root->operName->head->data.ptr_value))->val.str);
		...
		if (root->subLinkType == postgres::PG_ALL_SUBLINK) {
