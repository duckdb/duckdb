...
unique_ptr<ParsedExpression> Transformer::TransformCoalesce(postgres::A_Expr *root) {
	if (!root) {
		return nullptr;
	}
	auto coalesce_args = reinterpret_cast<postgres::List *>(root->lexpr);
	...
	for (auto cell = coalesce_args->head; cell && cell->next; cell = cell->next) {
		...
		auto value_expr = TransformExpression(reinterpret_cast<postgres::Node *>(cell->data.ptr_value));
