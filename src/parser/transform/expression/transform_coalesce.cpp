...
unique_ptr<ParsedExpression> Transformer::TransformCoalesce(postgres::PGAExpr *root) {
	if (!root) {
		return nullptr;
	}
	auto coalesce_args = reinterpret_cast<postgres::PGList *>(root->lexpr);
	...
	for (auto cell = coalesce_args->head; cell && cell->next; cell = cell->next) {
		...
		auto value_expr = TransformExpression(reinterpret_cast<postgres::PGNode *>(cell->data.ptr_value));
		...
		if (cell->next->next == nullptr) {
			...
			cur_root->result_if_false =
			    TransformExpression(reinterpret_cast<postgres::PGNode *>(cell->next->data.ptr_value));
		} else {
