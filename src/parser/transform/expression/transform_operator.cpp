...
unique_ptr<ParsedExpression> Transformer::TransformAExpr(postgres::PGAExpr *root) {
	if (!root) {
		return nullptr;
	}
	auto name = string((reinterpret_cast<postgres::PGValue *>(root->name->head->data.ptr_value))->val.str);

	switch (root->kind) {
	case postgres::PG_AEXPR_DISTINCT:
		break;
	case postgres::PG_AEXPR_IN: {
		...
		TransformExpressionList((postgres::PGList *)root->rexpr, result->children);
		return move(result);
	} break;
	// rewrite NULLIF(a, b) into CASE WHEN a=b THEN NULL ELSE a END
	case postgres::PG_AEXPR_NULLIF: {
	...
	case postgres::PG_AEXPR_BETWEEN:
	case postgres::PG_AEXPR_NOT_BETWEEN: {
		auto between_args = reinterpret_cast<postgres::PGList *>(root->rexpr);
		...
		auto between_left = TransformExpression(reinterpret_cast<postgres::PGNode *>(between_args->head->data.ptr_value));
		auto between_right =
		    TransformExpression(reinterpret_cast<postgres::PGNode *>(between_args->tail->data.ptr_value));
		...
		auto compare_between = make_unique<ConjunctionExpression>(ExpressionType::CONJUNCTION_AND, move(compare_left),
		                                                          move(compare_right));
		if (root->kind == postgres::PG_AEXPR_BETWEEN) {
	...
	// rewrite SIMILAR TO into regexp_matches('asdf', '.*sd.*')
	case postgres::PG_AEXPR_SIMILAR: {
		auto left_expr = TransformExpression(root->lexpr);
