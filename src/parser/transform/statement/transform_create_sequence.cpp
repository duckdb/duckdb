...
unique_ptr<CreateSequenceStatement> Transformer::TransformCreateSequence(postgres::PGNode *node) {
	auto stmt = reinterpret_cast<postgres::PGCreateSeqStmt *>(node);
	...
	if (stmt->options) {
		postgres::PGListCell *cell = nullptr;
		for_each_cell(cell, stmt->options->head) {
			auto *def_elem = reinterpret_cast<postgres::PGDefElem *>(cell->data.ptr_value);
			string opt_name = string(def_elem->defname);

			auto val = (postgres::PGValue *)def_elem->arg;
			if (def_elem->defaction == postgres::PG_DEFELEM_UNSPEC && !val) { // e.g. NO MINVALUE
				continue;
			}
			assert(val);

			if (opt_name == "increment") {
				assert(val->type == postgres::T_PGInteger);
