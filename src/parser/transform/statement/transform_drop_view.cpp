...
unique_ptr<DropViewStatement> Transformer::TransformDropView(DropStmt *stmt) {
	...
	if (view_list->length == 2) {
		...
		info.view_name = reinterpret_cast<postgres::Value *>(view_list->head->next->data.ptr_value)->val.str;
	} else {
		info.view_name = reinterpret_cast<postgres::Value *>(view_list->head->data.ptr_value)->val.str;
	}
