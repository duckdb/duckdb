...
unique_ptr<CreateTableStatement> Transformer::TransformCreateTableAs(postgres::PGNode *node) {
	auto stmt = reinterpret_cast<postgres::PGCreateTableAsStmt *>(node);
	assert(stmt);
	if (stmt->relkind == postgres::PG_OBJECT_MATVIEW) {
