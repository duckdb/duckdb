...
void Transformer::TransformValuesList(postgres::PGList *list, vector<vector<unique_ptr<ParsedExpression>>> &values) {
	for (auto value_list = list->head; value_list != NULL; value_list = value_list->next) {
		auto target = (postgres::PGList *)(value_list->data.ptr_value);
...
unique_ptr<InsertStatement> Transformer::TransformInsert(postgres::PGNode *node) {
	auto stmt = reinterpret_cast<postgres::PGInsertStmt *>(node);
	assert(stmt);
	...
	if (stmt->cols) {
		for (auto c = stmt->cols->head; c != NULL; c = lnext(c)) {
			auto target = (postgres::PGResTarget *)(c->data.ptr_value);
			result->columns.push_back(string(target->name));
		}
	}

	auto select_stmt = reinterpret_cast<postgres::PGSelectStmt *>(stmt->selectStmt);
