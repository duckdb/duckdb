...
unique_ptr<DropTableStatement> Transformer::TransformDropTable(DropStmt *stmt) {
	...
	if (table_list->length == 2) {
		info.schema = reinterpret_cast<postgres::Value *>(table_list->head->data.ptr_value)->val.str;
		info.table = reinterpret_cast<postgres::Value *>(table_list->head->next->data.ptr_value)->val.str;
	} else {
		info.table = reinterpret_cast<postgres::Value *>(table_list->head->data.ptr_value)->val.str;
	}
