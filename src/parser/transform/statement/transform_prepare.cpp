...
unique_ptr<PrepareStatement> Transformer::TransformPrepare(postgres::PGNode *node) {
	auto stmt = reinterpret_cast<postgres::PGPrepareStmt *>(node);
	assert(stmt);
...
unique_ptr<ExecuteStatement> Transformer::TransformExecute(postgres::PGNode *node) {
	auto stmt = reinterpret_cast<postgres::PGExecuteStmt *>(node);
	assert(stmt);
...
unique_ptr<DeallocateStatement> Transformer::TransformDeallocate(postgres::PGNode *node) {
	auto stmt = reinterpret_cast<postgres::PGDeallocateStmt *>(node);
	assert(stmt);
