...
unique_ptr<CreateIndexStatement> Transformer::TransformCreateIndex(postgres::PGNode *node) {
	auto stmt = reinterpret_cast<postgres::PGIndexStmt *>(node);
	assert(stmt);
	...
	for (auto cell = stmt->indexParams->head; cell != nullptr; cell = cell->next) {
		auto index_element = (postgres::PGIndexElem *)cell->data.ptr_value;
