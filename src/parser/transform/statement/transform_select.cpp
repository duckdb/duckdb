...
unique_ptr<SelectStatement> Transformer::TransformSelect(postgres::PGNode *node) {
	auto stmt = reinterpret_cast<postgres::PGSelectStmt *>(node);
	auto result = make_unique<SelectStatement>();

	if (stmt->windowClause) {
		for (auto window_ele = stmt->windowClause->head; window_ele != NULL; window_ele = window_ele->next) {
			auto window_def = reinterpret_cast<postgres::PGWindowDef *>(window_ele->data.ptr_value);
			assert(window_def);
	...
	if (stmt->withClause) {
		TransformCTE(reinterpret_cast<postgres::PGWithClause *>(stmt->withClause), *result);
	}
