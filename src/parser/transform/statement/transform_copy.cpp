...
void SetControlString(postgres::PGDefElem *def_elem, string option, string option_example, string &info_str) {
	auto *val = (postgres::PGValue *)(def_elem->arg);
	if (!val || val->type != postgres::T_PGString) {
...
void HandleOptions(postgres::PGCopyStmt *stmt, CopyInfo &info) {
	// option names
	...
	for_each_cell(cell, stmt->options->head) {
		...
		if (StringUtil::StartsWith(def_elem->defname, "delim") || StringUtil::StartsWith(def_elem->defname, "sep")) {
			...
			// format
			auto *format_val = (postgres::PGValue *)(def_elem->arg);
			if (!format_val || format_val->type != postgres::T_PGString) {
			...
			// header
			auto *header_val = (postgres::PGValue *)(def_elem->arg);
			if (!header_val) {
				info.header = true;
				continue;
			}
			...
			if (!force_quote_val ||
			    (force_quote_val->type != postgres::T_PGAStar && force_quote_val->type != postgres::T_PGList)) {
				throw ParserException("Unsupported parameter type for FORCE_QUOTE: expected e.g. FORCE_QUOTE *");
			}

			// * option (all columns)
			if (force_quote_val->type == postgres::T_PGAStar) {
				info.quote_all = true;
			}

			// list of columns
			if (force_quote_val->type == postgres::T_PGList) {
				auto column_list = (postgres::PGList *)(force_quote_val);
			...
			if (!force_not_null_val || force_not_null_val->type != postgres::T_PGList) {
				throw ParserException("Unsupported parameter type for FORCE_NOT_NULL: expected e.g. FORCE_NOT_NULL *");
			}

			auto column_list = (postgres::PGList *)(force_not_null_val);
			for (auto c = column_list->head; c != NULL; c = lnext(c)) {
				auto target = (postgres::PGResTarget *)(c->data.ptr_value);
			...
			// encoding
			auto *encoding_val = (postgres::PGValue *)(def_elem->arg);
			if (!encoding_val || encoding_val->type != postgres::T_PGString) {
...
unique_ptr<CopyStatement> Transformer::TransformCopy(postgres::PGNode *node) {
	auto stmt = reinterpret_cast<postgres::PGCopyStmt *>(node);
	assert(stmt);
	...
	if (stmt->attlist) {
		for (auto n = stmt->attlist->head; n != nullptr; n = n->next) {
			auto target = reinterpret_cast<postgres::PGResTarget *>(n->data.ptr_value);
			if (target->name) {
	...
	if (stmt->relation) {
		...
		result->select_statement = TransformSelectNode((postgres::PGSelectStmt *)stmt->query);
	}
