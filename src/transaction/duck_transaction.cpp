...
ErrorData DuckTransaction::WriteToWAL(ClientContext &context, AttachedDatabase &db,
	...
	try {
		...
		profiler.StartTimer(MetricType::COMMIT_LOCAL_STORAGE_LATENCY);
		storage->Commit(commit_state.get());
		profiler.EndTimer(MetricType::COMMIT_LOCAL_STORAGE_LATENCY);

		profiler.StartTimer(MetricType::WRITE_TO_WAL_LATENCY);
		undo_buffer.WriteToWAL(*wal, commit_state.get());
		profiler.EndTimer(MetricType::WRITE_TO_WAL_LATENCY);
