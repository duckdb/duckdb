...
void CleanupState::CleanupEntry(UndoFlags type, data_ptr_t data) {
	...
	case UndoFlags::CATALOG_ENTRY: {
		...
		// Cleaning temporary entries generated by TABLE RENAME
		// if parent wasn't deleted, check child
		if(!catalog_entry->parent->deleted) {
			// if this entry was deleted (an entry generated by TABLE RENAME), release it
			if(catalog_entry->deleted)
				catalog_entry = nullptr;
		} else {
			// otherwise we need do release the old table entry due to the TABLE RENAME
			catalog_entry->parent = nullptr;
			catalog_entry = nullptr;
		}
		break;
	}
