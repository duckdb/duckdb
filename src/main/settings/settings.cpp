...
void MergeJoinThreshold::SetLocal(ClientContext &context, const Value &input) {
	auto &config = ClientConfig::GetConfig(context);
	config.merge_join_threshold = static_cast<idx_t>(input.GetValue<int64_t>());
}
...
Value MergeJoinThreshold::GetSetting(const ClientContext &context) {
	auto &config = ClientConfig::GetConfig(context);
	return Value::BIGINT(static_cast<int64_t>(config.merge_join_threshold));
}
...
void NestedLoopJoinThreshold::SetLocal(ClientContext &context, const Value &input) {
	auto &config = ClientConfig::GetConfig(context);
	config.nested_loop_join_threshold = static_cast<idx_t>(input.GetValue<int64_t>());
}
...
Value NestedLoopJoinThreshold::GetSetting(const ClientContext &context) {
	auto &config = ClientConfig::GetConfig(context);
	return Value::BIGINT(static_cast<int64_t>(config.nested_loop_join_threshold));
}
