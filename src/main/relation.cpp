...
shared_ptr<Relation> Relation::Project(const string &select_list, const vector<string> &aliases) {
	auto expressions = Parser::ParseExpressionList(select_list, context.lock()->GetParserOptions());
...
shared_ptr<Relation> Relation::Project(const vector<string> &expressions, const vector<string> &aliases) {
	auto result_list = StringListToExpressionList(*context.lock(), expressions);
...
shared_ptr<Relation> Relation::Filter(const string &expression) {
	auto expression_list = Parser::ParseExpressionList(expression, context.lock()->GetParserOptions());
	if (expression_list.size() != 1) {
...
shared_ptr<Relation> Relation::Filter(const vector<string> &expressions) {
	...
	auto expression_list = StringListToExpressionList(*context.lock(), expressions);
...
shared_ptr<Relation> Relation::Order(const string &expression) {
	auto order_list = Parser::ParseOrderList(expression, context.lock()->GetParserOptions());
...
shared_ptr<Relation> Relation::Order(const vector<string> &expressions) {
	...
	for (auto &expression : expressions) {
		auto inner_list = Parser::ParseOrderList(expression, context.lock()->GetParserOptions());
...
shared_ptr<Relation> Relation::Join(const shared_ptr<Relation> &other, const string &condition, JoinType type) {
	auto expression_list = Parser::ParseExpressionList(condition, context.lock()->GetParserOptions());
	D_ASSERT(!expression_list.empty());
...
shared_ptr<Relation> Relation::Aggregate(const string &aggregate_list) {
	auto expression_list = Parser::ParseExpressionList(aggregate_list, context.lock()->GetParserOptions());
...
shared_ptr<Relation> Relation::Aggregate(const string &aggregate_list, const string &group_list) {
	auto expression_list = Parser::ParseExpressionList(aggregate_list, context.lock()->GetParserOptions());
	auto groups = Parser::ParseExpressionList(group_list, context.lock()->GetParserOptions());
...
shared_ptr<Relation> Relation::Aggregate(const vector<string> &aggregates) {
	auto aggregate_list = StringListToExpressionList(*context.lock(), aggregates);
...
shared_ptr<Relation> Relation::Aggregate(const vector<string> &aggregates, const vector<string> &groups) {
	auto aggregate_list = StringListToExpressionList(*context.lock(), aggregates);
	auto group_list = StringListToExpressionList(*context.lock(), groups);
...
unique_ptr<QueryResult> Relation::Execute() {
	return context.lock()->Execute(shared_from_this());
}
...
void Relation::Insert(const vector<vector<Value>> &values) {
	...
	auto rel = make_shared<ValueRelation>(context.lock(), values, move(column_names), "values");
	rel->Insert(GetAlias());
}
...
unique_ptr<QueryResult> Relation::Query(const string &sql) {
	return context.lock()->Query(sql, false);
}
...
shared_ptr<Relation> Relation::TableFunction(const std::string &fname, const vector<Value> &values,
	...
	return make_shared<TableFunctionRelation>(context.lock(), fname, values, named_parameters, shared_from_this());
}

shared_ptr<Relation> Relation::TableFunction(const std::string &fname, const vector<Value> &values) {
	return make_shared<TableFunctionRelation>(context.lock(), fname, values, shared_from_this());
}
