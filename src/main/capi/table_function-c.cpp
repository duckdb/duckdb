...
void duckdb_table_function_get_client_context(duckdb_bind_info info, duckdb_client_context *context) {
	if (!info || !context) {
		return;
	}
	auto &bind_info = GetCTableFunctionBindInfo(info);
	auto wrapper = new duckdb::CClientContextWrapper(bind_info.context);
	*context = reinterpret_cast<duckdb_client_context>(wrapper);
}
