...
duckdb_state duckdb_append_default_to_chunk(duckdb_appender appender, duckdb_data_chunk chunk, int32_t column) {
	if (!appender || !chunk) {
		return DuckDBError;
	}
	...
	try {
		appender_instance->appender->AppendDefault(*data_chunk, column);
	} catch (std::exception &ex) {
		ErrorData error(ex);
		appender_instance->error = error.RawMessage();
		return DuckDBError;
