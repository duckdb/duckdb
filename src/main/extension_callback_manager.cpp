...
struct ExtensionCallbackRegistry {
	...
	identifier_map_t<shared_ptr<GrammarExtension>> grammar_extensions;
...
void ExtensionCallbackManager::Register(shared_ptr<GrammarExtension> extension) {
	...
	if (!res.second) {
		...
		throw InvalidInputException("Can't add GrammarExtension %s, a GrammarExtension by that name already exists",
		                            name);
	}
	callback_registry.atomic_store(new_registry);
}
...
identifier_map_t<shared_ptr<GrammarExtension>> ExtensionCallbackManager::GrammarExtensions() const {
	auto registry = callback_registry.atomic_load();
...
optional_ptr<GrammarExtension> ExtensionCallbackManager::FindGrammarExtension(const Identifier &name) const {
	auto registry = callback_registry.atomic_load();
...
optional_ptr<DialectExtension> ExtensionCallbackManager::GetDialectExtension(const Identifier &name) const {
	auto registry = callback_registry.atomic_load();
