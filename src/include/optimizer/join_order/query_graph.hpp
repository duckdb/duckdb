...
struct FilterNode {
	...
	unordered_map<uint64_t, unique_ptr<FilterNode>> children;
};
...
public:
	...
	struct QueryEdge {
		...
		unordered_map<uint64_t, unique_ptr<QueryEdge>> children;
	};
...
public:
	...
	vector<uint64_t> GetNeighbors(RelationSet *node, unordered_set<uint64_t> &exclusion_set);
