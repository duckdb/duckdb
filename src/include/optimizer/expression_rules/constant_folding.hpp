...
class ConstantFoldingRule : public Rule {
  public:
	...
	Apply(AbstractExpression &root, std::vector<AbstractOperator> &bindings) {
		...
		// FIXME:  these produce incorrect results with NULLs. Possible fix
		// could be to match bottom-up
		...
		// switch (root.type) { 				case
		// ExpressionType::OPERATOR_ADD: case ExpressionType::OPERATOR_SUBTRACT:
		// if
		...
		//					break;
		//				case ExpressionType::OPERATOR_DIVIDE:
		//					if (Value::Equals(right_val->value, zero)) { // X /
		//0 = NULL 						return
		...
		// switch
		//(root.type) { 				case ExpressionType::OPERATOR_ADD: // X + 0 ==
		//X if (Value::Equals(left_val->value, zero)) { return
