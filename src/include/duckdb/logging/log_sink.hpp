...
protected:
	...
	// Invoken whenever buffers are full to flush the sink
...
// Implements a stdout-based log sink using log lines in CSV to allow for easy parsing. Supports only denormalized mode
// since there is only one output stream
...
// FileLogSink implements a file-based logging system in CSV. It implements CSVLogSink to provide persistent log sink in
// CSV files.
// The FileLogSink can operate in normalized (separate files for entries and contexts) or denormalized mode (single
// file)
