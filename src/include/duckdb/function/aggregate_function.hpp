...
public:
	...
	AggregateFunction(const string &name, const vector<LogicalType> &arguments, const LogicalType &return_type,
	                  ...
	                  aggregate_serialize_t serialize = nullptr, aggregate_deserialize_t deserialize = nullptr)
	    : AggregateFunction(name, arguments, return_type, state_size, initialize, update, combine, finalize,
	                        null_handling, bind, destructor, statistics, window, serialize, deserialize) {
	}
	...
	AggregateFunction(const vector<LogicalType> &arguments, const LogicalType &return_type, aggregate_size_t state_size,
	    ...
	    : AggregateFunction(arguments, return_type, state_size, initialize, update, combine, finalize, null_handling,
	                        bind, destructor, statistics, window, serialize, deserialize) {
	}
	...
	aggregate_cluster_update_t GetStateClusterUpdateCallback() const { return callbacks.cluster_update; }
	void SetStateClusterUpdateCallback(aggregate_cluster_update_t callback) { callbacks.cluster_update = callback; }
