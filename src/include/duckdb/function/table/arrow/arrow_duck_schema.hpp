...
public:
	...
	explicit ArrowType(LogicalType type_p, const ArrowTypeInfo& type_info)
	    : type(std::move(type_p)), type_info(type_info) {
	}

	explicit ArrowType(string error_message_p, bool not_implemented_p = false)
	    : type(LogicalTypeId::INVALID), error_message(std::move(error_message_p)),
