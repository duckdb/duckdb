...
public:
	...
	static void ProcessEntries(CanonicalFormat &vdata, const LogicalType &type, uint64_t hashes[], uint8_t counts[],
	                           idx_t count);
	//! Add the indices and counts to the logs
	static void AddToLogs(CanonicalFormat &vdata, idx_t count, uint64_t indices[], uint8_t counts[],
	                      HyperLogLog **logs[], const SelectionVector *log_sel);
	//! Add the indices and counts to THIS log
	void AddToLog(CanonicalFormat &vdata, idx_t count, uint64_t indices[], uint8_t counts[]);
