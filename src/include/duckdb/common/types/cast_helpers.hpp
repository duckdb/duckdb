...
struct UhugeintToStringCast {
	static int UnsignedLength(uhugeint_t value) {
		if (value.upper == 0) {
			return NumericHelper::UnsignedLength<uint64_t>(value.lower);
		}
		// search the length using the POWERS_OF_TEN array
		// the length has to be between [17] and [38], because the hugeint is bigger than 2^63
		// we use the same approach as above, but split a bit more because comparisons for hugeints are more expensive
		if (value >= Uhugeint::POWERS_OF_TEN[27]) {
			// [27..38]
			if (value >= Uhugeint::POWERS_OF_TEN[32]) {
				if (value >= Uhugeint::POWERS_OF_TEN[36]) {
					int length = 37;
					length += value >= Uhugeint::POWERS_OF_TEN[37];
					length += value >= Uhugeint::POWERS_OF_TEN[38];
					return length;
				} else {
					int length = 33;
					length += value >= Uhugeint::POWERS_OF_TEN[33];
					length += value >= Uhugeint::POWERS_OF_TEN[34];
					length += value >= Uhugeint::POWERS_OF_TEN[35];
					return length;
				}
			} else {
				if (value >= Uhugeint::POWERS_OF_TEN[30]) {
					int length = 31;
					length += value >= Uhugeint::POWERS_OF_TEN[31];
					length += value >= Uhugeint::POWERS_OF_TEN[32];
					return length;
				} else {
					int length = 28;
					length += value >= Uhugeint::POWERS_OF_TEN[28];
					length += value >= Uhugeint::POWERS_OF_TEN[29];
					return length;
				}
			}
		} else {
			// [17..27]
			if (value >= Uhugeint::POWERS_OF_TEN[22]) {
				// [22..27]
				if (value >= Uhugeint::POWERS_OF_TEN[25]) {
					int length = 26;
					length += value >= Uhugeint::POWERS_OF_TEN[26];
					return length;
				} else {
					int length = 23;
					length += value >= Uhugeint::POWERS_OF_TEN[23];
					length += value >= Uhugeint::POWERS_OF_TEN[24];
					return length;
				}
			} else {
				// [17..22]
				if (value >= Uhugeint::POWERS_OF_TEN[20]) {
					int length = 21;
					length += value >= Uhugeint::POWERS_OF_TEN[21];
					return length;
				} else {
					int length = 18;
					length += value >= Uhugeint::POWERS_OF_TEN[18];
					length += value >= Uhugeint::POWERS_OF_TEN[19];
					return length;
				}
			}
		}
	...
	// Formats value in reverse and returns a pointer to the beginning.
	static char *FormatUnsigned(uhugeint_t value, char *ptr) {
		// while integer division is slow, hugeint division is MEGA slow
		// we want to avoid doing as many divisions as possible
		// for that reason we start off doing a division by a large power of ten that uint64_t can hold
		// (100000000000000000) - this is the third largest
		...
		value = Uhugeint::DivMod(value, 100000000000000000ULL, remainder);

		auto startptr = ptr;
		// now we format the remainder: note that we need to pad with zero's in case
		// the remainder is small (i.e. less than 10000000000000000)
		ptr = NumericHelper::FormatUnsigned<uint64_t>(remainder.lower, ptr);
	...
	static int DecimalLength(uhugeint_t value, uint8_t width, uint8_t scale) {
		if (scale == 0) {
			// scale is 0: regular number
		...
		// in that case we print the integer number, but with one extra character ('.')
		auto extra_numbers = width > scale ? 2 : 1;
		return MaxValue(scale + extra_numbers, UnsignedLength(value) + 1);
	}

	static void FormatDecimal(uhugeint_t value, uint8_t width, uint8_t scale, char *dst, int len) {
		auto endptr = dst + len;
		...
		// and the numbers AFTER the decimal (minor)
		uhugeint_t minor;
		uhugeint_t major = Uhugeint::DivMod(value, Uhugeint::POWERS_OF_TEN[scale], minor);
	...
	static string_t FormatDecimal(uhugeint_t value, uint8_t width, uint8_t scale, Vector &vector) {
		int length = DecimalLength(value, width, scale);
		string_t result = StringVector::EmptyString(vector, length);
