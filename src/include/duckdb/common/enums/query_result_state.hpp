...
enum class QueryResultState : uint8_t { READY, NOT_READY, BLOCKED, NO_TASKS_AVAILABLE, FINISHED, ERROR };

//! Whether execution reached a terminal state
constexpr bool IsTerminal(QueryResultState state) {
	return state == QueryResultState::FINISHED || state == QueryResultState::ERROR;
}
