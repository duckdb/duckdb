...
public:
	...
	string ToString(const QueryProfiler::ProfilingNode &op);
	...
	void Render(const QueryProfiler::ProfilingNode &op, std::ostream &ss);
...
private:
	...
	unique_ptr<RenderTree> CreateTree(const QueryProfiler::ProfilingNode &op);
	...
	unique_ptr<RenderTreeNode> CreateNode(const QueryProfiler::ProfilingNode &op);
