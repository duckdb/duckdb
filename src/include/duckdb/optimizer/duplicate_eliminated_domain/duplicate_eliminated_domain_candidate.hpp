...
private:
	...
	DuplicateEliminatedDomainCandidate(LogicalOperator &source, vector<idx_t> source_path_p,
	                                   vector<idx_t> key_indices_p, DuplicateEliminatedDomainCoverage coverage_p,
	                                   idx_t source_cardinality_p, idx_t domain_cardinality_p,
	                                   idx_t payload_cardinality_p, idx_t payload_domain_cardinality_p,
	                                   bool has_selection_p)
	    : source_path(std::move(source_path_p)), key_indices(std::move(key_indices_p)), coverage(coverage_p),
	      ...
	      has_selection(has_selection_p), source_type(source.type), source_types(source.types),
...
public:
	...
	static optional<DuplicateEliminatedDomainCandidate> FindBest(ClientContext &context,
