...
public:
	...
	static void Eval(int64_t num_rows, int num_prtns, uint16_t *prtn_ranges, INPUT_PRTN_ID_FN prtn_id_impl,
	                 OUTPUT_POS_FN output_pos_impl) {
...
public:
	...
	bool AcquirePartitionLock(size_t thread_id, int num_prtns, const int *prtns_to_try, bool limit_retries,
	...
	arrow::Status ForEachPartition(size_t thread_id,
		...
		while (num_unprocessed_partitions > 0) {
			                     ...
			                     /*limit_retries=*/false, /*max_retries=*/-1, &locked_prtn_id, &locked_prtn_id_pos);
			{
				...
				public:
					AutoReleaseLock(PartitionLocks *locks, int prtn_id) : locks(locks), prtn_id(prtn_id) {
					}
					~AutoReleaseLock() {
			...
			if (locked_prtn_id_pos < num_unprocessed_partitions - 1) {
				temp_unprocessed_prtns[locked_prtn_id_pos] = temp_unprocessed_prtns[num_unprocessed_partitions - 1];
			}
...
private:
	...
	std::unique_ptr<pcg32_fast[]> rngs_;
};
} // namespace duckdb
