...
public:
	...
	enum EGbAggType
	{
		EgbaggtypeGlobal, EgbaggtypeLocal, EgbaggtypeIntermediate, EgbaggtypeSentinel
	};

	// coercion form
	enum ECoercionForm
	{
		EcfExplicitCall, EcfExplicitCast, EcfImplicitCast, EcfDontCare
	};
...
public:
	...
	static Operator* PexprRehydrate(CCostContext* pcc, duckdb::vector<Operator*> pdrgpexpr, CDrvdPropCtxtPlan* pdpctxtplan);
...
public:
	...
	virtual Operator* SelfRehydrate(CCostContext* pcc, duckdb::vector<Operator*> pdrgpexpr, CDrvdPropCtxtPlan* pdpctxtplan)
	{
		return nullptr;
	}
	...
	bool FLogical() const
	{
		return ((logical_type != LogicalOperatorType::LOGICAL_INVALID) && (physical_type == PhysicalOperatorType::INVALID));
	}
	...
	{
		return ((logical_type == LogicalOperatorType::LOGICAL_EXTENSION_OPERATOR) && (physical_type != PhysicalOperatorType::EXTENSION));
	}
...
public:
	...
	virtual duckdb::unique_ptr<Operator> CopywithNewChilds(CGroupExpression* pgexpr, duckdb::vector<duckdb::unique_ptr<Operator>> pdrgpexpr, double cost);
