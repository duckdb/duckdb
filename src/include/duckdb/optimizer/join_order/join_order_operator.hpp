...
//! Conflict descriptor for a binary operator occurrence in the original join tree. Independently movable INNER
//! conjuncts have separate occurrences; non-inner operators retain one occurrence owning their complete condition.
//!
...
struct JoinOrderOperator {
	JoinOrderOperator(idx_t index_p, JoinOrderOperatorType type_p, JoinRelationSet &left_relations_p,
	    ...
	    : index(index_p), type(type_p), left_relations(left_relations_p), right_relations(right_relations_p),
	      ...
	      right_total_set(syntactic_set_p) {
	}
	...
	//! Costing/filter predicates carried by this binary operator.
	vector<idx_t> predicate_indices;
	//! Present only when this descriptor owns atomic LEFT/SEMI/ANTI semantics.
