...
struct JoinRelationSet {
	JoinRelationSet(array_ptr<idx_t> relations, idx_t count) : relations(std::move(relations)), count(count) {
	}

	string ToString() const;

	array_ptr<idx_t> relations;
	idx_t count;
...
public:
	...
	JoinRelationSet &GetJoinRelation(array_ptr<idx_t> relations, idx_t count);
