...
public:
	LogicalCopyFromFile(idx_t table_index, CopyFunction function, unique_ptr<FunctionData> info, vector<LogicalType> sql_types)
	    : LogicalOperator(LogicalOperatorType::COPY_FROM_FILE), table_index(table_index), function(function), info(move(info)),
