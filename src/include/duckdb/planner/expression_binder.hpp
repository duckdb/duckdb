...
public:
	unique_ptr<Expression> Bind(reference<unique_ptr<ParsedExpression>> expr,
	                            optional_ptr<LogicalType> result_type = nullptr, bool root_expression = true);
	...
	string Bind(reference<unique_ptr<ParsedExpression>> expr, idx_t depth, bool root_expression = false);
	...
	virtual BindResult BindExpression(reference<unique_ptr<ParsedExpression>> expr_ptr, idx_t depth,
...
protected:
	...
	BindResult BindExpression(FunctionExpression &expr, idx_t depth, reference<unique_ptr<ParsedExpression>> expr_ptr);
