...
public:
	...
	//! Returns true if a cast is invertable (i.e. CAST(s -> t -> s) = s for all values of s). This is not true for e.g. boolean casts, because that can be e.g. -1 -> TRUE -> 1. This is necessary to prevent some optimizer bugs.
	static bool CastIsInvertable(SQLType source_type, SQLType target_type);
