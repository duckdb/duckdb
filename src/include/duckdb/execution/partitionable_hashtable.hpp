...
public:
	...
	idx_t AddChunk(DataChunk &groups, DataChunk &payload, bool do_partition, const vector<idx_t, false> &filter);
	void Partition();
	bool IsPartitioned();
...
private:
	...
	vector<idx_t, false> sel_vector_sizes;
...
private:
	idx_t ListAddChunk(HashTableList &list, DataChunk &groups, Vector &group_hashes, DataChunk &payload,
	                   const vector<idx_t, false> &filter);
