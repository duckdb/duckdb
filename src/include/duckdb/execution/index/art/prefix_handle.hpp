...
public:
	...
	static NodePointer &ChildRef(const ART &art, NodeHandle &handle) {
		return *reinterpret_cast<NodePointer *>(handle.GetPtr() + art.PrefixCount() + 1);
	}

	//! Get a mutable reference to the child slot using an explicit prefix byte count.
	static NodePointer &ChildRefWithCount(NodeHandle &handle, const idx_t count) {
		return ChildRefWithCount(handle.GetPtr(), count);
	}

	//! Get a mutable reference to the child slot using an explicit prefix byte count.
	static NodePointer &ChildRefWithCount(const data_ptr_t data, const idx_t count) {
		return *reinterpret_cast<NodePointer *>(data + count + 1);
	}

	//! Transform prefix chain to deprecated format.
	//! Returns an empty OptionalNode if the prefix was not loaded from storage (early out) or if the endpoint
	...
	static OptionalNode TransformToDeprecated(ART &art, NodePointer &node, TransformToDeprecatedState &state);
