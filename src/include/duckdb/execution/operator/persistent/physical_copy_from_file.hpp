...
public:
	PhysicalCopyFromFile(vector<TypeId> types, vector<SQLType> sql_types, unique_ptr<CopyInfo> info)
	    : PhysicalOperator(PhysicalOperatorType::COPY_FROM_FILE, move(types)), sql_types(sql_types), info(move(info)) {
	}
