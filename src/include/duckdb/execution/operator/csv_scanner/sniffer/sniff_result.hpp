...
struct SnifferResult {
	SnifferResult(vector<LogicalType> return_types_p, vector<Identifier> names_p)
...
struct AdaptiveSnifferResult : SnifferResult {
	AdaptiveSnifferResult(vector<LogicalType> return_types_p, vector<Identifier> names_p, bool more_than_one_row_p)
