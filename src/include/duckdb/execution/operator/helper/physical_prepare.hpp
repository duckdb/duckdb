...
public:
	PhysicalPrepare(PhysicalPlan &physical_plan, const Identifier &name_p, shared_ptr<PreparedStatementData> prepared,
	                idx_t estimated_cardinality)
	    : PhysicalOperator(physical_plan, PhysicalOperatorType::PREPARE, {LogicalType::BOOLEAN}, estimated_cardinality),
	      name(physical_plan.ArenaRef().MakeString(name_p.GetIdentifierName())), prepared(std::move(prepared)) {
	}
