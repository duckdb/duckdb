...
const char INLINED_PEG_GRAMMAR[] = {
	...
	"ExcludeList <- 'EXCLUDE' ExcludeNameList\n"
	"ExcludeNameList <- Parens(List(ExcludeName))\n"
	"ExcludeName <- DottedIdentifier / ColIdOrString\n"
	"ReplaceList <- 'REPLACE' ReplaceEntryList\n"
	"ReplaceEntryList <- Parens(List(AliasedExpression))\n"
	"RenameList <- 'RENAME' RenameEntryList\n"
	"RenameEntryList <- Parens(List(RenameEntry))\n"
	"RenameEntry <- ColonRenameEntry / RenameEntryAs\n"
	"ColonRenameEntry <- Identifier ':' ExcludeName\n"
	"RenameEntryAs <- ExcludeName 'AS'? Identifier\n"
