...
public:
	...
	static GroupByNode ParseGroupByList(const string &group_by, const ParserOptions &options);
	//! Parses a list as found in an ORDER BY expression (i.e. including optional ASCENDING/DESCENDING modifiers)
	static vector<OrderByNode> ParseOrderList(const string &select_list, const ParserOptions &options);
	...
	static ColumnList ParseColumnList(const string &column_list, const ParserOptions &options);
	static ColumnDefinition ParseColumnDefinition(const string &column_definition, const ParserOptions &options);
