...
enum class AlterType : uint8_t { INVALID = 0, ALTER_TABLE = 1, ALTER_VIEW = 2, ALTER_SEQUENCE = 3, CHANGE_OWNERSHIP = 4};
...
struct ChangeOwnershipInfo : public AlterInfo{
   ChangeOwnershipInfo(CatalogType catalog_type, string entry_schema, string entry_name, string owner_schema, string owner_name)
    : AlterInfo(AlterType::CHANGE_OWNERSHIP, entry_schema, entry_name),
      catalog_type(catalog_type),
      owner_schema(owner_schema),
      owner_name(owner_name)
    {}

    // Catalog type refers to the entry type, since struct is usually built from an
