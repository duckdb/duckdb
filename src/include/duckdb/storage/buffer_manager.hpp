...
public:
	...
	//! Register a buffer of arbitrary size, as long as it is >= BLOCK_SIZE. can_destroy signifies whether or not the
	//! buffer can be destroyed when unpinned, or whether or not it needs to be written to a temporary file so it can be
	//! reloaded. The resulting buffer will already be allocated, but needs to be pinned in order to be used.
	shared_ptr<BlockHandle> Allocate(idx_t alloc_size, bool can_destroy = false);
