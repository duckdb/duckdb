...
public:
	...
	explicit AlpCompressionState(ColumnDataCheckpointer &checkpointer, AlpAnalyzeState<T> *analyze_state)
		...
		input_vector.clear();
		input_vector.reserve(AlpConstants::ALP_VECTOR_SIZE);
	...
	vector<T> input_vector = vector<T>(AlpConstants::ALP_VECTOR_SIZE);
...
public:
	...
	idx_t UsedSpace() const {
		...
		return sizeof(uint32_t) + data_bytes_used;
	}
	...
	void CompressGroup(){
		alp::AlpCompression<T, false>::Compress(input_vector, state.alp_state);
		...
		// TODO: manage Nulls (which now are received as 0 here)
		for (auto &value : input_vector){
			NumericStats::Update<T>(current_segment->stats.statistics, value);
		}
		current_segment->count += input_vector.size();
		FlushGroup();
	}
	...
	void FlushGroup(){
		...
		if (state.alp_state.exceptions_count > 0){
			...
			memcpy((void *) data_ptr, (void*) state.alp_state.exceptions_positions,
			       sizeof(uint16_t) * state.alp_state.exceptions_count);
			data_ptr += sizeof(uint16_t) * state.alp_state.exceptions_count;
		}
