...
public:
	...
	// static void AppendVectorBool(STATE_TYPE &state, Vector &input, idx_t input_size) {
	// 	idx_t bits_per_value_DEBUG_ONLY = ValidityMask::BITS_PER_VALUE;
	//
	...
	// validity mask. 	for (idx_t i = 0; i<input_size; i++) { 		if (bitpacking_ptr[i] == 1) { // won't trigger on
	// null values
	// 			// True boolean
	// 			*bitpacking_ptr |= ValidityUncompressed::LOWER_MASKS[1] << i%8;
	// 		} else {
	// 			// False boolean
	// 			*bitpacking_ptr &= ValidityUncompressed::LOWER_MASKS[i%8];
	// 		}
	// 		if ((i+1) % 8 == 0) {
	// 			bitpacking_ptr += 1; // advance to the next byte slot
	// 		}
	// 	}
	// 	// 00000100
	// 	// if (!data) {
