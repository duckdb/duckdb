...
public:
	static CompressionFunction GetFunction(PhysicalType data_type);
	static void AlignedScan(const validity_t *input, idx_t input_start, ValidityMask &result_mask, idx_t scan_count);
	...
	//! still work).
	static void UnalignedScan(const validity_t *input, idx_t input_size, idx_t input_start, ValidityMask &result_mask,
