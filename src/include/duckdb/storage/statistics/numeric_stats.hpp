...
struct NumericStatsData {
	...
	uint64_t imprint_bitmap;
	uint8_t imprint_bins;
};
...
struct NumericStats {
	...
	// for column imprint
	// set the bit map and bit count
	DUCKDB_API static void SetImprint(BaseStatistics &stats, uint64_t bitmap, uint8_t bins);

	// reset the bitmap to 0
	DUCKDB_API static void ResetImprint(BaseStatistics &stats);

	DUCKDB_API static uint64_t GetImprintBitmapUnsafe(const BaseStatistics &stats);
	DUCKDB_API static uint8_t GetImprintBinsUnsafe(const BaseStatistics &stats);
	DUCKDB_API static bool HasImprint(const BaseStatistics &stats);
	...
	template <class T>
	static uint8_t ComputeBinIndex(T value, T min, T max, uint8_t bins);

	// for building stage, built bitmap for a specific segment and column
	DUCKDB_API static void BuildImprintForSegment(ColumnData &col_data, SegmentNode<ColumnSegment> &segment_node);
