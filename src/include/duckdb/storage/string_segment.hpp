...
namespace duckdb {
    ...
    protected:
        void
        Update(ColumnData &column_data, SegmentStatistics &stats, Transaction &transaction, Vector &update, row_t *ids,
    ...
    private:
        void
        AppendData(SegmentStatistics &stats, data_ptr_t target, data_ptr_t end, idx_t target_offset, Vector &source,
                   idx_t offset, idx_t count);

        //! Fetch all the strings of a vector from the base table and place their locations in the result vector
        void
        FetchBaseData(ColumnScanState &state, data_ptr_t base_data, idx_t vector_index, Vector &result, idx_t count);
        ...
        void Select_String(buffer_handle_set_t &handles, Vector &result, data_ptr_t baseptr, int32_t *dict_offset,
                           ...
                           idx_t &approved_tuple_count, unsigned long size, nullmask_t *source_nullmask,
                           bool use_valid_sel,
                           size_t vector_index) {
                ...
                for (idx_t i = 0; i < approved_tuple_count; i++) {
                    ...
                    if (string_updates && string_updates[vector_index]) {
                        ...
                        if (update_idx < info.count && info.ids[update_idx] == src_idx) {
                            result_data[src_idx] =
                                    ReadString(handles, info.block_ids[update_idx], info.offsets[update_idx]);
                            update_idx++;
                        } else {
    ...
    void
    Select_String_Between(buffer_handle_set_t &handles, Vector &result, data_ptr_t baseptr, int32_t *dict_offset,
