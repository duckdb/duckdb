...
public:
	...
	//! Bulk row fetch. For each `i` in [0, visible_count), fetches the row at `offsets[visible_sel.get_index(i)]` and
	//! writes every requested column into `result.data[col_idx][result_offset + i]`.
