...
public:
	...
	//! When the entire batch is visible (return value == count), `visible_sel` is NOT guaranteed to be
	//! populated -- the caller is expected to treat that case as the identity selection (use offsets[i]
	//! directly) and avoid the indirection.
	...
	//! Pass an empty (`sel_vector == nullptr`) `visible_sel` to indicate identity selection (i.e. fetch
	//! `offsets[0..visible_count-1]` in order). This skips the per-row indirection through the sel.
