...
public:
	...
	//! Bulk visibility check. For each offset in [0, count), writes the input index into
	//! `visible_sel` if that row is visible to the transaction. Returns the number of visible rows.
	...
	//! Bulk row fetch. For each `i` in [0, visible_count), fetches the row at
	//! `offsets[visible_sel.get_index(i)]` and writes every requested column into
	//! `result.data[col_idx][result_offset + i]`. Iterates column-major to amortise the per-column
	//! segment-tree state and the buffer handles pinned in `state.handles`.
