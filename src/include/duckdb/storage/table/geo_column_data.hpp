...
public:
	...
	//! The BLOB fallback column for mixed geometries
	shared_ptr<ColumnData> blob_column;
	//! The "shredded" geometry column
	shared_ptr<ColumnData> geom_column;

public:
	idx_t GetMaxEntry() override;

	bool IsShredded() const {
		return geom_column != nullptr;
	}
	...
	void SetBlobColumnData(shared_ptr<ColumnData> blob_column_p);
	void SetGeomColumnData(shared_ptr<ColumnData> geom_column_p);
...
private:
	...
	WriteShreddedData(const RowGroup &row_group, const LogicalType &shredded_type, BaseStatistics &stats);
};
