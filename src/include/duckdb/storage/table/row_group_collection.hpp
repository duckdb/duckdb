...
public:
	...
	void InitializeScanWithOffset(TableScanState &state, const vector<column_t> &column_ids, idx_t start_row, idx_t end_row);
	...
	void Fetch(Transaction &transaction, DataChunk &result, const vector<column_t> &column_ids,
	...
	void Update(Transaction &transaction, row_t *ids, const vector<column_t> &column_ids, DataChunk &updates, TableStatistics &stats);

	void Checkpoint(TableDataWriter &writer, vector<RowGroupPointer> &row_group_pointers, vector<unique_ptr<BaseStatistics>> &global_stats);
	...
	shared_ptr<RowGroupCollection> AddColumn(ColumnDefinition &new_column, Expression *default_value, BaseStatistics &stats);
	shared_ptr<RowGroupCollection> RemoveColumn(idx_t col_idx);
	shared_ptr<RowGroupCollection> AlterType(idx_t changed_idx, const LogicalType &target_type, vector<column_t> bound_columns, Expression &cast_expr, BaseStatistics &stats);
private:
