	...
	static idx_t StringAppendBase(BufferHandle &handle, ColumnSegment &segment, SegmentStatistics &stats,
		...
		for (idx_t i = 0; i < count; i++) {
...
#ifdef DEBUG
			GetDictionary(segment, handle).Verify(segment.block->GetBlockSize());
#endif
			...
			if (DUCKDB_UNLIKELY(required_space >=
			                    StringUncompressed::GetStringBlockLimit(segment.block->GetBlockSize()))) {
			...
			if (DUCKDB_UNLIKELY(use_overflow_block)) {
				...
				*dictionary_size += BIG_STRING_MARKER_SIZE;
				...
				D_ASSERT(NumericCast<idx_t>(*dictionary_size) <= segment.block->GetBlockSize());
			...
			D_ASSERT(RemainingSpace(segment, handle) <= segment.block->GetBlockSize());
#ifdef DEBUG
			GetDictionary(segment, handle).Verify(segment.block->GetBlockSize());
#endif
		}
...
public:
	...
	inline static string_t FetchStringFromDict(ColumnSegment &segment, uint32_t dict_end_offset, Vector &result,
		...
		D_ASSERT(dict_offset <= NumericCast<int32_t>(segment.block->GetBlockSize()));
