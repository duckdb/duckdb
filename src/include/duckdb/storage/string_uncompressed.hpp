...
public:
	...
	static idx_t StringAppend(ColumnSegment &segment, SegmentStatistics &stats, CanonicalFormat &data, idx_t offset,
	                          idx_t count) {
		return StringAppendBase(segment, stats, data, offset, count);
	}

	template <bool DUPLICATE_ELIMINATE = false>
	static idx_t StringAppendBase(ColumnSegment &segment, SegmentStatistics &stats, CanonicalFormat &data, idx_t offset,
