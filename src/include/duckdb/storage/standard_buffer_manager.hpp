...
public:
	...
	shared_ptr<BlockHandle> RegisterTransientMemory(const idx_t size, const idx_t block_size,
	                                                const idx_t block_header_size) final;
	...
	uint64_t GetBlockHeaderSize() const final;

	//! Allocate an in-memory buffer with a single pin.
	//! The allocated memory is released when the buffer handle is destroyed.
	DUCKDB_API BufferHandle Allocate(MemoryTag tag, idx_t block_size, bool can_destroy = true,
	                                 const idx_t block_header_size = DEFAULT_BLOCK_HEADER_STORAGE_SIZE) final;
...
protected:
	...
	shared_ptr<BlockHandle> RegisterMemory(MemoryTag tag, idx_t block_size, const idx_t block_header_size,
	                                       bool can_destroy);
