...
struct EnableColumnImprintSetting {
	using RETURN_TYPE = bool;
	static constexpr const char *Name = "enable_column_imprint";
	static constexpr const char *Description =
	    "Enable column imprint for fine-grained pruning using 64-bit bitmaps (default: false).";
	static constexpr const char *InputType = "BOOLEAN";
	static void SetGlobal(DatabaseInstance *db, DBConfig &config, const Value &parameter);
	static void ResetGlobal(DatabaseInstance *db, DBConfig &config);
	static Value GetSetting(const ClientContext &context);
...
struct GeneratedSettingInfo {
	static constexpr idx_t MaxSettingIndex = 94;
};
