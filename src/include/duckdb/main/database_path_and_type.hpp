...
struct DBPathAndType {
	...
	static void ExtractExtensionPrefix(string &path, Identifier &db_type);
	...
	static void CheckMagicBytes(QueryContext context, FileSystem &fs, string &path, Identifier &db_type,
	...
	static void ResolveDatabaseType(FileSystem &fs, string &path, Identifier &db_type,
	                                optional_ptr<PrefetchedFileData> out_prefetch = nullptr);
};
} // namespace duckdb
