...
inline duckdb_ext_api_v0 CreateAPIv0(idx_t minor_version, idx_t patch_version) {
	...
	if (patch_version >= 1) {
		...
		memset(&result.duckdb_open, 0, sizeof(result.duckdb_open) * 232);
	}
	if (patch_version >= 2) {
		...
		memset(&result.duckdb_get_profiling_info, 0, sizeof(result.duckdb_get_profiling_info) * 66);
	}
	return result;
}
