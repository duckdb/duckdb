...
class DataTable {
	...
	void Scan(Transaction &transaction, DataChunk &result, const std::vector<column_t> &column_ids,
	          ScanStructure &structure);
	//! Fetch data from the specific row identifiers from the base table
	void Fetch(Transaction &transaction, DataChunk &result, std::vector<column_t> &column_ids, Vector &row_ids);
	...
	void Update(TableCatalogEntry &table, ClientContext &context, Vector &row_ids, std::vector<column_t> &column_ids,
	            DataChunk &data);

	//! Scan used for creating an index, incrementally locks all storage chunks
	void CreateIndexScan(ScanStructure &structure, std::vector<column_t> &column_ids, DataChunk &result);
	...
	//! versioned tuples
	void RetrieveVersionedData(DataChunk &result, const std::vector<column_t> &column_ids,
	...
	void RetrieveBaseTableData(DataChunk &result, const std::vector<column_t> &column_ids, sel_t regular_entries[],
