...
constexpr const uint64_t VERSION_NUMBER = 1;
// Size of a memory slot managed by the StorageManager. This is the quantum of allocation for Blocks on DuckDB. We default to 256KB.
constexpr const uint64_t BLOCK_SIZE = 1 << 18;
//! The size of the headers. This should be small and written more or less atomically by the hard disk. We default to the page size, which is 4KB.
constexpr const uint64_t HEADER_SIZE = 1 << 12;

using block_id_t = int64_t;

//! The MainHeader is the first header in the storage file. The MainHeader is typically written only once for a database file.
struct MainHeader {
