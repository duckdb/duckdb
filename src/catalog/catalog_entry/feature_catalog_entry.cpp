...
FeatureCatalogEntry::FeatureCatalogEntry(Catalog &catalog, SchemaCatalogEntry &schema, CreateFeatureInfo &info)
    : StandardEntry(CatalogType::FEATURE_ENTRY, schema, catalog, info.feature_name), entity_table(info.entity_table),
      source_table(info.source_table), entity_columns(info.entity_columns), entity_key_columns(info.entity_key_columns),
      timestamp_column(info.timestamp_column), window_interval(info.window_interval),
      watermark_interval(info.watermark_interval), refresh_mode(info.refresh_mode),
      retain_versions(info.retain_versions), current_version(info.current_version),
