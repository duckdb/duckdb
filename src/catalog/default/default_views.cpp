...
static DefaultView internal_views[] = {
    ...
    {DEFAULT_SCHEMA, "pg_attribute", "SELECT * FROM pg_catalog.pg_attribute"},
    {DEFAULT_SCHEMA, "pg_attrdef", "SELECT * FROM pg_catalog.pg_attrdef"},
    {DEFAULT_SCHEMA, "pg_class", "SELECT * FROM pg_catalog.pg_class"},
    {DEFAULT_SCHEMA, "pg_constraint", "SELECT * FROM pg_catalog.pg_constraint"},
    {DEFAULT_SCHEMA, "pg_depend", "SELECT * FROM pg_catalog.pg_depend"},
    {DEFAULT_SCHEMA, "pg_description", "SELECT * FROM pg_catalog.pg_description"},
    {DEFAULT_SCHEMA, "pg_enum", "SELECT * FROM pg_catalog.pg_enum"},
    {DEFAULT_SCHEMA, "pg_index", "SELECT * FROM pg_catalog.pg_index"},
    {DEFAULT_SCHEMA, "pg_indexes", "SELECT * FROM pg_catalog.pg_indexes"},
    ...
    {"pg_catalog", "pg_constraint", "SELECT table_oid*1000000+constraint_index oid, constraint_text conname, schema_oid connamespace, CASE WHEN constraint_type='CHECK' then 'c' WHEN constraint_type='UNIQUE' then 'u' ELSE 'p' END contype, false condeferrable, false condeferred, true convalidated, table_oid conrelid, 0 contypid, 0 conindid, 0 conparentid, 0 confrelid, NULL confupdtype, NULL confdeltype, NULL confmatchtype, true conislocal, 0 coninhcount, false connoinherit, constraint_column_indexes conkey, NULL confkey, NULL conpfeqop, NULL conppeqop, NULL conffeqop, NULL conexclop, expression conbin FROM duckdb_constraints()"},
    ...
    {"pg_catalog", "pg_index", "SELECT index_oid indexrelid, table_oid indrelid, 0 indnatts, 0 indnkeyatts, is_unique indisunique, NULL indisprimary, false indisexclusion, true indimmediate, false indisclustered, true indisvalid, false indcheckxmin, true indisready, true indislive, false indisreplident, NULL indkey, NULL indcollation, NULL indclass, NULL indoption, expressions indexprs, NULL indpred FROM duckdb_indexes()"},
