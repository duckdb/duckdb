...
void SingleFileCheckpointWriter::CreateCheckpoint() {
	...
	partial_block_mgr.FlushPartialBlocks();
...
void CheckpointReader::ReadIndex(ClientContext &context, MetaBlockReader &reader) {
	...
	case IndexType::ART: {
		auto art = make_unique<ART>(info->column_ids, *table_catalog->storage->table_io_manager, move(unbound_expressions), info->constraint_type, *context.db,
...
void PartialBlockManager::RegisterPartialBlock(PartialBlockAllocation &&allocation) {
	...
	if (state.block_use_count < max_use_count) {
		...
		if (new_space_left >= free_space_threshold) {
