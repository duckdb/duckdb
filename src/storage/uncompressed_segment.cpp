...
//        auto vec = FlatVector::GetData<int64_t>(result);
//        VectorOperations::SelectFlatLoop<LEFT_TYPE, RIGHT_TYPE, OP, LEFT_CONSTANT, RIGHT_CONSTANT, NO_NULL, false, true>(
//			    result, rdata, sel, count, nullmask, true_sel, false_sel);
        return true;
    }
    else{
            auto read_lock = lock.GetSharedLock();
        //! Select the data from the base table
        Select(state, result, sel, approved_tuple_count, tableFilter);
        return false;
    }
