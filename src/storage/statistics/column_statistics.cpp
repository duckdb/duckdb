...
shared_ptr<ColumnStatistics> ColumnStatistics::CreateEmptyStats(const LogicalType &type) {
	return make_refcounted<ColumnStatistics>(BaseStatistics::CreateEmpty(type));
}
...
shared_ptr<ColumnStatistics> ColumnStatistics::Copy() const {
	return make_refcounted<ColumnStatistics>(stats.Copy(), distinct_stats ? distinct_stats->Copy() : nullptr);
}
...
shared_ptr<ColumnStatistics> ColumnStatistics::Deserialize(Deserializer &deserializer) {
	...
	return make_refcounted<ColumnStatistics>(std::move(stats), std::move(distinct_stats));
}
