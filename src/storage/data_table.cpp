...
DataTable::DataTable(ClientContext &context, DataTable &parent, ColumnDefinition &new_column, Expression &default_value,
	...
	if (add_column_checkpoint_state && is_replay_wal) {
		...
		stable_result = add_column_checkpoint_state->GetStableResult();
	}

	this->row_groups = parent.row_groups->AddColumn(context, new_column, default_executor, stable_result);
...
void DataTable::RollbackAddedColumnIfNeeded() {
	if (persist_added_column) {
		// ROLLBACK the last column we previously added and persisted
		CommitDropColumn(column_definitions.size() - 1);
	}
}
