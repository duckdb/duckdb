...
void WriteOverflowStringsToDisk::WriteString(string_t string, block_id_t &result_block, int32_t &result_offset) {
	...
	s.Compress(string.GetData(), uncompressed_size, data_ptr_cast<char>(compressed_buf.get()), &compressed_size);
	string_t compressed_string(data_ptr_cast<const char>(compressed_buf.get()), compressed_size);
