...
void ColumnDataCheckpointer::ScanSegmentsWithSample(const std::function<void(Vector &, idx_t)> &callback, double sample_rate) {
	if (sample_rate == 1) {
...
vector<CheckpointAnalyzeResult> ColumnDataCheckpointer::DetectBestCompressionMethod() {
	...
	ScanSegmentsWithSample([&](Vector &scan_vector, idx_t count) {
		for (idx_t i = 0; i < checkpoint_states.size(); i++) {
			if (!has_changes[i]) {
				continue;
			}
