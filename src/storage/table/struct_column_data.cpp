...
unique_ptr<ColumnCheckpointState> StructColumnData::CreateCheckpointState(RowGroup &row_group,
                                                                          ...
                                                                          SegmentLock &&lock) {
	return make_uniq<StructColumnCheckpointState>(row_group, *this, partial_block_manager, std::move(lock));
}
...
unique_ptr<ColumnCheckpointState> StructColumnData::Checkpoint(RowGroup &row_group,
	...
	auto checkpoint_state =
	    make_uniq<StructColumnCheckpointState>(row_group, *this, checkpoint_info.info.manager, std::move(lock));
