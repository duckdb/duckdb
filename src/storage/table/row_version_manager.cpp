...
idx_t RowVersionManager::GetDeletedCount(idx_t count) {
	ScanOptions options(TransactionData(0, NOT_DELETED_ID));
	options.insert_type = InsertedScanType::ALL_ROWS;
	options.delete_type = DeletedScanType::STANDARD;
	...
	for (idx_t r = 0, i = 0; r < count; r += STANDARD_VECTOR_SIZE, i++) {
		...
		idx_t not_deleted_count = vector_info[i]->GetSelVector(options, nullptr, max_count);
		deleted_count += max_count - not_deleted_count;
	}
	return deleted_count;
}
