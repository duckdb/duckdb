...
void RowGroupCollection::VacuumDeletes(vector<SegmentNode<RowGroup>> &segments) {
	...
	for (idx_t r_idx = 0; r_idx < segments.size(); r_idx++) {
		...
		for (target_count = 1; target_count <= MAX_MERGE_COUNT; target_count++) {
			...
			merge_rows = row_group_counts[r_idx];
			merge_count = 1;
			for (next_idx = r_idx + 1; next_idx < segments.size(); next_idx++) {
				if (row_group_counts[next_idx] == 0) {
					continue;
				}
				if (merge_rows + row_group_counts[next_idx] > total_target_size) {
					// does not fit
					break;
				}
		...
		for (idx_t target_idx = 0; target_idx < target_count; target_idx++) {
			...
			row_group_counts[r_idx + target_idx] = row_group->count;
			segments[r_idx + target_idx].node = std::move(row_group);
		...
		// skip forward by the row groups we have merged
		r_idx += merge_count - 1;
	}
}
...
void RowGroupCollection::Checkpoint(TableDataWriter &writer, TableStatistics &global_stats) {
	...
	// prior to checkpointing merge any rows
