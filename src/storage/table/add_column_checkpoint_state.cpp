...
void AddColumnCheckpointState::FlushToDisk(RowGroupCollection &data) {
	...
	info.stable_result = make_uniq<PersistentCollectionData>();
	...
	for (int64_t i = 0;; ++i) {
		...
		if (row_group_ptr == nullptr)
			break;
		row_group_ptr->WriteToDisk(write_info);
	}
	...
	for (int64_t i = 0;; ++i) {
		...
		if (row_group_ptr == nullptr)
			break;
		auto persistent_data = row_group_ptr->SerializeRowGroupInfo();
		persistent_data.types = data.GetTypes();
		info.stable_result->row_group_data.push_back(std::move(persistent_data));
	}
}

shared_ptr<RowGroupCollection> AddColumnCheckpointState::GetRowGroups() {
	auto &table_info = table.GetDataTableInfo();
	auto new_types = table.GetTypes();
	new_types.push_back(info.new_column.GetType());
	auto result = make_shared_ptr<RowGroupCollection>(table_info, table_info->GetIOManager(), new_types, 0);
	result->Initialize(*info.stable_result);
