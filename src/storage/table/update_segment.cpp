...
UpdateInfo *CreateEmptyUpdateInfo(TransactionData transaction, idx_t type_size, idx_t count, array_ptr<char> &data) {
	data = make_array<char>(sizeof(UpdateInfo) + (sizeof(sel_t) + type_size) * STANDARD_VECTOR_SIZE);
...
void UpdateSegment::Update(TransactionData transaction, idx_t column_index, Vector &update, row_t *ids, idx_t count,
	...
	if (root->info[vector_index]) {
		...
		array_ptr<char> update_info_data;
		if (!node) {
		...
		result->tuples = make_array<sel_t>(STANDARD_VECTOR_SIZE);
		result->tuple_data = make_array<data_t>(STANDARD_VECTOR_SIZE * type_size);
