...
void ColumnScanState::NextInternal(idx_t count) {
	...
	while (row_index >= current->start + current->count) {
		current = (ColumnSegment *)segment_tree->GetNextSegment(current);
		initialized = false;
		...
		if (result.size() > 0) {
			...
			do {
				current_row_group = row_group_state.row_group =
				    (RowGroup *)row_groups->GetNextSegment(current_row_group);
...
bool CollectionScanState::ScanCommitted(DataChunk &result, SegmentLock &l, TableScanType type) {
	...
	while (current_row_group) {
		...
		if (result.size() > 0) {
			...
			current_row_group = row_group_state.row_group =
			    (RowGroup *)row_groups->GetNextSegment(l, current_row_group);
			if (current_row_group) {
...
bool CollectionScanState::ScanCommitted(DataChunk &result, TableScanType type) {
	...
	while (current_row_group) {
		...
		if (result.size() > 0) {
			...
			current_row_group = row_group_state.row_group = (RowGroup *)row_groups->GetNextSegment(current_row_group);
			if (current_row_group) {
				current_row_group->InitializeScan(row_group_state);
			}
		}
