...
void TemporaryDirectoryHandle::ClaimOwner() {
	...
	for (;;) {
		...
		auto marker_path = fs.JoinPath(temp_directory, TemporaryOwnerClaimName(owner));
		try {
			auto claim = fs.OpenFile(marker_path, FileFlags::FILE_FLAGS_WRITE | FileFlags::FILE_FLAGS_FILE_CREATE_NEW);
			claim->Close();
			break;
		} catch (std::exception &) {
