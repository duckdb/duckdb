...
Block::Block(Allocator &allocator, const block_id_t id, const idx_t block_size, const optional_idx block_header_size)
    : FileBuffer(allocator, FileBufferType::BLOCK, block_size, block_header_size), id(id) {
}

Block::Block(Allocator &allocator, block_id_t id, uint32_t internal_size, const optional_idx block_header_size)
