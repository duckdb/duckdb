...
void LogicalRecursiveCTE::Serialize(Serializer &serializer) const {
	...
	serializer.WritePropertyWithDefault<unordered_map<idx_t, unique_ptr<Expression>>>(209, "payload_aggregate_dest_map", payload_aggregate_dest_map);
}
...
unique_ptr<LogicalOperator> LogicalRecursiveCTE::Deserialize(Deserializer &deserializer) {
	...
	deserializer.ReadPropertyWithDefault<unordered_map<idx_t, unique_ptr<Expression>>>(209, "payload_aggregate_dest_map", result->payload_aggregate_dest_map);
	return std::move(result);
}
