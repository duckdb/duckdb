...
CompressedSegment::CompressedSegment(ColumnSegment *parent_p, DatabaseInstance &db, PhysicalType type, idx_t row_start, CompressionFunction *function_p, block_id_t block_id)
    : db(db), type(type), tuple_count(0), row_start(row_start), function(function_p), parent(parent_p) {
	D_ASSERT(function);
...
void CompressedSegment::FilterSelection(SelectionVector &sel, Vector &result, const TableFilter &filter,
...
bool CompressedSegment::RowIdIsValid(idx_t row_id) const {
	return row_id <= tuple_count;
}

bool CompressedSegment::RowRangeIsValid(idx_t row_id, idx_t count) const {
