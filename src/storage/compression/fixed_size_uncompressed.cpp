...
//===--------------------------------------------------------------------===//
template <class T>
static void AppendLoop(SegmentStatistics &stats, data_ptr_t target, idx_t target_offset, CanonicalFormat &adata,
...
template <>
void AppendLoop<list_entry_t>(SegmentStatistics &stats, data_ptr_t target, idx_t target_offset, CanonicalFormat &adata,
                              idx_t offset, idx_t count) {
...
template <class T>
idx_t FixedSizeAppend(ColumnSegment &segment, SegmentStatistics &stats, CanonicalFormat &data, idx_t offset,
                      idx_t count) {
	auto &buffer_manager = BufferManager::GetBufferManager(segment.db);
	auto handle = buffer_manager.Pin(segment.block);
