...
void BitpackingScanPartialInternal(ColumnSegment &segment, ColumnScanState &state, idx_t scan_count, Vector &result,
	...
	// FOR vectors may only be produced for a top-level entire-vector scan. The partial path is used for nested
	// children, cross-segment scans, and ScanCount, all of which require a FLAT vector (validity / nested ScanCount
	// assert flat), so FOR is disallowed there.
...
void BitpackingScanPartial(ColumnSegment &segment, ColumnScanState &state, idx_t scan_count, Vector &result,
                           idx_t result_offset) {
	// partial scan (nested children / cross-segment / ScanCount) - must stay FLAT, never produce a FOR vector
...
void BitpackingScan(ColumnSegment &segment, ColumnScanState &state, idx_t scan_count, Vector &result) {
	// top-level entire-vector scan - FOR vectors allowed here
