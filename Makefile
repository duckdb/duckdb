...
release:
	...
	cd build/release && \
	cmake -DCMAKE_CXX_FLAGS="-flto=thin" -DCMAKE_EXE_LINKER_FLAGS="-flto=thin" -DCMAKE_MODULE_LINKER_FLAGS="-flto=thin" $(GENERATOR) $(FORCE_COLOR) ${WARNINGS_AS_ERRORS} ${FORCE_WARN_UNUSED_FLAG} ${FORCE_32_BIT_FLAG} ${DISABLE_UNITY_FLAG} ${DISABLE_SANITIZER_FLAG} ${OSX_BUILD_UNIVERSAL_FLAG} ${STATIC_LIBCPP} ${EXTENSIONS} -DCMAKE_BUILD_TYPE=Release ../.. && \
