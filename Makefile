...
codeql: ${EXTENSION_CONFIG_STEP}
	...
	cd build/codeql && \
	cmake $(GENERATOR) $(FORCE_COLOR) -DCMAKE_C_COMPILER_LAUNCHER= -DCMAKE_CXX_COMPILER_LAUNCHER= ${DISABLE_SANITIZER_FLAG} -DCLANG_TIDY=1 -DDISABLE_UNITY=1 -DBUILD_EXTENSIONS=parquet -DCMAKE_BUILD_TYPE=Debug ../.. && \
	cmake --build . --config Debug
