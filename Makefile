...
codeql: ${EXTENSION_CONFIG_STEP}
	...
	cd build/codeql && \
	cmake $(GENERATOR) $(FORCE_COLOR) -DCMAKE_C_COMPILER_LAUNCHER= -DCMAKE_CXX_COMPILER_LAUNCHER= ${DISABLE_SANITIZER_FLAG} -DDUCKDB_CLANG_TIDY=1 -DBUILD_UNITTESTS=FALSE -DDISABLE_UNITY=1 -DBUILD_EXTENSIONS=parquet -DCMAKE_C_FLAGS=-w -DCMAKE_CXX_FLAGS=-w -DCMAKE_BUILD_TYPE=Debug ../.. && \
	cmake --build . --config Debug
