...
int main() {
	...
	// select a,b, sum(a) over (partition by b order by a rows between unbounded preceding and 0 following) from integers;
	...
	// expressions can either be built-in specific window functions (e.g. row_number, rank, ntile, etc) or combinable aggregate functions
	// sum(a) over (partition by b order by a rows between unbounded preceding and 0 following)
	auto aggregate = SumFun::GetSumAggregate(PhysicalType::INT64);
	auto return_type = aggregate.return_type;
	auto sum = make_unique<BoundWindowExpression>(ExpressionType::WINDOW_AGGREGATE, return_type, make_unique<AggregateFunction>(aggregate), nullptr);
