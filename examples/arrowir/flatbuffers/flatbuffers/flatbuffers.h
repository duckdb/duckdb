...
#if defined(FLATBUFFERS_NAN_DEFAULTS) && defined(FLATBUFFERS_HAS_NEW_STRTOD) && (FLATBUFFERS_HAS_NEW_STRTOD > 0)
...
class Array {
	typedef typename flatbuffers::integral_constant<bool, flatbuffers::is_scalar<T>::value> scalar_tag;
	typedef typename flatbuffers::conditional<scalar_tag::value, T, const T *>::type IndirectHelperType;
...
public:
	...
	typename flatbuffers::conditional<scalar_tag::value, void, T *>::type GetMutablePointer(uoffset_t i) const {
		FLATBUFFERS_ASSERT(i < size());
...
inline uint8_t *ReallocateDownward(Allocator *allocator, uint8_t *old_p, size_t old_size, size_t new_size,
	...
	return allocator ? allocator->reallocate_downward(old_p, old_size, new_size, in_use_back, in_use_front)
	                 : DefaultAllocator().reallocate_downward(old_p, old_size, new_size, in_use_back, in_use_front);
}
...
public:
	DetachedBuffer()
	    : allocator_(nullptr), own_allocator_(false), buf_(nullptr), reserved_(0), cur_(nullptr), size_(0) {
	}
...
public:
	explicit vector_downward(size_t initial_size, Allocator *allocator, bool own_allocator, size_t buffer_minalign)
	    : allocator_(allocator), own_allocator_(own_allocator), initial_size_(initial_size),
	      buffer_minalign_(buffer_minalign), reserved_(0), buf_(nullptr), cur_(nullptr), scratch_(nullptr) {
	}
...
// clang-format off
  ...
  #endif  // defined(FLATBUFFERS_CPP98_STL)
	    // clang-format on
	    : allocator_(other.allocator_), own_allocator_(other.own_allocator_), initial_size_(other.initial_size_),
	      buffer_minalign_(other.buffer_minalign_), reserved_(other.reserved_), buf_(other.buf_), cur_(other.cur_),
...
public:
	Verifier(const uint8_t *buf, size_t buf_len, uoffset_t _max_depth = 64, uoffset_t _max_tables = 1000000,
	    ...
	    : buf_(buf), size_(buf_len), depth_(0), max_depth_(_max_depth), num_tables_(0), max_tables_(_max_tables),
	      upper_bound_(0), check_alignment_(_check_alignment) {
...
// clang-format off
	...
	bool VerifyBufferFromStart(const char *identifier, size_t start) {
		if (identifier &&
		    !Check((size_ >= 2 * sizeof(flatbuffers::uoffset_t) && BufferHasIdentifier(buf_ + start, identifier)))) {
			return false;
		}
